...
async function replacePlayer() {
  ...
  document.getElementById('player').onLoad = "await sleep(3000) document.getElementById('player').innerHTML = ''";
  ...
  fetch('https://immortal2willlose.xyz/api/dl/v1/', options)
    ...
    .then(async (data) => {
      await sleep(500);
      console.log(data);
      const ccurl = data.url.replace('http://152.53.80.20:9000/', 'https://immortal2willlose.xyz/api/dl/v1/');
      document.getElementById('player').innerHTML =
        "<video controls width='100%' style='border-radius: 10px;' id='ghost_player' autoplay><source src='" + ccurl + "' type='video/mp4'></video>";
