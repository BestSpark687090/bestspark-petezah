class EJS_GameManager {
  constructor(Module, EJS) {
    ...
    this.functions = {
      ...
      loadState: this.Module.cwrap("load_state", "number", [
        "string",
        "number",
      ]),
      screenshot: this.Module.cwrap("cmd_take_screenshot", "", []),
      simulateInput: this.Module.cwrap("simulate_input", "null", [
        "number",
        "number",
        "number",
      ]),
      ...
      setVariable: this.Module.cwrap("ejs_set_variable", "null", [
        "string",
        "string",
      ]),
      setCheat: this.Module.cwrap("set_cheat", "null", [
        "number",
        "number",
        "string",
      ]),
      ...
      toggleFastForward: this.Module.cwrap("toggle_fastforward", "null", [
        "number",
      ]),
      setFastForwardRatio: this.Module.cwrap("set_ff_ratio", "null", [
        "number",
      ]),
      toggleRewind: this.Module.cwrap("toggle_rewind", "null", ["number"]),
      setRewindGranularity: this.Module.cwrap(
        "set_rewind_granularity",
        "null",
        ["number"],
      ),
      toggleSlowMotion: this.Module.cwrap("toggle_slow_motion", "null", [
        "number",
      ]),
  ...
  loadExternalFiles() {
    return new Promise(async (resolve, reject) => {
      if (
        this.EJS.config.externalFiles &&
        this.EJS.config.externalFiles.constructor.name === "Object"
      ) {
        for (const key in this.EJS.config.externalFiles) {
          await new Promise((done) => {
            this.EJS.downloadFile(
              this.EJS.config.externalFiles[key],
              async (res) => {
                if (res === -1) {
                  if (this.EJS.debug)
                    console.warn(
                ...
                if (key.trim().endsWith("/")) {
                  ...
                  const name = this.EJS.config.externalFiles[key]
                    .split("/")
                    .pop()
                    .split("#")[0]
                    .split("?")[0]
                    ...
                    .trim();
                  if (!name) return done();
                  const files = await this.EJS.checkCompression(
                    new Uint8Array(res.data),
                    this.EJS.localization("Decompress Game Assets"),
                  );
  ...
  simulateInput(player, index, value) {
    ...
    if ([24, 25, 26, 27, 28, 29].includes(index)) {
      if (index === 24 && value === 1) {
        const slot = this.EJS.settings["save-state-slot"]
          ? this.EJS.settings["save-state-slot"]
          : "1";
        this.quickSave(slot);
        this.EJS.displayMessage(
          this.EJS.localization("SAVED STATE TO SLOT") + " " + slot,
        );
      }
      if (index === 25 && value === 1) {
        const slot = this.EJS.settings["save-state-slot"]
        ...
        this.quickLoad(slot);
        this.EJS.displayMessage(
          this.EJS.localization("LOADED STATE FROM SLOT") + " " + slot,
        );
      }
      if (index === 26 && value === 1) {
        let newSlot;
        ...
        this.EJS.displayMessage(
          this.EJS.localization("SET SAVE STATE SLOT TO") + " " + newSlot,
        );
        this.EJS.changeSettingOption("save-state-slot", newSlot.toString());
      }
      if (index === 27) {
        this.functions.toggleFastForward(
          this.EJS.isFastForward ? !value : value,
        );
      }
      if (index === 29) {
  ...
  createCueFile(fileNames) {
    try {
      if (fileNames.length > 1) {
        fileNames = fileNames.filter((item) => {
          return this.getFileNames().includes(
            item.split(".").pop().toLowerCase(),
          );
        });
        fileNames = fileNames.sort((a, b) => {
          if (isNaN(a.charAt()) || isNaN(b.charAt()))
            throw new Error("Incorrect file name format");
          return Number.parseInt(a.charAt()) > Number.parseInt(b.charAt())
            ? 1
            : -1;
        });
      }
    ...
    if (baseFileName.includes(".")) {
      baseFileName = baseFileName.substring(
        0,
        baseFileName.length - baseFileName.split(".").pop().length - 1,
      );
    }
    for (let i = 0; i < fileNames.length; i++) {
      const contents =
  ...
  loadPpssppAssets() {
    return new Promise((resolve) => {
      this.EJS.downloadFile(
        ...
        (res) => {
          this.EJS.checkCompression(
            new Uint8Array(res.data),
            this.EJS.localization("Decompress Game Data"),
          ).then((pspassets) => {
            if (pspassets === -1) {
