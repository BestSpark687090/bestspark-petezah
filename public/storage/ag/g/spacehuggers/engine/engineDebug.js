...
const debugCircle = (pos, radius, color, time, fill = 0) =>
  debugRect(pos, radius, color, time, fill);
const debugPoint = (pos, color, time, angle) =>
  debugRect(pos, 0, color, time, angle);
const debugLine = (posA, posB, color, thickness = 0.1, time) => {
...
const debugRender = () => {
  ...
  if (debugOverlay) {
    for (const o of engineObjects) {
      if (o.canvas) continue; // skip tile layers
      ...
      drawRect(
        o.pos,
        size.scale(0.8),
        o.parent ? new Color(1, 1, 1, 0.5) : new Color(0, 0, 0, 0.8),
      );
      o.parent && drawLine(o.pos, o.parent.pos, 0.1, new Color(0, 0, 1, 0.5));
    }
    ...
    if (bestObject) {
      ...
      raycastHitPos &&
        drawRect(
          raycastHitPos.int().add(vec2(0.5)),
          vec2(1),
          new Color(0, 1, 1, 0.3),
        );
      drawRect(
        mousePosWorld.int().add(vec2(0.5)),
        vec2(1),
        new Color(0, 0, 1, 0.5),
      );
      drawLine(
        ...
        bestObject.pos,
        0.1,
        !raycastHitPos ? new Color(0, 1, 0, 0.5) : new Color(1, 0, 0, 0.5),
      );

      let pos = mousePosWorld.copy(),
        height = vec2(0, 0.5);
      const printVec2 = (v) =>
        "(" +
        (v.x > 0 ? " " : "") +
        ...
        v.y.toFixed(2) +
        ")";
      const args = [0.5, new Color(), 0.05, undefined, undefined, "monospace"];

      drawText(
        "pos = " +
          printVec2(bestObject.pos) +
          (bestObject.angle > 0 ? "  " : " ") +
          ((bestObject.angle * 180) / PI).toFixed(1) +
          "°",
        (pos = pos.add(height)),
        ...args,
      );
      drawText(
        "vel = " + printVec2(bestObject.velocity),
        (pos = pos.add(height)),
        ...args,
      );
      drawText(
        "size = " + printVec2(bestObject.size),
        (pos = pos.add(height)),
        ...args,
      );
      drawText(
        "collision = " + getTileCollisionData(mousePosWorld),
        (pos = mousePosWorld.subtract(height)),
        ...args,
      );
    }
...

  {
    ...
    debugRects.forEach((r) => {
      ...
      if (r.size == 0 || (r.size.x === 0 && r.size.y === 0)) {
        ...
        // rect
        const w = (r.size.x * cameraScale) | 0,
          h = (r.size.y * cameraScale) | 0;
...
let debugParticleEditor = 0,
  debugParticleSystem,
  debugParticleSystemDiv,
  particleSystemCode;
...
const debugToggleParticleEditor = () => {
  ...
  if (!debugParticleSystemDiv) {
    ...
    for (const setting of debugParticleSettings) {
      ...
      } else input.value = debugParticleSystem[name] || "0";
      ...
      input.oninput = (e) => {
        ...
        if (type) {
          if (type == "color") {
            ...
            debugParticleSystem.tileSize = vec2(
              parseInt(input.value),
              debugParticleSystem.tileSize.y,
            );
          } else if (name == "tileSizeY") {
            debugParticleSystem.tileSize.y = vec2(
              debugParticleSystem.tileSize.x,
              parseInt(input.value),
            );
          }
        } else debugParticleSystem[name] = inputFloat;
    ...
    button.onclick = (e) =>
      navigator.clipboard.writeText(particleSystemCode.value);
    ...
    const updateCode = () => {
      ...
      for (const setting of debugParticleSettings) {
        ...
        const type = setting[1];
        let value;
        if (name == "tileSizeX" || type == "alpha") continue;
        ...
        } else value = debugParticleSystem[name];
        code += value;
      }
