...
function configureAvailableCustomDimensions03(json) {
  gameanalytics.GameAnalytics.configureAvailableCustomDimensions03(
    JSON.parse(json),
  );
}
...
function configureAvailableResourceItemTypes(json) {
  gameanalytics.GameAnalytics.configureAvailableResourceItemTypes(
    JSON.parse(json),
  );
}
...
function addProgressionEventWithScore(
  status,
  progression1,
  progression2,
  progression3,
...
function addErrorEvent(severity, message) {
  gameanalytics.GameAnalytics.addErrorEvent(Number.parseInt(severity), message);
}
...
function setGender(gender) {
  gameanalytics.GameAnalytics.setGender(Number.parseInt(gender));
}

function setBirthYear(year) {
  gameanalytics.GameAnalytics.setBirthYear(Number.parseInt(year));
}
