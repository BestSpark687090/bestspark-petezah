  ...
  M.init = function (div) {
    ...
    M.plants = {
      ...
      meddleweed: {
        ...
        onKill: function (x, y, age) {
          if (Math.random() < 0.2 * (age / 100))
            M.plot[y][x] = [
              M.plants[choose(["brownMold", "crumbspore"])].id + 1,
              0,
            ];
        },
      },
      ...
      crumbspore: {
        ...
        onDie: function (x, y) {
          var moni =
            Math.min(Game.cookies * 0.01, Game.cookiesPs * 60) * Math.random();
          if (moni != 0) {
            Game.Earn(moni);
            Game.Popup(
      ...
      doughshroom: {
        ...
        onDie: function (x, y) {
          var moni =
            Math.min(Game.cookies * 0.03, Game.cookiesPs * 60 * 5) *
            Math.random();
          if (moni != 0) {
            Game.Earn(moni);
            Game.Popup(
    ...
    M.dropUpgrade = function (upgrade, rate) {
      if (
        ...
        Math.random() <=
          rate *
            Game.dropRateMult() *
            (Game.HasAchiev("Seedless to nay") ? 1.05 : 1)
      ) {
        Game.Unlock(upgrade);
      }
    };
    ...
    M.getMuts = function (neighs, neighsM) {
      ...
      if (neighsM["bakerWheat"] >= 1 && neighsM["thumbcorn"] >= 1)
        muts.push(["cronerice", 0.01]);
      if (neighsM["thumbcorn"] >= 2)
        muts.push(["thumbcorn", 0.1], ["bakerWheat", 0.05]);
      if (neighsM["cronerice"] >= 1 && neighsM["thumbcorn"] >= 1)
        muts.push(["gildmillet", 0.03]);
      ...
      if (neighsM["clover"] >= 1 && neighsM["gildmillet"] >= 1)
        muts.push(["shimmerlily", 0.02]);
      ...
      if (neighsM["shimmerlily"] >= 1 && neighsM["cronerice"] >= 1)
        muts.push(["elderwort", 0.01]);
      if (neighsM["wrinklegill"] >= 1 && neighsM["cronerice"] >= 1)
        muts.push(["elderwort", 0.002]);
      if (neighsM["bakerWheat"] >= 1 && neighs["brownMold"] >= 1)
        muts.push(["chocoroot", 0.1]);
      if (neighsM["chocoroot"] >= 1 && neighs["whiteMildew"] >= 1)
        muts.push(["whiteChocoroot", 0.1]);
      if (neighsM["whiteMildew"] >= 1 && neighs["brownMold"] <= 1)
        muts.push(["brownMold", 0.5]);
      if (neighsM["brownMold"] >= 1 && neighs["whiteMildew"] <= 1)
        muts.push(["whiteMildew", 0.5]);
      if (neighsM["meddleweed"] >= 1 && neighs["meddleweed"] <= 3)
        muts.push(["meddleweed", 0.15]);
      ...
      if (neighsM["chocoroot"] >= 1 && neighsM["keenmoss"] >= 1)
        muts.push(["drowsyfern", 0.005]);
      if (
        (neighsM["cronerice"] >= 1 && neighsM["keenmoss"] >= 1) ||
        (neighsM["cronerice"] >= 1 && neighsM["whiteMildew"] >= 1)
      )
        muts.push(["wardlichen", 0.005]);
      if (neighsM["wardlichen"] >= 1 && neighs["wardlichen"] < 2)
        muts.push(["wardlichen", 0.05]);
      if (neighsM["greenRot"] >= 1 && neighsM["brownMold"] >= 1)
        muts.push(["keenmoss", 0.1]);
      if (neighsM["keenmoss"] >= 1 && neighs["keenmoss"] < 2)
        muts.push(["keenmoss", 0.05]);
      if (neighsM["chocoroot"] >= 1 && neighsM["bakeberry"] >= 1)
        muts.push(["queenbeet", 0.01]);
      if (neighsM["queenbeet"] >= 8) muts.push(["queenbeetLump", 0.001]);
      if (neighsM["queenbeet"] >= 2) muts.push(["duketater", 0.001]);

      if (neighsM["crumbspore"] >= 1 && neighs["crumbspore"] <= 1)
        muts.push(["crumbspore", 0.07]);
      if (neighsM["crumbspore"] >= 1 && neighsM["thumbcorn"] >= 1)
        muts.push(["glovemorel", 0.02]);
      if (neighsM["crumbspore"] >= 1 && neighsM["shimmerlily"] >= 1)
        muts.push(["cheapcap", 0.04]);
      if (neighsM["doughshroom"] >= 1 && neighsM["greenRot"] >= 1)
        muts.push(["foolBolete", 0.04]);
      ...
      if (neighsM["crumbspore"] >= 1 && neighsM["brownMold"] >= 1)
        muts.push(["wrinklegill", 0.06]);
      if (neighsM["whiteMildew"] >= 1 && neighsM["clover"] >= 1)
        muts.push(["greenRot", 0.05]);
      ...
      if (neighs["shriekbulb"] >= 1 && neighs["shriekbulb"] < 2)
        muts.push(["shriekbulb", 0.005]);

      if (neighsM["bakerWheat"] >= 1 && neighsM["whiteChocoroot"] >= 1)
        muts.push(["tidygrass", 0.002]);
      if (neighsM["tidygrass"] >= 3 && neighsM["elderwort"] >= 3)
        muts.push(["everdaisy", 0.002]);
      if (neighsM["elderwort"] >= 1 && neighsM["crumbspore"] >= 1)
        muts.push(["ichorpuff", 0.002]);
      ...
      info: {
        ...
        descFunc: function () {
          ...
          if (M.freeze) str = "Your garden is frozen, providing no effects.";
            ...
            if (effStr == "")
              effStr =
                '<div style="font-size:10px;margin-left:64px;"><b>None.</b></div>';
    ...
    M.getPlantDesc = function (me) {
      ...
      if (me.children.length > 0) {
        ...
        for (var i in me.children) {
          if (!M.plants[me.children[i]])
            console.log("No plant named " + me.children[i]);
          else {
      ...
      return (
        ...
        (!me.immortal
          ? '<div style="margin:6px 0px;font-size:11px;"><b>Average lifespan :</b> ' +
            Game.sayTime(
              (100 / (me.ageTick + me.ageTickR / 2)) * M.stepT * 30,
              -1,
            ) +
            " <small>(" +
        ...
        " <small>(" +
        Beautify(
          Math.ceil((100 / (me.ageTick + me.ageTickR / 2)) * (me.mature / 100)),
        ) +
        " ticks)</small></div>" +
        (me.weed
          ? '<div style="margin:6px 0px;font-size:11px;"><b>Is a weed</b></div>'
          : "") +
        (me.fungus
          ? '<div style="margin:6px 0px;font-size:11px;"><b>Is a fungus</b></div>'
          : "") +
        (me.detailsStr
      ...
      return function () {
        ...
        var str =
          ...
          (M.parent.amount < me.req
            ? '<div style="text-align:center;">Soil unlocked at ' +
              me.req +
              " farms.</div>"
              ...
              (M.soil == me.id
                ...
                : M.nextSoil > Date.now()
                  ? "You will be able to change your soil again in " +
                    Game.sayTime(
                      ((M.nextSoil - Date.now()) / 1000) * 30 + 30,
                      -1,
                    ) +
                    "."
          ...
          var str =
            ...
            (M.plotBoost[y][x] != [1, 1, 1]
              ? "<small>" +
                (M.plotBoost[y][x][0] != 1
                  ? "<br>Aging multiplier : " +
                    Beautify(M.plotBoost[y][x][0] * 100) +
                    "%"
                  : "") +
                (M.plotBoost[y][x][1] != 1
                  ? "<br>Effect multiplier : " +
                    Beautify(M.plotBoost[y][x][1] * 100) +
                    "%"
                  : "") +
                (M.plotBoost[y][x][2] != 1
          ...
          var str =
            ...
            (stage < 4
              ? "Mature in about " +
                Game.sayTime(
                  (100 /
                    (M.plotBoost[y][x][0] * (me.ageTick + me.ageTickR / 2))) *
        ...
        AddEvent(
          ...
          (function (me) {
            return function () {
              ...
              M.nextSoil =
                Date.now() +
                (Game.Has("Turbo-charged soil") ? 1 : 1000 * 60 * 10);
              M.toCompute = true;
              M.soil = me.id;
              M.computeStepT();
              for (var i in M.soils) {
    ...
    M.getTile = function (x, y) {
      if (x < 0 || x > 5 || y < 0 || y > 5 || !M.isTileUnlocked(x, y))
        return [0, 0];
      return M.plot[y][x];
    };
    ...
    M.askConvert = function () {
      ...
      Game.Prompt(
        ...
        [
          [
            "Yes!",
            "Game.ClosePrompt();Game.ObjectsById[" +
              M.parent.id +
              "].minigame.convert();",
          ],
          "No",
    ...
    str +=
      '<div id="gardenDrag"><div id="gardenCursor" class="shadowFilter"></div></div>';

    str += '<div id="gardenPanel" class="framed">';
    str +=
      '<div class="title gardenPanelLabel">Tools</div><div class="line"></div>';
  ...
  M.draw = function () {
    ...
    if (M.cursorL) {
      if (!M.cursor || M.seedSelected < 0) {
        ...
        M.cursorL.style.backgroundPosition =
          -icon[0] * 48 + "px " + -icon[1] * 48 + "px";
    ...
    if (Game.drawT % 10 == 0) {
      ...
      if (M.freeze)
        l("gardenNextTick").innerHTML = "Garden is frozen. Unfreeze to resume.";
      else
        l("gardenNextTick").innerHTML =
          "Next tick in " +
          Game.sayTime(((M.nextStep - Date.now()) / 1000) * 30 + 30, -1) +
          "";
      ...
      for (var i in M.soils) {
        var me = M.soils[i];
        if (M.parent.amount < me.req)
          l("gardenSoil-" + me.id).classList.add("disabled");
