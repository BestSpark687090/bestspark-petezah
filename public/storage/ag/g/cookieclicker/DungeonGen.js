...
var DungeonGen = function () {
  ...
  new this.Pattern("Pillars", function (x, y, room) {
    if ((x + room.x) % 2 == 0 && (y + room.y) % 2 == 0 && Math.random() < 0.8)
      return TILE_PILLAR;
    return 0;
  });
  new this.Pattern("Large pillars", function (x, y, room) {
    if ((x + room.x) % 3 < 2 && (y + room.y) % 3 < 2 && Math.random() < 0.8)
      return TILE_PILLAR;
    return 0;
  });
  new this.Pattern("Sparse pillars", function (x, y, room) {
    if ((x + room.x) % 3 == 0 && (y + room.y) % 3 == 0 && Math.random() < 0.8)
      return TILE_PILLAR;
    return 0;
  });
  new this.Pattern("Lines", function (x, y, room) {
    if (room.x % 2 == 0)
      if ((x + room.x) % 2 == 0 && Math.random() < 0.98) return TILE_PILLAR;
    if (room.x % 2 == 1)
      if ((y + room.y) % 2 == 0 && Math.random() < 0.98) return TILE_PILLAR;
    return 0;
  });
  ...
  this.Map.prototype.getFloorTileInRoom = function (room) {
    ...
    for (var i in room.tiles) {
      if (
        room.tiles[i].type == TILE_FLOOR_EDGE ||
        room.tiles[i].type == TILE_FLOOR_CENTER
      )
  ...
  this.Map.prototype.canPlaceRoom = function (rx, ry, rw, rh) {
    if (rx < 2 || ry < 2 || rx + rw >= this.w - 1 || ry + rh >= this.h - 1)
      return false;
  ...
  this.Map.prototype.expandRoom = function (room, rx, ry, rw, rh) {
    ...
    for (var x = rx; x < rx + rw; x++) {
      for (var y = ry; y < ry + rh; y++) {
        if (
          room.tiles[this.getRoomTile(room, x, y)].type == TILE_FLOOR_CENTER
        ) {
          var tile = 0;
  ...
  this.Map.prototype.planRoom = function (room) {
    ...
    for (var i = 0; i < branches; i++) {
      ...
      steps = Math.max(
        room.w + room.h,
        Math.ceil(steps * (1 - Math.random() * this.sizeVariance)),
      );
  ...
  this.Map.prototype.carve = function (room) {
    ...
    for (var i in room.tiles) {
      ...
      if (
        (type == TILE_WALL || type == TILE_WALL_CORNER) &&
        this.isWall(x, y) != -1
      ) {
      ...
      if (
        ...
        if (type == TILE_FLOOR_EDGE || type == TILE_FLOOR_CENTER)
  ...
  this.Map.prototype.getDeepestRoom = function () {
    ...
    for (var i in this.rooms) {
      if (
        this.rooms[i].gen + Math.sqrt(this.rooms[i].freeTiles) * 0.05 >=
          deepest &&
  ...
  this.Map.prototype.dig = function () {
    ...
    if (this.digs == 0) {
      ...
      var h = rand(3, 7);
      var room = this.newRoom(
        Math.floor(this.w / 2 - w / 2),
        Math.floor(this.h / 2 - h / 2),
        w,
        h,
      );
      room.corridor = 0;
    ...
    if (finished == 1) {
      ...
      for (var i = 0; i < 10; i++) {
        var newRoom = this.newRandomRoom({
          corridor: 0,
          w: rand(3, 7),
          h: rand(3, 7),
        });
  ...
  this.Map.prototype.getPic = function (x, y) {
    ...
    if (Tiles[this.data[x][y][2]]) {
      if (Tiles[this.data[x][y][2]].joinType == "join") {
        ...
        else if (this.data[x][y][0] == TILE_DOOR)
          joinWith.push(TILE_WALL, TILE_WALL_CORNER);
    ...
    for (var y = 0; y < this.h; y++) {
      for (var x = 0; x < this.w; x++) {
        ...
        if (room != -1) {
          ...
          if (
            this.data[x][y][0] == TILE_ENTRANCE ||
            this.data[x][y][0] == TILE_EXIT
          )
            opacity = 1;
          title =
