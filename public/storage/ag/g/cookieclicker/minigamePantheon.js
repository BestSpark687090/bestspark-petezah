...
M.launch = function () {
  ...
  M.init = function (div) {
    ...
    M.gods = {
      ...
      ages: {
        ...
        activeDescFunc: function () {
          ...
          if (godLvl == 1)
            mult *=
              0.15 *
              Math.sin((Date.now() / 1000 / (60 * 60 * 3)) * Math.PI * 2);
          else if (godLvl == 2)
            mult *=
              0.15 *
              Math.sin((Date.now() / 1000 / (60 * 60 * 12)) * Math.PI * 2);
          else if (godLvl == 3)
            mult *=
              0.15 *
              Math.sin((Date.now() / 1000 / (60 * 60 * 24)) * Math.PI * 2);
          return (
            "Current bonus : " +
            (mult < 0 ? "-" : "+") +
            Beautify(Math.abs(mult) * 100, 2) +
            "%."
          );
        },
        descBefore:
      ...
      seasons: {
        ...
        icon: [25, 18],
        descBefore:
          '<span class="green">Some seasonal effects are boosted.</span>',
        desc1:
      ...
      scorn: {
        ...
        desc3:
          '<span class="green">Wrinklers appear 50% faster and digest 5% more cookies.</span>',
        quote:
        ...
        activeDescFunc: function () {
          if (Game.BuildingsOwned % 10 == 0)
            ...
            return (
              "Buildings owned : " +
              Beautify(Game.BuildingsOwned) +
              ".<br>Effect is inactive."
            );
        },
        desc1: '<span class="green">Sugar lumps ripen an hour sooner.</span>',
        desc2:
          '<span class="green">Sugar lumps ripen 40 minutes sooner.</span>',
        desc3:
          '<span class="green">Sugar lumps ripen 20 minutes sooner.</span>',
        descAfter:
    ...
    M.godTooltip = function (id) {
      return function () {
        ...
        var str =
          ...
          (me.descBefore
            ? '<div class="templeEffect">' + me.descBefore + "</div>"
            : "") +
          (me.desc1
          ...
          (me.desc3
            ...
            : "") +
          (me.descAfter
            ? '<div class="templeEffect">' + me.descAfter + "</div>"
            : "") +
    ...
    M.hoverSlot = function (what) {
      ...
      if (M.dragging) {
        if (M.slotHovered == -1)
          l("templeGodPlaceholder" + M.dragging.id).style.display =
            "inline-block";
    ...
    for (var i in M.gods) {
      ...
      str +=
        ...
        '"></div></div>';
      str +=
        '<div class="templeGodPlaceholder" id="templeGodPlaceholder' +
        me.id +
        '"></div>';
  ...
  M.draw = function () {
    ...
    if (M.dragging) {
      ...
      l("templeGod" + M.dragging.id).style.transform =
        "translate(" + x + "px," + y + "px)";
    }
    var t = 1000 * 60 * 60;
    if (M.swaps == 0) t = 1000 * 60 * 60 * 16;
    ...
    M.swapsL.innerHTML =
      ...
      "</span>" +
      (M.swaps < 3
        ? " (next in " + Game.sayTime((t2 / 1000 + 1) * Game.fps, -1) + ")"
        : "");
  };
