...
class JobWorker {
  ...
  _FindAvailableJob() {
    for (let i = 0, len = self.jobQueue.length; i < len; ++i) {
      const job = self.jobQueue[i];
      if (
        typeof job["maxWorkerNum"] === "number" &&
        this._number >= job["maxWorkerNum"]
      )
        continue;
      if (
        !job["isBroadcast"] ||
        (this._number < job["doneFlags"].length &&
          !job["doneFlags"][this._number])
      )
        return i;
    }
    return -1;
...
function OnInputPortMessage(e) {
  ...
  if (msg["isBroadcast"]) {
    ...
    const useWorkerCount =
      typeof maxWorkerNum === "number"
        ? Math.min(maxWorkerNum, curWorkerCount)
        : curWorkerCount;
    msg["doneFlags"] = new Array(useWorkerCount).fill(false);
    msg["transferables"] = [];
    self.lastBroadcasts.delete(msg["type"]);
