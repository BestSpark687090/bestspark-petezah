...
("use strict");
{
  ...
  window.RuntimeInterface = class RuntimeInterface {
    ...
    async _Init(opts) {
      ...
      else if (this.IsWebView2Wrapper()) {
    ...
    _InitWrapper() {
      if (!this.IsWebView2Wrapper()) return Promise.resolve();
      return new Promise((resolve) => {
  ...
  const HANDLER_CLASS = class BrowserDOMHandler extends self.DOMHandler {
    ...
    _OnNavigate(e) {
      ...
      if (type === "back")
        ...
        else if (exportType === "preview" || this._iRuntime.IsWebView2Wrapper())
    ...
    _OnRequestFullscreen(e) {
      if (
        this._iRuntime.IsWebView2Wrapper() ||
