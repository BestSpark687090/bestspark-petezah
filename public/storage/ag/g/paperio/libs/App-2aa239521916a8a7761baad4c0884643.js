...
var App = (function () {
  ...
  App.prototype.onFrameworkInfoHandler = function (event) {
    MGDelegate.removeEventListener(
      MGEvent.FRAMEWORK_INFO_RESPONSE || "FRAMEWORK_INFO_RESPONSE",
      this.onFrameworkInfoHandler,
      this,
    );
    ...
    App.ScreenshotEnabled = App.frameworkInfo["HasScreenshot"];
    App.CreditsEnabled = App.frameworkInfo.hasOwnProperty("showCredits")
      ? App.frameworkInfo["showCredits"]
      : true;
    App.MoreGamesButtonEnabled = App.frameworkInfo.hasOwnProperty(
      "showMoreGamesButton",
    )
      ? App.frameworkInfo["showMoreGamesButton"]
      : true;
    App.FullscreenEnabled = App.frameworkInfo.hasOwnProperty(
      "fullscreenEnabled",
    )
      ? App.frameworkInfo["fullscreenEnabled"]
      : true;
  ...
  App.Screenshot = function (
    ...
    if (!newCanvas) {
      ...
      newCanvas
        .getContext("2d")
        .clearRect(0, 0, newCanvas.width, newCanvas.height);
      newCanvas.width = clipBounds.width;
      newCanvas.height = clipBounds.height;
    }
    setTimeout(function () {
      try {
        ...
        newCanvas
          .getContext("2d")
          .putImageData(data, 0, 0, 0, 0, clipBounds.width, clipBounds.height);
        var evt = new MGEvent(MGEvent.SCREENSHOT || "SCREENSHOT");
        evt.data = {
          rect: clipBounds,
          msg: msg,
