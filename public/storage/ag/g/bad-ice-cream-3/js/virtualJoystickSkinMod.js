...
(function () {
  ...
  scope.getPortraitOffset = function () {
    ...
    const isIpad =
      (!landscape && aspect >= 0.75) || (landscape && aspect <= 1.34);
  ...
  scope.placeSprites = function () {
    scope.edgeLeft = scope.target.domElement.parentNode.appendChild(
      scope.createTile(scope.skinnerObject.sprites["edge.png"]),
    );
    scope.edgeRight = scope.target.domElement.parentNode.appendChild(
      scope.createTile(scope.skinnerObject.sprites["edge.png"]),
    );

    scope.logo = scope.target.domElement.parentNode.appendChild(
      scope.skinnerObject.createImage(
        scope.skinnerObject.sprites["nitrome_small.png"],
      ),
    );
    ...
    scope.logo.addEventListener("touchstart", () => {
      ...
      if (scope.logoPresses === 3) {
        const gameboySkin =
          "sepia(1) hue-rotate(20deg) brightness(0.65) saturate(0.9)";
        document.documentElement.style.transition =
          "filter 0.5s cubic-bezier(.96,.29,.83,.67) 0s";
        if (document.documentElement.style.filter === gameboySkin)
    ...
    if (!scope.isOnPoki) {
      scope.home = scope.target.domElement.parentNode.appendChild(
        scope.skinnerObject.createImage(
          scope.skinnerObject.sprites["home.png"],
        ),
      );
    ...
    const pauseButtonImage = scope.skinnerObject.createImage(
      scope.skinnerObject.sprites["pause.png"],
    );
    ...
    scope.pauseBut.ontouchstart = () => {
      ...
      setTimeout(() => {
        scope.target.triggerKeyEvent("keyup", "P".charCodeAt(0));
      }, 50);
      try {
        if (scope.gameCanvas) {
          ...
          if (scope.isPaused) {
            ...
            touchEvent.touches = touchEvent.changedTouches = [
              {
                clientX: bounds.left + bounds.width - 10,
    ...
    scope.pokiLogo = scope.target.domElement.parentNode.appendChild(
      scope.skinnerObject.createImage(
        scope.skinnerObject.sprites["poki_large.png"],
      ),
    );
      ...
      const searchCanvasInterval = setInterval(() => {
        ...
        if (canvas) {
          ...
          scope.gameCanvas.addEventListener("touchstart", () => {
            if (
              !scope.cancelNextPress &&
              parseFloat(scope.target.domElement.style.opacity) <=
                scope.target.settings.offOpacity
            ) {
