var Loader = (function () {
  ...
  function runLoadingProcess(
    ...
    const reporters = Array.from({ length: jsCount + binCount }, () =>
      createReporter(),
    );
  ...
  function runGame(progressEvent = (f) => f, completeEvent = (f) => f) {
    ...
    const avmReporter = createReporter(
      (f) => {
        console.log("AVM Load", f);
      },
      null,
      __config.progressParserWeigth ? __config.progressParserWeigth : 0.001,
    );
    ...
    const complete = (f) => {
      ...
      if (__config.start) {
        ...
        let onCLick = (e) => {
          ...
          if (!f) throw "PokiPlayer did not send a callback for starting game";
          f();
          window.setTimeout(() => {
    ...
    runLoadingProcess(jss, bins, loadReporter.report, __config.debug).then(
      (data) => {
    ...
    handleResize = () => {
      let x =
        typeof config.x === "string"
          ? (parseFloat(config.x.replace("%", "")) / 100) * window.innerWidth
          : config.x;
      let y =
        typeof config.y === "string"
          ? (parseFloat(config.y.replace("%", "")) / 100) * window.innerHeight
          : config.y;
      let w =
        typeof config.w === "string"
          ? (parseFloat(config.w.replace("%", "")) / 100) * window.innerWidth
          : config.w;
      let h =
        typeof config.h === "string"
          ? (parseFloat(config.h.replace("%", "")) / 100) * window.innerHeight
          : config.h;
