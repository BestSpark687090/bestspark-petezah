...
async function GetMainPageUrl() {
  ...
  for (const b of a) {
    let a = b.url;
    if (
      (a.startsWith(self.registration.scope) &&
        (a = a.substring(self.registration.scope.length)),
      a && "/" !== a)
    )
...
async function CreateCacheFromFileList(a, b, c) {
  ...
  try {
    ...
    throw (
      console.error("[SW] Error writing cache entries: ", b),
      caches.delete(a),
      b
    );
  }
...
async function UpdateCheck(a) {
  try {
    ...
    if (!b.ok)
      throw new Error(
        "offline.json responded with " + b.status + " " + b.statusText,
      );
