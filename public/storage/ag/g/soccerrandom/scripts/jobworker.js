...
function SendResult(a, b) {
  if (!a) {
    ...
    self.outputPort.postMessage(
      { type: "result", jobId: self.activeJobId, result: b.result },
      a,
    );
  }
  SendDone();
...
function OnDispatchWorkerMessage(a) {
  ...
  if ("_send_buffer" === c)
    return void self.sentBuffers.set(b["id"], b["buffer"]);
  ...
  g && g.then
    ? g
        .then((a) => SendResult(f, a))
        .catch((a) => SendError(f, "Rejection in job handler: " + a))
    : SendResult(f, g);
}
