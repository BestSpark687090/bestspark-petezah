    ...
    constructor(opts) {
      ...
      if (this._exportType === "cordova" && this._useWorker)
        if (isAndroid) {
          ...
          if (!chromeVer || !(Number.parseInt(chromeVer[1], 10) >= 90))
            this._useWorker = false;
...
("use strict");
{
  ...
  self["C3_GetSvgImageSize"] = async (blob) => {
  ...
  self["C3_RasterSvgImageBlob"] = async (
    blob,
    imageWidth,
    imageHeight,
    surfaceWidth,
    ...
    constructor(iRuntime) {
      ...
      if (iRuntime.IsiOSWebView())
        if (window["visualViewport"]) {
          let lastVisualViewportHeight = Number.POSITIVE_INFINITY;
  ...
  const HANDLER_CLASS = class PokiHandler extends self.DOMHandler {
    ...
    NotifyGameplayStart() {
      ...
      if (!this._finishedLoadingSent) {
        console.log(
          "Don't forget to send the Loading Finished Notification, or configure the Poki Plugin to manage it automatically.",
        );
      }
