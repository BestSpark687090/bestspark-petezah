...
const b = {
  ...
  fireAlwaysFire() {
    //added  && player.speed < 0.5 && m.onGround  //removed input.fire && (!input.field || m.fieldFire)
    if (b.inventory.length && b.activeGun !== null) {
      if (
        m.fireCDcycle < m.cycle &&
        player.speed < 0.5 &&
  ...
  fireWithAmmo() {
    //triggers after firing when you have ammo
  ...
  outOfAmmo() {
    //triggers after firing when you have NO ammo
    simulation.makeTextLog(
      `${b.guns[b.activeGun].name}.<span class='color-g'>ammo</span><span class='color-symbol'>:</span> 0`,
    );
    ...
    if (tech.isAmmoFromHealth) {
      ...
      if (tech.isEnergyHealth) {
        if (m.maxEnergy > amount * 2) {
          ...
          for (let i = 0; i < 4; i++)
            powerUps.spawn(
              m.pos.x + 50 * (Math.random() - 0.5),
              m.pos.y + 50 * (Math.random() - 0.5),
              "ammo",
            );
        }
      } else {
  ...
  refundAmmo() {
    //triggers after firing when you removed ammo for a gun, but didn't need to
  ...
  giveGuns(gun = "random", ammoPacks = 22) {
    ...
    if (tech.isOneGun && b.inventory > 0) {
      ...
      for (let i = 0, len = tech.tech.length; i < len; i++) {
        if (
          tech.tech[i].isGunTech &&
          tech.tech[i].count > 0 &&
          !tech.tech[i].isInstant &&
          !tech.tech[i].isRemoveGun
        ) {
  ...
  setFireCD() {
    b.fireCDscale =
      tech.fireRate * tech.slowFire * tech.researchHaste * tech.aimDamage;
    if (m.fieldMode === 6) b.fireCDscale *= 0.8;
    if (tech.isFastTime) b.fireCDscale *= 0.666;
    if (tech.isFireRateForGuns)
      b.fireCDscale *= Math.pow(0.76923, Math.max(0, b.inventory.length - 1));
  ...
  explosionRange() {
    return (
      tech.explosiveRadius *
      (tech.isExplosionHarm ? 1.7 : 1) *
      (tech.isSmallExplosion ? 0.66 : 1) *
      (tech.isExplodeRadio ? 1.25 : 1)
    );
  },
  explosion(where, radius, color = "rgba(255,25,0,0.6)") {
    ...
    if (tech.isExplodeRadio) {
      ...
      if (tech.isSmartRadius)
        radius = Math.max(
          Math.min(
            radius,
            Vector.magnitude(Vector.sub(where, player.position)) - 25,
          ),
          1,
        );
      ...
      if (Vector.magnitude(Vector.sub(where, player.position)) < radius) {
        const DRAIN =
          (tech.isExplosionHarm ? 0.6 : 0.45) *
          (tech.isRadioactiveResistance ? 0.2 : 1);
        if (m.immuneCycle < m.cycle) m.energy -= DRAIN;
        if (m.energy < 0) {
          m.energy = 0;
          if (simulation.dmgScale)
            m.damage(
              tech.radioactiveDamage *
                0.03 *
                (tech.isRadioactiveResistance ? 0.2 : 1),
            );
        }
      ...
      for (let i = 0, len = mob.length; i < len; ++i) {
        if (mob[i].alive && !mob[i].isShielded) {
          ...
          if (dist < radius) {
            ...
            if (Matter.Query.ray(map, mob[i].position, where).length > 0)
              dmg *= 0.5; //reduce damage if a wall is in the way
      ...
      if (m.immuneCycle < m.cycle) {
        ...
        if (dist < radius) {
          if (simulation.dmgScale) {
            ...
            if (tech.isImmuneExplosion && m.energy > 0.25) {
              ...
              knock = Vector.mult(
                ...
                -0.6 *
                  player.mass *
                  Math.max(0, Math.min(0.15 - 0.002 * player.speed, 0.15)),
              );
              ...
              knock = Vector.mult(
                Vector.normalise(sub),
                -Math.sqrt(dmg) * player.mass * 0.013,
              );
          ...
          knock = Vector.mult(
            Vector.normalise(sub),
            -Math.sqrt(dmg) * player.mass * 0.005,
          );
          player.force.x += knock.x;
          player.force.y += knock.y;
        }
      ...
      for (let i = body.length - 1; i > -1; i--) {
        if (!body[i].isNotHoldable) {
          ...
          if (dist < radius) {
            knock = Vector.mult(
              Vector.normalise(sub),
              -Math.sqrt(dmg) * body[i].mass * 0.022,
            );
            ...
            knock = Vector.mult(
              Vector.normalise(sub),
              -Math.sqrt(dmg) * body[i].mass * 0.011,
            );
            body[i].force.x += knock.x;
            body[i].force.y += knock.y;
          }
      ...
      for (let i = 0, len = powerUp.length; i < len; ++i) {
        ...
        if (dist < radius) {
          knock = Vector.mult(
            Vector.normalise(sub),
            -Math.sqrt(dmg) * powerUp[i].mass * 0.013,
          );
          ...
          knock = Vector.mult(
            Vector.normalise(sub),
            -Math.sqrt(dmg) * powerUp[i].mass * 0.007,
          );
          powerUp[i].force.x += knock.x;
          powerUp[i].force.y += knock.y;
        }
      ...
      for (let i = 0, len = mob.length; i < len; ++i) {
        if (mob[i].alive && !mob[i].isShielded) {
          ...
          if (dist < radius) {
            ...
            knock = Vector.mult(
              Vector.normalise(sub),
              -Math.sqrt(dmg * damageScale) *
                mob[i].mass *
                (mob[i].isBoss ? 0.003 : 0.01),
            );
            if (tech.isStun) {
  ...
  clusterExplode(where, size) {
    //can occur after grenades detonate
    const cycle = () => {
      if (m.alive) {
        if (simulation.paused || m.isBodiesAsleep) {
          ...
          if (!(count % 7)) {
            ...
            b.explosion(
              Vector.add(
                where,
                Vector.mult(unit, size * (count * 0.01 + 0.03 * Math.random())),
              ),
              size * (0.4 + Math.random() * 0.35),
              `hsla(${360 * Math.random()},100%,66%,0.6)`,
      ...
      if (m.alive) {
        if (simulation.paused || m.isBodiesAsleep) {
          ...
          if (count % 2) {
            ...
            b.explosion(
              Vector.add(where, Vector.mult(unit, size * 0.75)),
              size * 0.7,
              color,
            ); //makes bullet do explosive damage at end
          }
        ...
        if (simulation.paused || m.isBodiesAsleep) {
          ...
          if (count === 8) {
            ...
            for (let i = 0, len = 6; i < len; i++) {
              ...
              b.explosion(
                Vector.add(where, Vector.mult(unit, 1.1 * range)),
                size * 0.6,
                color,
              ); //makes bullet do explosive damage at end
            }
          ...
          if (count === 16) {
            ...
            for (let i = 0, len = 10; i < len; i++) {
              ...
              b.explosion(
                Vector.add(where, Vector.mult(unit, 1.4 * range)),
                size * 0.45,
                color,
              ); //makes bullet do explosive damage at end
            }
  ...
  grenadeEnd() {
    ...
    if (tech.fragments)
      b.targetedNail(
        this.position,
        tech.fragments * Math.floor(2 + 1.5 * Math.random()),
      );
  },
  grenade() {},
  setGrenadeMode() {
    grenadeDefault = function (
      where = {
      ...
      bullet[me].endCycle =
        simulation.cycle +
        Math.floor(m.crouch ? 120 : 80) * tech.bulletsLastLonger;
    ...
    grenadeRPG = function (
      ...
      bullet[me].do = function () {
        ...
        if (
          ...
          Matter.Query.collides(this, body).length
        ) {
          this.endCycle = 0; //explode if touching map or blocks
        }
      };
    };
    grenadeRPGVacuum = function (
      where = {
        x: m.pos.x + 30 * Math.cos(m.angle),
        y: m.pos.y + 30 * Math.sin(m.angle),
      },
      ...
      bullet[me].explodeRad =
        350 * size + Math.floor(Math.random() * 50) + tech.isBlockExplode * 100;
      bullet[me].onEnd = b.grenadeEnd;
      bullet[me].minDmgSpeed = 1;
      bullet[me].beforeDmg = function () {
        this.endCycle = 0; //bullet ends cycle after doing damage  //this also triggers explosion
      ...
      bullet[me].suck = function () {
        ...
        //draw suck
        const radius =
          (2.75 * this.explodeRad * (this.endCycle - simulation.cycle)) /
          this.suckCycles;
      ...
      bullet[me].do = function () {
        if (simulation.cycle > this.endCycle - this.suckCycles) {
          //suck
          this.do = this.suck;
        } else if (
          Matter.Query.collides(this, map).length ||
          ...
          Matter.Body.setPosition(
            this,
            Vector.sub(this.position, this.velocity),
          ); //undo last movement
          this.do = this.suck;
        } else {
          this.force.x += this.thrust.x;
    ...
    grenadeVacuum = function (
      ...
      bullet[me].do = function () {
        ...
        const suckCycles = 40;
        if (simulation.cycle > this.endCycle - suckCycles) {
          //suck
          const that = this;
    ...
    grenadeNeutron = function (
      ...
      b.fireProps(
        (m.crouch ? 45 : 25) / Math.pow(0.92, tech.missileCount),
        m.crouch ? 35 : 20,
        angle,
        me,
      ); //cd , speed
      ...
      bullet[me].radiusDecay =
        (0.81 + 0.15 * tech.isNeutronSlow) / tech.bulletsLastLonger;
      ...
      bullet[me].do = function () {
        ...
        if (mobCollisions.length) {
          ...
          if (this.stuckTo.isVerticesChange) {
            ...
            this.stuckToRelativePosition = Vector.rotate(
              Vector.sub(this.position, this.stuckTo.position),
              -this.stuckTo.angle,
            );
          }
          this.stuck = function () {
            if (this.stuckTo && this.stuckTo.alive) {
              const rotate = Vector.rotate(
                ...
                this.stuckTo.angle,
              ); //add in the mob's new angle to the relative position vector
              Matter.Body.setPosition(
                this,
                Vector.add(
                  Vector.add(rotate, this.stuckTo.velocity),
                  this.stuckTo.position,
                ),
              );
              Matter.Body.setVelocity(this, this.stuckTo.velocity); //so that it will move properly if it gets unstuck
            } else {
              this.collisionFilter.mask =
                cat.map | cat.body | cat.player | cat.mob; //non collide with everything but map
              this.stuck = function () {
      ...
      bullet[me].radiationMode = function () {
        //the do code after the bullet is stuck on something,  projects a damaging radiation field
        this.stuck(); //runs different code based on what the bullet is stuck to
        this.damageRadius =
          this.damageRadius * 0.85 + 0.15 * this.maxDamageRadius; //smooth radius towards max
        ...
        if (this.damageRadius < 15) {
          ...
          if (
            Vector.magnitude(Vector.sub(player.position, this.position)) <
            this.damageRadius
          ) {
            ...
            if (m.energy > DRAIN) {
              ...
              if (simulation.dmgScale)
                m.damage(
                  (tech.isRadioactiveResistance ? 0.00016 * 0.2 : 0.00016) *
                    tech.radioactiveDamage,
                ); //0.00015
            }
          }
          ...
          for (let i = 0, len = mob.length; i < len; i++) {
            if (
              Vector.magnitude(Vector.sub(mob[i].position, this.position)) <
              this.damageRadius + mob[i].radius
            ) {
              if (
                Matter.Query.ray(map, mob[i].position, this.position).length > 0
              )
                dmg *= 0.2; //reduce damage if a wall is in the way
      ...
      b.guns[5].do = function () {
        if (!input.field && m.crouch) {
          ...
          for (let i = 1.6, len = 19; i < len + 1; i++) {
            ...
            ctx.lineTo(
              m.pos.x + time * v.x,
              m.pos.y + time * v.y + 0.34 * time * time,
            );
          }
          ctx.stroke();
        }
  ...
  grapple(where, angle = m.angle) {
    ...
    bullet[me] = Bodies.fromVertices(
      ...
      ],
      {
        ...
        collisionFilter: {
          category: cat.bullet,
          mask: tech.isShieldPierce
            ? cat.body | cat.mob | cat.mobBullet
            : cat.body | cat.mob | cat.mobBullet | cat.mobShield,
        },
        minDmgSpeed: 4,
        // lookFrequency: Math.floor(7 + Math.random() * 3),
        ...
        powerUpDamage:
          ...
          simulation.cycle - 480 < tech.harpoonPowerUpCycle,
        draw() {
          // draw rope
          const where = {
            x: m.pos.x + 30 * Math.cos(m.angle),
          ...
          if (this.attached) {
            ...
            const perpendicular = Vector.mult(
              Vector.normalise(Vector.perp(sub)),
              this.drawStringFlip *
                Math.min(
                  0.7 * long,
                  10 +
                    this.drawStringControlMagnitude /
                      (10 + Vector.magnitude(sub)),
                ),
            );
            const controlPoint = Vector.add(
              Vector.add(where, Vector.mult(sub, -0.5)),
              perpendicular,
            );
            ctx.quadraticCurveTo(
              controlPoint.x,
          ...
          ctx.beginPath();
          ctx.lineTo(this.vertices[0].x, this.vertices[0].y);
          const spike = Vector.add(
            this.vertices[3],
            Vector.mult(Vector.sub(this.vertices[3], this.vertices[2]), 2),
          );
        ...
        beforeDmg(who) {
          if (tech.isShieldPierce && who.isShielded) {
            //disable shields
            who.isShielded = false;
          ...
          if (tech.hookNails) {
            ...
            for (let i = 0; i < 4; i++)
              b.nail(
                this.position,
                { x: 10.5 * Math.cos(ANGLE), y: 10.5 * Math.sin(ANGLE) },
                1.2,
              );
          }
        ...
        onEnd() {
          if (
            ...
            if (index !== null) {
              ...
              if (tech.isHarpoonPowerUp)
                tech.harpoonPowerUpCycle = simulation.cycle;
            } else {
        ...
        retract() {
          ...
          if (this.angularSpeed < 0.5)
            this.torque += this.inertia * 0.001 * (Math.random() - 0.5); //(Math.round(Math.random()) ? 1 : -1)
          this.collisionFilter.mask = 0; //cat.map | cat.mob | cat.mobBullet | cat.mobShield // | cat.body
          //recoil on pulling grapple back
          const mag = this.pickUpTarget
            ? Math.max(this.pickUpTarget.mass, 0.5)
            : 0.5;
          const momentum = Vector.mult(
            Vector.sub(this.position, m.pos),
            mag * (m.crouch ? 0.0001 : 0.0002),
          );
        ...
        returnToPlayer() {
          ...
          if (
            Vector.magnitude(Vector.sub(this.position, m.pos)) < returnRadius
          ) {
            //near player
            this.endCycle = 0;
            //recoil on catching grapple
            const momentum = Vector.mult(
              Vector.sub(this.velocity, player.velocity),
              m.crouch ? 0.0001 : 0.0002,
            );
            player.force.x += momentum.x;
            player.force.y += momentum.y;
            if (this.pickUpTarget) {
              ...
              totalMomentum = Vector.add(
                Vector.mult(player.velocity, player.mass),
                Vector.mult(
                  Vector.normalise(this.velocity),
                  15 * Math.min(20, this.pickUpTarget.mass),
                ),
              );
              Matter.Body.setVelocity(
                ...
                Vector.mult(
                  totalMomentum,
                  1 / (m.defaultMass + this.pickUpTarget.mass),
                ),
              );

              m.definePlayerMass(
                m.defaultMass + this.pickUpTarget.mass * m.holdingMassScale,
              );
            ...
            const rangeScale =
              1 + 0.000001 * Vector.magnitude(sub) * Vector.magnitude(sub); //return faster when far from player
            const returnForce = Vector.mult(
              Vector.normalise(sub),
              rangeScale * this.thrustMag * this.mass,
            );
            ...
            simulation.ephemera.push({
              ...
              do() {
                ...
                for (let j = 1; j < block.length; j++)
                  ctx.lineTo(block[j].x, block[j].y);
        ...
        grabBlocks() {
          if (this.pickUpTarget) {
            //if always attached to a block
            //position block on hook
            Matter.Body.setPosition(
              this.pickUpTarget,
              Vector.add(this.vertices[2], this.velocity),
            );
            ...
            if (blocks.length) {
              ...
              for (let i = 0; i < blocks.length; i++) {
                if (
                  blocks[i].bodyA.classType === "body" &&
                  !blocks[i].bodyA.isNotHoldable &&
                  blocks[0].bodyA.mass < 60
                ) {
                  this.retract();
                  if (tech.hookNails) {
                    // if (m.immuneCycle < m.cycle + m.collisionImmuneCycles) m.immuneCycle = m.cycle + 5; //player is immune to damage for 5 cycles
                    ...
                    simulation.ephemera.push({
                      ...
                      do() {
                        ...
                        for (let j = 1; j < blockVertices.length; j++)
                          ctx.lineTo(blockVertices[j].x, blockVertices[j].y);
                    ...
                    this.blockDist = Vector.magnitude(
                      Vector.sub(this.pickUpTarget.position, m.pos),
                    );
                  }
                }
        ...
        grabPowerUp() {
          //grab power ups near the tip of the harpoon
          if (this.caughtPowerUp) {
            Matter.Body.setPosition(
              this.caughtPowerUp,
              Vector.add(this.vertices[2], this.velocity),
            ...
            for (let i = 0, len = powerUp.length; i < len; ++i) {
              ...
              if (
                Vector.magnitudeSquared(
                  Vector.sub(this.vertices[2], powerUp[i].position),
                ) <
                radius * radius
              ) {
                if (
            ...
            if (Matter.Query.collides(this, map).length) {
              ...
              this.do = () => {
                ...
                if (input.field) {
                  ...
                  const drag =
                    1 -
                    30 / Math.min(Math.max(100, dist), 700) -
                    0.1 * (player.speed > 66);
                  // console.log(player.speed)
                  Matter.Body.setVelocity(player, {
                    x: player.velocity.x * drag,
                    y: player.velocity.y * drag,
                  });
                  const pullScale = 0.0004;
                  const pull = Vector.mult(
                    Vector.normalise(sub),
                    pullScale *
                      Math.min(Math.max(15, dist), this.isSlowPull ? 70 : 200),
                  );
    ...
    bullet[me] = Bodies.fromVertices(
      ...
      ],
      {
        ...
        collisionFilter: {
          category: cat.bullet,
          mask: tech.isShieldPierce
            ? cat.map | cat.body | cat.mob | cat.mobBullet
            : cat.map | cat.body | cat.mob | cat.mobBullet | cat.mobShield,
        },
        minDmgSpeed: 4,
        lookFrequency: Math.floor(7 + Math.random() * 3),
        ...
        beforeDmg(who) {
          ...
          if (tech.fragments) {
            b.targetedNail(
              this.vertices[2],
              tech.fragments * Math.floor(2 + Math.random()),
            );
          ...
          if (tech.isFoamBall) {
            for (
              let i = 0, len = Math.min(30, 2 + 2 * Math.sqrt(this.mass));
              i < len;
              i++
            ) {
              ...
              b.foam(
                this.position,
                Vector.rotate(velocity, 6.28 * Math.random()),
                radius,
              );
            }
          }
        ...
        returnToPlayer() {
          if (
            ...
            if (m.energy < 0.05) {
              ...
              //if (m.cycle + 20 * b.fireCDscale < m.fireCDcycle)
              this.force.x -= returnForce.x;
              this.force.y -= returnForce.y;
            }
            this.grabPowerUp();
          ...
          if (isReturn || target) {
            if (isReturn) {
              if (this.cycle > totalCycles) {
                ...
                const momentum = Vector.mult(
                  Vector.sub(this.velocity, player.velocity),
                  m.crouch ? 0.00015 : 0.0003,
                ); //recoil on jerking line
                player.force.x += momentum.x;
                player.force.y += momentum.y;
                requestAnimationFrame(() => {
                  //delay this for 1 cycle to get the proper hit graphics
                  this.collisionFilter.category = 0;
                  this.collisionFilter.mask = 0;
                });
              } else {
            ...
            if (target) {
              ...
              const vectorGoal = Vector.normalise(
                Vector.sub(this.position, target.position),
              );
  ...
  missile(where, angle, speed, size = 1) {
    ...
    bullet[me] = Bodies.rectangle(where.x, where.y, 30 * size, 4 * size, {
      ...
      endCycle:
        simulation.cycle +
        Math.floor(
          (230 + 40 * Math.random()) * tech.bulletsLastLonger +
            120 * tech.isMissileBiggest +
            60 * tech.isMissileBig,
        ),
      collisionFilter: {
        category: cat.bullet,
        mask: cat.map | cat.body | cat.mob | cat.mobBullet | cat.mobShield,
      ...
      tryToLockOn() {
        ...
        for (let i = 0, len = mob.length; i < len; ++i) {
          if (
            ...
            if (
              Vector.magnitude(
                Vector.sub(this.position, mob[i].position) < this.explodeRad,
              )
            ) {
              this.endCycle = 0; //bullet ends cycle after doing damage  //also triggers explosion
      ...
      do() {
        ...
        ctx.arc(
          this.position.x -
            Math.cos(this.angle) * (25 * size - 3) +
            (Math.random() - 0.5) * 4,
          this.position.y -
            Math.sin(this.angle) * (25 * size - 3) +
            (Math.random() - 0.5) * 4,
          11 * size,
          0,
          2 * Math.PI,
    ...
    const thrust =
      0.0066 *
      bullet[me].mass *
      (tech.isMissileBig ? (tech.isMissileBiggest ? 0.3 : 0.7) : 1);
    Matter.Body.setVelocity(bullet[me], {
      x: m.Vx / 2 + speed * Math.cos(angle),
      y: m.Vy / 2 + speed * Math.sin(angle),
    });
  ...
  extruder() {
    ...
    if (m.energy > DRAIN && b.canExtruderFire) {
      ...
      bullet[me] = Bodies.polygon(
        where.x + 20 * Math.cos(m.angle),
        where.y + 20 * Math.sin(m.angle),
        4,
        0.01,
        {
          cycle: -0.5,
          ...
          do() {
            ...
            if (Matter.Query.point(map, this.position).length) {
              //check if inside map   //|| Matter.Query.point(body, this.position).length
              this.isBranch = true;
              this.do = () => {
                if (this.endCycle < simulation.cycle + 1) this.isWave = false;
              };
            ...
            this.cycle++;
            const wiggleMag =
              (m.crouch ? 6 : 12) * Math.cos(simulation.cycle * 0.09);
            const wiggle = Vector.mult(
              transverse,
              wiggleMag * Math.cos(this.cycle * 0.36),
            ); //+ wiggleMag * Math.cos(simulation.cycle * 0.3))
            const velocity = Vector.mult(player.velocity, 0.4); //move with player
            Matter.Body.setPosition(
              this,
              Vector.add(velocity, Vector.add(this.position, wiggle)),
            );
          },
        },
      );
      ...
      if (
        180 - Math.abs(Math.abs(b.lastAngle - m.angle) - 180) > 0.13 ||
        !b.wasExtruderOn
      ) {
  ...
  plasma() {
    ...
    if (m.energy > DRAIN) {
      ...
      let range =
        tech.isPlasmaRange *
        (120 + (m.crouch ? 400 : 300) * Math.sqrt(Math.random())); //+ 100 * Math.sin(m.cycle * 0.3);
        ...
        if (best.who.alive) {
          ...
          if (best.who.speed > 3) {
            const force = Vector.mult(
              Vector.normalise(Vector.sub(m.pos, path[1])),
              -0.005 * Math.min(5, best.who.mass),
            );
            Matter.Body.applyForce(best.who, path[1], force);
            Matter.Body.setVelocity(best.who, {
              x: best.who.velocity.x * 0.4,
              y: best.who.velocity.y * 0.4,
            });
          } else {
            const force = Vector.mult(
              Vector.normalise(Vector.sub(m.pos, path[1])),
              -0.01 * Math.min(5, best.who.mass),
            );
            Matter.Body.applyForce(best.who, path[1], force);
            Matter.Body.setVelocity(best.who, {
          ...
          const force = Vector.mult(
            Vector.normalise(Vector.sub(m.pos, path[1])),
            -0.007 * Math.sqrt(Math.sqrt(best.who.mass)),
          );
          Matter.Body.applyForce(best.who, path[1], force);
        }
      }
  ...
  laser(
    ...
    dmg = tech.laserDamage,
    reflections = tech.laserReflections,
    isThickBeam = false,
    push = 1,
  ) {
    ...
    const checkForCollisions = function () {
      best = vertexCollision(path[path.length - 2], path[path.length - 1], [
        mob,
        map,
        body,
      ]);
      ...
      if (best.who.alive) {
        ...
        if (best.who.damageReduction) {
          if (
            ...
            Vector.dot(
              Vector.normalise(
                Vector.sub(best.who.position, path[path.length - 1]),
              ),
              Vector.normalise(
                Vector.sub(path[path.length - 1], path[path.length - 2]),
              ),
            ) >
              0.999 - 0.5 / best.who.radius
          ) {
        ...
        if (tech.isLaserPush) {
          ...
          const force = Vector.mult(
            Vector.normalise(
              Vector.sub(path[index], path[Math.max(0, index - 1)]),
            ),
            0.003 * push * Math.min(6, best.who.mass),
          );
  ...
  AoEStunEffect(where, range, cycles = 120 + 60 * Math.random()) {
    for (let i = 0, len = mob.length; i < len; ++i) {
      if (
        ...
        if (
          Vector.magnitude(Vector.sub(where, mob[i].position)) - mob[i].radius <
          range
        )
          mobs.statusStun(mob[i], cycles);
      }
    }
    simulation.drawList.push({
      x: where.x,
  ...
  laserMine(
    ...
    bullet[me] = Bodies.polygon(position.x, position.y, 3, 25, {
      ...
      onEnd() {},
      do() {
        if (!(simulation.cycle % this.lookFrequency) && m.energy > this.drain) {
          //find mob targets
          for (let i = 0, len = mob.length; i < len; ++i) {
            if (
              Vector.magnitude(Vector.sub(this.position, mob[i].position)) <
                1300 &&
              !mob[i].isBadTarget &&
              Matter.Query.ray(map, this.position, mob[i].position).length ===
                0 &&
              Matter.Query.ray(body, this.position, mob[i].position).length ===
                0
            ) {
              ...
              if (this.angularSpeed < 0.5)
                this.torque += this.inertia * this.torqueMagnitude * 200; //spin
      ...
      laserSpin() {
        ...
        if (m.energy > this.drain) {
          m.energy -= this.drain;
          if (this.angularSpeed < 0.05)
            this.torque += this.inertia * this.torqueMagnitude; //spin
    ...
    bullet[bIndex] = Bodies.rectangle(where.x, where.y, 45, 16, {
          ...
          if (tech.isFoamMine) {
            ...
            for (let i = 0; i < 12; i++) {
              ...
              b.foam(
                ...
                Vector.rotate(
                  velocity,
                  this.angle + 1.57 + 3 * (Math.random() - 0.5),
                ),
                radius,
              ); //6.28 * Math.random()
            }
      ...
      do() {
        ...
        if (collide.length > 0) {
          for (let i = 0; i < collide.length; i++) {
            if (collide[i].bodyA.collisionFilter.category === cat.map) {
              ...
              for (let j = 0; j < 10; j++) {
                ...
                if (Matter.Query.collides(this, map).length > 0) {
                  ...
                  setTimeout(() => {
                    if (
                      Matter.Query.collides(this, map).length === 0 ||
                      Matter.Query.point(map, this.position).length > 0
                    ) {
      ...
      arm() {
        this.collisionFilter.mask =
          ...
          cat.mobBullet |
          cat.mobShield |
          cat.bullet; //can now collide with other bullets
                ...
                for (let i = 0, len = mob.length; i < len; ++i) {
                  if (
                    ...
                    Vector.magnitude(
                      ...
                      this.range + mob[i].radius + random &&
                    ...
                    if (tech.isMineSentry) {
                      this.lookFrequency = Math.floor(
                        7 +
                          7 * b.fireCDscale +
                          10 * (tech.oneSuperBall && tech.isSuperMine) +
                      ...
                      this.do = function () {
                        ...
                        if (!(simulation.cycle % this.lookFrequency)) {
                          ...
                          if (tech.isFoamMine) {
                            ...
                            for (
                              let i = 0, len = tech.extraSuperBalls / 4;
                              i < len;
                              i++
                            ) {
                              if (Math.random() < 0.33)
  ...
  worm(where, isFreeze = tech.isSporeFreeze) {
    //used with the tech upgrade in mob.death()
    const bIndex = bullet.length;
    const wormSize = 6 + tech.wormSize * 4.2 * Math.random();
    if (bIndex < 500) {
      //can't make over 500 spores
      bullet[bIndex] = Bodies.polygon(where.x, where.y, 3, 3, {
        ...
        frictionAir: 0.025,
        thrust:
          (tech.isSporeFollow ? 0.0012 : 0.00055) *
        ...
        endCycle:
          simulation.cycle +
          Math.floor(
            (600 + Math.floor(Math.random() * 420)) * tech.bulletsLastLonger,
          ),
        minDmgSpeed: 0,
        playerOffPosition: {
          //used when moving towards player to keep spores separate
              ...
              setTimeout(() => {
                if (!who.alive) {
                  for (let i = 0; i < 3; i++) {
                    ...
                    bullet[bullet.length - 1].endCycle = Math.min(
                      simulation.cycle +
                        Math.floor(420 * tech.bulletsLastLonger),
                      this.endCycle + 180 + Math.floor(60 * Math.random()),
                    ); //simulation.cycle + Math.floor(420 * tech.bulletsLastLonger)
                  }
                }
        ...
        do() {
          ...
          const dir = Vector.mult(
            Vector.normalise(this.velocity),
            -0.6 * this.wormTail * speed,
          );
          const tail = Vector.add(this.position, dir);
          const wiggle = Vector.add(
            Vector.add(tail, dir),
            Vector.rotate(dir, Math.sin(this.tailCycle)),
          );
          ...
          if (this.lockedOn && this.lockedOn.alive) {
            ...
            if (!(simulation.cycle % this.lookFrequency)) {
              ...
              for (let i = 0, len = mob.length; i < len; ++i) {
                if (
                  ...
                  const targetVector = Vector.sub(
                    this.position,
                    mob[i].position,
                  );
                  const dist =
                    Vector.magnitude(targetVector) * (Math.random() + 0.5);
                  if (dist < closeDist) {
            ...
            if (tech.isSporeFollow && this.lockedOn === null) {
              //move towards player //checking for null means that the spores don't go after the player until it has looked and not found a target
              ...
              if (dx * dx + dy * dy > 10000) {
                this.force = Vector.mult(
                  Vector.normalise(
                    Vector.sub(
                      m.pos,
                      Vector.add(this.playerOffPosition, this.position),
                    ),
                  ),
                  this.mass * this.thrust,
                );
              ...
              const force = Vector.mult(
                Vector.rotate(unit, 0.005 * this.playerOffPosition.x),
                0.000003,
              );
  ...
  spore(where, isFreeze = tech.isSporeFreeze) {
    //used with the tech upgrade in mob.death()
    const bIndex = bullet.length;
    const size = 4;
    if (bIndex < 500) {
      ...
      bullet[bIndex] = Bodies.polygon(where.x, where.y, size, size, {
        ...
        frictionAir: 0.025,
        thrust:
          (tech.isSporeFollow ? 0.0011 : 0.0005) *
          (1 + 0.3 * (Math.random() - 0.5)),
        ...
        endCycle:
          simulation.cycle +
          Math.floor(
            (540 + Math.floor(Math.random() * 420)) * tech.bulletsLastLonger,
          ),
        minDmgSpeed: 0,
        playerOffPosition: {
          //used when moving towards player to keep spores separate
  ...
  iceIX(
    speed = 0,
    dir = m.angle + Math.PI * 2 * Math.random(),
    where = {
      x: m.pos.x + 30 * Math.cos(m.angle),
      y: m.pos.y + 30 * Math.sin(m.angle),
    },
    ...
    bullet[me] = Bodies.polygon(where.x, where.y, 3, RADIUS, {
      ...
      // inertia: Infinity,
      spin:
        0.00004 * (0.1 + Math.random()) * (Math.round(Math.random()) ? 1 : -1),
      friction: 0,
      frictionAir: 0.02,
      restitution: 0.9,
      ...
      do() {
        ...
        if (this.lockedOn) {
          ...
          this.force = Vector.mult(
            Vector.normalise(Vector.sub(this.lockedOn.position, this.position)),
            this.mass * THRUST,
          );
        } else {
          this.force = Vector.mult(
  ...
  flea(
    where,
    velocity,
    radius = 6 + 3 * Math.random() + 10 * tech.wormSize * Math.random(),
  ) {
    const me = bullet.length;
    bullet[me] = Bodies.polygon(where.x, where.y, 5, radius, {
      ...
      endCycle:
        simulation.cycle +
        Math.floor(
          900 * tech.bulletsLastLonger +
            420 * Math.random() +
            Math.max(0, 150 - bullet.length),
        ), // 13 - 19s
      classType: "bullet",
      collisionFilter: {
        category: cat.bullet,
      ...
      dmg: 0,
      setDamage() {
        //dmg is set to zero after doing damage once, and set back to normal after jumping
      ...
      beforeDmg(who) {
        Matter.Body.setVelocity(
          this,
          Vector.mult(
            Vector.normalise(Vector.sub(this.position, who.position)),
            10 + 10 * Math.random(),
          ),
      ...
      do() {
        ...
        if (
          ...
          //if on the ground and not on jump cooldown //
          this.cd = simulation.cycle + this.delay;
          ...
          if (
            tech.isSporeFollow &&
            !this.lockedOn &&
            Matter.Query.ray(map, this.position, m.pos).length === 0
          ) {
            this.lockedOn = {
              //make target player if there are no mobs to target
              position: m.pos,
              velocity: {
                x: 0,
                y: 0,
          ...
          if (this.lockedOn) {
            //hop towards mob target
            const where = Vector.add(
              this.lockedOn.position,
              Vector.mult(this.lockedOn.velocity, 5),
            ); //estimate where the mob will be in 5 cycles
            ...
            const capMaxY =
              0.12 + 0.04 * Math.random() + 0.05 * tech.isSporeFollow;
            const capMinY =
              closeDist > 500
                ? 0.05 + 0.02 * Math.random()
                : 0.02 + 0.01 * Math.random(); //don't jump super low, unless you are very close to mob target
            this.force.x = Math.max(-capX, Math.min(capX, Vx)) * this.mass;
            this.force.y =
              -Math.max(capMinY, Math.min(capMaxY, Vy)) * this.mass;
          } else {
            //random hops
            if (Math.random() < 0.5) {
              //chance to continue in the same horizontal direction
              this.force.x =
                (0.01 + 0.03 * Math.random()) *
                this.mass *
                (this.velocity.x > 0 ? 1 : -1); //random move
            } else {
              this.force.x =
                (0.01 + 0.03 * Math.random()) *
                this.mass *
                (Math.random() < 0.5 ? 1 : -1); //random move
            }
      ...
      if (droneCount > 0) {
        ...
        if (!simulation.paused && !simulation.isChoosing && m.alive) {
          ...
          if (tech.isDroneRadioactive) {
            b.droneRadioactive(
              {
                x: where.x + 50 * (Math.random() - 0.5),
                y: where.y + 50 * (Math.random() - 0.5),
              },
              0,
            );
          } else {
            ...
            if (tech.isDroneGrab && deliveryCount > 0) {
              ...
              who.endCycle +=
                3000 * tech.droneCycleReduction * tech.bulletsLastLonger;
              deliveryCount--;
            }
          }
        }
  ...
  drone(
    ...
    bullet[me] = Bodies.polygon(where.x, where.y, 8, RADIUS, {
      ...
      lookFrequency:
        (tech.isDroneFastLook ? 20 : 70) + Math.floor(17 * Math.random()),
      endCycle:
        simulation.cycle +
        Math.floor(
          (950 + 400 * Math.random()) *
            tech.bulletsLastLonger *
            tech.droneCycleReduction,
        ) +
        5 * RADIUS +
        Math.max(0, 150 - bullet.length),
      classType: "bullet",
      ...
      beforeDmg(who) {
        if (who.isInvulnerable) {
          ...
          if (
            tech.isIncendiary &&
            simulation.cycle + this.deathCycles < this.endCycle &&
            !tech.isForeverDrones
          ) {
            const max = Math.max(
              Math.min(
                this.endCycle - simulation.cycle - this.deathCycles,
                1500,
              ),
              0,
            );
            b.explosion(
              this.position,
              max * 0.14 + this.isImproved * 110 + 60 * Math.random(),
            ); //makes bullet do explosive damage at end
      ...
      onEnd() {
        if (tech.isDroneRespawn) {
          ...
          const canSee = body.filter(
            (a) =>
              Matter.Query.ray(map, this.position, a.position).length === 0 &&
              !a.isNotHoldable &&
              Vector.magnitude(Vector.sub(this.position, a.position)) <
                70 + 30 * a.mass,
          );
          if (canSee.length) {
            ...
            const found = canSee.reduce((a, b) => {
              const distA = Vector.magnitude(
                Vector.sub(this.position, a.position),
              );
              const distB = Vector.magnitude(
                Vector.sub(this.position, b.position),
              );
      ...
      doRespawning() {
        ...
        if (this.bodyTarget) {
          this.force = Vector.mult(
            Vector.normalise(
              Vector.sub(this.position, this.bodyTarget.position),
            ),
            -this.mass * THRUST,
          );
        } else {
      ...
      do() {
        ...
        if (!(simulation.cycle % this.lookFrequency)) {
          ...
          if (tech.isDroneTeleport && this.lockedOn) {
            ...
            Matter.Body.setVelocity(
              this,
              Vector.mult(unit, Math.max(20, this.speed * 1.5)),
            );
            ctx.beginPath();
            ctx.moveTo(this.position.x, this.position.y);
            Matter.Body.translate(
              ...
              Vector.mult(
                unit,
                Math.min(350, distMag - this.lockedOn.radius + 10),
              ),
            );
          ...
          if (!this.isImproved && !simulation.isChoosing) {
            if (this.lockedOn) {
              for (let i = 0, len = powerUp.length; i < len; ++i) {
                ...
                if (
                  Vector.magnitudeSquared(
                    Vector.sub(this.position, powerUp[i].position),
                  ) < 20000 &&
                  (powerUp[i].name !== "heal" ||
                    m.health < 0.97 * m.maxHealth ||
                    tech.isDroneGrab) &&
                ...
                if (
                  ...
                  if (
                    Matter.Query.ray(map, this.position, powerUp[i].position)
                      .length === 0 //&& Matter.Query.ray(body, this.position, powerUp[i].position).length === 0
                  ) {
        ...
        if (this.lockedOn) {
          ...
          this.force = Vector.mult(
            Vector.normalise(Vector.sub(this.position, simulation.mouseInGame)),
            -this.mass * THRUST,
          );
        }
  ...
  droneRadioactive(
    ...
    const me = bullet.length;
    const THRUST =
      (tech.isFastDrones ? 0.003 : 0.0012) + 0.0005 * (Math.random() - 0.5);
    ...
    bullet[me] = Bodies.polygon(where.x, where.y, 8, RADIUS, {
      ...
      endCycle:
        simulation.cycle +
        Math.floor(
          ((900 + 110 * Math.random()) * tech.bulletsLastLonger) /
            tech.droneRadioDamage,
        ) +
        5 * RADIUS +
        Math.max(0, 150 - 2 * bullet.length),
      classType: "bullet",
      isDrone: true,
      collisionFilter: {
        category: cat.bullet,
      ...
      do() {
        //radioactive zone
        this.radioRadius =
          this.radioRadius * 0.993 + 0.007 * this.maxRadioRadius; //smooth radius towards max
        //aoe damage to player
        if (
          Vector.magnitude(Vector.sub(player.position, this.position)) <
          this.radioRadius
          ...
          if (m.energy > DRAIN) {
            ...
            if (simulation.dmgScale)
              m.damage(
                (tech.isRadioactiveResistance ? 0.00004 : 0.0002) *
                  tech.radioactiveDamage,
              ); //0.00015
          }
        }
        ...
        for (let i = 0, len = mob.length; i < len; i++) {
          if (
            Vector.magnitude(Vector.sub(mob[i].position, this.position)) <
            this.radioRadius + mob[i].radius
          ) {
            if (
              Matter.Query.ray(map, mob[i].position, this.position).length > 0
            )
              dmg *= 0.25; //reduce damage if a wall is in the way
            mob[i].damage(mob[i].shield ? dmg * 3 : dmg);
            mob[i].locatePlayer();
          }
        }
        ...
        if (simulation.cycle + this.deathCycles > this.endCycle) {
          ...
          if (this.lockedOn) {
            ...
            this.force = Vector.mult(
              Vector.normalise(
                Vector.sub(this.position, simulation.mouseInGame),
              ),
              -this.mass * THRUST,
            );
          }
  ...
  superBall(where, velocity, radius) {
    ...
    Composite.add(engine.world, bullet[me]); //add bullet to world
    Matter.Body.setVelocity(bullet[me], velocity);
    bullet[me].calcDensity = function () {
      return (
        0.0007 + 0.0007 * tech.isSuperHarm + 0.0004 * tech.isBulletTeleport
      );
    };
    Matter.Body.setDensity(bullet[me], bullet[me].calcDensity());
    bullet[me].endCycle =
      simulation.cycle + Math.floor(270 + 90 * Math.random());
    bullet[me].minDmgSpeed = 0;
    ...
    if (tech.isSuperHarm) {
      ...
      bullet[me].do = function () {
        this.force.y += this.mass * gravity;
        if (this.nextPortCycle < simulation.cycle) {
          //teleport around if you have tech.isBulletTeleport
          this.nextPortCycle = simulation.cycle + this.portFrequency;
          ...
          Matter.Body.setPosition(
            ...
            Vector.add(
              this.position,
              Vector.rotate({ x: range, y: 0 }, 2 * Math.PI * Math.random()),
            ),
          );
          Matter.Body.setVelocity(
            this,
            Vector.rotate(
              this.velocity,
              2 * (Math.random() * Math.random() - 0.25),
            ),
          );
        }
      };
      ...
      if (!who.isInvulnerable) {
        ...
        if (tech.isIncendiary) {
          ...
          const cycle = () => {
            ...
            this.endCycle =
              simulation.cycle + Math.floor(300 + 90 * Math.random()); //reset to full duration of time
            Matter.Body.setVelocity(
              this,
              Vector.mult(Vector.normalise(this.velocity), 60),
            ); //reset to high velocity
            let count = 5;
            const wait = () => {
              count--;
  ...
  targetedBall(
    ...
    for (let i = 0; i < num; i++) {
      if (targets.length > 0) {
        ...
        b.superBall(
          position,
          Vector.mult(Vector.normalise(Vector.sub(WHERE, position)), speed),
          radius,
        );
        shotsFired++;
      } else if (isRandomAim) {
    ...
    for (let i = 0; i < num; i++) {
      if (targets.length > 0) {
        ...
        b.foam(
          position,
          Vector.mult(
            Vector.normalise(Vector.sub(where, position)),
            speed - radius * 0.25,
          ),
          radius,
        );
  ...
  foam(position, velocity, radius) {
    ...
    bullet[me] = Bodies.polygon(position.x, position.y, 20, radius, {
      ...
      damage:
        tech.foamDamage *
        (tech.isFastFoam ? 2.8 : 1) *
        (tech.isBulletTeleport ? 1.53 : 1), //damage done over time
      scale:
        1 - (0.006 / tech.bulletsLastLonger) * (tech.isFastFoam ? 1.65 : 1),
      classType: "bullet",
      collisionFilter: {
        category: cat.bullet,
      ...
      beforeDmg(who) {
        if (!this.target && who.alive) {
          ...
          if (who.radius < 20) {
            ...
            const normal = Matter.Query.collides(this, [who])[0].normal;
            this.targetRelativePosition = Vector.rotate(
              Vector.sub(
                Vector.sub(this.position, who.position),
                Vector.mult(normal, -this.radius),
              ),
              -who.angle,
            ); //find relative position vector for zero mob rotation
          } else {
        ...
        if (this.target && this.target.alive) {
          ...
          if (this.target.isVerticesChange) {
            ...
            Matter.Body.setPosition(
              this,
              Vector.add(
                Vector.add(rotate, this.target.velocity),
                this.target.position,
              ),
            );
          }
          if (this.target.isBoss) {
            if (this.target.speed > 6.5)
              Matter.Body.setVelocity(
                this.target,
                Vector.mult(this.target.velocity, 0.975),
              );
          } else {
            if (this.target.speed > 2.5)
              Matter.Body.setVelocity(
                this.target,
                Vector.mult(this.target.velocity, 0.94),
              );
          }
          ...
          if (
            tech.isSpawnBulletsOnDeath &&
            bullet.length < 180 &&
            !this.target.isMobBullet
          ) {
            let targets = [];
            for (let i = 0, len = mob.length; i < len; i++) {
              const dist = Vector.magnitudeSquared(
            ...
            for (let i = 0; i < len; i++) {
              if (targets.length - i > 0) {
                ...
                const velocity = Vector.mult(
                  Vector.normalise(
                    Vector.sub(targets[index].position, this.position),
                  ),
                  speed,
                );
                b.foam(
                  this.position,
                  Vector.rotate(velocity, 0.5 * (Math.random() - 0.5)),
                  radius,
                );
              } else {
                b.foam(
          ...
          this.target = null;
        } else if (Matter.Query.point(map, this.position).length > 0) {
          //slow when touching map
          const slow = 0.87;
          Matter.Body.setVelocity(this, {
            x: this.velocity.x * slow,
            y: this.velocity.y * slow,
          ...
          if (tech.isFoamAttract) {
            for (let i = 0, len = mob.length; i < len; i++) {
              const range = Vector.magnitude(
                Vector.sub(mob[i].position, this.position),
              );
              if (
                !mob[i].isBadTarget &&
                mob[i].alive &&
                ...
                this.force = Vector.mult(
                  Vector.normalise(Vector.sub(mob[i].position, this.position)),
                  this.mass * mag,
                );
                const slow = 0.98;
                Matter.Body.setVelocity(this, {
  ...
  targetedBlock(who, speed = 50 - Math.min(20, who.mass * 2), range = 1600) {
    ...
    for (let i = 0, len = mob.length; i < len; i++) {
      if (who !== mob[i] && !mob[i].isBadTarget && !mob[i].isInvulnerable) {
        ...
        if (
          dist < range &&
          Matter.Query.ray(map, who.position, mob[i].position).length === 0
        ) {
          //&& Matter.Query.ray(body, position, mob[i].position).length === 0
          closestMob = mob[i];
          range = dist;
    ...
    if (closestMob) {
      const where = Vector.add(
        closestMob.position,
        Vector.mult(closestMob.velocity, dist / 60),
      );
      const velocity = Vector.mult(
        Vector.normalise(Vector.sub(where, who.position)),
        speed,
      );
  ...
  crit(mob, bullet) {
    if (
      !mob.shield &&
      Vector.dot(
        Vector.normalise(Vector.sub(mob.position, bullet.position)),
        Vector.normalise(bullet.velocity),
      ) >
        0.999 - 1 / mob.radius
    ) {
      if (
  ...
  nail(pos, velocity, dmg = 1) {
    ...
    bullet[me].beforeDmg = function (who) {
      //beforeDmg is rewritten with ice crystal tech
      if (tech.isNailRadiation)
        mobs.statusDoT(
          who,
          dmg * (tech.isFastRadiation ? 1.3 : 0.44),
          tech.isSlowRadiation ? 360 : tech.isFastRadiation ? 60 : 180,
        ); // one tick every 30 cycles
      if (tech.isNailCrit) {
        //makes bullet do explosive damage if it hits center
        if (
          !who.shield &&
          Vector.dot(
            Vector.normalise(Vector.sub(who.position, this.position)),
            Vector.normalise(this.velocity),
          ) >
            0.97 - 1 / who.radius
        ) {
          b.explosion(
            this.position,
            80 + 90 * (b.activeGun === 0) + 30 * Math.random(),
          ); //larger explosions for human aimed nail gun, smaller for auto aimed sources, like bots, and mine
        }
      }
      this.ricochet(who);
    };
    bullet[me].ricochet = function (who) {
      //use for normal nails, and ice crystal nails
      if (tech.isRicochet) {
        const targets = []; //target nearby mobs
        for (let i = 0, len = mob.length; i < len; i++) {
          const dist = Vector.magnitude(
          ...
          if (
            ...
            targets.push(
              Vector.add(
                mob[i].position,
                Vector.mult(mob[i].velocity, dist / 60),
              ),
            ); //predict where the mob will be in a few cycles
          }
        }
        if (targets.length > 0) {
          ...
          Matter.Body.setVelocity(
            this,
            Vector.mult(
              Vector.normalise(Vector.sub(targets[index], this.position)),
              45,
            ),
          );
          Matter.Body.setAngle(
    ...
    if (tech.needleTunnel) {
      ...
      bullet[me].do = function () {
        ...
        if (whom.length && this.speed > 20) {
          ...
          for (let i = 0, len = whom.length; i < len; i++) {
            ...
            if (who && who.mob) {
              ...
              for (let i = 0; i < this.immuneList.length; i++) {
                //check if this needle has hit this mob already
              ...
              if (!immune) {
                ...
                if (tech.isNailRadiation) {
                  mobs.statusDoT(
                    who,
                    (tech.isFastRadiation ? 6 : 2) * tech.bulletSize,
                    tech.isSlowRadiation
                      ? 360
          ...
          //penetrate walls
          if (!this.isInMap) {
            //turn after entering the map, but only turn once
            this.isInMap = true;
            Matter.Body.setVelocity(
              this,
              Vector.rotate(this.velocity, 0.25 * (Math.random() - 0.5)),
            );
            Matter.Body.setAngle(
              this,
              Math.atan2(this.velocity.y, this.velocity.x),
          ...
          Matter.Body.setPosition(
            this,
            Vector.add(this.position, Vector.mult(this.velocity, -0.98)),
          ); //move back 1/2 your velocity = moving at 1/2 speed
        } else if (Matter.Query.collides(this, body).length) {
          //penetrate blocks
          Matter.Body.setAngularVelocity(this, 0);
          Matter.Body.setPosition(
            this,
            Vector.add(this.position, Vector.mult(this.velocity, -0.94)),
          ); //move back 1/2 your velocity = moving at 1/2 speed
      ...
      bullet[me].collisionFilter.mask = tech.isShieldPierce
        ? cat.body
        : cat.body | cat.mobShield;
      bullet[me].do = function () {
        const whom = Matter.Query.collides(this, mob);
        if (whom.length && this.speed > 20) {
          //if touching a mob
  ...
  convertBotsTo(type) {
    //type can be a string like "dynamoBotCount"
    ...
    for (let i = 0, len = totalPermanentBots - totalTechToConvert; i < len; i++)
      tech.tech[index].effect(); //also convert any permanent bots that didn't come from a tech
  ...
  clearPermanentBots() {
    for (let i = 0; i < bullet.length; i++) {
      if (bullet[i].botType && bullet[i].endCycle === Infinity)
        bullet[i].endCycle = 0; //remove active bots, but don't remove temp bots
    }
  },
  removeBot() {
  ...
  respawnBots() {
    for (let i = 0; i < tech.dynamoBotCount; i++)
      b.dynamoBot({
    ...
    for (let i = 0; i < tech.soundBotCount; i++)
      b.soundBot({
        x: player.position.x + 50 * (Math.random() - 0.5),
        y: player.position.y + 50 * (Math.random() - 0.5),
      });
    ...
    if (tech.isIntangible && m.isCloak) {
      for (let i = 0; i < bullet.length; i++) {
        if (bullet[i].botType)
          bullet[i].collisionFilter.mask =
            cat.map | cat.bullet | cat.mobBullet | cat.mobShield;
      }
    }
  },
  randomBot(where = player.position, isKeep = true, isLaser = true) {
    if (Math.random() < 0.5) {
      //chance to match bot to your upgrade
      ...
      //else don't match bot to upgrade
    ...
    for (let i = 0; i < bullet.length; i++) {
      if (bullet[i].botType === "dynamo") {
        ...
        if (bullet[i].isKeep) {
          ...
          bullet[i].followDelay =
            Math.floor(step * bullet.length * Math.random()) % 600;
        }
      }
    }
  },
  dynamoBot(position = player.position, isKeep = true) {
    ...
    bullet[me] = Bodies.polygon(position.x, position.y, 5, 10, {
      ...
      do() {
        ...
        //check for damage
        if (m.immuneCycle < m.cycle && !((m.cycle + this.phase) % 30)) {
          //twice a second
          if (
            Vector.magnitude(Vector.sub(this.position, player.position)) <
              250 &&
            m.immuneCycle < m.cycle
          ) {
            //give energy
        ...
        if (!m.isCloak) {
          //if cloaking field isn't active
          const size = 33;
        ...
        mask:
          b.totalBots() < 50
            ? cat.map |
              ...
              cat.mobBullet |
              cat.mobShield
            : cat.map | cat.body | cat.mob | cat.mobBullet | cat.mobShield, //if over 50 bots, they no longer collide with each other
      },
      beforeDmg() {},
      onEnd() {},
      do() {
        ...
        if (distanceToPlayer > this.range) {
          ...
          this.force = Vector.mult(
            Vector.normalise(Vector.sub(m.pos, this.position)),
      ...
      do() {
        ...
        if (distanceToPlayer > this.range) {
          ...
          if (
            ...
            for (let i = 0, len = mob.length; i < len; i++) {
              const dist2 = Vector.magnitudeSquared(
                Vector.sub(this.position, mob[i].position),
              );
              if (
                mob[i].alive &&
                ...
                this.torque +=
                  this.inertia * 0.00004 * (Math.round(Math.random()) ? 1 : -1);
                this.force = Vector.mult(
                  Vector.normalise(Vector.sub(this.position, mob[i].position)),
                  this.mass * 0.02,
                );
                ...
                if (tech.missileCount > 1) {
                  ...
                  const push = Vector.mult(
                    Vector.perp(direction),
                    (0.015 * countReduction) / Math.sqrt(tech.missileCount),
                  );
                  for (let i = 0; i < tech.missileCount; i++) {
                    setTimeout(
                      () => {
                        b.missile(this.position, angle, -8, size);
                        bullet[bullet.length - 1].force.x +=
                          push.x * (i - (tech.missileCount - 1) / 2);
                        bullet[bullet.length - 1].force.y +=
                          push.y * (i - (tech.missileCount - 1) / 2);
                      },
  ...
  foamBot(
    ...
    bullet[me] = Bodies.polygon(position.x, position.y, 6, RADIUS, {
      ...
      minDmgSpeed: 2,
      lookFrequency:
        60 + Math.floor(17 * Math.random()) - 50 * tech.isFoamBotUpgrade,
      cd: 0,
      fireCount: 0,
      ...
      fire() {
        ...
        b.foam(
          this.position,
          Vector.rotate(velocity, 0.07 * (Math.random() - 0.5)),
          radius + 6 * this.isUpgraded,
        );
        ...
        const force = Vector.mult(
          velocity,
          0.0001 * this.mass * (tech.isFoamCavitation ? 2 : 1),
        );
        this.force.x -= force.x;
        this.force.y -= force.y;
      },
      do() {
        if (this.fireCount === 0) {
          //passive mode: look for targets and following player
          const distanceToPlayer = Vector.magnitude(
            Vector.sub(this.position, m.pos),
          );
          if (distanceToPlayer > this.range) {
          ...
          if (
            ...
            for (let i = 0, len = mob.length; i < len; i++) {
              ...
              if (
                ...
                this.fireTarget = Vector.add(
                  mob[i].position,
                  Vector.mult(mob[i].velocity, Math.sqrt(dist2) / 60),
                ); //set target to where the mob will be in 1 second
                this.fire();
                break;
              }
            }
          ...
          //fire mode: quickly fire at targets and doesn't follow player
          this.fire();
        }
  ...
  soundBot(
    ...
    bullet[me] = Bodies.rectangle(position.x, position.y, 12, 30, {
      ...
      minDmgSpeed: 2,
      lookFrequency:
        17 + Math.floor(7 * Math.random()) - 5 * tech.isSoundBotUpgrade,
      cd: 0,
      fireCount: 0,
      ...
      addWave(where, angle) {
        const halfArc =
          0.2 * (tech.isBulletTeleport ? 0.66 + (Math.random() - 0.5) : 1) +
          0.05 * tech.isSoundBotUpgrade; //6.28 is a full circle, but these arcs needs to stay small because we are using small angle linear approximation, for collisions
        this.waves.push({
          position: where,
          ...
          resonanceCount: 0,
          dmg:
            (tech.isUpgraded ? 4 : 1.5) *
            m.dmgScale *
            tech.wavePacketDamage *
            tech.waveBeamDamage *
            (tech.isBulletTeleport ? 1.5 : 1),
        });
      },
      fire() {
        if (!(simulation.cycle % (6 - 2 * tech.isSoundBotUpgrade))) {
          ...
          this.addWave(
            ...
            Math.atan2(
              this.fireTarget.y - this.position.y,
              this.fireTarget.x - this.position.x,
            ) +
              tech.isBulletTeleport * 0.3 * (Math.random() - 0.5),
          ); //add wave to waves array
          //face target
          Matter.Body.setAngle(
            this,
            Vector.angle(this.position, this.fireTarget),
          );
        }
      },
      do() {
        ...
        if (!m.isBodiesAsleep) {
          ...
          for (let i = this.waves.length - 1; i > -1; i--) {
            const v1 = Vector.add(
              this.waves[i].position,
              Vector.mult(this.waves[i].unit1, this.waves[i].radius),
            );
            const v2 = Vector.add(
              this.waves[i].position,
              Vector.mult(this.waves[i].unit2, this.waves[i].radius),
            );
            //draw wave
            ctx.moveTo(v1.x, v1.y);
            ctx.arc(
              this.waves[i].position.x,
              this.waves[i].position.y,
              this.waves[i].radius,
              this.waves[i].angle,
              this.waves[i].angle + this.waves[i].arc,
            );
            ...
            for (let j = 0; j < hits.length; j++) {
              ...
              if (!who.isShielded) {
                ...
                Matter.Body.setVelocity(who, {
                  x: who.velocity.x * 0.98,
                  y: who.velocity.y * 0.98,
                });
                let vertices = who.vertices;
                const vibe = 50 + who.radius * 0.15;
                ctx.moveTo(
                  vertices[0].x + vibe * (Math.random() - 0.5),
                  vertices[0].y + vibe * (Math.random() - 0.5),
                );
                for (let j = 1; j < vertices.length; j++)
                  ctx.lineTo(
                    vertices[j].x + vibe * (Math.random() - 0.5),
                    vertices[j].y + vibe * (Math.random() - 0.5),
                  );
                ctx.lineTo(
                  vertices[0].x + vibe * (Math.random() - 0.5),
                  vertices[0].y + vibe * (Math.random() - 0.5),
                ...
                if (tech.isPhononWave && this.phononWaveCD < m.cycle) {
                  this.phononWaveCD =
                    m.cycle + 10 * (1 + this.waves[i].resonanceCount);
                  let closestMob, dist;
                  ...
                  for (let i = 0, len = mob.length; i < len; i++) {
                    if (
                      ...
                      dist = Vector.magnitude(
                        Vector.sub(who.position, mob[i].position),
                      );
                      if (dist < range) {
                        closestMob = mob[i];
                        range = dist;
                  ...
                  if (closestMob) {
                    ...
                    this.addWave(
                      who.position,
                      Math.atan2(
                        closestMob.position.y - who.position.y,
                        closestMob.position.x - who.position.x,
                      ) +
                        tech.isBulletTeleport * 0.3 * (Math.random() - 0.5),
                    );
                  } else {
                  ...
                  this.waves[this.waves.length - 1].resonanceCount =
                    this.waves[i].resonanceCount + 1;
                  break;
                }
              }
            }
            ...
            for (let j = 0, len = Math.min(30, hits.length); j < len; j++) {
              ...
              who.force.y +=
                (0.005 * (Math.random() - 0.5) - simulation.g * 0.1) * who.mass; //remove force of gravity
              let vertices = who.vertices;
              const vibe = 25;
              ctx.moveTo(
                vertices[0].x + vibe * (Math.random() - 0.5),
              ...
              if (
                ...
                !who.isNotHoldable &&
                who.speed < 5 &&
                who.angularSpeed < 0.1
              ) {
                if (Math.random() < 0.5)
                  b.targetedBlock(who, 50 - Math.min(25, who.mass * 3)); //    targetedBlock(who, speed = 50 - Math.min(20, who.mass * 2), range = 1600) {
            ...
            if (
              this.waves[i].radius >
              end - 30 * this.waves[i].resonanceCount
            ) {
  ...
  laserBot(
    ...
    bullet[me] = Bodies.polygon(position.x, position.y, 3, RADIUS, {
      ...
      minDmgSpeed: 2,
      lookFrequency:
        20 + Math.floor(7 * Math.random()) - 13 * tech.isLaserBotUpgrade,
      range: (600 + 375 * tech.isLaserBotUpgrade) * (1 + 0.12 * Math.random()),
      drainThreshold:
        0.15 + 0.5 * Math.random() + (tech.isEnergyHealth ? 0.3 : 0), // laser bot will not attack if the player is below this energy
      ...
      do() {
        const playerPos = Vector.add(
          Vector.add(this.offPlayer, m.pos),
        ...
        if (!(simulation.cycle % this.lookFrequency)) {
          ...
          if (Math.random() < 0.1) {
            if (isKeep) {
              ...
              this.offPlayer = Vector.mult(
                Vector.rotate({ x: 1, y: 0 }, 6.28 * Math.random()),
                range,
              );
            }
          }
      ...
      laser() {
        ...
        const perp2 = Vector.mult(
          Vector.rotate({ x: 1, y: 0 }, m.angle + Math.PI / 2),
          0.6 *
            this.lockedOn.radius *
            Math.sin(simulation.cycle / this.lookFrequency),
        );
        const path = [
          { x: this.vertices[0].x, y: this.vertices[0].y },
          Vector.add(this.lockedOn.position, perp2),
        ];
            ...
            if (best.who.damageReduction) {
              if (
                ...
                simulation.drawList.push({
                  ...
                  y: path[path.length - 1].y,
                  radius:
                    Math.sqrt(2500 * damage * best.who.damageReduction) + 5,
                  color: `hsla(${60 + 283 * Math.random()},100%,70%,0.5)`, // random hue, but not red
                  time: 16,
                });
              } else {
                simulation.drawList.push({
                  ...
                  y: path[path.length - 1].y,
                  radius:
                    Math.sqrt(2000 * damage * best.who.damageReduction) + 2,
                  color: tech.laserColorAlpha,
                  time: simulation.drawTime,
                });
              }
  ...
  boomBot(
    ...
    bullet[me] = Bodies.polygon(position.x, position.y, 4, RADIUS, {
      ...
      minDmgSpeed: 0,
      lookFrequency:
        43 + Math.floor(7 * Math.random()) - 13 * tech.isBoomBotUpgrade,
      acceleration: (isKeep ? 0.005 : 0.001) * (1 + 0.5 * Math.random()),
      attackAcceleration: 0.012 + 0.006 * tech.isBoomBotUpgrade,
      range:
        500 * (1 + 0.1 * Math.random()) +
        350 * tech.isBoomBotUpgrade +
        !isKeep * 100,
      endCycle: Infinity,
      classType: "bullet",
      collisionFilter: {
      ...
      beforeDmg() {
        if (this.lockedOn) {
          const explosionRadius = Math.min(
            136 + 230 * this.isUpgraded,
            Vector.magnitude(Vector.sub(this.position, m.pos)) - 30,
          );
      ...
      do() {
        const distanceToPlayer = Vector.magnitude(
          Vector.sub(this.position, player.position),
        );
        if (distanceToPlayer > 100) {
          ...
          if (this.explode) {
            ...
            b.explosion(
              ...
              Math.max(
                ...
                Math.min(
                  this.explode,
                  (distanceToPlayer - 70) / b.explosionRange(),
                ),
              ),
            );
            this.explode = 0;
          ...
          this.force = Vector.mult(
            Vector.normalise(Vector.sub(player.position, this.position)),
            this.mass * this.acceleration,
          );
        ...
        if (this.lockedOn && this.lockedOn.alive && !m.isCloak) {
          ...
          if (
            DIST - this.lockedOn.radius < this.range &&
            Matter.Query.ray(map, this.position, this.lockedOn.position)
              .length === 0
          ) {
            ...
            this.force = Vector.add(
              ...
              Vector.mult(
                ...
                this.attackAcceleration * this.mass,
              ),
            );
          }
        }
      ...
      do() {
        ...
        if (this.lockedOn && this.lockedOn.alive && m.fieldCDcycle < m.cycle) {
          ...
          if (
            ...
            let best;
            let range =
              tech.isPlasmaRange * (120 + 300 * Math.sqrt(Math.random()));
            const path = [
              { x: this.position.x, y: this.position.y },
              {
              ...
              if (best.who.alive) {
                ...
                if (best.who.damageReduction) {
                  simulation.drawList.push({
                    ...
                    radius:
                      Math.sqrt(2000 * dmg * best.who.damageReduction) + 2,
                    color: "rgba(255,0,255,0.2)",
                    time: simulation.drawTime * 4,
                  });
                }
  ...
  orbitBot(position = player.position, isKeep = true) {
    ...
    bullet[me] = Bodies.polygon(position.x, position.y, 9, 12, {
      ...
      range:
        190 +
        130 * tech.isOrbitBotUpgrade +
        !isKeep * 60 * (0.5 - Math.random()), //range is set in bot upgrade too!
      orbitalSpeed: 0,
      ...
      do() {
        ...
        Matter.Body.setPosition(
          this,
          Vector.add(m.pos, Vector.mult(orbit, this.range)),
        ); //bullets move with player
      },
    });
  ...
  guns: [
    {
      ...
      fireRecoilNails() {
        ...
        const CD = Math.max(
          11 - 0.06 * (m.cycle - this.startingHoldCycle),
          0.99,
        ); //CD scales with cycles fire is held down
        this.nextFireCycle = m.cycle + CD * b.fireCDscale; //predict next fire cycle if the fire button is held down

        m.fireCDcycle = m.cycle + Math.floor(CD * b.fireCDscale); // cool down
        this.baseFire(
          m.angle + ((Math.random() - 0.5) * (m.crouch ? 0.04 : 0.13)) / CD,
          45 + 6 * Math.random(),
        );
        ...
        if (m.onGround) {
          ...
          player.force.x -=
            0.06 *
            Math.cos(m.angle) *
            Math.min(1, 3 / (0.1 + Math.abs(player.velocity.x)));
      ...
      fireNormal() {
        ...
        m.fireCDcycle = m.cycle + Math.floor(CD * b.fireCDscale); // cool down
        this.baseFire(
          m.angle + ((Math.random() - 0.5) * (m.crouch ? 0.05 : 0.3)) / CD,
        );
      },
      fireNeedles() {
        if (m.crouch) {
      ...
      fireRivets() {
        m.fireCDcycle =
          m.cycle + Math.floor((m.crouch ? 22 : 14) * b.fireCDscale); // cool down
        const me = bullet.length;
        const size = tech.bulletSize * 8;
        bullet[me] = Bodies.rectangle(
          m.pos.x + 35 * Math.cos(m.angle),
          m.pos.y + 35 * Math.sin(m.angle),
          5 * size,
          size,
        ...
        bullet[me].beforeDmg = function (who) {
          ...
          if (tech.isNailRadiation)
            mobs.statusDoT(
              who,
              7 * (tech.isFastRadiation ? 0.7 : 0.24),
              tech.isSlowRadiation ? 360 : tech.isFastRadiation ? 60 : 180,
            ); // one tick every 30 cycles
          if (this.speed > 4 && tech.fragments) {
            b.targetedNail(
              this.position,
              1.25 * tech.fragments * tech.bulletSize,
            );
        ...
        bullet[me].rotateToVelocity = function () {
        ...
        if (tech.isIceCrystals) {
          bullet[bullet.length - 1].beforeDmg = function (who) {
            ...
            if (tech.isNailRadiation)
              mobs.statusDoT(
                who,
                1 * (tech.isFastRadiation ? 1.3 : 0.44),
                tech.isSlowRadiation ? 360 : tech.isFastRadiation ? 60 : 180,
              ); // one tick every 30 cycles
            if (tech.isNailCrit) {
              if (
      ...
      fire() {
        ...
        const coolDown = function () {
          if (m.crouch) {
            ...
            m.fireCDcycle =
              m.cycle +
              Math.floor((73 + 36 * tech.shotgunExtraShots) * b.fireCDscale); // cool down
            if (
              tech.isShotgunImmune &&
              m.immuneCycle < m.cycle + Math.floor(60 * b.fireCDscale)
            )
              m.immuneCycle = m.cycle + Math.floor(60 * b.fireCDscale); //player is immune to damage for 30 cycles
            knock = 0.01;
          } else {
            m.fireCDcycle =
              m.cycle +
              Math.floor((56 + 28 * tech.shotgunExtraShots) * b.fireCDscale); // cool down
            if (
              tech.isShotgunImmune &&
              m.immuneCycle < m.cycle + Math.floor(47 * b.fireCDscale)
            )
              m.immuneCycle = m.cycle + Math.floor(47 * b.fireCDscale); //player is immune to damage for 30 cycles
            spread = 1.3;
            knock = 0.1;
          }
          ...
          if (tech.isShotgunReversed) {
            ...
            player.force.y +=
              1.5 * knock * Math.sin(m.angle) - 3 * player.mass * simulation.g;
        ...
        const spray = (num) => {
          ...
          for (let i = 0; i < num; i++) {
            ...
            bullet[me].endCycle =
              simulation.cycle + 40 * tech.bulletsLastLonger;
            bullet[me].minDmgSpeed = 15;
            if (tech.isShotgunReversed)
              Matter.Body.setDensity(bullet[me], 0.0015);
            // bullet[me].restitution = 0.4
        ...
        const chooseBulletType = function () {
          if (tech.isRivets) {
            ...
            Matter.Body.setDensity(
              bullet[me],
              0.005 * (tech.isShotgunReversed ? 1.5 : 1),
            );
            Composite.add(engine.world, bullet[me]); //add bullet to world
            ...
            bullet[me].beforeDmg = function (who) {
              if (this.speed > 4) {
                if (tech.fragments) {
                  b.targetedNail(
                    this.position,
                    6 * tech.fragments * tech.bulletSize,
                  );
                  this.endCycle = 0; //triggers despawn
                }
            ...
            for (let i = 0; i < totalBullets; i++) {
              ...
              const me = bullet.length;
              bullet[me] = Bodies.rectangle(
                m.pos.x + 50 * Math.cos(m.angle),
                m.pos.y + 50 * Math.sin(m.angle),
                17,
                4,
                b.fireAttributes(dir),
              );
              ...
              bullet[me].endCycle =
                2 * end * tech.bulletsLastLonger + simulation.cycle;
              ...
              bullet[me].onEnd = function () {
                b.explosion(
                  this.position,
                  180 * (tech.isShotgunReversed ? 1.4 : 1) +
                    (Math.random() - 0.5) * 30,
                ); //makes bullet do explosive damage at end
              };
              bullet[me].beforeDmg = function () {
            ...
            for (let i = 0; i < number; i++) {
              ...
              const speed = m.crouch
                ? 35 * (1 + 0.05 * Math.random())
                : 30 * (1 + 0.15 * Math.random());
              b.flea(where, {
                x: speed * Math.cos(angle),
                y: speed * Math.sin(angle),
              });
            ...
            for (
              let i = 0, len = 10 * (tech.isShotgunReversed ? 1.5 : 1);
              i < len;
              i++
            ) {
              b.iceIX(
                23 + 10 * Math.random(),
                m.angle + spread * (Math.random() - 0.5),
              );
            }
            spray(10); //fires normal shotgun bullets
    ...
    },
    {
      ...
      fireOne() {
        m.fireCDcycle =
          m.cycle + Math.floor((m.crouch ? 27 : 19) * b.fireCDscale); // cool down
      ...
      fireMulti() {
        m.fireCDcycle =
          m.cycle + Math.floor((m.crouch ? 23 : 15) * b.fireCDscale); // cool down
    ...
    },
    {
      ...
      do360Longitudinal() {
        if (!m.isBodiesAsleep) {
          ...
          const damage =
            ...
            tech.wavePacketDamage *
            tech.waveBeamDamage *
            (tech.isBulletTeleport ? 1.43 : 1) *
            (tech.isInfiniteWaveAmmo ? 0.75 : 1); //damage is lower for large radius mobs, since they feel the waves longer

          for (let i = this.waves.length - 1; i > -1; i--) {
            //draw wave
            ctx.moveTo(
              this.waves[i].position.x + this.waves[i].radius,
              this.waves[i].position.y,
            );
            ctx.arc(
              this.waves[i].position.x,
            ...
            for (let j = 0, len = mob.length; j < len; j++) {
              if (!mob[j].isShielded) {
                const dist = Vector.magnitude(
                  Vector.sub(this.waves[i].position, mob[j].position),
                );
                const r = mob[j].radius + 30;
                if (
                  dist + r > this.waves[i].radius &&
                  dist - r < this.waves[i].radius
                ) {
                  //make them shake around
                  if (!mob[j].isBadTarget) {
                    mob[j].force.x +=
                      0.01 * (Math.random() - 0.5) * mob[j].mass;
                    mob[j].force.y +=
                      0.01 * (Math.random() - 0.5) * mob[j].mass;
                  }
                  // if (!mob[j].isShielded) {
                  ...
                  for (let k = 1; k < vertices.length; k++) {
                    ctx.lineTo(
                      vertices[k].x + vibe * (Math.random() - 0.5),
                      vertices[k].y + vibe * (Math.random() - 0.5),
                    );
                  }
                  ctx.lineTo(
                    vertices[0].x + vibe * (Math.random() - 0.5),
                  ...
                  if (tech.isPhononWave && this.phononWaveCD < m.cycle) {
                    this.phononWaveCD =
                      m.cycle + 8 * (1 + this.waves[i].resonanceCount);
                    this.waves.push({
            ...
            for (let j = 0, len = Math.min(30, body.length); j < len; j++) {
              const dist = Vector.magnitude(
                Vector.sub(this.waves[i].position, body[j].position),
              );
              const r = 20;
              if (
                dist + r > this.waves[i].radius &&
                ...
                who.force.x += 0.01 * (Math.random() - 0.5) * who.mass;
                who.force.y +=
                  (0.01 * (Math.random() - 0.5) - simulation.g * 0.25) *
                  who.mass; //remove force of gravity
                //draw vibes
                let vertices = who.vertices;
                const vibe = 25;
                ctx.moveTo(
            ...
            if (
              ...
              //* Math.pow(0.9, this.waves[i].resonanceCount)
              this.waves.splice(i, 1); //end
            }
          }
          ctx.stroke();
      ...
      fire360Longitudinal() {
        m.fireCDcycle =
          m.cycle + Math.floor((m.crouch ? 4 : 8) * b.fireCDscale); // cool down
        this.waves.push({
        ...
        if (!m.isBodiesAsleep) {
          ...
          for (let i = this.waves.length - 1; i > -1; i--) {
                ...
                if (tech.isPhononWave && this.phononWaveCD < m.cycle) {
                  ...
                  if (closestMob) {
                    const dir = Vector.normalise(
                      Vector.sub(closestMob.position, who.position),
                    );
            ...
            for (let j = 0, len = Math.min(30, hits.length); j < len; j++) {
              ...
              who.force.x += 0.01 * (Math.random() - 0.5) * who.mass;
              who.force.y +=
                (0.01 * (Math.random() - 0.5) - simulation.g * 0.25) * who.mass; //remove force of gravity
              let vertices = who.vertices;
              const vibe = 25;
              ctx.moveTo(
                vertices[0].x + vibe * (Math.random() - 0.5),
      ...
      fireLongitudinal() {
        m.fireCDcycle =
          m.cycle + Math.floor((m.crouch ? 4 : 8) * b.fireCDscale); // cool down
        const halfArc =
          (m.crouch ? 0.0785 : 0.275) *
          (tech.isBulletTeleport ? 0.66 + (Math.random() - 0.5) : 1); //6.28 is a full circle, but these arcs needs to stay small because we are using small angle linear approximation, for collisions
        const angle =
          m.angle + tech.isBulletTeleport * 0.3 * (Math.random() - 0.5);
        this.waves.push({
          position: {
            x: m.pos.x + 25 * Math.cos(m.angle),
            y: m.pos.y + 25 * Math.sin(m.angle),
      ...
      fireTransverse() {
        totalCycles = Math.floor(
          (3.5 * 35 * tech.waveReflections * tech.bulletsLastLonger) /
            Math.sqrt(tech.waveReflections * 0.5),
        );
        const me = bullet.length;
        bullet[me] = Bodies.polygon(
          m.pos.x + 25 * Math.cos(m.angle),
          m.pos.y + 25 * Math.sin(m.angle),
          5,
          4,
          {
            angle: m.angle,
            ...
            amplitude:
              (m.crouch ? 6 : 12) *
              (this.wavePacketCycle % 2 ? -1 : 1) *
              Math.sin(this.wavePacketCycle * 0.088) *
              Math.sin(this.wavePacketCycle * 0.04), //0.0968 //0.1012 //0.11 //0.088 //shorten wave packet
            minDmgSpeed: 0,
            dmg:
              m.dmgScale *
              tech.waveBeamDamage *
              tech.wavePacketDamage *
              (tech.isBulletTeleport ? 1.43 : 1) *
              (tech.isInfiniteWaveAmmo ? 0.75 : 1), //also control damage when you divide by mob.mass
            dmgCoolDown: 0,
            classType: "bullet",
            collisionFilter: {
              category: 0,
            ...
            query() {
              ...
              if (this.dmgCoolDown < 1) {
                ...
                for (let i = 0; i < q.length; i++) {
                  this.dmgCoolDown =
                    5 + Math.floor(8 * Math.random() * b.fireCDscale);
                  let dmg = this.dmg;
                  q[i].damage(dmg);
                  if (q[i].alive) {
                    q[i].foundPlayer();
                  ...
                  if (q[i].damageReduction) {
                    simulation.drawList.push({
                      ...
                      y: this.position.y,
                      radius:
                        Math.log(dmg + 1.1) * 40 * q[i].damageReduction + 3,
                      color: "rgba(0,0,0,0.4)",
                      time: simulation.drawTime,
                    });
                  }
            ...
            wiggle() {
              this.cycle++;
              const where = Vector.mult(
                transverse,
                this.amplitude * Math.cos(this.cycle * tech.waveFrequency),
              );
          ...
          bullet[me].wiggle = function () {
            ...
            if (Math.random() < 0.005) {
              if (Math.random() < 0.33) {
                ...
                //randomize position in velocity direction
                const velocityScale = Vector.mult(
                  this.velocity,
                  50 * (Math.random() - 0.5),
                );
                Matter.Body.setPosition(
                  this,
                  Vector.add(velocityScale, Vector.add(this.position, where)),
                );
              }
            } else {
              Matter.Body.setPosition(this, Vector.add(this.position, where));
    ...
    },
    {
      ...
      fire() {
        ...
        // } else {
        m.fireCDcycle =
          m.cycle + (tech.missileFireCD * b.fireCDscale) / countReduction; // cool down
        const direction = {
          x: Math.cos(m.angle),
          y: Math.sin(m.angle),
        };
        ...
        if (tech.missileCount > 1) {
          const push = Vector.mult(
            Vector.perp(direction),
            (0.2 * countReduction) / Math.sqrt(tech.missileCount),
          );
          ...
          const fireMissile = () => {
            if (m.crouch) {
              ...
              bullet[bullet.length - 1].force.x +=
                0.5 * push.x * (Math.random() - 0.5);
              bullet[bullet.length - 1].force.y +=
                0.004 + 0.5 * push.y * (Math.random() - 0.5);
            } else {
              b.missile(
                {
                  x: m.pos.x + 30 * direction.x,
          ...
          const cycle = () => {
            if ((simulation.paused || m.isBodiesAsleep) && m.alive) {
              ...
              if (count < tech.missileCount * launchDelay && m.alive)
                requestAnimationFrame(cycle);
            }
          };
          requestAnimationFrame(cycle);
    ...
    },
    {
      ...
      fire() {
        ...
        m.fireCDcycle =
          m.cycle +
          Math.floor(((m.crouch ? 35 : 27) * b.fireCDscale) / countReduction); // cool down
        const where = {
          x: m.pos.x + 30 * Math.cos(m.angle),
          y: m.pos.y + 30 * Math.sin(m.angle),
        };
    ...
    },
    {
      ...
      fire() {
        ...
        const dir = m.angle;
        bullet[me] = Bodies.polygon(
          m.pos.x + 30 * Math.cos(m.angle),
          m.pos.y + 30 * Math.sin(m.angle),
          20,
          4.5,
          b.fireAttributes(dir, false),
        );
        ...
        Matter.Body.setDensity(bullet[me], 0.000001);
        bullet[me].endCycle =
          simulation.cycle + 480 + Math.max(0, 120 - 2 * bullet.length);
        bullet[me].frictionAir = 0;
        ...
        bullet[me].minDmgSpeed = 0;
        bullet[me].totalSpores =
          8 + 2 * tech.isSporeFreeze + 5 * tech.isSporeColony;
        bullet[me].stuck = function () {};
        bullet[me].beforeDmg = function () {};
        bullet[me].do = function () {
          ...
          if (mobCollisions.length) {
            ...
            if (bodyCollisions.length) {
              ...
              if (Matter.Query.collides(this, map).length) {
                ...
                //if colliding with nothing just fall
                this.force.y += this.mass * 0.0006;
        ...
        bullet[me].grow = function () {
          this.stuck(); //runs different code based on what the bullet is stuck to
          let scale = 1.01;
          if (tech.isSporeGrowth && !(simulation.cycle % 40)) {
            //release a spore
            if (tech.isSporeFlea) {
        ...
        bullet[me].onEnd = function () {
          ...
          // }
          if (tech.isStun)
            b.AoEStunEffect(this.position, 600, 270 + 120 * Math.random()); //AoEStunEffect(where, range, cycles = 120 + 60 * Math.random()) {
        };
      },
    },
    {
        ...
        if (tech.isDroneRadioactive) {
          if (m.crouch) {
            b.droneRadioactive(
              {
                x:
                  m.pos.x + 30 * Math.cos(m.angle) + 10 * (Math.random() - 0.5),
                y:
                  m.pos.y + 30 * Math.sin(m.angle) + 10 * (Math.random() - 0.5),
              },
              45,
            );
    ...
    },
    {
      ...
      fireStream() {
        ...
        const radius =
          5 + 8 * Math.random() + (tech.isAmmoFoamSize && this.ammo < 300) * 12;
      ...
      doCharges() {
        if (this.charge > 0) {
          ...
          const mag = 11 + radius;
          ctx.arc(
            m.pos.x + mag * Math.cos(m.angle),
            m.pos.y + mag * Math.sin(m.angle),
            radius,
            0,
            2 * Math.PI,
          );
          ...
          if (this.isDischarge && m.cycle % 2) {
            ...
            const radius =
              5 +
              8 * Math.random() +
              (tech.isAmmoFoamSize && this.ammo < 300) * 12;
            const SPEED =
              (m.crouch ? 1.2 : 1) * 10 -
              radius * 0.4 +
              Math.min(5, Math.sqrt(this.charge));
            const dir = m.angle + 0.15 * (Math.random() - 0.5);
            const velocity = {
              x: SPEED * Math.cos(dir),
              y: SPEED * Math.sin(dir),
    ...
    },
    {
      ...
      railDo() {
        if (this.charge > 0) {
          ...
          if (!input.fire && this.charge > 0.6) {
            ...
            for (let i = 0, len = mob.length; i < len; ++i) {
              //push away mobs when firing
              if (!mob[i].isUnblockable) {
                const SUB = Vector.sub(mob[i].position, m.pos);
                const DISTANCE = Vector.magnitude(SUB);
                if (DISTANCE < range + mob[i].radius) {
                  const DEPTH =
                    100 + Math.min(range - DISTANCE + mob[i].radius, 1500);
                  const FORCE = Vector.mult(
                    Vector.normalise(SUB),
                    0.0015 * Math.sqrt(DEPTH) * mob[i].mass,
                  );
                  ...
                  simulation.drawList.push({
                    ...
                    y: mob[i].position.y,
                    radius:
                      Math.log(dmg + 1.1) * 40 * mob[i].damageReduction + 3,
            ...
            for (let i = 0, len = powerUp.length; i < len; ++i) {
              ...
              if (DISTANCE < range) {
                ...
                const FORCE = Vector.mult(
                  Vector.normalise(SUB),
                  0.002 * Math.sqrt(DEPTH) * powerUp[i].mass,
                );
                powerUp[i].force.x += FORCE.x;
                powerUp[i].force.y +=
                  FORCE.y - powerUp[i].mass * simulation.g * 1.5; //kick up a bit to give them some arc
              }
            }
            ...
            for (let i = 0, len = 10 + 25 * this.charge; i < len; i++) {
              ...
              const where = Vector.add(
                m.pos,
                Vector.mult(unit, range * (0.6 + 0.3 * Math.random())),
              );
              simulation.drawList.push({
                x: where.x,
                y: where.y,
            ...
            const recoil = Vector.mult(
              Vector.normalise(Vector.sub(where, m.pos)),
              m.crouch ? 0.03 : 0.06,
            );
            player.force.x -= recoil.x;
            player.force.y -= recoil.y;
            const harpoonSize = tech.isLargeHarpoon
              ? 1 + 0.1 * Math.sqrt(this.ammo)
              : 1;
            ...
            if (tech.extraHarpoons) {
              ...
              for (let i = 0, len = mob.length; i < len; ++i) {
                if (
                  ...
                  const dist = Vector.magnitude(
                    Vector.sub(where, mob[i].position),
                  );
                  if (dot > 0.95 - Math.min(dist * 0.00015, 0.3)) {
                    //lower dot product threshold for targeting then if you only have one harpoon //target closest mob that player is looking at and isn't too close to target
                    // if (this.ammo > -1) {
                    //     this.ammo--
                    b.harpoon(
                      where,
              ...
              for (let i = 0, len = mob.length; i < len; ++i) {
                if (
                  ...
                  if (
                    ...
                    dot > 0.98 - Math.min(dist * 0.00014, 0.3)
                  ) {
                    //target closest mob that player is looking at and isn't too close to target
                    closest.distance = dist;
                    closest.target = mob[i];
                  }
                }
            ...
            const rate =
              Math.sqrt(b.fireCDscale) *
              tech.railChargeRate *
              (tech.isCapacitor ? 0.6 : 1) *
              (m.crouch ? 0.8 : 1);
      ...
      harpoonFire() {
        ...
        const totalCycles =
          6.5 *
          (tech.isFilament ? 1 + 0.013 * Math.min(110, this.ammo) : 1) *
          Math.sqrt(harpoonSize);

        if (tech.extraHarpoons && !m.crouch) {
          //multiple harpoons
          const SPREAD = 0.2;
          let angle = m.angle - (SPREAD * tech.extraHarpoons) / 2;
          const dir = {
          ...
          const range =
            450 * (tech.isFilament ? 1 + 0.012 * Math.min(110, this.ammo) : 1);
          let targetCount = 0;
          for (let i = 0, len = mob.length; i < len; ++i) {
            if (
              mob[i].alive &&
              ...
              if (dist < range && dot > 0.9) {
                ...
                if (this.ammo > 0) {
                  ...
                  b.harpoon(
                    ...
                    true,
                    totalCycles,
                  ); //Vector.angle(Vector.sub(where, mob[i].position), { x: 0, y: 0 })
                  angle += SPREAD;
                  targetCount++;
                  if (targetCount > tech.extraHarpoons) break;
                }
          ...
          //m.crouch makes a single harpoon with longer range
          const dir = {
            x: Math.cos(m.angle),
            y: Math.sin(m.angle),
          }; //make a vector for the player's direction of length 1; used in dot product
          ...
          if (m.crouch && m.onGround) {
            b.harpoon(
              ...
              true,
              1.6 * totalCycles,
              m.crouch && tech.crouchAmmoCount && (tech.crouchAmmoCount - 1) % 2
                ? false
                : true,
        ...
        m.fireCDcycle =
          ...
          35 * b.fireCDscale +
          60 * (m.energy < 0.05) +
          tech.extraHarpoons; // cool down is set when harpoon bullet returns to player
        const recoil = Vector.mult(
          Vector.normalise(Vector.sub(where, m.pos)),
          m.crouch ? 0.015 : 0.035,
        );
        player.force.x -= recoil.x;
        player.force.y -= recoil.y;
      },
      ...
      fire() {
        if (m.crouch) {
          if (tech.isLaserMine) {
            ...
            b.mine(
              pos,
              { x: speed * Math.cos(m.angle), y: speed * Math.sin(m.angle) },
              0,
            );
    ...
    },
    {
        ...
        if (tech.isPulseLaser) {
          this.fire = () => {
            const drain = Math.min(
              0.9 * m.maxEnergy,
              (0.01 * (tech.isCapacitor ? 10 : 1)) / b.fireCDscale,
            );
                ...
                if (!input.fire) {
                  if (this.charge > 5) {
                    ...
                    for (let i = 0; i < len; i++) {
                      ...
                      const off = history.yOff - 24.2859;
                      b.pulse(
                        1.65 * this.charge * this.lensDamage,
                        history.angle,
                        {
                          x: history.position.x,
                ...
                if (!input.fire) {
                  if (this.charge > 5) {
                    ...
                    if (tech.beamSplitter) {
                      ...
                      const angle =
                        m.angle - (tech.beamSplitter * divergence) / 2;
                      for (let i = 0; i < 1 + tech.beamSplitter; i++)
                        b.pulse(this.charge, angle + i * divergence);
                    } else {
      ...
      fireWideBeam() {
        ...
        if (m.energy < drain) {
          ...
          const dmg =
            ((0.7 * tech.laserDamage) / b.fireCDscale) * this.lensDamage; //  3.5 * 0.55 = 200% more damage
          const where = {
            x: m.pos.x + 30 * Math.cos(m.angle),
            y: m.pos.y + 30 * Math.sin(m.angle),
          };
          ...
          ctx.beginPath();
          if (
            Matter.Query.ray(map, eye, where).length === 0 &&
            Matter.Query.ray(body, eye, where).length === 0
          ) {
            b.laser(
              eye,
              {
          ...
          for (let i = 1; i < tech.wideLaser; i++) {
            ...
            if (
              Matter.Query.ray(map, eye, whereOff).length === 0 &&
              Matter.Query.ray(body, eye, whereOff).length === 0
            ) {
              ctx.moveTo(eye.x, eye.y);
              ctx.lineTo(whereOff.x, whereOff.y);
              b.laser(
      ...
      fireHistory() {
        ...
        if (m.energy < drain) {
          ...
          m.energy -= drain;
          const dmg =
            ((0.5 * tech.laserDamage) / b.fireCDscale) * this.lensDamage; //  3.5 * 0.55 = 200% more damage
