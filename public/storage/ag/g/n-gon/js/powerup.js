...
const powerUps = {
  ...
  attractHeal() {
    for (let i = 0; i < powerUp.length; i++) {
      //attract heal power ups to player
      if (powerUp[i].name === "heal") {
        let attract = Vector.mult(
          Vector.normalise(Vector.sub(m.pos, powerUp[i].position)),
          0.015 * powerUp[i].mass,
        );
        powerUp[i].force.x += attract.x;
        powerUp[i].force.y += attract.y - powerUp[i].mass * simulation.g; //negate gravity
        Matter.Body.setVelocity(
          powerUp[i],
          Vector.mult(powerUp[i].velocity, 0.7),
        );
      }
    }
  },
  dupExplode() {
    for (let i = 0, len = powerUp.length; i < len; ++i) {
      if (powerUp[i].isDuplicated) {
        if (Math.random() < 0.003 && !m.isBodiesAsleep) {
          //  (1-0.003)^240 = chance to be removed after 4 seconds,   240 = 4 seconds * 60 cycles per second
          b.explosion(
            powerUp[i].position,
            175 + (11 + 3 * Math.random()) * powerUp[i].size,
          );
          if (powerUp[i]) {
  ...
  choose(type, index) {
    if (type === "gun") {
      ...
      if (b.inventory.length === 1)
        text += `<br>input.key.gun<span class='color-symbol'>:</span> ["<span class='color-text'>MouseLeft</span>"]`;
      ...
      simulation.makeTextLog(
        `<span class='color-var'>tech</span>.giveTech("<span class='color-text'>${tech.tech[index].name}</span>")`,
      );
      tech.giveTech(index);
    }
  ...
  endDraft(type, isCanceled = false) {
    //type should be a gun, tech, or field
    if (isCanceled) {
      if (tech.isCancelDuplication) {
        const value = 0.05;
        tech.duplication += value;
        simulation.makeTextLog(
          `tech.duplicationChance() <span class='color-symbol'>+=</span> ${value}`,
        );
      ...
      if (tech.isCancelRerolls) {
        for (let i = 0, len = 8 + 4 * Math.random(); i < len; i++) {
          ...
          powerUps.spawn(
            m.pos.x + 40 * (Math.random() - 0.5),
            m.pos.y + 40 * (Math.random() - 0.5),
            spawnType,
            false,
          );
        }
  ...
  difficulty: {
    ...
    effect() {
      ...
      requestAnimationFrame(() => {
        //add a background behind the power up menu
      ...
      //build level info
      document
        .getElementById("choose-grid")
        .classList.add("choose-grid-no-images");
      document.getElementById("choose-grid").classList.remove("choose-grid");
      document.getElementById("choose-grid").style.gridTemplateColumns =
        "405px"; //adjust this to increase the width of the whole menu, but mostly the center column
      ...
      document
        ...
        .addEventListener("click", () => {
          ...
          if (localSettings.isHideImages) {
            ...
            document
              .getElementById("choose-grid")
              .classList.remove("choose-grid");
          } else {
            document.getElementById("choose-grid").classList.add("choose-grid");
            document
              .getElementById("choose-grid")
              .classList.remove("choose-grid-no-images");
          }
          if (
            level.levelsCleared === 0 &&
            initialDifficultyMode !== simulation.difficultyMode
          ) {
  ...
  boost: {
    ...
    effect() {
      ...
      powerUps.boost.endCycle =
        m.cycle +
        Math.floor(Math.max(0, powerUps.boost.endCycle - m.cycle) * 0.6) +
        powerUps.boost.duration; //duration+seconds plus 2/3 of current time left
    },
    draw() {
      ...
      if (powerUps.boost.endCycle > m.cycle) {
        ...
        ctx.beginPath();
        const arc =
          (powerUps.boost.endCycle - m.cycle) / powerUps.boost.duration;
        ctx.arc(
          m.pos.x,
          m.pos.y,
          28,
  ...
  research: {
    ...
    changeRerolls(amount) {
      ...
      if (tech.isDeathAvoid && document.getElementById("tech-anthropic")) {
        document.getElementById("tech-anthropic").innerHTML =
    ...
    use(type) {
      //runs when you actually research a list of selections, type can be field, gun, or tech
      ...
      if (tech.isResearchDamage) {
        tech.damage *= 1.05;
        simulation.makeTextLog(
          `<strong>1.05x</strong> <strong class='color-d'>damage</strong>`,
        );
        tech.addJunkTechToPool(0.01);
      }
      ...
      if (tech.isResearchReality) {
        ...
        simulation.trails();
        simulation.makeTextLog(
          `simulation.amplitude <span class='color-symbol'>=</span> ${Math.random()}`,
        );
      }
      powerUps[type].effect();
    },
  ...
  heal: {
    ...
    size() {
      return (
        Math.sqrt(0.1 + 0.25) *
        40 *
        simulation.healScale ** 0.25 *
        Math.sqrt(tech.largerHeals * (tech.isHalfHeals ? 0.5 : 1))
      ); //(simulation.healScale ** 0.25)  gives a smaller radius as heal scale goes down
    },
    effect() {
      if (!tech.isEnergyHealth && m.alive) {
        ...
        if (heal > 0) {
          ...
          const healOutput =
            Math.min(m.maxHealth - m.health, heal) * simulation.healScale;
          m.addHealth(heal);
          if (healOutput > 0)
            simulation.makeTextLog(
              `<span class='color-var'>m</span>.health <span class='color-symbol'>+=</span> ${healOutput.toFixed(3)}`,
            ); // <br>${m.health.toFixed(3)}
          if (tech.isOverHeal && overHeal > 0) {
            //tech quenching
            overHeal *= 2; //double the over heal converted to max health
            //make sure overHeal doesn't kill player
            if (m.health - overHeal * m.defense() < 0)
              overHeal = m.health - 0.01;
            ...
            simulation.makeTextLog(
              `<span class='color-var'>m</span>.health <span class='color-symbol'>-=</span> ${overHeal.toFixed(3)}`,
            ); // <br>${m.health.toFixed(3)}
            simulation.drawList.push({
          ...
          } else if (overHeal > 0.13) {
            //if leftover heals spawn a new spammer heal power up
            requestAnimationFrame(() => {
              powerUps.directSpawn(
                ...
                true,
                null,
                overHeal * 40 * simulation.healScale ** 0.25,
          ...
          if (tech.isHealBrake) {
            ...
            for (let i = 0; i < simulation.ephemera.length; i++) {
              if (simulation.ephemera[i].name === "healPush") {
                ...
                simulation.ephemera[i].count =
                  0.5 * simulation.ephemera[i].count + totalTime; //add time
                simulation.ephemera[i].scale =
                  0.5 *
                  (simulation.ephemera[i].scale +
                    Math.min(Math.max(0.6, heal * 6), 2.3)); //take average of scale
              }
            }
            if (!foundActiveEffect) {
              simulation.ephemera.push({
                ...
                do() {
                  ...
                  this.range =
                    this.range * 0.99 +
                    0.01 * (300 * this.scale + 100 * Math.sin(m.cycle * 0.022));
      ...
      if (powerUps.healGiveMaxEnergy) {
        tech.healMaxEnergyBonus +=
          0.14 * tech.largerHeals * (tech.isHalfHeals ? 0.5 : 1);
        m.setMaxEnergy();
      }
    },
    spawn(x, y, size) {
      //used to spawn a heal with a specific size / heal amount, not normally used
  ...
  ammo: {
    ...
    effect() {
      const couplingExtraAmmo =
        m.fieldMode === 10 || m.fieldMode === 0 ? 1 + 0.04 * m.coupling : 1;
      if (b.inventory.length > 0) {
        powerUps.animatePowerUpGrab("rgba(68, 102, 119,0.25)");
        if (tech.isAmmoForGun && b.activeGun !== null) {
          //give extra ammo to one gun only with tech logistics
          const target = b.guns[b.activeGun];
          if (target.ammo !== Infinity) {
            if (tech.ammoCap) {
              target.ammo = Math.ceil(
                2 * target.ammoPack * tech.ammoCap * couplingExtraAmmo,
              );
            } else {
              target.ammo += Math.ceil(
                2 *
          ...
          for (let i = 0, len = b.inventory.length; i < len; i++) {
            ...
            if (target.ammo !== Infinity) {
              if (tech.ammoCap) {
                target.ammo = Math.ceil(
                  target.ammoPack * tech.ammoCap * couplingExtraAmmo,
                );
              } else {
                //default ammo behavior
                target.ammo += Math.ceil(
                  (Math.random() + Math.random()) *
                    target.ammoPack *
                    couplingExtraAmmo,
  ...
  cancelText(type) {
    ...
    if (tech.isSuperDeterminism) {
      ...
      level.levelsCleared === 0 &&
      localSettings.isTrainingNotAttempted &&
      b.inventory.length === 0
    ) {
      //don't show cancel if on initial level and haven't done tutorial
  ...
  researchText(type) {
    ...
    if (type === "entanglement") {
      ...
      tech.isJunkResearch &&
      powerUps.research.currentRerollCount < 2
    ) {
      ...
      text += `<div onclick="powerUps.research.use('${type}')" class='research-card' >`; // style = "margin-left: 192px; margin-right: -192px;"
      text += `<div><div><span style="position:relative;">`;
      for (let i = 0, len = Math.min(powerUps.research.count, 30); i < len; i++)
  ...
  researchAndCancelText(type) {
    ...
    if (type === "entanglement") {
      ...
      for (
        let i = 0, len = Math.min(powerUps.research.count, researchCap);
        i < len;
        i++
      ) {
        text += `<div class="circle-grid research" style="font-size:0.82em; position:absolute; top:0; left:${(18 - len * 0.21) * i}px ;opacity:0.8; border: 1px #fff solid;"></div>`;
  ...
  gunText(choose, click) {
    const style = localSettings.isHideImages
      ? powerUps.hideStyle
      : `style="background-image: url('img/gun/${b.guns[choose].name}.webp');"`;
  ...
  fieldText(choose, click) {
    const style = localSettings.isHideImages
      ? powerUps.hideStyle
      : `style="background-image: url('img/field/${m.fieldUpgrades[choose].name}${choose === 0 ? Math.floor(Math.random() * 10) : ""}.webp');"`;
    return `<div class="choose-grid-module card-background" onclick="${click}" onauxclick="${click}"${style}>
        <div class="card-text">
  ...
  techText(choose, click) {
    const techCountText =
      tech.tech[choose].count > 0 ? `(${tech.tech[choose].count + 1}x)` : "";
    const style =
      localSettings.isHideImages || tech.tech[choose].isLore
        ? powerUps.hideStyle
        : `style="background-image: url('img/${tech.tech[choose].name}.webp');"`;
    return `<div class="choose-grid-module card-background" onclick="${click}" onauxclick="${click}"${style}>
                <div class="card-text">
  ...
  junkTechText(choose, click) {
    ...
    const style = localSettings.isHideImages
      ? powerUps.hideStyle
      : `style="background-size: contain;background-repeat: no-repeat;background-image: url('img/junk.webp');"`;
    if (!localSettings.isHideImages) {
      // setTimeout(() => { //delay so that the html element exists
      //     if (tech.tech[choose].url === undefined) { //if on url has been set yet
    ...
    effect() {
      if (m.alive) {
        ...
        if (options.length > 0 || !tech.isSuperDeterminism) {
          ...
          for (let i = 0; i < b.guns.length; i++)
            b.guns[i].isRecentlyShown = false; //reset recently shown back to zero
          // if (options.length > 0) {
          let text = powerUps.buildColumns(totalChoices, "gun");
          for (let i = 0; i < totalChoices; i++) {
            const choose =
              options[Math.floor(Math.seededRandom(0, options.length))]; //pick an element from the array of options
            ...
            text += powerUps.gunText(
              choose,
              `powerUps.choose('gun',${choose})`,
            );
          ...
          if (tech.isExtraBotOption) {
            ...
            if (botTech.length > 0) {
              //pick random bot tech
              const choose =
                botTech[Math.floor(Math.random() * botTech.length)];
              const techCountText =
                tech.tech[choose].count > 0
                  ? `(${tech.tech[choose].count + 1}x)`
                  : "";
              const style = localSettings.isHideImages
                ? powerUps.hideStyle
                : `style="background-image: url('img/${tech.tech[choose].name}.webp');"`;
          ...
          if (tech.isOneGun && b.inventory.length > 0)
            text += `<div style = "color: #f24">replaces your current gun</div>`;
          document.getElementById("choose-grid").innerHTML = text;
          powerUps.showDraft();
        }
        // }
  ...
  field: {
    ...
    effect() {
      if (m.alive) {
        let options = [];
        for (let i = 1; i < m.fieldUpgrades.length; i++) {
          //skip field emitter
        ...
        for (let i = 0; i < m.fieldUpgrades.length; i++)
          m.fieldUpgrades[i].isRecentlyShown = false; //reset recently shown back to zero
        ...
        if (options.length > 0 || tech.isExtraBotOption) {
          ...
          for (let i = 0; i < totalChoices; i++) {
            ...
            text += powerUps.fieldText(
              choose,
              `powerUps.choose('field',${choose})`,
            );
      ...
      if (m.alive) {
        ...
        for (let i = 0; i < tech.tech.length; i++) {
          if (
            tech.tech[i].count < tech.tech[i].maxCount &&
            tech.tech[i].allowed() &&
            !tech.tech[i].isBanished
          ) {
            if (tech.tech[i].frequency > 0) optionLengthNoDuplicates++;
            for (let j = 0, len = tech.tech[i].frequency; j < len; j++)
              options.push(i);
          }
        }
        function removeOption(index) {
        ...
        if (optionLengthNoDuplicates < totalChoices + 1) {
          //if not enough options for all the choices
          totalChoices = optionLengthNoDuplicates;
          if (tech.isBanish) {
            //when you run out of options eject banish
            for (let i = 0, len = tech.tech.length; i < len; i++) {
              if (tech.tech[i].name === "decoherence")
                powerUps.ejectTech(i, true);
            }
            simulation.makeTextLog(
              `decoherence <span class='color-var'>tech</span> ejected`,
            );
        ...
        for (let i = 0; i < tech.tech.length; i++)
          tech.tech[i].isRecentlyShown = false; //reset recently shown back to zero
        if (options.length > 0) {
          ...
          for (let i = 0; i < totalChoices; i++) {
            ...
            if (tech.isBanish) {
              ...
              if (i === 0)
                simulation.makeTextLog(
                  `options.length = ${optionLengthNoDuplicates} <span class='color-text'>//tech removed from pool by decoherence</span>`,
                );
            }
            ...
            if (Math.random() < tech.junkChance) {
              // choose is set to a random JUNK tech
              const list = [];
              for (let i = 0; i < tech.tech.length; i++) {
                if (tech.tech[i].isJunk) list.push(i);
              }
              chooseJUNK = list[Math.floor(Math.random() * list.length)];
              text += powerUps.junkTechText(
                chooseJUNK,
                `powerUps.choose('tech',${chooseJUNK})`,
              );
            } else {
              if (tech.tech[choose].isFieldTech) {
                text += powerUps.fieldTechText(
                  choose,
                  `powerUps.choose('tech',${choose})`,
                );
              } else if (tech.tech[choose].isGunTech) {
                text += powerUps.gunTechText(
                  choose,
                  `powerUps.choose('tech',${choose})`,
                );
                ...
                text += powerUps.skinTechText(
                  choose,
                  `powerUps.choose('tech',${choose})`,
                );
              } else if (tech.tech[choose].isInstant) {
                text += powerUps.instantTechText(
                  choose,
                  `powerUps.choose('tech',${choose})`,
                );
              } else {
                //normal tech
                text += powerUps.techText(
                  choose,
                  `powerUps.choose('tech',${choose})`,
                );
              }
          ...
          if (tech.isMassProduction) {
            ...
            for (let i = 0, len = tech.tech.length; i < len; i++) {
              if (tech.tech[i].isMassProduction) {
                const style = localSettings.isHideImages
                  ? powerUps.hideStyle
                  : `style="background-image: url('img/${tech.tech[i].name}.webp');"`;
          ...
          if (tech.isExtraGunField) {
            if (Math.random() > 0.5 && b.inventory.length < b.guns.length) {
              ...
              const pick =
                gunOptions[Math.floor(Math.seededRandom(0, gunOptions.length))]; //pick an element from the array of options
              ...
              const pick =
                fieldOptions[
                  Math.floor(Math.seededRandom(0, fieldOptions.length))
                ]; //pick an element from the array of options
          ...
          if (
            ...
            const cycle = (timestamp) => {
              ...
              if (
                timestamp - timeStart > tech.brainStormDelay * count &&
                simulation.isChoosing
              ) {
                count++;
                powerUps.tech.effect();
                document.getElementById("choose-grid").style.pointerEvents =
                  "auto"; //turn off the normal 500ms delay
                document.body.style.cursor = "auto";
  ...
  entanglement: {
    ...
    effect() {
      if (m.alive && localSettings.entanglement) {
        ...
        if (
          localSettings.entanglement.fieldIndex &&
          localSettings.entanglement.fieldIndex !== m.fieldMode
        ) {
        ...
        for (let i = 0; i < localSettings.entanglement.gunIndexes.length; i++) {
          //add guns
        ...
        for (
          let i = 0;
          i < localSettings.entanglement.techIndexes.length;
          i++
        ) {
          //add tech
          ...
          for (let j = 0; j < tech.tech.length; j++) {
            if (
              localSettings.entanglement.techIndexes[i] === tech.tech[j].name
            ) {
              choose = j;
              found = true;
              break;
          ...
          if (found && tech.tech[choose]) {
            ...
            if (
              choose === null ||
              tech.tech[choose].count + 1 > tech.tech[choose].maxCount ||
              !tech.tech[choose].allowed()
            ) {
              // text += `<div class="choose-grid-module" style = "background-color: #efeff5; border: 0px; opacity:0.5; font-size: 60%; line-height: 130%; margin: 1px; padding-top: 6px; padding-bottom: 6px;"><div class="grid-title">${tech.tech[choose].name} <span style = "color: #aaa;font-weight: normal;font-size:80%;">- incoherent</span></div></div>`
  ...
  spawnRandomPowerUp(x, y) {
    //mostly used after mob dies,  doesn't always return a power up
    if (
      (!tech.isEnergyHealth &&
        Math.random() * Math.random() - 0.3 > Math.sqrt(m.health)) ||
      Math.random() < 0.04
    ) {
    ...
    if (Math.random() < 0.0007 * (3 - b.inventory.length)) {
      //a new gun has a low chance for each not acquired gun up to 3
  ...
  spawnBossPowerUp(x, y) {
    //boss spawns field and gun tech upgrades
  ...
  chooseRandomPowerUp(x, y) {
    //100% chance to drop a random power up    //used in spawn.debris
    if (Math.random() < 0.5) {
  ...
  addResearchToLevel() {
    //add a random power up to a location that has a mob,  mostly used to give each level a research
    if (simulation.difficultyMode < 4 && mob.length) {
      //don't spawn on higher difficulty settings
      const index = Math.floor(Math.random() * mob.length);
      powerUps.spawn(mob[index].position.x, mob[index].position.y, "research");
    }
  },
  spawnStartingPowerUps(x, y) {
    //used for map specific power ups, mostly to give player a starting gun
    if (level.levelsCleared < 4) {
      //runs on first 4 levels on all difficulties
      if (level.levelsCleared > 1) powerUps.spawn(x, y, "tech");
      if (b.inventory.length === 0) {
        powerUps.spawn(x, y, "gun", false); //first gun
      } else if (tech.totalCount === 0) {
        //first tech
        powerUps.spawn(x, y, "tech", false);
      } else if (b.inventory.length === 1) {
        //second gun or extra ammo
        if (Math.random() < 0.4) {
      ...
      //after the first 4 levels just spawn a random power up
    ...
    if (!simulation.isChoosing || isOverride) {
      ...
      if (choose === "random") {
        ...
        if (have.length) {
          ...
          simulation.makeTextLog(
            `<span class='color-var'>tech</span>.remove("<span class='color-text'>${tech.tech[choose].name}</span>")`,
          );
  ...
  pauseEjectTech(index) {
    if (
      (tech.isPauseEjectTech || simulation.testing) &&
      !simulation.isChoosing &&
      !tech.tech[index].isInstant
    ) {
      ...
      document.getElementById(`${index}-pause-tech`).style.textDecoration =
        "line-through";
  ...
  randomize(where) {
    //makes a random power up convert into a random different power up
    //put 10 power ups close together
    const len = Math.min(10, powerUp.length);
    for (let i = 0; i < len; i++) {
      //collide the first 10 power ups
      const unit = Vector.rotate({ x: 1, y: 0 }, 6.28 * Math.random());
      Matter.Body.setPosition(
        powerUp[i],
        Vector.add(where, Vector.mult(unit, 20 + 25 * Math.random())),
      );
    ...
    if (smallIndexes.length > 2 && Math.random() < 0.66) {
      ...
      for (let j = 0; j < 3; j++) {
        for (let i = 0; i < powerUp.length; i++) {
          if (
            powerUp[i].name === "heal" ||
            powerUp[i].name === "research" ||
            powerUp[i].name === "ammo" ||
            powerUp[i].name === "coupling" ||
            powerUp[i].name === "boost"
          ) {
            Matter.Composite.remove(engine.world, powerUp[i]);
            powerUp.splice(i, 1);
            break;
  ...
  directSpawn(
    ...
    let polygonSides;
    if (isDuplicated) {
      polygonSides = tech.isPowerUpsVanish
        ? 3
        : Math.floor(4 + 2 * Math.random());
