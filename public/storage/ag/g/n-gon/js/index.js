...
document.getElementById("seed").placeholder = Math.initialSeed = String(
...
function vertexCollision(v1, v1End, domains) {
  //= [map, body, [playerBody, playerHead]]     //m.isCloak ? [map, body] : [map, body, [playerBody, playerHead]]
  let results;
  let best = {
    x: null,
    y: null,
  ...
  for (let j = 0; j < domains.length; j++) {
    ...
    for (let i = 0; i < domain.length; ++i) {
      ...
      const len = vertices.length - 1;
      for (let j = 0; j < len; j++) {
        results = simulation.checkLineIntersection(
          v1,
          v1End,
          vertices[j],
          vertices[j + 1],
...
function beforeUnloadEventListener(event) {
  ...
  if (tech.isExitPrompt) {
    ...
    simulation.makeTextLog(
      `damage <span class='color-symbol'>*=</span> ${1.25}`,
    );
    if (Math.random() < 0.25) {
  ...
  if (Object.keys(set).length !== 0) {
    ...
    for (const property in set) {
      ...
      requestAnimationFrame(() => {
        build.sortTech("have", true);
      });
    }
  } else if (
    localSettings.isTrainingNotAttempted &&
    localSettings.runCount < 30
  ) {
    //make training button more obvious for new players
    ...
    let myanim = document.createElementNS(
...
for (let i = 0, len = tech.tech.length; i < len; i++) {
  if (!tech.tech[i].link)
    tech.tech[i].link =
      `<a target="_blank" href='https://en.wikipedia.org/w/index.php?search=${encodeURIComponent(tech.tech[i].name).replace(/'/g, "%27")}&title=Special:Search' class="link">${tech.tech[i].name}</a>`;
}
const build = {
  pixelDraw() {
    let count = 0;
  ...
  showImages(from) {
    //on click event:  from all 3 different places to hide / show images
        ...
        do() {
          if (!(m.cycle % 15)) {
            ...
            if (m.lastCalculatedDefense !== defense) {
              document.getElementById("defense-bar").style.width =
                Math.floor(300 * m.maxHealth * (1 - defense)) + "px";
              m.lastCalculatedDefense = defense;
            }
            const damage = tech.damageFromTech(); //update damage bar
            if (m.lastCalculatedDamage !== damage) {
              document.getElementById("damage-bar").style.height =
                Math.floor(
                  (Math.atan(0.25 * damage - 0.25) + 0.25) *
                    0.53 *
                    canvas.height,
                ) + "px";
  ...
  generatePauseLeft() {
    ...
    if (tech.orbitBotCount)
      botText += `<br>orbital-bots: ${tech.orbitBotCount}`;
    ...
    if (tech.dynamoBotCount)
      botText += `<br>dynamo-bots: ${tech.dynamoBotCount}`;
    if (tech.plasmaBotCount)
      botText += `<br>plasma-bots: ${tech.plasmaBotCount}`;
    if (tech.missileBotCount)
      botText += `<br>missile-bots: ${tech.missileBotCount}`;
...
<details> <summary>difficulty</summary>
    ...
    if (tech.isPauseSwitchField || simulation.testing) {
      ...
      const style = localSettings.isHideImages
        ? `style="height:auto;"`
        : `style="background-image: url('img/field/${m.fieldUpgrades[m.fieldMode].name}${m.fieldMode === 0 ? m.fieldUpgrades[0].imageNumber : ""}.webp');"`;
      text += `<div class="pause-grid-module card-background" id="pause-field" ${style} >
                                                    <div class="card-text">
                                                        <div class="grid-title"><div class="circle-grid field"></div> &nbsp; ${build.nameLink(m.fieldUpgrades[m.fieldMode].name)}</div>
                                                        ${m.fieldUpgrades[m.fieldMode].description}</div> </div>`;
    ...
    for (let i = 0, len = b.inventory.length; i < len; i++) {
      const style = localSettings.isHideImages
        ? `style="height:auto;"`
        : `style="background-image: url('img/gun/${b.guns[b.inventory[i]].name}.webp');"`;
  ...
  generatePauseRight() {
    ...
    const ejectClass =
      tech.isPauseEjectTech && !simulation.isChoosing ? "pause-eject" : "";
    for (let i = 0, len = tech.tech.length; i < len; i++) {
      if (tech.tech[i].count > 0) {
        const style =
          localSettings.isHideImages ||
          tech.tech[i].isJunk ||
          tech.tech[i].isLore
            ? `style="height:auto;"`
            : `style = "background-image: url('img/${tech.tech[i].name}.webp');"`;
        const techCountText =
          tech.tech[i].count > 1 ? `(${tech.tech[i].count}x)` : "";
    ...
    function pressEnterSort(event) {
      if (event.key === "Enter") {
        requestAnimationFrame(() => {
          document.getElementById("sort-input").focus();
        });
    ...
    document
      .getElementById("sort-input")
      .addEventListener("keydown", pressEnterSort);
    requestAnimationFrame(() => {
      document.getElementById("sort-input").focus();
    });
  },
  sortTech(find, isExperiment = false) {
    const sortKeyword = (a, b) => {
      let aHasKeyword =
        (a.descriptionFunction
          ? a.descriptionFunction()
          : a.description
        ).includes(find) || a.name.includes(find);
      let bHasKeyword =
        (b.descriptionFunction
          ? b.descriptionFunction()
          : b.description
        ).includes(find) || b.name.includes(find);
  ...
  choosePowerUp(index, type, isAllowed = false) {
    if (type === "gun") {
      ...
      for (let i = 0, len = b.inventory.length; i < len; i++) {
        //look for selection in inventory
        if (b.guns[b.inventory[i]].name === b.guns[index].name) {
          //if already clicked, remove gun
          isDeselect = true;
          document
            .getElementById("gun-" + b.inventory[i])
            .classList.remove("build-gun-selected");
          //remove gun
      ...
      if (!isDeselect) {
        //add gun
        document
          .getElementById("gun-" + index)
          .classList.add("build-gun-selected");
        b.giveGuns(index);
      }
    } else if (type === "field") {
      ...
      if (tech.tech[index].count < tech.tech[index].maxCount) {
        ...
        if (
          !document
            ...
            .classList.contains("build-tech-selected")
        )
          document
            .getElementById("tech-" + index)
            .classList.add("build-tech-selected");
        tech.giveTech(index);
      } else if (!tech.tech[index].isInstant) {
        // tech.totalCount -= tech.tech[index].count
        document
          .getElementById("tech-" + index)
          .classList.remove("build-tech-selected");
  ...
  updateExperimentText(isAllowed = false) {
    for (let i = 0, len = tech.tech.length; i < len; i++) {
      ...
      if (
        (!tech.tech[i].isJunk || localSettings.isJunkExperiment) &&
        !tech.tech[i].isLore
      ) {
        if (tech.tech[i].allowed() || isAllowed || tech.tech[i].count > 0) {
          ...
          if (
            tech.tech[i].count === 0 &&
            techID.classList.contains("build-tech-selected")
          )
          ...
          if (techID.classList.contains("experiment-grid-disabled")) {
            ...
            techID.setAttribute(
              "onClick",
              `javascript: build.choosePowerUp(${i},'tech')`,
            );
          }
        } else {
          //disabled color for disabled tech
          ...
          if (techID.classList.contains("build-tech-selected"))
            techID.classList.remove("build-tech-selected");
          if (tech.tech[i].isFieldTech) {
            techID.innerHTML = build.fieldTechText(i);
          } else if (tech.tech[i].isGunTech) {
  ...
  populateGrid() {
    ...
    for (let i = 0, len = m.fieldUpgrades.length; i < len; i++) {
      const style = localSettings.isHideImages
        ? hideStyle
        : `style="background-image: url('img/field/${m.fieldUpgrades[i].name}${i === 0 ? m.fieldUpgrades[0].imageNumber : ""}.webp');"`;
    ...
    for (let i = 0, len = b.guns.length; i < len; i++) {
      const style = localSettings.isHideImages
        ? hideStyle
        : `style="background-image: url('img/gun/${b.guns[i].name}.webp');"`;
    ...
    for (let i = 0, len = tech.tech.length; i < len; i++) {
      if (
        ...
        const style =
          localSettings.isHideImages || tech.tech[i].isJunk
            ? hideStyle
            : `style="background-image: url('img/${tech.tech[i].name}.webp');"`;
        if (
          (tech.tech[i].allowed() || tech.tech[i].count > 0) &&
          (!tech.tech[i].isInstant || localSettings.isJunkExperiment)
        ) {
          // || tech.tech[i].name === "+1 cardinality") { //|| tech.tech[i].name === "leveraged investment"
    ...
    for (let i = 0, len = tech.tech.length; i < len; i++) {
      if (document.getElementById(`tech-${i}`)) {
        document
          .getElementById(`tech-${i}`)
          .setAttribute("data-descr", tech.tech[i].requires); //add tooltip
  ...
  nameLink(text) {
    //converts text into a clickable wikipedia search
  ...
  shareURL(isCustom = false) {
    ...
    if (isCustom) {
      ...
      simulation.makeTextLog(
        "n-gon build URL copied to clipboard.<br>Paste into browser address bar.",
      );
    }
    console.log(
  ...
  startExperiment() {
    //start playing the game after exiting the experiment menu
    ...
    if (!simulation.isCheating) {
      ...
      if (b.inventory.length !== 0 || m.fieldMode !== 0)
        simulation.isCheating = true;
    }
    if (simulation.isCheating) {
      //if you are cheating remove any lore you might have gotten
      lore.techCount = 0;
      for (let i = 0, len = tech.tech.length; i < len; i++) {
...
const input = {
  ...
  controlTextUpdate() {
    ...
    document.getElementById("key-next-gun").innerHTML = cleanText(
      input.key.nextGun,
    );
    document.getElementById("key-previous-gun").innerHTML = cleanText(
      input.key.previousGun,
    );
    document.getElementById("key-testing").innerHTML = cleanText(
      input.key.testing,
    ); //if (localSettings.loreCount > 0)

    document.getElementById("splash-up").innerHTML = cleanText(input.key.up)[0];
    document.getElementById("splash-down").innerHTML = cleanText(
      input.key.down,
    )[0];
    document.getElementById("splash-left").innerHTML = cleanText(
      input.key.left,
    )[0];
    document.getElementById("splash-right").innerHTML = cleanText(
      input.key.right,
    )[0];
    document.getElementById("splash-next-gun").innerHTML = cleanText(
      input.key.nextGun,
    )[0];
    document.getElementById("splash-previous-gun").innerHTML = cleanText(
      input.key.previousGun,
    )[0];
  ...
  setTextFocus() {
    ...
    document.getElementById("key-previous-gun").style.background =
      backgroundColor;
...
document
  .getElementById("control-details")
...
document
  ...
  .addEventListener("click", input.setDefault);
...
window.addEventListener("keydown", function (event) {
    ...
    case input.key.pause:
      if (input.isPauseKeyReady && m.alive && !build.isExperimentSelection) {
        ...
        if (simulation.isChoosing) {
          ...
          if (tech.isPauseSwitchField || simulation.testing) {
            document
              .getElementById("pause-field-previous")
              .addEventListener("click", () => {
                const energy = m.energy; //save current energy
                if (m.fieldMode === 4 && simulation.molecularMode > 0) {
                  simulation.molecularMode--;
                  m.fieldUpgrades[4].description =
                    m.fieldUpgrades[4].setDescription();
                } else {
                  m.setField(
                    m.fieldMode < 2
                      ? m.fieldUpgrades.length - 1
                      : m.fieldMode - 1,
                  ); //cycle to previous field, skip field emitter
                  if (m.fieldMode === 4) {
                ...
                document.getElementById("pause-field").style.backgroundImage =
                  `url('img/field/${m.fieldUpgrades[m.fieldMode].name}${m.fieldMode === 0 ? Math.floor(Math.random() * 10) : ""}.webp')`;
                document.getElementById("pause-field").innerHTML =
                  `<div class="card-text"> <div class="grid-title"><div class="circle-grid field"></div> &nbsp; ${build.nameLink(m.fieldUpgrades[m.fieldMode].name)}</div>${m.fieldUpgrades[m.fieldMode].description}</div>`;
              });

            document
              .getElementById("pause-field-next")
              .addEventListener("click", () => {
                const energy = m.energy; //save current energy
                if (m.fieldMode === 4 && simulation.molecularMode < 3) {
                  ...
                  m.setField(
                    m.fieldMode === m.fieldUpgrades.length - 1
                      ? 1
                      : m.fieldMode + 1,
                  ); //cycle to next field, skip field emitter
                  if (m.fieldMode === 4) {
    ...
    case input.key.testing:
      if (
        ...
        if (simulation.difficultyMode > 4) {
          simulation.makeTextLog(
            "<em>testing mode disabled for this difficulty</em>",
          );
          break;
        }
  ...
  if (simulation.testing) {
    ...
    switch (event.key.toLowerCase()) {
      ...
      case "f":
        const mode =
          m.fieldMode === m.fieldUpgrades.length - 1 ? 0 : m.fieldMode + 1;
        m.setField(mode);
        break;
      case "g":
        b.giveGuns("all", 1000);
...
if (localSettings.isAllowed && !localSettings.isEmpty) {
  ...
  document.getElementById("community-maps").checked =
    localSettings.isCommunityMaps;

  if (localSettings.fpsCapDefault === undefined)
    localSettings.fpsCapDefault = "max";
  if (localSettings.personalSeeds === undefined)
    localSettings.personalSeeds = [];
  ...
  if (
    localSettings.banList.length === 0 ||
    localSettings.banList === "undefined"
  ) {
  ...
  if (localSettings.difficultyCompleted === undefined) {
    localSettings.difficultyCompleted = [
      null,
      false,
      false,
      false,
      ...
      false,
      false,
    ]; //null because there isn't a difficulty zero
    localStorage.setItem("localSettings", JSON.stringify(localSettings)); //update local storage
  }

  if (localSettings.difficultyMode === undefined)
    localSettings.difficultyMode = "2";
...
document.getElementById("updates").addEventListener("toggle", function () {
  ...
  loadJSON(
    "https://api.github.com/repos/landgreen/n-gon/commits",
    function (data) {
  ...
  portamento(frequency, end = 1000, shiftRate = 10, gain = 0.05) {
    ...
    for (let i = 0, len = end * 0.1; i < len; i++)
      oscillator.frequency.setValueAtTime(
        frequency + i * shiftRate,
        audioCtx.currentTime + i * 0.01,
      );
    setTimeout(() => {
      audioCtx.suspend();
      audioCtx.close();
...
if (!localSettings.isHideImages) {
  addEventListener("load", () => {
    let urls = new Array();
    for (let i = 0, len = b.guns.length; i < len; i++)
      urls.push("img/gun/" + b.guns[i].name + ".webp");
    for (let i = 1, len = m.fieldUpgrades.length; i < len; i++)
      urls.push("img/field/" + m.fieldUpgrades[i].name + ".webp");
    for (let i = 0, len = tech.tech.length; i < len; i++) {
      if (!tech.tech[i].isJunk && !tech.tech[i].isLore)
        urls.push("img/" + tech.tech[i].name + ".webp");
    }
...
function cycle() {
  ...
  if (elapsed > simulation.fpsInterval) {
