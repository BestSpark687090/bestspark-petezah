...
const spawn = {
    ...
    if (options.length === 0) {
      const boss =
        spawn.bossTypeSpawnOrder[
          spawn.bossTypeSpawnIndex++ % spawn.bossTypeSpawnOrder.length
        ];
      spawn[boss](x, y);
    } else {
  ...
  setSpawnList() {
    ...
    const push =
      spawn.mobTypeSpawnOrder[
        spawn.mobTypeSpawnIndex++ % spawn.mobTypeSpawnOrder.length
      ];
  ...
  spawnChance(chance) {
    const difficultyChance =
      simulation.difficultyMode === 1 ? 1 : simulation.difficulty;
    return (
      Math.random() < chance + 0.07 * difficultyChance &&
      mob.length < -1 + 16 * Math.log10(simulation.difficulty + 1)
    );
  },
  ...
  randomSmallMob(
    ...
    y,
    num = Math.max(
      Math.min(Math.round(Math.random() * simulation.difficulty * 0.2), 4),
      0,
    ),
  ...
  randomGroup(x, y, chance = 1) {
    if (
      (spawn.spawnChance(chance) && simulation.difficulty > 2) ||
      chance === Infinity
    ) {
      ...
      if (canBeGroup) {
        ...
        if (Math.random() < 0.1) {
          ...
          spawn.spawnOrbitals(
            mob[mob.length - 1],
            mob[mob.length - 1].radius + 50 + 200 * Math.random(),
            1,
          );
  ...
  //mob templates *********************************************************************************************
  //***********************************************************************************************************
  MACHO(x = m.pos.x, y = m.pos.y) {
    //immortal mob that follows player         //if you have the tech it spawns at start of every level at the player
      ...
      if (!simulation.isTimeSkipping) {
        ...
        if (tech.isMoveMACHO && m.crouch && input.down) {
          Matter.Body.setVelocity(
            this,
            Vector.add(
              Vector.mult(this.velocity, 0.97),
              Vector.mult(player.velocity, 0.03),
            ),
          );
          Matter.Body.setPosition(
            this,
            Vector.add(
              Vector.mult(this.position, 0.95),
              Vector.mult(player.position, 0.05),
            ),
          );
        }
        ...
        if (mag < this.radius) {
          //buff to player when inside radius
  ...
  WIMP(
    x = level.exit.x + tech.wimpCount * 200 * (Math.random() - 0.5),
    y = level.exit.y + tech.wimpCount * 200 * (Math.random() - 0.5),
  ) {
    //immortal mob that follows player //if you have the tech it spawns at start of every level at the exit
    mobs.spawn(x, y, 3, 0.1, "transparent");
    let me = mob[mob.length - 1];
    me.stroke = "transparent";
    me.isShielded = true; //makes it immune to damage
    ...
    me.awake = function () {
      ...
      const where = Vector.add(
        this.position,
        Vector.mult(Vector.normalise(sub), this.chaseSpeed),
      );
      ...
      if (
        ...
        if (m.energy > DRAIN) {
          ...
          m.energy = 0;
          m.damage(
            (tech.isRadioactiveResistance ? 0.005 * 0.2 : 0.005) *
              simulation.dmgScale,
          );
          simulation.drawList.push({
            //add dmg to draw queue
  ...
  finalBoss(x, y, radius = 300) {
    ...
    let me = mob[mob.length - 1];
    setTimeout(() => {
      //fix mob in place, but allow rotation
    ...
    me.damageReductionDecay = function () {
      //slowly make the boss take more damage over time  //damageReduction resets with each invulnerability phase
      if (!(me.cycle % 60) && this.lastDamageCycle + 240 > this.cycle)
        this.damageReduction *= 1.04; //only decay once a second   //only decay if the player has done damage in the last 4 seconds
    };
    me.mobType =
      spawn.fullPickList[Math.floor(Math.random() * spawn.fullPickList.length)];
    me.spawnMobs = function (index = 0) {
      ...
      spawn[me.mobType](
        where.x + 50 * (Math.random() - 0.5),
        where.y + 50 * (Math.random() - 0.5),
      );
    ...
    me.mode = [
        ...
        do() {
          if (!(me.cycle % this.spawnRate) && mob.length < me.maxMobs) {
            ...
            const index = Math.floor(
              (me.cycle % (this.spawnRate * 6)) / this.spawnRate,
            ); //int from 0 to 5
            if (index === 0)
              me.mobType =
                spawn.fullPickList[
        ...
        do() {
          if (!(me.cycle % this.spawnRate) && mob.length < me.maxMobs) {
            ...
            for (let i = 0; i < 6; i++) {
              ...
              spawn.hopBullet(
                vertex.x + 50 * (Math.random() - 0.5),
                vertex.y + 50 * (Math.random() - 0.5),
                13 + Math.ceil(Math.random() * 8),
              ...
              const velocity = Vector.mult(
                Vector.perp(Vector.normalise(Vector.sub(me.position, vertex))),
                -18,
              ); //give the mob a rotational velocity as if they were attached to a vertex
              Matter.Body.setVelocity(mob[mob.length - 1], {
                x: me.velocity.x + velocity.x,
                y: me.velocity.y + velocity.y,
              });
      ...
      },
      {
        ...
        do() {
          ...
          this.bombCycle++;
          if (
            !(this.bombCycle % this.bombInterval) &&
            this.bombCycle % 660 > 330
          ) {
            //mines above player
            if (simulation.isHorizontalFlipped) {
              const x = m.pos.x + 200 * (Math.random() - 0.5);
              if (x > -750) {
                //mines above player IN tunnel
                spawn.mine(
                  Math.min(Math.max(-730, x), 100),
                  -450 - yOff * Math.random(),
                ); //player in main room
                mob[mob.length - 1].fallHeight = -209;
              } else {
                //mines above player NOT in tunnel
                spawn.mine(
                  Math.min(Math.max(-5375, x), -765),
                  -1500 - yOff * Math.random(),
                ); //player in tunnel
                mob[mob.length - 1].fallHeight = -9;
              }
              if (Math.random() < 0.5) {
                spawn.mine(
                  -5350 + 4550 * Math.random(),
                  -1500 - yOff * Math.random(),
                ); //random mines
                mob[mob.length - 1].fallHeight = -9;
              }
              ...
              if (x < 750) {
                ...
                //mines above player NOT in tunnel
                spawn.mine(
                  Math.min(Math.max(760, x), 5375),
                  -1500 - yOff * Math.random(),
                ); //player in tunnel
                mob[mob.length - 1].fallHeight = -9;
              }
              if (Math.random() < 0.5) {
                //random mines, but not in tunnel
                spawn.mine(
                  800 + 4550 * Math.random(),
                  -1500 - yOff * Math.random(),
                ); //random mines
                mob[mob.length - 1].fallHeight = -9;
              }
      ...
      },
      {
        ...
        do() {
          this.orbitersCycle++;
          if (
            !(this.orbitersCycle % this.spawnRate) &&
            this.orbitersCycle % 660 > 600 &&
            mob.length < me.maxMobs
          ) {
            const speed =
              (0.01 + 0.0005 * simulation.difficultyMode) *
              (Math.random() < 0.5 ? 0.85 : -1.15);
            ...
            me.orbitalNoVelocity(
              me,
              dist + 900 * (Math.random() - 0.5),
              0.1 * Math.random() + phase,
              speed,
      ...
      },
      {
        ...
        do() {
          ...
          if (this.fadeCycle > 0) {
            ...
            if (this.fadeCycle > 360)
              this.fadeCycle =
                -200 + simulation.difficultyMode * simulation.difficultyMode; //turn laser off and reset
            ctx.strokeStyle = "#50f";
            ctx.setLineDash([70 + 300 * Math.random(), 55 * Math.random()]);
            ctx.lineWidth = 1.5;
            ctx.beginPath();
            if (this.fadeCycle < 120) {
              //damage scales up over 2 seconds to give player time to move as it fades in
              const scale = this.fadeCycle / 120;
              const dmg =
                this.fadeCycle < 60 ? 0 : 0.1 * simulation.dmgScale * scale;
      ...
      },
      {
        ...
        do() {
          ...
          this.eventHorizon = Math.max(
            0,
            this.eventHorizonRadius * Math.sin(this.eventHorizonCycle * 0.007),
          ...
          if (
            Vector.magnitude(Vector.sub(me.position, player.position)) <
            this.eventHorizon
          ) {
            if (m.immuneCycle < m.cycle) {
              if (m.energy > 0) m.energy -= 0.018;
              if (m.energy < 0.05 && m.immuneCycle < m.cycle)
                m.damage(0.0003 * simulation.dmgScale);
            }
            const angle = Math.atan2(
              player.position.y - me.position.y,
              player.position.x - me.position.x,
            );
            player.force.x -=
              0.0017 * Math.cos(angle) * player.mass * (m.onGround ? 1.7 : 1);
      ...
      },
      {
        ...
        do() {
          ...
          if (this.cycle % this.cycleDuration === 0) {
            ...
            this.whereX =
              (simulation.isHorizontalFlipped ? -3000 : 3000) +
              this.width * this.zone;
          }
          //draw slow zone
          if (this.cycle % this.cycleDuration > 0.45 * this.cycleDuration) {
            ...
            if (
              player.position.x > this.whereX &&
              player.position.x < this.whereX + this.width
            ) {
            ...
            //show where slow zone is about to show up
          ...
          if (this.playerSlowTime > 0) {
            ...
            Matter.Body.setVelocity(player, {
              x:
                Math.max(0.05, 1 - 0.01 * Math.max(10, this.playerSlowTime)) *
                player.velocity.x,
              y:
                Math.max(0.2, 1 - 0.0025 * this.playerSlowTime) *
                player.velocity.y,
            });
      ...
      },
      {
        ...
        do() {
          ...
          if (this.cycle === this.startCycle) {
            ...
            //antigravity
            for (let i = 0, len = body.length; i < len; ++i) {
              //push blocks away horizontally
              body[i].force.y -= simulation.g * body[i].mass;
              Matter.Body.setVelocity(
                body[i],
                Vector.mult(body[i].velocity, 0.98),
              ); //friction
            }
            for (let i = 0, len = powerUp.length; i < len; ++i) {
            ...
            Matter.Body.setVelocity(
              player,
              Vector.mult(player.velocity, 0.985),
            ); //friction
            //graphics
    ...
    me.spawnRate =
      5800 - 30 * simulation.difficultyMode * simulation.difficultyMode;
    me.spawnBoss = function () {
      //if the fight lasts too long start spawning bosses
      if (!(me.cycle % this.spawnRate) && this.health < 1) {
        this.spawnRate = Math.max(
          300,
          this.spawnRate -
            10 * simulation.difficultyMode * simulation.difficultyMode,
        ); //reduce the timer each time a boss spawns
        spawn.randomLevelBoss(
          3000 * (simulation.isHorizontalFlipped ? -1 : 1) +
            2000 * (Math.random() - 0.5),
          -1100 + 200 * (Math.random() - 0.5),
        );
      }
    };
    me.pushAway = function (magX = 0.13, magY = 0.05) {
      for (let i = 0, len = body.length; i < len; ++i) {
        //push blocks away horizontally
        body[i].force.x +=
          magX * body[i].mass * (body[i].position.x > this.position.x ? 1 : -1);
      ...
      for (let i = 0, len = bullet.length; i < len; ++i) {
        ...
        bullet[i].force.x +=
          magX *
          bullet[i].mass *
          (bullet[i].position.x > this.position.x ? 1 : -1);
      ...
      for (let i = 0, len = powerUp.length; i < len; ++i) {
        ...
        powerUp[i].force.x +=
          magX *
          powerUp[i].mass *
          (powerUp[i].position.x > this.position.x ? 1 : -1);
        powerUp[i].force.y -= magY * powerUp[i].mass;
      }
      player.force.x +=
        magX * player.mass * (player.position.x > this.position.x ? 1 : -1);
    ...
    me.boulder = function (x, y) {
      ...
      me.spin =
        ...
        0.000005 *
        (1 + Math.random()) *
        (m.pos.x > me.position.x ? 1 : -1);
      Matter.Body.setAngularVelocity(
        me,
        0.1 * (1 + 0.3 * Math.random()) * (m.pos.x > me.position.x ? 1 : -1),
      );
    ...
    me.orbitalNoVelocity = function (who, radius, phase, speed) {
      //orbitals that don't include their host velocity  //specifically for finalBoss
      let boss = this;
      ...
      me.do = function () {
        ...
        Matter.Body.setPosition(
          this,
          Vector.add(who.position, Vector.mult(orbit, radius)),
        );
        if (
          Matter.Query.collides(this, [player]).length > 0 &&
          !(m.isCloak && tech.isIntangible) &&
          m.immuneCycle < m.cycle
        ) {
    ...
    me.lasers = function (where, angle, dmg = 0.1 * simulation.dmgScale) {
      ...
      best = vertexCollision(
        where,
        look,
        m.isCloak ? [map, body] : [map, body, [playerBody, playerHead]],
      );
      if (
        best.who &&
        (best.who === playerBody || best.who === playerHead) &&
        m.immuneCycle < m.cycle
      ) {
        if (m.immuneCycle < m.cycle + 60 + m.collisionImmuneCycles)
          m.immuneCycle = m.cycle + 60 + m.collisionImmuneCycles; //player is immune to damage extra time
        m.damage(dmg);
        simulation.drawList.push({
          //add dmg to draw queue
          x: best.x,
    ...
    me.onDeath = function () {
      if (!this.hasRunDeathScript) {
        ...
        const v = Matter.Vertices.hull(
          Matter.Vertices.clockwiseSort(this.vertices),
        ); //might help with vertex collision issue, not sure
        ...
        if (lore.techCount > lore.techGoal - 1 && !simulation.isCheating) {
          simulation.makeTextLog(
            `<span class="lore-text">undefined</span> <span class='color-symbol'>=</span> ${lore.techCount}/${lore.techGoal}`,
            360,
          );
          setTimeout(function () {
            simulation.makeTextLog(
              `level.levels.push("<span class='lore-text'>null</span>")`,
              720,
            );
            unlockExit();
            level.levels.push("null");
          ...
          function loop() {
            if (!simulation.paused && !simulation.onTitlePage) {
              count++;
              if (count < 660) {
                if (count === 1 && simulation.difficultyMode < 5)
                  simulation.makeTextLog(
                    `<em>//enter testing mode to set level.levels.length to <strong>Infinite</strong></em>`,
                  );
                if (!(count % 60))
                  simulation.makeTextLog(
                    `simulation.analysis <span class='color-symbol'>=</span> ${((count / 60 - Math.random()) * 0.1).toFixed(3)}`,
                  );
              } else if (count === 660) {
                simulation.makeTextLog(
                  `simulation.analysis <span class='color-symbol'>=</span> 1 <em>//analysis complete</em>`,
                );
              } else if (count === 780) {
                simulation.makeTextLog(
                  `<span class="lore-text">undefined</span> <span class='color-symbol'>=</span> ${lore.techCount}/${lore.techGoal}`,
                ...
                simulation.makeTextLog(
                  `Engine.clear(engine) <em>//simulation successful</em>`,
                );
              } else if (count === 1260) {
            ...
            if (simulation.testing || simulation.difficultyMode > 4) {
              ...
              setTimeout(function () {
                simulation.makeTextLog(
                  `level.levels.length <span class='color-symbol'>=</span> <strong>Infinite</strong>`,
                );
              }, 1500);
            } else {
        ...
        for (let i = 0, len = body.length; i < len; ++i) {
          const velocity = Vector.mult(
            Vector.normalise(Vector.sub(this.position, body[i].position)),
            65,
          );
          const pushUp = Vector.add(velocity, { x: 0, y: -0.5 });
          Matter.Body.setVelocity(
            body[i],
            Vector.add(body[i].velocity, pushUp),
          );
        }
        //damage all mobs
        for (let j = 0; j < 8; j++) {
          //in case some mobs leave things after they die
          for (let i = 0, len = mob.length; i < len; ++i) {
            if (mob[i] !== this) {
  ...
  zombie(x, y, radius, sides, color) {
    //mob that attacks other mobs
    ...
    me.lookForMobTargets = function () {
      if (
        ...
        this.target &&
        (!this.target.alive ||
          Matter.Query.ray(map, this.position, this.target.position).length !==
            0)
      ) {
    ...
    me.zombieHealthBar = function () {
      ...
      if ((this.health < 0.01 || isNaN(this.health)) && this.alive)
        this.death();
      ...
      if (this.hitCD < simulation.cycle && !this.isStunned && !this.isSlowed) {
        if (this.target) {
          this.force = Vector.mult(
            Vector.normalise(Vector.sub(this.target.position, this.position)),
            this.accelMag * this.mass,
          );
        } else {
          //wonder around
        ...
        const hit = (who) => {
          if (!who.isZombie && who.damageReduction) {
            ...
            who.locatePlayer();
            if ((who.health < 0.01 || isNaN(who.health)) && who.alive)
              who.death();
      ...
      if (this.seePlayer.recall) {
        ...
        for (let i = 0, len = mob.length; i < len; i++) {
          if (mob[i].isGrouper && mob[i] != this && mob[i].isDropPowerUp) {
            //don't tether to self, bullets, shields, ...
            const distance2 = Vector.magnitudeSquared(
              Vector.sub(this.position, mob[i].position),
            );
  ...
  blockBoss(x, y, radius = 60) {
    ...
    me.onDeath = function () {
      ...
      for (const who of mob) {
        if (who.isNecroMob) {
          //blockMobs leave their body, and die
          who.leaveBody = true;
          who.damage(Infinity);
        }
      }
    ...
    me.do = function () {
      ...
      this.seePlayerCheck();
      if (this.target) {
        //(this.target === player && this.seePlayer.yes) || this.target !== player
        const force = Vector.mult(
          Vector.normalise(Vector.sub(this.target.position, this.position)),
          this.accelMag * this.mass,
        );
      ...
      if (!(simulation.cycle % 30)) {
        ...
        for (let i = 0; i < body.length; i++) {
          if (
            Vector.magnitude(Vector.sub(this.position, body[i].position)) <
              700 &&
            !body[i].isNotHoldable
          ) {
            // check distance for each block
        ...
        if (this.distanceToPlayer() > 1500 && this.target === null) {
          ...
          if (body.length) {
            // look for a new target by finding the closest block
            let min = Infinity;
            let closestBlock = null;
            for (const block of body) {
              const dist = Vector.magnitudeSquared(
                Vector.sub(this.position, block.position),
              );
              if (
                dist < min &&
                Matter.Query.ray(map, this.position, block.position).length ===
                  0
              ) {
                min = dist;
                closestBlock = block;
        ...
        if (!(simulation.cycle % 90)) {
          ...
          if (count < 20 * Math.random() * Math.random()) {
            //limit number of spawns if there are already too many blockMobs
            const unit = Vector.normalise(
              Vector.sub(player.position, this.position),
            );
            for (let i = 0, len = 3 * Math.random(); i < len; i++) {
              ...
              const where = Vector.add(
                Vector.mult(unit, radius + 200 * Math.random()),
                this.position,
              );
              spawn.blockMob(
                where.x + 100 * (Math.random() - 0.5),
                where.y + 100 * (Math.random() - 0.5),
                null,
              );
      ...
      for (let i = 0; i < activeBeams.length; i++) {
        ...
        if (newBlockMob.alive) {
          const vertexIndex = Math.floor(
            ((newBlockMob.vertices.length - 1) * duration) / beamTotalDuration,
          );
          ctx.beginPath();
          ctx.moveTo(this.position.x, this.position.y);
          ctx.lineTo(
            newBlockMob.vertices[vertexIndex].x,
            newBlockMob.vertices[vertexIndex].y,
          );
  ...
  blockMob(x, y, host, growCycles = 60) {
    if (host === null) {
      ...
      const sideLength =
        Vector.magnitude(Vector.sub(host.vertices[0], host.vertices[1])) +
        Vector.magnitude(Vector.sub(host.vertices[1], host.vertices[2])) / 2; //average of first 2 sides
      mobs.spawn(x, y, 4, Math.min(70, sideLength), "#999");
      if (
        host.bounds.max.x - host.bounds.min.x < 150 &&
        host.bounds.max.y - host.bounds.min.y < 150
      ) {
    ...
    me.cycle = 0;
    me.do = function () {
      //grow phase only occurs for growCycles
      this.checkStatus();
      this.seePlayerCheck();
      this.cycle++;
  ...
  cellBoss(x, y, radius = 20, cellID) {
    ...
    me.onDamage = function (dmg) {
      if (
        Math.random() < 0.34 * dmg * Math.sqrt(this.mass) &&
        this.health > dmg
      )
        this.split();
    };
  ...
  spawnerBossCulture(
    ...
    y,
    radius = 50,
    num = 8 + Math.min(20, simulation.difficulty * 0.4),
  ) {
  ...
  spawnerBoss(x, y, radius, spawnID) {
    ...
    me.do = function () {
      ...
      if (this.seePlayer.recall) {
        ...
        for (let i = 0, len = mob.length; i < len; i++) {
          if (mob[i].isSpawnBoss && mob[i].spawnID === this.spawnID)
            mob[i].seePlayer.recall = 1;
        }
      }
    };
    me.onDeath = function () {
      ...
      const len = Math.min(
        12,
        spawns * Math.ceil(Math.random() * simulation.difficulty * spawns),
      );
      for (let i = 0; i < len; i++) {
        spawn.spawns(
          this.position.x + (Math.random() - 0.5) * radius * 2.5,
          this.position.y + (Math.random() - 0.5) * radius * 2.5,
        );
        Matter.Body.setVelocity(mob[mob.length - 1], {
  ...
  growBossCulture(
    ...
    y,
    radius = 17,
    nodes = 12 + Math.min(10, simulation.difficulty * 0.25),
  ) {
    ...
    for (let i = 0; i < nodes; ++i) {
      ...
      const mag = Math.max(
        radius,
        sideLength * (1 - Math.pow(Math.random(), 1.5)),
      ); //working on a distribution that is circular, random, but not too focused in the center
      spawn.growBoss(
  ...
  powerUpBossBaby(x, y, vertices = 9, radius = 60) {
    ...
    me.do = function () {
      ...
      if (this.alive) {
        for (
          let i = 0;
          i < Math.min(powerUp.length, this.vertices.length);
          i++
        ) {
    ...
    me.do = function () {
      ...
      if (this.seePlayer.recall) {
        if (
          ...
          this.force.y +=
            forceMag * Math.sin(angle) -
            (Math.random() * 0.06 + 0.1) * this.mass; //antigravity
        }
      } else {
        //randomly hob if not aware of player
        if (
          this.randomHopCD < simulation.cycle &&
          (Matter.Query.collides(this, map).length ||
            Matter.Query.collides(this, body).length)
        ) {
          this.randomHopCD = simulation.cycle + this.randomHopFrequency;
          //slowly change randomHopFrequency after each hop
          this.randomHopFrequency = Math.max(
            100,
            this.randomHopFrequency + (0.5 - Math.random()) * 200,
          );
          const forceMag =
            (this.accelMag + this.accelMag * Math.random()) *
            this.mass *
            (0.1 + Math.random() * 0.3);
    ...
    me.dropEgg = function () {
      if (mob.length < 360) {
        ...
        for (let i = 0; i < 30; i++) {
          //find the ground
          if (
            Matter.Query.point(map, where).length > 0 ||
            Matter.Query.point(body, where).length > 0
          )
            break;
          where.y += 1;
        }
  ...
  hopEgg(x, y) {
    mobs.spawn(
      ...
      y,
      10,
      9 + Math.floor(3 * Math.random()),
      "rgba(50, 150, 150,0.3)",
    ); //"rgb(100,170,150)" //"rgb(61, 125, 121)"
    let me = mob[mob.length - 1];
    me.stroke = "transparent";
      ...
      if (
        ...
        if (this.isRandomMove || Math.random() < 0.2) {
          ...
          this.force.x +=
            (0.01 + 0.03 * Math.random()) *
            this.mass *
            (player.position.x > this.position.x ? 1 : -1); //chase player
        }
  ...
  hopMotherBoss(x, y, radius = 120) {
    ...
    me.do = function () {
      ...
      if (
        ...
        for (let i = 0, len = 1 + 0.05 * simulation.difficulty; i < len; ++i)
          spawn.hopBullet(
            this.position.x + 100 * (Math.random() - 0.5),
            this.position.y + 100 * (Math.random() - 0.5),
          );

        this.force.x +=
          (0.02 + 0.06 * Math.random()) *
          this.mass *
          (player.position.x > this.position.x ? 1 : -1);
  ...
  spinner(x, y, radius = 30 + Math.ceil(Math.random() * 35)) {
    ...
    me.look = function () {
      ...
      if (this.seePlayer.recall && this.cd < simulation.cycle) {
        this.burstDir = Vector.normalise(
          Vector.sub(this.seePlayer.position, this.position),
        );
  ...
  sucker(x, y, radius = 30 + Math.ceil(Math.random() * 25)) {
    ...
    me.do = function () {
      ...
      //eventHorizon waves in and out
      const eventHorizon =
        this.eventHorizon * (0.93 + 0.17 * Math.sin(simulation.cycle * 0.011));
      //draw darkness
      ctx.beginPath();
      ctx.arc(
        this.position.x,
      ...
      if (
        ...
        player.force.x -=
          0.00125 * player.mass * Math.cos(angle) * (m.onGround ? 1.8 : 1);
    ...
    me.do = function () {
      ...
      if (!(simulation.cycle % this.seePlayerFreq)) {
        if (this.distanceToPlayer2() < this.seeAtDistance2) {
          //&& !m.isCloak   ignore cloak for black holes
          this.locatePlayer();
          if (!this.seePlayer.yes) this.seePlayer.yes = true;
        } else if (this.seePlayer.recall) {
          this.lostPlayer();
      ...
      if (this.seePlayer.recall) {
        ...
        if (!(simulation.cycle % 90)) {
          ...
          const velocity = Vector.mult(
            ...
            -20,
          ); //set direction to turn to fire                    //Vector.mult(Vector.perp(Vector.normalise(Vector.sub(this.position, this.vertices[index]))), -35)
        ...
        //eventHorizon waves in and out
        const eventHorizon =
          this.eventHorizon * (1 + 0.2 * Math.sin(simulation.cycle * 0.008)); //  zoom camera in and out with the event horizon
        //draw darkness
        ctx.beginPath();
        ctx.arc(
          this.position.x,
        ...
        if (
          ...
          player.force.x -=
            0.0013 * Math.cos(angle) * player.mass * (m.onGround ? 1.7 : 1);
        ...
        for (let i = 0; i < powerUp.length; i++) {
          //attract heal power ups
          const sub = Vector.sub(this.position, powerUp[i].position);
          const mag =
            0.0015 *
            Math.min(1, (Vector.magnitude(sub) - 200) / this.eventHorizon);
          const attract = Vector.mult(
            Vector.normalise(sub),
            mag * powerUp[i].mass,
          );
  ...
  spiderBoss(x, y, radius = 60 + Math.ceil(Math.random() * 10)) {
    ...
    for (let i = 0; i < nodes; ++i) {
      spawn.stabber(
        x + sideLength * Math.sin(i * angle),
        y + sideLength * Math.cos(i * angle),
        radius,
        12,
      );
  ...
  mantisBoss(x, y, radius = 35, isSpawnBossPowerUp = true) {
    ...
    me.do = function () {
      ...
      if (
        ...
        Matter.Query.ray(map, this.position, this.seePlayer.position).length ===
          0
      ) {
    ...
    me.onDeath = function () {
      this.removeCons();
      if (isSpawnBossPowerUp)
        powerUps.spawnBossPowerUp(this.position.x, this.position.y);
      for (let i = 0; i < this.babyList.length; i++) {
        if (this.babyList[i].alive) {
          this.babyList[i].collisionFilter.mask =
            cat.map | cat.bullet | cat.player;
          this.babyList[i].isInvulnerable = false;
          this.babyList[i].damageReduction = this.startingDamageReduction;
          this.babyList[i].collisionFilter.mask =
            cat.bullet | cat.player | cat.map | cat.body;
        }
      }
    };
    ...
    for (let i = 0; i < nodes; ++i) {
      ...
      babyMob.delay =
        60 + 55 * simulation.CDScale + Math.floor(Math.random() * 20);
          ...
          for (let j = 0; j < numRemoved; j++) {
            for (let i = 0; i < b.inventory.length; i++) {
              ...
              if (gun.ammo > 0 && gun.ammo !== Infinity) {
                gun.ammo -= Math.ceil(
                  (Math.random() + Math.random()) * gun.ammoPack,
                );
    ...
    me.awake = function () {
      ...
      if (this.distanceToPlayer() < this.laserRange) {
        ...
        requestAnimationFrame(() => {
          if (!simulation.paused && m.alive) {
            ctx.transform(
              1,
              this.warpIntensity * (Math.random() - 0.5),
              this.warpIntensity * (Math.random() - 0.5),
              1,
              0,
              0,
            ); //ctx.transform(Horizontal scaling. A value of 1 results in no scaling,  Vertical skewing,   Horizontal skewing,   Vertical scaling. A value of 1 results in no scaling,   Horizontal translation (moving),   Vertical translation (moving)) //https://developer.mozilla.org/en-US/docs/Web/API/CanvasRenderingContext2D/setTransform
        ...
        ctx.lineTo(m.pos.x, m.pos.y);
        ctx.lineTo(
          m.pos.x + (Math.random() - 0.5) * 3000,
          m.pos.y + (Math.random() - 0.5) * 3000,
        );
        ctx.lineWidth = 2;
      ...
      for (let i = 0, len = 6; i < len; i++) {
        ...
        ctx.ellipse(
          this.position.x,
          this.position.y,
          this.laserRange * Math.abs(Math.sin(phase + (i / len) * Math.PI)),
          this.laserRange,
          rotation,
          0,
          2 * Math.PI,
  ...
  focuser(x, y, radius = 30 + Math.ceil(Math.random() * 10)) {
    ...
    me.do = function () {
      ...
      if (this.seePlayer.yes && dist2 < 4000000) {
        ...
        this.laserPos = Vector.add(
          this.laserPos,
          Vector.mult(Vector.sub(player.position, this.laserPos), 0.1),
        );
        ...
        if (dist2 > 80000) {
          ...
          let laserOffR = Vector.rotateAbout(
            this.laserPos,
            (targetDist - r) * laserWidth,
            this.position,
          );
          ...
          let laserOffL = Vector.rotateAbout(
            this.laserPos,
            (targetDist - r) * -laserWidth,
            this.position,
          );
    ...
    me.do = function () {
      ...
      if (this.seePlayer.recall) {
        ...
        if (!(simulation.cycle % this.seePlayerFreq)) {
          ...
          const diff =
            mod(Math.atan2(sub.y, sub.x) - this.angle + Math.PI, 2 * Math.PI) -
            Math.PI;
          if (Math.abs(diff) > 2.8)
            this.torque += 0.0002 * this.inertia * Math.random();
        }
        ...
        this.wing(
          this.angle +
            Math.PI / 2 +
            flapArc * Math.sin(simulation.cycle * this.flapRate),
          this.flapRadius,
        );
        this.wing(
          this.angle -
  ...
  stinger(x, y, radius = 18 + 4 * Math.random()) {
    ...
    me.do = function () {
      ...
      if (this.seePlayer.recall) {
        ...
        this.frictionAir =
          0.11 +
          0.09 * Math.sin(simulation.cycle * this.flapRate - Math.PI / 2);
  ...
  beetleBoss(x, y, radius = 50) {
    ...
    me.pushAway = function (magX = 0.13, magY = 0.05) {
      for (let i = 0, len = body.length; i < len; ++i) {
        //push blocks away horizontally
        if (
          Vector.magnitudeSquared(Vector.sub(body[i].position, this.position)) <
          4000000
        ) {
          //2000
          body[i].force.x +=
            magX *
            body[i].mass *
            (body[i].position.x > this.position.x ? 1 : -1);
          body[i].force.y -= magY * body[i].mass;
        }
      }
      for (let i = 0, len = bullet.length; i < len; ++i) {
        ...
        if (
          Vector.magnitudeSquared(
            Vector.sub(bullet[i].position, this.position),
          ) < 4000000
        ) {
          //2000
          bullet[i].force.x +=
      ...
      for (let i = 0, len = powerUp.length; i < len; ++i) {
        ...
        if (
          Vector.magnitudeSquared(
            Vector.sub(powerUp[i].position, this.position),
          ) < 4000000
        ) {
          //2000
          powerUp[i].force.x +=
      ...
      if (
        Vector.magnitudeSquared(Vector.sub(player.position, this.position)) <
        4000000
      ) {
        //2000
        player.force.x +=
    ...
    me.do = function () {
      ...
      if (this.isInvulnerable) {
        ...
        if (this.invulnerableCount < 0) {
          ...
          this.wingGoal = 250;
          this.pushAway(
            Math.sqrt(this.flapRate) * 0.13,
            Math.sqrt(this.flapRate) * 0.06,
          ); //this.flapRate = 0.2, +0.13x3 -> 0.6
          me.babies(0.05 * simulation.difficulty + 1);
        }
        //draw invulnerable
        ctx.beginPath();
        ...
        ctx.fillStyle =
          this.fill = `hsla(${160 + 40 * Math.random()}, 100%, ${25 + 25 * Math.random() * Math.random()}%, 0.9)`; //"rgba(0,235,255,0.3)";   // ctx.fillStyle = `hsla(44, 79%, 31%,0.4)`; //"rgba(0,235,255,0.3)";
        this.wing(
          this.angle +
            Math.PI / 2 +
            flapArc * Math.sin(simulation.cycle * this.flapRate),
  ...
  laserTargetingBoss(x, y, radius = 80) {
    ...
    for (
      let i = 0, len = 2 + 0.3 * Math.sqrt(simulation.difficulty);
      i < len;
      i++
    )
      spawn.spawnOrbitals(me, radius + 40 + 10 * i, 1);
    // me.onHit = function() { };
      ...
      if (this.seePlayer.recall) {
        ...
        if (simulation.cycle % this.laserInterval > this.laserInterval / 2) {
          ...
          ctx.arc(
            ...
            1 +
              0.3 *
                (this.laserInterval - (simulation.cycle % this.laserInterval)),
            0,
            2 * Math.PI,
          ); //* this.fireCycle / this.fireDelay
          ctx.fillStyle = color;
          ctx.fill();
        } else {
          ctx.beginPath();
          ctx.arc(
            this.vertices[1].x,
            this.vertices[1].y,
            1 + 0.3 * (simulation.cycle % this.laserInterval),
            0,
            2 * Math.PI,
          ); //* this.fireCycle / this.fireDelay
          ctx.fillStyle = color;
  ...
  laserBombingBoss(x, y, radius = 80) {
    ...
    me.lookTorque =
      0.0000055 *
      (Math.random() > 0.5 ? -1 : 1) *
      (1 + 0.1 * Math.sqrt(simulation.difficulty));
    me.fireDir = { x: 0, y: 0 };
    ...
    me.do = function () {
      ...
      if (this.seePlayer.recall) {
        ...
        best = vertexCollision(
          this.position,
          look,
          m.isCloak ? [map] : [map, [playerBody, playerHead]],
        );
        ...
        if (best.who === playerBody || best.who === playerHead) {
          ...
          if (this.targetingCount > this.targetingTime) {
            ...
            spawn.grenade(
              ...
              this.vertices[1].y,
              dist / speed,
              Math.min(550, 250 + simulation.difficulty * 3),
              6,
            ); //    grenade(x, y, lifeSpan = 90 + Math.ceil(60 / simulation.accelScale), pulseRadius = Math.min(550, 250 + simulation.difficulty * 3), size = 4) {
    ...
    me.do = function () {
      ...
      if (this.nextBlinkCycle < simulation.cycle && this.seePlayer.yes) {
        ...
        if (distMag < this.blinkRange) {
          //if player is inside teleport range
          Matter.Body.setPosition(this, this.seePlayer.position);
        } else {
          Matter.Body.translate(
            this,
            Vector.mult(Vector.normalise(dist), this.blinkRange),
          );
        }
        spawn.grenade(
          this.position.x,
        ...
        Matter.Body.setVelocity(this, { x: 0, y: 0 });
        this.torque +=
          (0.00004 + 0.00003 * Math.random()) *
          this.inertia *
          (Math.round(Math.random()) * 2 - 1); //randomly spin around after firing
      }
      this.checkStatus();
    };
  },
      ...
      if (!m.isCloak && !this.isStunned) {
        if (this.isFiring) {
          if (this.fireCycle > this.fireDelay) {
            ...
            this.fireCycle = 0;
            this.torque +=
              (0.00008 + 0.00007 * Math.random()) *
              this.inertia *
              (Math.round(Math.random()) * 2 - 1); //randomly spin around after firing
            //is player in beam path
            if (
              Matter.Query.ray([player], this.fireTarget, this.position).length
            ) {
              unit = Vector.mult(
                Vector.normalise(Vector.sub(this.vertices[1], this.position)),
            ...
            if (
              Vector.magnitude(Vector.sub(player.position, this.fireTarget)) <
                this.pulseRadius &&
              m.immuneCycle < m.cycle
            ) {
          ...
          const angle = this.angle + Math.PI / 2;
          const c =
            Math.cos(angle) * this.fireDir.x + Math.sin(angle) * this.fireDir.y;
          ...
          if (c > threshold) {
            ...
            this.fireTarget = Vector.add(this.vertices[1], unit);
            if (Vector.magnitude(Vector.sub(m.pos, this.fireTarget)) < 1000) {
              //if's possible for this to be facing 180 degrees away from the player, this makes sure that doesn't occur
  ...
  pulsar(x, y, radius = 40) {
    ...
    me.canSeeTarget = function () {
      ...
      if (
        ...
        Matter.Query.ray(map, this.fireTarget, this.position).length ||
        Matter.Query.ray(body, this.fireTarget, this.position).length || //something blocking line of sight
  ...
  laserLayer(x, y, radius = 18 + Math.floor(6 * Math.random())) {
    ...
    me.vertices = Matter.Vertices.rotate(me.vertices, Math.PI, me.position); //make the pointy side of triangle the front
    for (let i = 0; i < 4; i += 2) {
      let spike = Vector.mult(
        Vector.normalise(Vector.sub(me.vertices[i], me.position)),
        radius * 2,
      );
    ...
    me.addLaser = function () {
      if (this.cycle > this.laserDelay) {
        ...
        const v1 = {
          x: this.position.x + seeRange * Math.cos(angle),
          y: this.position.y + seeRange * Math.sin(angle),
        };
        const v2 = {
          x: this.position.x + seeRange * Math.cos(angle + Math.PI),
          y: this.position.y + seeRange * Math.sin(angle + Math.PI),
        };
        ...
        if (best1.who === null) {
          //if the path never hits the map , just stop at seeRange
          best1.x = v1.x;
          best1.y = v1.y;
        }
        if (best1.y > best2.y) {
          //make laser beams always fire from top to bottom so they are predicable, and not stopped by blocks on the ground
        ...
        this.lasers.push({
          a: { x: best1.x, y: best1.y },
          b: { x: best2.x, y: best2.y },
          fade: 0,
        });
        ...
        if (
          !this.seePlayer.recall &&
          (Vector.magnitude(Vector.sub(this.position, this.driftGoal)) < 200 ||
            0.3 > Math.random())
        ) {
          //used in drift when can't find player
          const radius = Math.random() * 1000;
          const angle = Math.random() * 2 * Math.PI;
          this.driftGoal = Vector.add(this.driftCenter, {
    ...
    me.fireLaser = function () {
      for (let i = 0; i < this.lasers.length; i++) {
        //fire all lasers in the array
        let best = vertexCollision(
          this.lasers[i].a,
          this.lasers[i].b,
          m.isCloak ? [body] : [body, [playerBody, playerHead]],
        ); //not checking map to fix not hitting player bug, this might make some lasers look strange when the map changes
    ...
    me.driftGoal = Vector.add(me.driftCenter, {
      x: r * Math.cos(a),
      y: r * Math.sin(a),
    }); //updated in addLaser()
    me.drift = function () {
      ...
      if (this.seePlayer.recall) {
        ...
        //drift
        const force = Vector.mult(
          Vector.normalise(Vector.sub(this.driftGoal, this.position)),
          0.00002 * this.mass,
        );
    ...
    me.addLaser = function () {
      if (this.cycle > this.laserDelay) {
        ...
        let add = (where, angle) => {
          ...
          const v2 = {
            x: where.x + seeRange * Math.cos(angle + Math.PI),
            y: where.y + seeRange * Math.sin(angle + Math.PI),
          };
          //find where v1,v2 hit walls and make them stop there
  ...
  laserBoss(x, y, radius = 30) {
    ...
    me.rotateVelocity =
      Math.min(0.0045, 0.0015 * simulation.accelScale * simulation.accelScale) *
      (level.levelsCleared > 8 ? 1 : -1) *
      (simulation.isHorizontalFlipped ? -1 : 1);
    me.do = function () {
  ...
  stabber(x, y, radius = 25 + Math.ceil(Math.random() * 12), spikeMax = 7) {
    ...
    me.onDeath = function () {
      if (this.spikeLength > 4) {
        this.spikeLength = 4;
        const spike = Vector.mult(
          Vector.normalise(
            Vector.sub(this.vertices[this.spikeVertex], this.position),
          ),
          this.radius * this.spikeLength,
        );
  ...
  striker(x, y, radius = 14 + Math.ceil(Math.random() * 25)) {
    ...
    me.do = function () {
      this.gravity();
      if (!(simulation.cycle % this.seePlayerFreq)) {
        // this.seePlayerCheck();  from mobs
        if (
          this.distanceToPlayer2() < this.seeAtDistance2 &&
          Matter.Query.ray(map, this.position, this.playerPosRandomY())
            .length === 0 &&
          ...
          this.foundPlayer();
          if (this.cd === Infinity)
            this.cd = simulation.cycle + this.delay * 0.7;
        } else if (this.seePlayer.recall) {
          this.lostPlayer();
          this.cd = Infinity;
        }
      ...
      if (this.cd < simulation.cycle && this.seePlayer.recall) {
        ...
        if (distMag < 400) {
          Matter.Body.translate(
            this,
            Vector.mult(Vector.normalise(dist), distMag - 20 - radius),
          );
        } else {
          Matter.Body.translate(
            this,
            Vector.mult(Vector.normalise(dist), this.strikeRange),
          );
        }
        ctx.lineTo(this.position.x, this.position.y);
  ...
  revolutionBoss(x, y, radius = 70) {
    ...
    me.do = function () {
      ...
      for (let i = 0, len = this.vertices.length; i < len; i++) {
        ...
        const bend =
          bendFactor * Math.cos(this.laserAngle + (2 * Math.PI * i) / len);
        const long =
          this.swordRadiusMax *
          Math.sin(this.laserAngle + (2 * Math.PI * i) / len);
        if (long > 0)
          this.laserSword(
            this.vertices[i],
            bend + this.angle + ((i + 0.5) / sides) * 2 * Math.PI,
            Math.abs(long),
          );
      }
    };
    me.laserSword = function (where, angle, length) {
      ...
      const look = {
        x: where.x + length * Math.cos(angle),
        y: where.y + length * Math.sin(angle),
      };
      best = vertexCollision(
        where,
        look,
  ...
  sprayBoss(x, y, radius = 40, isSpawnBossPowerUp = true) {
    ...
    Matter.Body.setDensity(
      me,
      0.002 + 0.0001 * Math.sqrt(simulation.difficulty),
    ); //extra dense //normal is 0.001 //makes effective life much larger
    me.damageReduction = 0.09;
    me.startingDamageReduction = me.damageReduction;
    me.isInvulnerable = false;
    me.nextHealthThreshold = 0.75;
    ...
    me.radialLines = function () {
      ...
      for (let i = 0, len = this.vertices.length; i < len; i++) {
        ...
        const unit = Vector.add(
          Vector.mult(
            Vector.normalise(Vector.sub(this.vertices[i], this.position)),
            1000,
          ),
          this.vertices[i],
        );
    ...
    me.normalDoStuff = function () {
      ...
      if (this.speed < 0.01) {
        ...
        if (Math.abs(this.velocity.y) < 9)
          Matter.Body.setVelocity(this, {
            x: this.velocity.x,
            y: this.velocity.y * 1.03,
          });
        if (Math.abs(this.velocity.x) < 7)
          Matter.Body.setVelocity(this, {
            x: this.velocity.x * 1.03,
            y: this.velocity.y,
          });
      }
    };
    me.burstFire = function () {
      ...
      if (!(simulation.cycle % this.burstFireFreq)) {
        ...
        if (this.phaseCycle > this.burstTotalPhases) {
          ...
          Matter.Body.setVelocity(
            this,
            Vector.rotate({ x: 20, y: 0 }, 2 * Math.PI * Math.random()),
          );
          if (this.isShielded) {
            //remove shield
            for (let i = 0; i < mob.length; i++) {
        ...
        if (this.phaseCycle > -1) {
          ...
          for (let i = 0, len = this.vertices.length; i < len; i++) {
            ...
            const velocity = Vector.mult(
              Vector.normalise(Vector.sub(this.position, this.vertices[i])),
              -15,
            );
            Matter.Body.setVelocity(mob[mob.length - 1], {
              x: velocity.x,
  ...
  mineBoss(x, y, radius = 120, isSpawnBossPowerUp = true) {
    ...
    me.restitution = 1;
    me.friction = 0;
    me.collisionFilter.mask =
      cat.bullet | cat.player | cat.body | cat.map | cat.mob;
    me.explodeRange = 400;
    Matter.Body.setVelocity(me, {
      x: 10 * (Math.random() - 0.5),
      y: 10 * (Math.random() - 0.5),
    ...
    me.do = function () {
      ...
      this.checkStatus();
      if (!(simulation.cycle % 15) && mob.length < 360)
  ...
  bounceBoss(x, y, radius = 80, isSpawnBossPowerUp = true) {
    ...
    me.do = function () {
      ...
      if (this.isInvulnerable) {
        ...
        // for (let i = 0; i < 1; i++) {
        const velocity = Vector.rotate(
          Vector.mult(Vector.normalise(this.velocity), -5 - 10 * Math.random()),
          0.5 * (Math.random() - 0.5),
        );
  ...
  timeBoss(x, y, radius = 50, isSpawnBossPowerUp = true) {
    ...
    for (
      ...
      spawn.spawnOrbitals(me, radius + 10 + 10 * i, 1);
    ...
    me.do = function () {
      ...
      if (this.speed < 0.01) {
        ...
        if (Math.abs(this.velocity.y) < 10)
          Matter.Body.setVelocity(this, {
            x: this.velocity.x,
            y: this.velocity.y * 1.02,
          });
        if (Math.abs(this.velocity.x) < 7)
          Matter.Body.setVelocity(this, {
            x: this.velocity.x * 1.02,
            y: this.velocity.y,
          });
      }
  ...
  slashBoss(x, y, radius = 80) {
    ...
    me.swordRadiusGrowRate =
      me.swordRadiusMax * (0.005 + 0.0003 * simulation.difficulty);
    me.isSlashing = false;
    ...
    me.swordWaiting = function () {
      if (
        ...
        !m.isCloak &&
        Matter.Query.ray(map, this.position, this.playerPosRandomY()).length ===
          0 &&
        Matter.Query.ray(body, this.position, this.playerPosRandomY())
          .length === 0
      ) {
        //find vertex farthest away from player
        let dist = 0;
        for (let i = 0, len = this.vertices.length; i < len; i++) {
          const D = Vector.magnitudeSquared(
            Vector.sub({ x: this.vertices[i].x, y: this.vertices[i].y }, m.pos),
          );
          if (D > dist) {
            dist = D;
            this.swordVertex = i;
    ...
    me.swordGrow = function () {
      this.laserSword(
        this.vertices[this.swordVertex],
    ...
    me.laserSword = function (where, angle) {
      ...
      const look = {
        x: where.x + this.swordRadius * Math.cos(angle),
        y: where.y + this.swordRadius * Math.sin(angle),
      };
      best = vertexCollision(
        where,
        look,
  ...
  slasher(x, y, radius = 33 + Math.ceil(Math.random() * 30)) {
    ...
    me.swordRadiusGrowRate =
      me.swordRadiusMax * (0.018 + 0.0006 * simulation.difficulty);
    me.isSlashing = false;
  ...
  slasher2(x, y, radius = 33 + Math.ceil(Math.random() * 30)) {
    ...
    me.swordRadiusGrowRate =
      me.swordRadiusMax * (0.011 + 0.0002 * simulation.difficulty);
    me.isSlashing = false;
  ...
  slasher3(x, y, radius = 33 + Math.ceil(Math.random() * 30)) {
    ...
    me.swordWaiting = function () {
      ...
      if (
        ...
        this.laserAngle =
          (this.swordVertex / sides) * 2 * Math.PI + Math.PI / sides;
        this.sword = this.swordGrow;
        this.cycle = 0;
        ...
        const laserStartVector = Vector.sub(
          this.position,
          this.vertices[this.swordVertex],
        );
  ...
  sneakBoss(x, y, radius = 70) {
    ...
    me.onDeath = function () {
      powerUps.spawnBossPowerUp(this.position.x, this.position.y);
      for (let i = 0; i < simulation.difficultyMode / 2 - 0.5; i++)
        spawn.sneaker(
          this.position.x + 10 * Math.random(),
          this.position.y + 10 * Math.random(),
        );
    ...
    me.onDamage = function () {
      if (this.vanishesLeft > 0 && this.health < 0.1) {
        //if health is below 10% teleport to a random spot on player history, heal, and cloak
        this.vanishesLeft--;
        ...
        const index = Math.floor(
          (m.history.length - 1) * (0.66 + 0.2 * Math.random()),
        );
    ...
    me.deCloak = function () {
      if (this.isCloaked) {
        ...
        this.collisionFilter.mask =
          cat.player | cat.map | cat.body | cat.bullet | cat.mob; //can touch player
      }
    };
    me.do = function () {
      if (this.damageReduction === 0) {
        ...
        while (i--) {
          if (this.status[i].type === "stun" || this.status[i].type === "dot")
    ...
    me.onDamage = function () {
      if (
        ...
        Matter.Body.setPosition(
          this,
          m.history[
            Math.floor((m.history.length - 1) * (0.3 + 0.4 * Math.random()))
          ].position,
        );
        Matter.Body.setVelocity(this, { x: 0, y: 0 });
        this.damageReduction = 0; //immune to harm for the rest of this game cycle
  ...
  ghoster(x, y, radius = 50 + Math.ceil(Math.random() * 90)) {
    ...
    if (map.length)
      me.searchTarget =
        map[Math.floor(Math.random() * (map.length - 1))].position; //required for search
        ...
        if (b.inventory.length) {
          ...
          for (let j = 0; j < numRemoved; j++) {
            for (let i = 0; i < b.inventory.length; i++) {
              ...
              if (gun.ammo > 0 && gun.ammo !== Infinity) {
                gun.ammo -= Math.ceil(
                  1.1 * (Math.random() + Math.random()) * gun.ammoPack,
                );
                if (gun.ammo < 0) gun.ammo = 0;
                isRemovedAmmo = true;
              }
  ...
  bomberBoss(x, y, radius = 88) {
    ...
    Matter.Body.setDensity(
      me,
      0.0025 + 0.00013 * Math.sqrt(simulation.difficulty),
    ); //extra dense //normal is 0.001 //makes effective life much larger
    ...
    me.accelMag =
      Math.floor(10 * (Math.random() + 4.5)) * 0.00001 * simulation.accelScale;
    ...
    const speed =
      0.007 + 0.003 * Math.random() + 0.004 * Math.sqrt(simulation.difficulty);
    let radiusOrbitals = radius + 125 + 350 * Math.random();
    for (let i = 0; i < len; i++)
      spawn.orbital(me, radiusOrbitals, (i / len) * 2 * Math.PI, speed);
  ...
  shooter(x, y, radius = 25 + Math.ceil(Math.random() * 50)) {
    ...
    me.fireDir = { x: 0, y: 0 };
    me.onDeath = function () {
      //helps collisions functions work better after vertex have been changed
      // this.vertices = Matter.Vertices.hull(Matter.Vertices.clockwiseSort(this.vertices))
    };
    spawn.shield(me, x, y);
    me.do = function () {
  ...
  shooterBoss(x, y, radius = 110, isSpawnBossPowerUp = true) {
    ...
    Matter.Body.setDensity(
      me,
      0.01 + 0.0004 * Math.sqrt(simulation.difficulty),
    ); //extra dense //normal is 0.001 //makes effective life much larger
    me.damageReduction = 0.2;
    ...
    setTimeout(() => {
      for (
        let i = 0, len = 3 + 0.5 * Math.sqrt(simulation.difficulty);
        i < len;
        i++
      )
        spawn.spawnOrbitals(me, radius + 40 + 10 * i, 1);
    ...
    me.do = function () {
      ...
      if (this.seePlayer.recall) {
        ...
        if (!(simulation.cycle % this.seePlayerFreq)) {
          ...
          this.fireDir.y -=
            Math.abs(this.seePlayer.position.x - this.position.x) / 5000; //gives the bullet an arc  //was    / 1600
        }
        //rotate towards fireAngle
        const angle = this.angle + Math.PI / 2;
        const dot = Vector.dot(
        ...
        if (dot > threshold) {
          ...
          //fire
          for (
            let i = 0, len = 2 + 0.07 * simulation.difficulty;
            i < len;
            i++
          ) {
            spawn.bullet(
              this.vertices[1].x,
              this.vertices[1].y,
              10 + Math.ceil(this.radius / 25),
            );
            const spread = Vector.rotate(
              { x: Math.sqrt(len) + 4, y: 0 },
  ...
  sniper(x, y, radius = 35 + Math.ceil(Math.random() * 30)) {
    ...
    me.memory = 30; //140;
    me.fireFreq =
      0.005 + Math.random() * 0.002 + 0.0005 * simulation.difficulty; //larger = fire more often
    me.noseLength = 0;
    me.fireAngle = 0;
    me.accelMag = 0.0005 * simulation.accelScale;
    me.frictionAir = 0.05;
  ...
  sniperBullet(x, y, radius = 9, sides = 5) {
    //bullets
    mobs.spawn(x, y, sides, radius, "rgb(255,0,155)");
    let me = mob[mob.length - 1];
    me.stroke = "transparent";
    ...
    me.collisionFilter.mask = cat.player | cat.map | cat.body | cat.bullet;
    me.onDeath = function () {
      if (simulation.difficulty > 11) {
        //explode AoE
        const radius = 100 + 0.5 * simulation.difficulty + 50 * Math.random();
        if (
          m.immuneCycle < m.cycle &&
          Vector.magnitude(Vector.sub(this.position, player.position)) < radius
        )
          m.damage(0.0003 * radius * simulation.dmgScale);
        simulation.drawList.push({
          //add dmg to draw queue
          x: this.position.x,
          y: this.position.y,
    ...
    me.do = function () {
      ...
      if (
        ...
        (Matter.Query.collides(this, body).length > 0 && this.speed < 10)
      ) {
        this.isDropPowerUp = false;
  ...
  launcherOne(x, y, radius = 30 + Math.ceil(Math.random() * 40)) {
    ...
    me.do = function () {
      ...
      if (this.seePlayer.recall && !(simulation.cycle % this.fireFreq)) {
        ...
        const velocity = Vector.mult(
          Vector.perp(
            Vector.normalise(Vector.sub(this.position, this.vertices[0])),
          ),
          -6,
        );
        Matter.Body.setVelocity(who, {
  ...
  launcherBoss(x, y, radius = 90, isSpawnBossPowerUp = true) {
    ...
    Matter.Body.setDensity(
      me,
      0.0022 + 0.0002 * Math.sqrt(simulation.difficulty),
    ); //extra dense //normal is 0.001 //makes effective life much larger
    me.damageReduction = 0.2;
  ...
  grenadierBoss(x, y, radius = 95) {
    ...
    Matter.Body.setDensity(
      me,
      0.004 + 0.0002 * Math.sqrt(simulation.difficulty),
    ); //extra dense //normal is 0.001 //makes effective life much larger
    me.onDeath = function () {
      //helps collisions functions work better after vertex have been changed
      setTimeout(() => {
    ...
    me.onDamage = function () {
      if (this.grenadeLimiter < 240 && this.health > 0) {
        ...
        spawn.grenade(
          this.position.x,
          this.position.y,
          80 + Math.floor(60 * Math.random()),
        );
        who = mob[mob.length - 1];
        const velocity = Vector.mult(
          Vector.normalise(Vector.sub(player.position, who.position)),
          3 * Math.sqrt(simulation.accelScale) + 4 * Math.random(),
        );
        Matter.Body.setVelocity(who, {
          x: this.velocity.x + velocity.x,
          y: this.velocity.y + velocity.y,
  ...
  grenade(
    x,
    y,
    lifeSpan = 90 + Math.ceil(60 / simulation.accelScale),
    pulseRadius = Math.min(550, 250 + simulation.difficulty * 3),
    size = 3,
  ) {
    ...
    me.do = function () {
      ...
      ctx.arc(
        this.position.x,
        this.position.y,
        pulseRadius * (1.01 - this.timeLeft / this.lifeSpan),
        0,
        2 * Math.PI,
      ); //* this.fireCycle / this.fireDelay
  ...
  timeSkipBoss(x, y, radius = 50) {
    ...
    me.do = function () {
      ...
      if (!simulation.isTimeSkipping) {
        if (
          Vector.magnitude(Vector.sub(this.position, m.pos)) < this.eventHorizon
        ) {
          this.attraction();
          this.damageReduction = this.startingDamageReduction;
          this.isInvulnerable = false;
    ...
    me.do = function () {
      ...
      if (this.seePlayer.recall && this.cycle % 15 === 0) {
        if (this.canFire) {
          ...
          spawn.seeker(
            this.vertices[this.closestVertex1].x,
            this.vertices[this.closestVertex1].y,
            6,
          );
          Matter.Body.setDensity(mob[mob.length - 1], 0.000001); //normal is 0.001
          const velocity = Vector.mult(
            Vector.normalise(
              Vector.sub(this.position, this.vertices[this.closestVertex1]),
            ),
            -10,
          );
          Matter.Body.setVelocity(mob[mob.length - 1], {
          ...
          spawn.seeker(
            this.vertices[this.closestVertex2].x,
            this.vertices[this.closestVertex2].y,
            6,
          );
          Matter.Body.setDensity(mob[mob.length - 1], 0.000001); //normal is 0.001
          const velocity2 = Vector.mult(
            Vector.normalise(
              Vector.sub(this.position, this.vertices[this.closestVertex2]),
            ),
            -10,
          );
          Matter.Body.setVelocity(mob[mob.length - 1], {
          ...
          for (let i = 0; i < this.vertices.length; i++) {
            const d = Vector.magnitudeSquared(
              Vector.sub(this.vertices[i], player.position),
            );
            if (d < distance2) {
              distance2 = d;
          ...
          if (this.closestVertex2 === this.closestVertex1) {
            this.closestVertex2++;
            if (this.closestVertex2 === this.vertices.length)
              this.closestVertex2 = 0;
          }
        }
      }
    };
  ...
  spawner(x, y, radius = 55 + Math.ceil(Math.random() * 50)) {
    ...
    me.onDeath = function () {
      ...
      for (
        let i = 0;
        i < Math.ceil(this.mass * 0.15 + Math.random() * 2.5);
        ++i
      ) {
        spawn.spawns(
          this.position.x + (Math.random() - 0.5) * radius * 2.5,
  ...
  spawns(x, y, radius = 15) {
    ...
    // me.stroke = "transparent"
    me.collisionFilter.mask =
      cat.player | cat.bullet | cat.body | cat.map | cat.mob;
    me.showHealthBar = false;
    Matter.Body.setDensity(me, 0.0001); //normal is 0.001
  ...
  snakeSpitBoss(x, y, radius = 50) {
    ...
    mobs.spawn(
      x + tailRadius * Math.cos(angle),
      y + tailRadius * Math.sin(angle),
      8,
      radius,
      color1,
    ); //"rgb(55,170,170)"
    ...
    Matter.Body.setDensity(
      me,
      0.0022 + 0.00022 * Math.sqrt(simulation.difficulty),
    ); //extra dense //normal is 0.001 //makes effective life much larger
    ...
    me.do = function () {
      ...
      if (this.seePlayer.recall && this.cycle % 10 === 0) {
        if (this.canFire) {
          ...
          const velocity = Vector.mult(
            Vector.normalise(
              Vector.sub(this.vertices[this.closestVertex1], this.position),
            ),
            15,
          );
          Matter.Body.setVelocity(mob[mob.length - 1], {
  ...
  dragonFlyBoss(x, y, radius = 42) {
    ...
    Matter.Body.setDensity(
      me,
      0.00165 + 0.00011 * Math.sqrt(simulation.difficulty),
    ); //extra dense //normal is 0.001 //makes effective life much larger
    ...
    me.do = function () {
      ...
      this.wing(
        a +
          Math.PI / 2 +
          this.angleOff +
          this.flapArc * Math.sin(simulation.cycle * this.flapRate),
        this.wingLength,
        this.ellipticity,
      );
      this.wing(
  ...
  tetherBoss(x, y, constraint, radius = 90) {
    ...
    Matter.Body.setDensity(
      me,
      0.0005 + 0.0002 * Math.sqrt(simulation.difficulty),
    ); //extra dense //normal is 0.001 //makes effective life much larger
    me.damageReduction = 0.25;
    me.startingDamageReduction = me.damageReduction;
    me.isInvulnerable = false;
  ...
  shield(
    ...
    x,
    y,
    chance = (simulation.difficultyMode === 6 ? 3 : 1) *
      Math.min(0.02 + simulation.difficulty * 0.005, 0.2),
  ) {
  ...
  spawnOrbitals(
    who,
    radius,
    chance = Math.min(0.25 + simulation.difficulty * 0.005),
  ) {
    if (Math.random() < chance) {
      const len = Math.floor(
        Math.min(15, 3 + Math.sqrt(simulation.difficulty)),
      );
      const speed =
        (0.003 +
          0.004 * Math.random() +
          0.002 * Math.sqrt(simulation.difficulty)) *
        (Math.random() < 0.5 ? 1 : -1);
      const offSet = 6.28 * Math.random();
      for (let i = 0; i < len; i++)
        spawn.orbital(who, radius, (i / len) * 2 * Math.PI + offSet, speed);
    }
  },
  orbital(who, radius, phase, speed) {
    ...
    me.do = function () {
      ...
      Matter.Body.setPosition(
        this,
        Vector.add(
          Vector.add(who.position, who.velocity),
          Vector.mult(orbit, radius),
        ),
      );
      //damage player
      if (
  ...
  orbitalBoss(x, y, radius = 70) {
    ...
    const nodes = Math.min(
      15,
      Math.floor(2 + 4 * nodeBalance + 0.75 * Math.sqrt(simulation.difficulty)),
    );
    ...
    Matter.Body.setDensity(
      me,
      0.0017 + 0.0002 * Math.sqrt(simulation.difficulty),
    ); //extra dense //normal is 0.001 //makes effective life much larger
    me.damageReduction = 0.1;
    ...
    let speed =
      (0.006 + 0.001 * Math.sqrt(simulation.difficulty)) *
      (Math.random() < 0.5 ? 1 : -1);
    let range = radius + 350 + 200 * rangeInnerVsOuter + nodes * 7;
    for (let i = 0; i < nodes; i++)
      spawn.orbital(me, range, (i / nodes) * 2 * Math.PI, speed);
    ...
    range = Math.max(
      60,
      100 + 100 * Math.random() - nodes * 3 - rangeInnerVsOuter * 80,
    );
    speed = speed * (1.25 + 2 * Math.random());
    const subNodes = Math.max(
      2,
      Math.floor(6 - 5 * nodeBalance + 0.5 * Math.sqrt(simulation.difficulty)),
    );
    for (let j = 0; j < nodes; j++) {
      for (let i = 0, len = subNodes; i < len; i++)
        spawn.orbital(
          mob[orbitalIndexes[j]],
          range,
          (i / len) * 2 * Math.PI,
          speed,
        );
  ...
  nodeGroup(
    ...
    spawn = "striker",
    nodes = Math.min(
      2 + Math.ceil(Math.random() * (simulation.difficulty + 2)),
      8,
    ),
    ...
    for (let i = 0; i < nodes; ++i) {
      ...
      if (spawn === "random") {
        whoSpawn =
          this.fullPickList[
            Math.floor(Math.random() * this.fullPickList.length)
          ];
      } else if (spawn === "randomList") {
        whoSpawn =
          this.pickList[Math.floor(Math.random() * this.pickList.length)];
      }
      this[whoSpawn](
        x + sideLength * Math.sin(i * angle),
        y + sideLength * Math.cos(i * angle),
    ...
    if (nodes > 2 && Math.random() < 0.998) {
      this.groupShield(
        ...
        x,
        y,
        sideLength + 2.5 * radius + nodes * 6 - 25,
      );
    }
  ...
  lineGroup(
    ...
    spawn = "striker",
    nodes = Math.min(
      3 + Math.ceil(Math.random() * simulation.difficulty + 2),
      8,
    ),
    //Math.ceil(Math.random() * 3) + Math.min(4,Math.ceil(simulation.difficulty/2)),
    ...
    me.do = function () {
      ...
      if (this.freeOfWires) {
        ...
        if (m.pos.x > 700 && player.velocity.x > -2) {
          let wireFriction =
            0.75 * Math.min(0.6, Math.max(0, 100 / (breakingPoint - m.pos.x)));
      ...
      if (!this.freeOfWires)
        ctx.lineTo(
          m.pos.x + 30 * Math.cos(m.angle + Math.PI),
          m.pos.y + 30 * Math.sin(m.angle + Math.PI),
        );
      ctx.lineCap = "butt";
      ctx.lineWidth = 15;
      ctx.strokeStyle = "#000";
  ...
  bodyRect(
    ...
    height,
    chance = 1,
    properties = { friction: 0.05, frictionAir: 0.001 },
  ) {
    //this is the command that adds blocks to the world in the middle of a level
    if (Math.random() < chance) {
  ...
  bodyVertex(x, y, vector, properties) {
    //this is the command that adds blocks to the world in the middle of a level
    body[body.length] = Matter.Bodies.fromVertices(
      x,
      y,
  ...
  mapRectNow(x, y, width, height, properties, isRedrawMap = true) {
    //adds rectangle to map array in the middle of a level
    map[map.length] = Bodies.rectangle(
      x + width / 2,
      y + height / 2,
      width,
  ...
  mapVertexNow(x, y, vector, properties, isRedrawMap = true) {
    //adds shape to map array in the middle of a level
    map[map.length] = Matter.Bodies.fromVertices(
      x,
      y,
      Vertices.fromPath(vector),
