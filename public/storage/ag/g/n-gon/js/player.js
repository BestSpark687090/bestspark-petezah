...
const m = {
  spawn() {
    ...
    jumpSensor = Bodies.rectangle(0, 46, 36, 6, {
      //(0, 46, 50, 6, { //for wall jumping
      //this sensor check if the player is on the ground to enable jumping
      sleepThreshold: 99999999999,
      isSensor: true,
    });
    vertices = Vertices.fromPath(
      "16 -82  2 -66  2 -37  43 -37  43 -66  30 -82",
    );
  ...
  setMovement() {
    ...
    m.Fx =
      (tech.baseFx *
        m.fieldFx *
        tech.squirrelFx *
        (tech.isFastTime ? 1.5 : 1)) /
      player.mass; //base player mass is 5
    m.jumpForce =
      (tech.baseJumpForce *
        m.fieldJump *
        tech.squirrelJump *
        (tech.isFastTime ? 1.13 : 1)) /
      player.mass /
      player.mass; //base player mass is 5
  },
  ...
  lookDefault() {
    ...
    const scale = 0.8;
    m.transSmoothX =
      canvas.width2 - m.pos.x - (simulation.mouse.x - canvas.width2) * scale;
    m.transSmoothY =
      canvas.height2 - m.pos.y - (simulation.mouse.y - canvas.height2) * scale;
  ...
  groundControl() {
    ...
    if (input.left) {
      ...
      Matter.Body.setVelocity(player, {
        x: m.moverX * 0.08 + player.velocity.x * stoppingFriction,
        y: player.velocity.y * stoppingFriction,
      });
    }

    if (Math.abs(moveX) > 4) {
      //come to a stop if fast     // if (player.speed > 4) { //come to a stop if fast
      const stoppingFriction = m.crouch ? 0.65 : 0.89; // this controls speed when crouched
      Matter.Body.setVelocity(player, {
        x:
          m.moverX * (1 - stoppingFriction) +
          player.velocity.x * stoppingFriction,
        y: player.velocity.y * stoppingFriction,
      });
    }
  ...
  airControl() {
    ...
    if (input.left) {
      if (player.velocity.x > -m.airSpeedLimit / player.mass / player.mass)
  ...
  switchWorlds() {
    ...
    for (let i = 0, len = b.inventory.length; i < len; i++) {
      if (b.guns[b.inventory[i]].ammo !== Infinity) {
        ammoCount +=
          b.guns[b.inventory[i]].ammo / b.guns[b.inventory[i]].ammoPack;
      } else {
        ammoCount += 5;
      }
    }
    ...
    for (let i = 0, len = b.inventory.length; i < len; i++) {
      if (b.guns[b.inventory[i]].ammo !== Infinity)
        b.guns[b.inventory[i]].ammo = Math.max(
          0,
          Math.floor(
            (ammoCount / b.inventory.length) *
              b.guns[b.inventory[i]].ammoPack *
              (2.5 + 0.3 * (Math.random() - 0.5)),
          ),
        );
    }
    ...
    for (let i = 0; i < totalTech; i++) {
      ...
      if (options.length > 0)
        tech.giveTech(options[Math.floor(Math.random() * options.length)]);
    }
    b.respawnBots();
  ...
  death() {
    if (tech.isImmortal) {
      //if player has the immortality buff, spawn on the same level with randomized damage
        ...
        setTimeout(
          function () {
            ...
            simulation.makeTextLog(
              `simulation.amplitude <span class='color-symbol'>=</span> 0.${len - i - 1}`,
              swapPeriod,
            );
      ...
      setTimeout(function () {
        ...
        simulation.isTextLogOpen = true;
        simulation.makeTextLog(
          "simulation.amplitude <span class='color-symbol'>=</span> null",
        );
        tech.isImmortal = false; //disable future immortality
      }, 6 * swapPeriod);
    } else if (m.alive) {
      //normal death code here
      m.storeTech();
      m.alive = false;
      ...
      setTimeout(function () {
        ...
        if (
          (level.levelsCleared < 4 || level.levelsCleared > 12) &&
          !simulation.isTraining &&
  ...
  drawHealth() {
    if (m.health < 1) {
      ...
      ctx.fillStyle = "#f00";
      ctx.fillRect(m.pos.x - m.radius, m.pos.y - 50, 60 * m.health, 10);
    }
  },
  displayHealth() {
    ...
    id.style.width =
      Math.floor(300 * m.maxHealth * Math.pow(m.health / m.maxHealth, 1.4)) +
      "px";
  ...
  setMaxHealth(isMessage) {
    ...
    document.getElementById("health-bg").style.width =
      `${Math.floor(300 * m.maxHealth)}px`;
    if (isMessage)
      simulation.makeTextLog(
        `<span class='color-var'>m</span>.<span class='color-h'>maxHealth</span> <span class='color-symbol'>=</span> ${m.maxHealth.toFixed(2)}`,
      );
    if (m.health > m.maxHealth) m.health = m.maxHealth;
    m.displayHealth();
  },
  ...
  defense() {
    ...
    if (tech.isSpeedHarm && tech.speedAdded + player.speed > 0.1)
      dmg *= 1 - Math.min((tech.speedAdded + player.speed) * 0.0193, 0.8); //capped at speed of 55
    if (tech.isHarmReduce && input.field) dmg *= 0.1;
    if (tech.isNeutronium && input.field && m.fieldCDcycle < m.cycle)
      dmg *= 0.05;
    ...
    if (tech.isFirstDer && b.inventory[0] === b.activeGun)
      dmg *= 0.85 ** b.inventory.length;
    // return tech.isEnergyHealth ? Math.pow(dmg, 0.7) : dmg //defense has less effect
    // dmg *= m.fieldHarmReduction
    return dmg * m.fieldHarmReduction;
  },
  rewind(steps) {
    // m.rewind(Math.floor(Math.min(599, 137 * m.energy)))
    if (tech.isRewindGrenade) {
      ...
      const immunityCycle =
        ...
        immunityDuration +
        10 +
        tech.isPetalsExplode * 30 +
        tech.isCircleExplode * 21;
    ...
    const shortPause = function () {
      if (m.defaultFPSCycle < m.cycle) {
        ...
        if (isDrawPlayer) {
          ...
          ctx.scale(
            simulation.zoom / simulation.edgeZoomOutSmooth,
            simulation.zoom / simulation.edgeZoomOutSmooth,
          ); //zoom in once centered
    ...
    if (tech.isRewindBot) {
      ...
      for (let i = 0; i < len; i++) {
        ...
        bullet[bullet.length - 1].endCycle =
          simulation.cycle + 440 + Math.floor(120 * Math.random()); //8-10 seconds
      }
    }
  },
  ...
  damage(dmg) {
    if (
      ...
      m.energy + 0.05 > Math.min(0.95, m.maxEnergy) &&
      dmg > 0.01
    ) {
      const steps = Math.floor(Math.min(299, 150 * m.energy));
      simulation.makeTextLog(
        `<span class='color-var'>m</span>.rewind(${steps})`,
      );
      m.rewind(steps);
      return;
    }
    m.lastHarmCycle = m.cycle;
    if (tech.isDroneOnDamage && bullet.length < 150) {
      //chance to build a drone on damage  from tech
      const len =
        (Math.min((dmg - 0.06 * Math.random()) * 80, 60) /
          tech.droneEnergyReduction) *
        (tech.isEnergyHealth ? 0.5 : 1);
      for (let i = 0; i < len; i++) {
        if (Math.random() < 0.5)
          b.drone({
    ...
    if (tech.isEnergyHealth) {
      ...
      if (m.energy < 0 || isNaN(m.energy)) {
        //taking deadly damage
        if (
          tech.isDeathAvoid &&
          powerUps.research.count &&
          !tech.isDeathAvoidedThisLevel
        ) {
      ...
      document.getElementById("dmg").style.opacity =
        0.1 + Math.min(0.6, dmg * 4);
    }
    if (dmg > 0.03) {
      m.lastHit = dmg;
    ...
    if (dmg > 0.05) {
      // freeze game for high damage hits
      simulation.fpsCap = 4; //40 - Math.min(25, 100 * dmg)
      simulation.fpsInterval = 1000 / simulation.fpsCap;
      if (tech.isHarmFreeze) {
        for (let i = 0, len = mob.length; i < len; i++)
          mobs.statusSlow(mob[i], 480); //freeze all mobs
      }
    } else {
      simulation.fpsCap = simulation.fpsCapDefault;
      simulation.fpsInterval = 1000 / simulation.fpsCap;
  ...
  calcLeg(cycle_offset, offset) {
    ...
    m.stepSize =
      0.8 * m.stepSize +
      0.2 * (7 * Math.sqrt(Math.min(9, Math.abs(m.Vx))) * m.onGround);
    //changes to stepsize are smoothed by adding only a percent of the new value each cycle
    const stepAngle = 0.034 * m.walk_cycle + cycle_offset;
    m.foot.x = 2.2 * m.stepSize * Math.cos(stepAngle) + offset;
    m.foot.y =
      offset + 1.2 * m.stepSize * Math.sin(stepAngle) + m.yOff + m.height;
    const Ymax = m.yOff + m.height;
    if (m.foot.y > Ymax) m.foot.y = Ymax;

    //calculate knee position as intersection of circle from hip and foot
    const d = Math.sqrt(
      (m.hip.x - m.foot.x) * (m.hip.x - m.foot.x) +
        (m.hip.y - m.foot.y) * (m.hip.y - m.foot.y),
    );
    const l =
      (m.legLength1 * m.legLength1 - m.legLength2 * m.legLength2 + d * d) /
      (2 * d);
    const h = Math.sqrt(m.legLength1 * m.legLength1 - l * l);
    m.knee.x =
      (l / d) * (m.foot.x - m.hip.x) -
      (h / d) * (m.foot.y - m.hip.y) +
      m.hip.x +
      offset;
    m.knee.y =
      (l / d) * (m.foot.y - m.hip.y) + (h / d) * (m.foot.x - m.hip.x) + m.hip.y;
  },
  draw() {},
  isAltSkin: false,
  resetSkin() {
  ...
  skin: {
    ...
    favicon() {
      //used to render the favicon, not actually in game
      m.yOffWhen.jump = 70;
      m.yOffWhen.stand = 49;
      m.yOffWhen.crouch = 22;
      m.isAltSkin = false;
    ...
    mech() {
      ...
      m.drawLeg = function (stroke) {
        ...
        const off = Vector.mult(
          Vector.rotate(Vector.normalise(sub), Math.PI / 2),
          8,
        );
    ...
    polar() {
      ...
      m.draw = function () {
        ...
        const diff =
          (m.lastKillCycle - m.cycle + tech.isDamageCooldownTime) /
          tech.isDamageCooldownTime;
        ...
        ctx.fillStyle =
          diff < 0
            ? `hsl(${hue}, 80%, 40%)`
            : `rgba(255,255,255,${Math.min(Math.max(0, diff + 0.3), 1)})`;
    ...
    CPT() {
      ...
      m.draw = function () {
        ...
        ctx.arc(0, 0, 30, 0, 2 * Math.PI);
        ctx.fillStyle =
          m.energy > 0.85 * Math.min(1, m.maxEnergy) ? m.bodyGradient : "#fff";
        ctx.fill();
        ctx.restore();

        ctx.beginPath();
        ctx.arc(
          m.pos.x + 15 * Math.cos(m.angle),
          m.pos.y + 15 * Math.sin(m.angle),
          5,
          0,
          2 * Math.PI,
        );
        ...
        ctx.rotate(
          m.angle -
            (m.fireCDcycle !== Infinity
              ? m.flipLegs *
                0.25 *
                Math.pow(Math.max(m.fireCDcycle - m.cycle, 0), 0.5)
              : 0),
        );
    ...
    cat() {
      ...
      m.draw = function () {
        ...
        ctx.bezierCurveTo(
          ...
          -75,
          -5,
          150 + 5 * Math.sin(simulation.cycle / 10),
          -70 + 10 * Math.sin(simulation.cycle / 10),
          0 + 10 * Math.sin(simulation.cycle / 10),
        );
        ctx.strokeStyle = "#333";
        ctx.lineWidth = 4;
        ctx.stroke();
    ...
    pareidolia() {
      ...
      m.draw = function () {
        ...
        ctx.lineWidth = 2;
        if (!(m.angle > -Math.PI / 2 && m.angle < Math.PI / 2))
          ctx.scale(1, -1); //here is the flip
        ctx.stroke();
        ctx.beginPath();
  ...
  setHoldDefaults() {
    if (tech.isFreeWormHole && m.fieldMode !== 9) {
      //not wormhole
    ...
    if (tech.isNeutronium && m.fieldMode !== 3) {
      //not negative mass field
    ...
    player.collisionFilter.mask =
      cat.body | cat.map | cat.mob | cat.mobBullet | cat.mobShield;
    m.airSpeedLimit = 125;
    m.fieldFx = 1;
    m.fieldJump = 1;
    m.setFieldRegen();
  ...
  setMaxEnergy(isMessage = true) {
    m.maxEnergy =
      (tech.isMaxEnergyTech ? 0.5 : 1) +
      tech.bonusEnergy +
      tech.healMaxEnergyBonus +
      tech.harmonicEnergy +
      3 * tech.isGroundState +
      1.5 * (m.fieldMode === 1) +
      (m.fieldMode === 0 || m.fieldMode === 1) * 0.05 * m.coupling +
      0.77 * tech.isStandingWaveExpand;
    if (isMessage)
      simulation.makeTextLog(
        `<span class='color-var'>m</span>.<span class='color-f'>maxEnergy</span> <span class='color-symbol'>=</span> ${m.maxEnergy.toFixed(2)}`,
      );
  },
  fieldMeterColor: "#0cf",
  ...
  drawRegenEnergyCloaking: function () {
    if (m.energy < m.maxEnergy) {
      // replaces m.drawRegenEnergy() with custom code
      m.regenEnergy();
      const xOff = m.pos.x - m.radius * m.maxEnergy;
      const yOff = m.pos.y - 50;
  ...
  setFieldRegen() {
    ...
    if (m.fieldMode === 0 || m.fieldMode === 4)
      m.fieldRegen += 0.000133 * m.coupling;
  ...
  regenEnergy() {
    //used in drawRegenEnergy  // rewritten by some tech
    if (m.immuneCycle < m.cycle && m.fieldCDcycle < m.cycle)
  ...
  drop() {
    ...
    if (m.holdingTarget) {
      ...
      m.holdingTarget.collisionFilter.mask =
  ...
  definePlayerMass(mass = m.defaultMass) {
    ...
    m.yOffWhen.stand = Math.max(
      m.yOffWhen.crouch,
      Math.min(49, 49 - (mass - 5) * 6),
    );
  ...
  throwBlockDefault() {
    if (m.holdingTarget) {
      if (input.field) {
        if (m.energy > 0.001) {
          ...
          if (tech.isTokamak) {
            ...
            ctx.moveTo(x, y);
            ctx.lineTo(
              m.holdingTarget.vertices[len].x,
              m.holdingTarget.vertices[len].y,
            );
            ctx.lineTo(
              m.holdingTarget.vertices[0].x,
              m.holdingTarget.vertices[0].y,
            );
            ctx.fill();
            for (let i = 0; i < len; i++) {
              ctx.beginPath();
              ctx.moveTo(x, y);
              ctx.lineTo(
                m.holdingTarget.vertices[i].x,
                m.holdingTarget.vertices[i].y,
              );
              ctx.lineTo(
                m.holdingTarget.vertices[i + 1].x,
                m.holdingTarget.vertices[i + 1].y,
              );
              ctx.fill();
            }
          } else {
            ...
            const speed = tech.isPrinter
              ? 15 +
                80 *
                  charge *
                  Math.min(0.85, 0.8 / Math.pow(m.holdingTarget.mass, 0.1))
              : 80 *
                charge *
                Math.min(0.85, 0.8 / Math.pow(m.holdingTarget.mass, 0.25));
            const v = {
      ...
      } else if (m.throwCharge > 0) {
        //Matter.Query.region(mob, player.bounds)
        ...
        if (tech.isTokamak && m.throwCharge > 4) {
          //remove the block body and pulse  in the direction you are facing
          ...
          if (m.immuneCycle < m.cycle)
            m.energy +=
              0.25 *
              Math.sqrt(m.holdingTarget.mass) *
              Math.min(5, m.throwCharge);
          m.throwCharge = 0;
          ...
          if (tech.isBlockRestitution) {
            ...
            m.holdingTarget.friction =
              m.holdingTarget.frictionStatic =
              m.holdingTarget.frictionAir =
                0.001;
          }
          ...
          const solid = function (that) {
            ...
            if (dx * dx + dy * dy > 10000 && that !== m.holdingTarget) {
              ...
              that.collisionFilter.mask =
                cat.player |
                cat.map |
                cat.body |
                cat.bullet |
                cat.mob |
                cat.mobBullet; //can hit player now
            } else {
          ...
          if (Matter.Query.collides(m.holdingTarget, map).length !== 0) {
            ...
            if (
              Matter.Query.ray(map, m.holdingTarget.position, m.pos).length !==
              0
            )
              speed = 0; //drop to zero if the center of the block can't see the center of the player through the map
          }
          m.throwCharge = 0;
          m.throwCycle = m.cycle + 180; //used to detect if a block was thrown in the last 3 seconds
          ...
          Matter.Body.setVelocity(player, {
            x:
              player.velocity.x -
              ((Math.cos(m.angle) * speed) / (m.crouch ? 30 : 10)) *
                Math.sqrt(m.holdingTarget.mass),
            y:
              player.velocity.y -
              ((Math.sin(m.angle) * speed) / 30) *
                Math.sqrt(m.holdingTarget.mass),
          });
          m.definePlayerMass(); //return to normal player mass
  ...
  drawField() {
    if (m.holdingTarget) {
      ctx.fillStyle =
        "rgba(110,170,200," + m.energy * (0.05 + 0.05 * Math.random()) + ")";
      ctx.strokeStyle =
        "rgba(110, 200, 235, " + (0.3 + 0.08 * Math.random()) + ")"; //"#9bd" //"rgba(110, 200, 235, " + (0.5 + 0.1 * Math.random()) + ")"
    } else {
      ctx.fillStyle =
        "rgba(110,170,200," +
        (0.02 + m.energy * (0.15 + 0.15 * Math.random())) +
        ")";
      ctx.strokeStyle =
        "rgba(110, 200, 235, " + (0.6 + 0.2 * Math.random()) + ")"; //"#9bd" //"rgba(110, 200, 235, " + (0.5 + 0.1 * Math.random()) + ")"
    }
    ...
    ctx.quadraticCurveTo(
      cp1x,
      cp1y,
      m.pos.x + 1 * range * Math.cos(m.angle - Math.PI * m.fieldArc),
      m.pos.y + 1 * range * Math.sin(m.angle - Math.PI * m.fieldArc),
    );
    ctx.fill();
    ...
    ctx.lineTo(
      m.pos.x + range * Math.cos(offAngle),
      m.pos.y + range * Math.sin(offAngle),
    );
  ...
  grabPowerUp() {
    //look for power ups to grab with field
    if (m.fireCDcycle < m.cycle) m.fireCDcycle = m.cycle - 1;
    for (let i = 0, len = powerUp.length; i < len; ++i) {
      ...
      if (
        ...
        if (!tech.isHealAttract || powerUp[i].name !== "heal") {
          //if you have accretion heals are already pulled in a different way
          powerUp[i].force.x +=
            0.04 * (dxP / Math.sqrt(dist2)) * powerUp[i].mass;
          powerUp[i].force.y +=
            0.04 * (dyP / Math.sqrt(dist2)) * powerUp[i].mass -
            powerUp[i].mass * simulation.g; //negate gravity
        ...
        if (
          ...
          (powerUp[i].name !== "heal" ||
            m.maxHealth - m.health > 0.01 ||
            tech.isOverHeal)
        ) {
          powerUps.onPickUp(powerUp[i]);
          Matter.Body.setVelocity(player, {
            //player knock back, after grabbing power up
            x:
              player.velocity.x +
              (powerUp[i].velocity.x / player.mass) * 4 * powerUp[i].mass,
            y:
              player.velocity.y +
              (powerUp[i].velocity.y / player.mass) * 4 * powerUp[i].mass,
          });
          powerUp[i].effect();
  ...
  pushMass(
    who,
    fieldBlockCost = (0.025 +
      Math.sqrt(who.mass) *
        Vector.magnitude(Vector.sub(who.velocity, player.velocity)) *
        0.002) *
      m.fieldShieldingScale,
  ) {
    if (m.energy > m.minEnergyToDeflect) {
      //shield needs at least some of the cost to block
      ...
      if (m.energy < m.minEnergyToDeflect) {
        ...
        if (tech.isLaserField) {
          simulation.ephemera.push({
            ...
            count:
              20 + Math.floor((m.maxEnergy * 30 * 0.0018) / tech.laserDrain), //how many cycles the ephemera lasts, scales with max energy
            do() {
              this.count--;
              if (this.count < 0) simulation.removeEphemera(this.name);
              for (let i = 0, num = 12; i < num; i++) {
                //draw random lasers
                ...
                b.laser(
                  ...
                  {
                    x: m.pos.x + 3000 * Math.cos(angle),
                    y: m.pos.y + 3000 * Math.sin(angle),
                  },
                  tech.laserDamage * 2.5,
      ...
      if (
        ...
        //for field emitter iceIX
        for (let i = 0; i < m.coupling; i++) {
          if (0.1 * m.coupling - i > 1.25 * Math.random()) {
            const sub = Vector.mult(
              Vector.normalise(Vector.sub(who.position, m.pos)),
              m.fieldRange * m.harmonicRadius * (0.4 + 0.3 * Math.random()),
            ); //m.harmonicRadius should be 1 unless you are standing wave expansion
            const rad = Vector.rotate(sub, 1 * (Math.random() - 0.5));
            const angle = Math.atan2(sub.y, sub.x);
            b.iceIX(
              6 + 6 * Math.random(),
              angle + 3 * (Math.random() - 0.5),
              Vector.add(m.pos, rad),
            );
          }
        }
      }
      ...
      if (tech.blockDmg) {
        ...
        if (who.isShielded) {
          for (let i = 0, len = mob.length; i < len; i++) {
            if (mob[i].id === who.shieldID)
              mob[i].damage(
                tech.blockDmg * m.dmgScale * (tech.isBlockRadiation ? 6 : 2),
                true,
              );
          }
  ...
  pushMobsFacing() {
    // find mobs in range and in direction looking
    for (let i = 0, len = mob.length; i < len; ++i) {
      if (
        Vector.magnitude(Vector.sub(mob[i].position, m.pos)) - mob[i].radius <
          m.fieldRange &&
        m.lookingAt(mob[i]) &&
        !mob[i].isUnblockable &&
  ...
  pickUp() {
    ...
    totalMomentum = Vector.add(
      ...
      Vector.mult(m.holdingTarget.velocity, m.holdingTarget.mass),
    );
    Matter.Body.setVelocity(
      player,
      Vector.mult(totalMomentum, 1 / (m.defaultMass + m.holdingTarget.mass)),
    );

    m.definePlayerMass(
      m.defaultMass + m.holdingTarget.mass * m.holdingMassScale,
    );
  ...
  couplingChange(change = 0) {
    if (change > 0 && level.onLevel !== -1)
      simulation.makeTextLog(
        `m.coupling <span class='color-symbol'>+=</span> ${change}`,
        60,
      ); //level.onLevel !== -1  means not on lore level
    m.coupling += change;
    if (m.coupling < 0) {
  ...
  setField(index) {
    ...
    simulation.makeTextLog(
      `<span class='color-var'>m</span>.setField("<span class='color-text'>${m.fieldUpgrades[m.fieldMode].name}</span>")`,
    );
  },
  fieldUpgrades: [
    {
      ...
      effect: () => {
        m.hold = function () {
          if (m.isHolding) {
            ...
            //not hold but field button is pressed
            ...
            //holding, but field button is released
            m.pickUp();
          } else {
    ...
    },
    {
      ...
      effect: () => {
        ...
        m.harmonic3Phase = () => {
          //normal standard 3 different 2-d circles
          const fieldRange1 =
            (0.75 + 0.3 * Math.sin(m.cycle / 23)) *
            m.fieldRange *
            m.harmonicRadius;
          const fieldRange2 =
            (0.68 + 0.37 * Math.sin(m.cycle / 37)) *
            m.fieldRange *
            m.harmonicRadius;
          const fieldRange3 =
            (0.7 + 0.35 * Math.sin(m.cycle / 47)) *
            m.fieldRange *
            m.harmonicRadius;
          ...
          ctx.fillStyle =
            ...
            Math.min(
              0.6,
              0.04 + 0.7 * m.energy * (0.1 + 0.11 * Math.random()),
            ) +
            ")";
          ctx.beginPath();
          ...
          for (let i = 0, len = mob.length; i < len; ++i) {
            if (
              Vector.magnitude(Vector.sub(mob[i].position, m.pos)) -
                mob[i].radius <
                netFieldRange &&
              !mob[i].isUnblockable
            ) {
        ...
        m.harmonicAtomic = () => {
          //several ellipses spinning about different axises
          ...
          for (let i = 0; i < tech.harmonics; i++) {
            ...
            ctx.ellipse(
              ...
              m.pos.y,
              radius *
                Math.abs(Math.sin(phase + (i / tech.harmonics) * Math.PI)),
              radius,
              rotation + (i / tech.harmonics) * Math.PI,
              0,
              2 * Math.PI,
            );
            ctx.fill();
    ...
    },
    {
      ...
      description:
        "<strong>deflecting</strong> does not drain <strong class='color-f'>energy</strong><br>maintains <strong>functionality</strong> while <strong>inactive</strong><br><strong>5</strong> <strong class='color-f'>energy</strong> per second",
      effect: () => {
          ...
          if (m.fieldCDcycle < m.cycle) {
            for (let i = 0, len = mob.length; i < len; ++i) {
              if (
                ...
                !mob[i].isUnblockable &&
                Vector.dot(
                  { x: Math.cos(m.fieldAngle), y: Math.sin(m.fieldAngle) },
                  Vector.normalise(
                    Vector.sub(mob[i].position, m.fieldPosition),
                  ),
                ) > m.fieldThreshold &&
                Matter.Query.ray(map, mob[i].position, m.fieldPosition)
                  .length === 0
              ) {
                mob[i].locatePlayer();
                const unit = Vector.normalise(
                  Vector.sub(m.fieldPosition, mob[i].position),
                );
                m.fieldCDcycle =
                  m.cycle + m.fieldBlockCD + (mob[i].isShielded ? 10 : 0);
                if (!mob[i].isInvulnerable && bullet.length < 250) {
                  for (let i = 0; i < m.coupling; i++) {
                    if (0.1 * m.coupling - i > Math.random()) {
                      const angle =
                        m.fieldAngle + 4 * m.fieldArc * (Math.random() - 0.5);
                ...
                if (tech.blockDmg) {
                  ...
                  if (mob[i].isShielded) {
                    for (let j = 0, len = mob.length; j < len; j++) {
                      if (mob[j].id === mob[i].shieldID)
                        mob[j].damage(
                          tech.blockDmg *
                            m.dmgScale *
                            (tech.isBlockRadiation ? 6 : 2),
                          true,
                        );
                    }
                  } else if (tech.isBlockRadiation) {
                    if (mob[i].isMobBullet) {
                      ...
                      mobs.statusDoT(
                        mob[i],
                        tech.blockDmg * m.dmgScale * 0.42,
                        180,
                      ); //200% increase -> x (1+2) //over 7s -> 360/30 = 12 half seconds -> 3/12
                    }
                  } else {
                  ...
                  ctx.beginPath();
                  ctx.moveTo(
                    mob[i].vertices[len].x + mag * (Math.random() - 0.5),
                    mob[i].vertices[len].y + mag * (Math.random() - 0.5),
                  );
                  for (let j = 0; j < len; j++) {
                    ctx.lineTo(
                      mob[i].vertices[j].x + mag * (Math.random() - 0.5),
                      mob[i].vertices[j].y + mag * (Math.random() - 0.5),
                    );
                  }
                  ctx.lineTo(
                    mob[i].vertices[len].x + mag * (Math.random() - 0.5),
                  ...
                  ctx.beginPath();
                  ctx.moveTo(
                    m.fieldPosition.x + eye * Math.cos(m.fieldAngle),
                    m.fieldPosition.y + eye * Math.sin(m.fieldAngle),
                  );
                  ...
                  for (let j = 0; j < len; j++) {
                    ...
                    ctx.lineTo(
                      mob[i].vertices[j + 1].x,
                      mob[i].vertices[j + 1].y,
                    );
                    ctx.fill();
                    ctx.stroke();
                  }
                ...
                if (mob[i].isUnstable) {
                  if (m.fieldCDcycle < m.cycle + 10)
                    m.fieldCDcycle = m.cycle + 6;
                  mob[i].death();
                }
                if (!isFree) {
                  //player knock backs
                  if (mob[i].isDropPowerUp && player.speed < 12) {
                    const massRootCap = Math.sqrt(
                      Math.min(10, Math.max(0.2, mob[i].mass)),
                    );
                    Matter.Body.setVelocity(player, {
        ...
        m.hold = function () {
          ...
          if (m.isHolding) {
            ...
            const angleReduction =
              0.5 +
              0.7 *
                (Math.PI / 2 -
                  Math.min(Math.PI / 2, Math.abs(m.angle + Math.PI / 2)));
            // console.log(angleReduction)
            if (player.velocity.y > 1) {
              player.force.y -=
                angleReduction *
                (tech.isBigField ? 0.95 : 0.5) *
                player.mass *
                simulation.g;
              Matter.Body.setVelocity(player, {
                x: player.velocity.x,
            ...
            ctx.quadraticCurveTo(
              ...
              cp1y,
              m.pos.x +
                1 * m.fieldRange * Math.cos(m.angle - Math.PI * m.fieldArc),
              m.pos.y +
                1 * m.fieldRange * Math.sin(m.angle - Math.PI * m.fieldArc),
            );
            ctx.fill();
            m.perfectPush();
          } else if (m.holdingTarget && m.fieldCDcycle < m.cycle) {
            ...
            if (!input.field) {
              ...
              ctx.quadraticCurveTo(
                m.fieldPosition.x + curve * m.fieldRange * Math.cos(a),
                m.fieldPosition.y + curve * m.fieldRange * Math.sin(a),
                m.fieldPosition.x +
                  1 *
                    m.fieldRange *
                    Math.cos(m.fieldAngle - Math.PI * m.fieldArc),
                m.fieldPosition.y +
                  1 *
                    m.fieldRange *
                    Math.sin(m.fieldAngle - Math.PI * m.fieldArc),
              );
              ctx.fill();
              m.perfectPush(true);
            }
          ...
          if (tech.isPerfectBrake) {
            //cap mob speed around player
            const range = 200 + 140 * wave + 150 * m.energy;
            for (let i = 0; i < mob.length; i++) {
              const distance = Vector.magnitude(
                Vector.sub(m.pos, mob[i].position),
              );
              if (distance < range) {
                ...
                if (
                  ...
                  // if velocity is directed towards player
                  Matter.Body.setVelocity(
                    mob[i],
                    Vector.mult(Vector.normalise(mob[i].velocity), cap),
                  ); //set velocity to cap, but keep the direction
                }
              }
            }
    ...
    },
    {
      ...
      description:
        "use <strong class='color-f'>energy</strong> to nullify &nbsp;<strong style='letter-spacing: 7px;'>gravity</strong><br><strong>0.4x</strong> <strong class='color-defense'>damage taken</strong><br><strong>6</strong> <strong class='color-f'>energy</strong> per second",
      fieldDrawRadius: 0,
      effect: () => {
        m.fieldFire = true;
            ...
            if (m.energy > DRAIN && m.fieldCDcycle < m.cycle) {
              if (tech.isFlyFaster) {
                ...
                function moveThis(who, range, mag = 1.06) {
                  for (let i = 0, len = who.length; i < len; ++i) {
                    ...
                    if (dist < range) {
                      ...
                      if (input.left) {
                        //blocks move horizontally with the same force as the player
                ...
                if (input.down) {
                  ...
                  this.fieldDrawRadius =
                    this.fieldDrawRadius * 0.97 + 500 * 0.03;
                  ...
                  m.energy -= 5 * DRAIN;
                  this.fieldDrawRadius =
                    this.fieldDrawRadius * 0.97 + 1100 * 0.03;
                  ...
                  m.energy -= DRAIN;
                  this.fieldDrawRadius =
                    this.fieldDrawRadius * 0.97 + 800 * 0.03;
                ...
                if (input.down) {
                  ...
                  player.force.y -= 0.5 * player.mass * simulation.g;
                  this.fieldDrawRadius =
                    this.fieldDrawRadius * 0.97 + 400 * 0.03;
                  ...
                  m.energy -= 5 * DRAIN;
                  this.fieldDrawRadius =
                    this.fieldDrawRadius * 0.97 + 850 * 0.03;
                  ...
                  m.energy -= DRAIN;
                  this.fieldDrawRadius =
                    this.fieldDrawRadius * 0.97 + 650 * 0.03;
                  player.force.y -= 1.07 * player.mass * simulation.g; // slow upward drift
        ...
        m.hold = function () {
          if (
            ...
            if (simulation.molecularMode === 0) {
              if (tech.isSporeFlea) {
                const drain =
                  0.18 + (Math.max(bullet.length, 130) - 130) * 0.02;
                if (m.energy > drain) {
                  m.energy -= drain;
                ...
                const drain =
                  0.095 + (Math.max(bullet.length, 130) - 130) * 0.01;
              ...
              if (tech.isDroneRadioactive) {
                ...
                const drain =
                  (0.45 + (Math.max(bullet.length, 100) - 100) * 0.006) *
                  tech.droneEnergyReduction;
                if (m.energy > drain) {
                  m.energy -= drain;
                  b.drone();
          ...
          if (m.isHolding) {
            ...
            if (tech.isPrinter && m.holdingTarget.isPrinted && input.field) {
              ...
              m.holdingTarget.radius += Math.min(
                1.1,
                1.3 / m.holdingTarget.mass,
              ); //grow up to a limit
              const r1 =
                m.holdingTarget.radius * (1 + 0.12 * Math.sin(m.cycle * 0.11));
              const r2 =
                m.holdingTarget.radius * (1 + 0.12 * Math.cos(m.cycle * 0.11));
              let angle = (m.cycle * 0.01) % (2 * Math.PI); //rotate the object
              let vertices = [];
              for (
                let i = 0, len = m.holdingTarget.vertices.length;
                i < len;
                i++
              ) {
                angle += (2 * Math.PI) / len;
                vertices.push({
                  x: m.holdingTarget.position.x + r1 * Math.cos(angle),
                  y: m.holdingTarget.position.y + r2 * Math.sin(angle),
                });
              }
    ...
    },
    {
      name: "plasma torch",
      description:
        "use <strong class='color-f'>energy</strong> to emit short range <strong class='color-plasma'>plasma</strong><br><strong class='color-d'>damages</strong> and <strong>pushes</strong> mobs away<br><strong>10</strong> <strong class='color-f'>energy</strong> per second",
      set() {
        ...
        if (tech.isPlasmaBall) {
          ...
          m.plasmaBall = Bodies.circle(
            m.pos.x + 10 * Math.cos(m.angle),
            m.pos.y + 10 * Math.sin(m.angle),
            1,
            {
              // collisionFilter: {
              //     group: 0,
              ...
              setPositionToNose() {
                ...
                Matter.Body.setPosition(
                  ...
                  Vector.add(
                    nose,
                    Vector.mult(
                      Vector.normalise(Vector.sub(nose, m.pos)),
                      circleRadiusScale * this.circleRadius,
                    ),
                  ),
                );
              },
              ...
              do() {
                if (this.isOn) {
                  ...
                  if (Matter.Query.collides(this, map).length > 0) {
                    if (this.isAttached) {
                      this.scale(
                        Math.max(0.9, 0.998 - 0.1 / m.plasmaBall.circleRadius),
                      );
                    } else {
                      this.isPopping = true;
                    }
                  ...
                  if (this.isPopping) {
                    ...
                    if (this.alpha < 0.1) {
                      ...
                      const scale =
                        1.04 + 4 / Math.max(1, m.plasmaBall.circleRadius);
                      Matter.Body.scale(m.plasmaBall, scale, scale); //grow
                    }
                    // if (this.speed > 2.5) {
                    //     const slow = 0.9
                  ...
                  const dischargeRange =
                    150 + 1600 * tech.plasmaDischarge + 1.3 * damageRadius;
                  for (let i = 0, len = mob.length; i < len; i++) {
                    if (
                      mob[i].alive &&
                      ...
                      if (sub < damageRadius + mob[i].radius) {
                        ...
                        sub < dischargeRange + mob[i].radius &&
                        Matter.Query.ray(map, mob[i].position, this.position)
                          .length === 0
                      ) {
                  ...
                  for (let i = 0; i < arcList.length; i++) {
                    if (tech.plasmaDischarge > Math.random()) {
                      const who =
                        arcList[Math.floor(Math.random() * arcList.length)];
                      who.damage(dmg * 4);
                      //draw arcs
          ...
          m.hold = function () {
            if (m.isHolding) {
              ...
              if (m.fieldCDcycle < m.cycle) {
                ...
                if (!m.plasmaBall.isAttached) {
                ...
                } else if (m.energy > m.plasmaBall.drain) {
                  //charge up when attached
                  if (tech.isCapacitor) {
                    m.energy -= m.plasmaBall.drain * 2;
                    const scale =
                      1 +
                      48 *
                        Math.pow(Math.max(1, m.plasmaBall.circleRadius), -1.8);
                    Matter.Body.scale(m.plasmaBall, scale, scale); //grow
                  } else {
                  ...
                  const slowY =
                    player.velocity.y > 0
                      ? Math.max(
                          0.8,
                          1 - 0.002 * player.velocity.y * player.velocity.y,
                        )
                      : Math.max(0.98, 1 - 0.001 * Math.abs(player.velocity.y)); //down : up
                  Matter.Body.setVelocity(player, {
                    x:
                      Math.max(0.95, 1 - 0.003 * Math.abs(player.velocity.x)) *
                      player.velocity.x,
                    y: slowY * player.velocity.y,
                  });
          ...
          m.hold = function () {
            ...
            for (let i = 1, len = bullet.length; i < len; i++) {
              //skip the first bullet (which is is oldest bullet)
              if (bullet[i].isWave) {
            ...
            if (b.wasExtruderOn && b.isExtruderOn)
              ctx.lineTo(
                m.pos.x + 15 * Math.cos(m.angle),
                m.pos.y + 15 * Math.sin(m.angle),
              );
    ...
    },
    {
      name: "time dilation",
      description:
        "use <strong class='color-f'>energy</strong> to <strong style='letter-spacing: 2px;'>stop time</strong><br><strong>1.2x</strong> movement and <strong><em>fire rate</em></strong><br><strong>12</strong> <strong class='color-f'>energy</strong> per second",
      set() {
        ...
        if (tech.isRewindField) {
          ...
          m.hold = function () {
            ...
            if (m.isHolding) {
              ...
              if (!m.holdingTarget) {
                if (this.rewindCount === 0) {
                  //large upfront energy cost to enter rewind mode
                  if (m.energy > 0.3) {
                    m.energy -= 0.3;
                  } else {
                    this.rewindCount = 0;
                    m.resetHistory();
                    if (m.fireCDcycle < m.cycle + 60)
                  ...
                  if (tech.isRewindBot && !(this.rewindCount % 60)) {
                    for (let i = 0; i < tech.isRewindBot; i++) {
                      ...
                      bullet[bullet.length - 1].endCycle =
                        simulation.cycle +
                        300 +
                        Math.floor(180 * Math.random()); //8-9 seconds
                    }
                  }
      ...
      effect: () => {
        ...
        m.hold = function () {
          if (m.isHolding) {
            ...
            //not hold and field button is pressed
            if (m.energy > m.fieldRegen) m.energy -= m.fieldRegen;
            m.grabPowerUp();
            m.lookForPickUp();
          } else if (m.holdingTarget && m.fieldCDcycle < m.cycle) {
            //holding target exists, and field button is not pressed
            m.pickUp();
          } else {
            m.holdingTarget = null; //clears holding target (this is so you only pick up right after the field button is released and a hold target exists)
          }
          //not shooting (or using field) enable cloak
          if (m.energy < 0.05 && m.fireCDcycle < m.cycle && !input.fire)
            m.fireCDcycle = m.cycle;
          if (m.fireCDcycle + 10 < m.cycle && !input.fire) {
            //automatically cloak if not firing
            // const drain = 0.02
            if (!m.isCloak) {
              //&& m.energy > drain + 0.03
              // m.energy -= drain
              ...
              if (tech.isIntangible) {
                for (let i = 0; i < bullet.length; i++) {
                  if (bullet[i].botType && bullet[i].botType !== "orbit")
            ...
            if (tech.isCloakStun) {
              //stun nearby mobs after exiting cloak
          ...
          if (
            m.isSneakAttack &&
            m.sneakAttackCycle +
              Math.min(100, 0.66 * (m.cycle - m.enterCloakCycle)) >
              m.cycle
          ) {
        ...
        m.hold = function () {
          if (tech.isPrinter) {
            ...
            if (m.isHolding) {
              ...
              if (tech.isPrinter && m.holdingTarget.isPrinted && input.field) {
                ...
                const r1 =
                  m.holdingTarget.radius *
                  (1 + 0.12 * Math.sin(m.cycle * 0.11));
                const r2 =
                  m.holdingTarget.radius *
                  (1 + 0.12 * Math.cos(m.cycle * 0.11));
                let angle = (m.cycle * 0.01) % (2 * Math.PI); //rotate the object
                let vertices = [];
                for (
                  let i = 0, len = m.holdingTarget.vertices.length;
          ...
          if (input.field) {
            if (m.fieldCDcycle < m.cycle) {
              ...
              if (!m.fieldOn) {
                // if field was off, and it starting up, teleport to new mouse location
                m.fieldOn = true;
                ...
                // }
                m.fieldPosition = {
                  //smooth the mouse position, set to mouse's current location
                  x: simulation.mouseInGame.x,
                  y: simulation.mouseInGame.y,
                };
                m.lastFieldPosition = {
                  //used to find velocity of field changes
                  x: m.fieldPosition.x,
                  y: m.fieldPosition.y,
                };
              } else {
                ...
                m.fieldPosition = {
                  ...
                  x:
                    m.fieldPosition.x * smooth +
                    simulation.mouseInGame.x * (1 - smooth),
                  y:
                    m.fieldPosition.y * smooth +
                    simulation.mouseInGame.y * (1 - smooth),
                };
              }
              ...
              for (let i = 0, len = powerUp.length; i < len; ++i) {
                ...
                if (
                  ...
                  powerUp[i].force.x +=
                    0.05 * (dxP / Math.sqrt(dist2)) * powerUp[i].mass;
                  powerUp[i].force.y +=
                    0.05 * (dyP / Math.sqrt(dist2)) * powerUp[i].mass -
                    powerUp[i].mass * simulation.g; //negate gravity
                  //extra friction
                  Matter.Body.setVelocity(powerUp[i], {
                    x: powerUp[i].velocity.x * 0.11,
              ...
              if (m.energy > 0.01) {
                ...
                if (
                  Matter.Query.ray(map, m.fieldPosition, player.position).length
                ) {
                  //is there something block the player's view of the field
                  radius = 0;
                  radiusSmooth = Math.max(
                    0,
                    isInMap ? 0.96 - 0.02 * speed : 0.995,
                  ); //0.99
                } else {
                ...
                m.fieldRadius =
                  m.fieldRadius * radiusSmooth + radius * (1 - radiusSmooth);

                for (let i = 0, len = body.length; i < len; ++i) {
                  if (
                    Vector.magnitude(
                      Vector.sub(body[i].position, m.fieldPosition),
                    ) < m.fieldRadius &&
                    ...
                    if (m.energy > DRAIN) {
                      ...
                      const push = Vector.mult(
                        ...
                        0.0001 * body[i].mass * Vector.magnitude(sub),
                      );
                ...
                ctx.ellipse(
                  m.fieldPosition.x,
                  m.fieldPosition.y,
                  1.2 * m.fieldRadius * off1,
                  1.2 * m.fieldRadius * off2,
                  rotate,
                  0,
                  2 * Math.PI,
                );
                ...
                ctx.ellipse(
                  ...
                  rotate,
                  0,
                  (2 * Math.PI * m.energy) / m.maxEnergy,
                );
                ctx.strokeStyle = "#000";
                ctx.lineWidth = 4;
                ctx.stroke();
      ...
      effect: function () {
        ...
        m.hold = function () {
          ...
          if (m.hole.isOn) {
            ...
            m.fieldRange =
              0.97 * m.fieldRange +
              0.03 * (50 + 10 * Math.sin(simulation.cycle * 0.025));
            const semiMajorAxis = m.fieldRange + 30;
            const edge1a = Vector.add(
              Vector.mult(m.hole.unit, semiMajorAxis),
              m.hole.pos1,
            );
            const edge1b = Vector.add(
              Vector.mult(m.hole.unit, -semiMajorAxis),
              m.hole.pos1,
            );
            const edge2a = Vector.add(
              Vector.mult(m.hole.unit, semiMajorAxis),
              m.hole.pos2,
            );
            const edge2b = Vector.add(
              Vector.mult(m.hole.unit, -semiMajorAxis),
              m.hole.pos2,
            );
            ctx.beginPath();
            ...
            for (let i = 0, len = powerUp.length; i < len; ++i) {
              ...
              if (dist2 < 600000) {
                ...
                powerUp[i].force.y +=
                  4 * (dyP / dist2) * powerUp[i].mass -
                  powerUp[i].mass * simulation.g; //negate gravity
                Matter.Body.setVelocity(powerUp[i], {
                  x: powerUp[i].velocity.x * 0.05,
                  y: powerUp[i].velocity.y * 0.05,
                });
                if (dist2 < 1000 && !simulation.isChoosing) {
                  //use power up if it is close enough
            ...
            for (let i = 0, len = body.length; i < len; i++) {
              if (!body[i].isNotHoldable) {
                const dist1 = Vector.magnitude(
                  Vector.sub(m.hole.pos1, body[i].position),
                );
                const dist2 = Vector.magnitude(
                  Vector.sub(m.hole.pos2, body[i].position),
                );
                if (dist1 < dist2) {
                  if (dist1 < suckRange) {
                    ...
                    if (
                      ...
                      if (body[i].mass < 0.05) {
                        ...
                        m.fieldRange *= 0.8;
                        if (
                          (m.fieldMode === 0 || m.fieldMode === 9) &&
                          m.immuneCycle < m.cycle
                        )
                          m.energy += 0.02 * m.coupling;
                        if (tech.isWormholeWorms) {
                          //pandimensional spermia
                          b.worm(
                            Vector.add(
                              m.hole.pos2,
                              Vector.rotate(
                                { x: m.fieldRange * 0.4, y: 0 },
                          ...
                          Matter.Body.setVelocity(
                            ...
                            Vector.mult(
                              Vector.rotate(m.hole.unit, Math.PI / 2),
                              -10,
                            ),
                          );
                  ...
                  const pull = Vector.mult(
                    Vector.normalise(Vector.sub(m.hole.pos2, body[i].position)),
                    1,
                  );
                  const slow = Vector.mult(body[i].velocity, slowScale);
                  Matter.Body.setVelocity(body[i], Vector.add(slow, pull));
                  ...
                  if (
                    ...
                    if (body[i].mass < 0.05) {
                      ...
                      if (
                        ...
                        m.energy += 0.02 * m.coupling;
                      if (m.fieldMode === 0 || m.fieldMode === 9)
                        m.energy += 0.02 * m.coupling;
                      if (tech.isWormholeWorms) {
                        //pandimensional spermia
                        b.worm(
            ...
            if (tech.isWormHoleBullets) {
              ...
              for (let i = 0, len = bullet.length; i < len; ++i) {
                //teleport bullets from hole1 to hole2
                if (!bullet[i].botType && !bullet[i].isInHole) {
                  //don't teleport bots
                  if (
                    Vector.magnitude(
                      Vector.sub(m.hole.pos1, bullet[i].position),
                    ) < m.fieldRange
                  ) {
                    //find if bullet is touching hole1
                    Matter.Body.setPosition(
                      bullet[i],
                      Vector.add(
                        m.hole.pos2,
                    ...
                    Vector.magnitude(
                      Vector.sub(m.hole.pos2, bullet[i].position),
                    ) < m.fieldRange
                  ) {
                    //find if bullet is touching hole1
                    Matter.Body.setPosition(
              ...
              for (let i = 0, len = mob.length; i < len; i++) {
                if (
                  Vector.magnitude(Vector.sub(m.hole.pos1, mob[i].position)) <
                  200
                ) {
                  const pull = Vector.mult(
                    Vector.normalise(Vector.sub(m.hole.pos1, mob[i].position)),
                    -0.07,
                  );
                  Matter.Body.setVelocity(
                    mob[i],
                    Vector.add(mob[i].velocity, pull),
                  );
                }
                if (
                  Vector.magnitude(Vector.sub(m.hole.pos2, mob[i].position)) <
                  200
                ) {
                  const pull = Vector.mult(
                    Vector.normalise(Vector.sub(m.hole.pos2, mob[i].position)),
                    -0.07,
                  );
                  Matter.Body.setVelocity(
                    mob[i],
          ...
          if (m.fieldCDcycle < m.cycle) {
            ...
            if (input.field) {
              ...
              m.fieldRange =
                ...
                0.03 * (50 + 10 * Math.sin(simulation.cycle * 0.025));
              const edge2a = Vector.add(
                Vector.mult(unit, 1.5 * m.fieldRange),
                simulation.mouseInGame,
              );
              const edge2b = Vector.add(
                Vector.mult(unit, -1.5 * m.fieldRange),
        ...
        m.hold = function () {
          if (m.isHolding) {
            ...
            if (m.fieldCDcycle < m.cycle) {
              ...
              b.grapple(
                {
                  x: m.pos.x + 40 * Math.cos(m.angle),
                  y: m.pos.y + 40 * Math.sin(m.angle),
                },
                m.angle,
              );
            ...
            if (
              ...
              for (let i = 0; i < mob.length; i++) {
                if (
                  !mob[i].isBadTarget &&
                  !mob[i].isInvulnerable &&
                  Vector.magnitude(Vector.sub(m.pos, mob[i].position)) <
                    range &&
                  Matter.Query.ray(map, m.pos, mob[i].position).length === 0
                ) {
                  m.energy -= 0.1;
                  if (m.fieldCDcycle < m.cycle + 30)
                    m.fieldCDcycle = m.cycle + 30;
                  const angle = Math.atan2(
                    mob[i].position.y - player.position.y,
                    mob[i].position.x - player.position.x,
                  );
  ...
  //************************************************************************************
  isShipMode: false,
  shipMode(thrust = 0.03, drag = 0.99, torque = 1.15, rotationDrag = 0.92) {
    //  m.shipMode() //thrust = 0.03, drag = 0.99, torque = 1.15, rotationDrag = 0.92
    if (!m.isShipMode) {
      ...
      m.airControl = () => {
        ...
        Matter.Body.setAngularVelocity(
          player,
          player.angularVelocity * rotationDrag,
        );
        if (input.right) {
      ...
      simulation.camera = () => {
        ...
        simulation.edgeZoomOutSmooth =
          (1 + 4 * d * d) * 0.04 + simulation.edgeZoomOutSmooth * 0.96;
        ...
        simulation.mouseInGame.x =
          ((simulation.mouse.x - canvas.width2) / simulation.zoom) *
            simulation.edgeZoomOutSmooth +
          canvas.width2 -
          m.transX;
        simulation.mouseInGame.y =
          ((simulation.mouse.y - canvas.height2) / simulation.zoom) *
            simulation.edgeZoomOutSmooth +
          canvas.height2 -
          m.transY;
      };
            ...
            if (mob[k].alive && m.alive) {
              ...
              function collideMob(obj) {
                ...
                if (
                  ...
                  let dmg =
                    Math.min(
                      Math.max(0.025 * Math.sqrt(mob[k].mass), 0.05),
                      0.3,
                    ) * simulation.dmgScale; //player damage is capped at 0.3*dmgScale of 1.0
                  if (
                    tech.isRewindAvoidDeath &&
                    m.energy + 0.05 > Math.min(0.95, m.maxEnergy) &&
                    dmg > 0.01
                  ) {
                    //CPT reversal runs in m.damage, but it stops the rest of the collision code here too
                    m.damage(dmg);
                    return;
                  }
                  m.damage(dmg);
                  ...
                  let angle = Math.atan2(
                    player.position.y - mob[k].position.y,
                    player.position.x - mob[k].position.x,
                  );
                  Matter.Body.setVelocity(player, {
                ...
                if (obj.classType === "bullet" && obj.speed > obj.minDmgSpeed) {
                  ...
                  let dmg =
                    ...
                    (obj.dmg +
                      0.15 *
                        obj.mass *
                        Vector.magnitude(
                          Vector.sub(mob[k].velocity, obj.velocity),
                        ));
                  ...
                  if (mob[k].damageReduction) {
                    simulation.drawList.push({
                      ...
                      y: pairs[i].activeContacts[0].vertex.y,
                      radius:
                        Math.log(dmg + 1.1) * 40 * mob[k].damageReduction + 3,
                ...
                if (obj.classType === "body" && obj.speed > 6) {
                  ...
                  if (v > 9) {
                    ...
                    if (stunTime > 0.5)
                      mobs.statusStun(mob[k], 30 + 60 * Math.sqrt(stunTime));
                    if (
                      mob[k].alive &&
                      mob[k].distanceToPlayer2() < 1000000 &&
                      !m.isCloak
                    )
                      mob[k].foundPlayer();
                    if (
