const tech = {
  ...
  setupAllTech() {
    for (let i = 0, len = tech.tech.length; i < len; i++) {
      ...
      if (
        tech.tech[i].name === "heals" ||
        tech.tech[i].name === "ammo" ||
        tech.tech[i].name === "research"
      )
        tech.tech[i].value = tech.tech[i].defaultValue;
    }
    ...
    lore.techCount = 0;
    if (simulation.isCheating || localSettings.runCount < 1) {
      //simulation.isCommunityMaps ||
      for (let i = 0, len = tech.tech.length; i < len; i++) {
  ...
  removeTech(index = "random") {
    ...
    simulation.makeTextLog(
      `<span class='color-var'>tech</span>.removeTech("<span class='color-text'>${tech.tech[index].name}</span>")`,
      360,
    );
  ...
  junkChance: 0,
  addJunkTechToPool(percent) {
    //percent is number between 0-1
    simulation.makeTextLog(
      `<strong>+${(100 * percent).toFixed(0)}%</strong> <span class='color-text'>JUNK</span><span class='color-var'>tech</span> chance`,
    );
  ...
  removeJunkTechFromPool(percent) {
    ...
    if (percent > 0) {
      ...
      if (tech.junkChance < 0.001 || tech.junkChance === undefined)
        tech.junkChance = 0;
    }
  },
  giveRandomJUNK() {
    ...
    simulation.makeTextLog(
      `<span class='color-var'>tech</span>.giveTech("<span class='color-text'>${name}</span>")<em>`,
    );
    tech.giveTech(name);
  },
  giveTech(index = "random") {
    ...
    if (index === "random") {
      ...
      if (options.length > 0) {
        ...
        simulation.makeTextLog(
          `<span class='color-var'>tech</span>.giveTech("<span class='color-text'>${tech.tech[newTech].name}</span>")<em> //random tech</em>`,
        );
      ...
      if (tech.isMetaAnalysis && tech.tech[index].isJunk) {
        simulation.makeTextLog(
          `//tech: meta-analysis replaced junk tech with random tech`,
        );
        tech.giveTech("random");
        for (let i = 0; i < 2; i++)
          powerUps.spawn(
            m.pos.x + 40 * Math.random(),
            m.pos.y + 40 * Math.random(),
            "research",
          );
        return;
      }
      ...
      if (tech.isBanish && tech.tech[index].isBanished)
        tech.tech[index].isBanished = false; //stops the bug where you can't gets stacks of tech you take with decoherence, I think
  ...
  haveGunCheck(name, needActive = true) {
    ...
    if (build.isExperimentSelection || !needActive) {
      ...
      //must be holding gun, this is the standard while playing
  ...
  hasExplosiveDamageCheck() {
    return (
      ...
      (m.fieldMode === 4 && simulation.molecularMode === 1) ||
      tech.missileBotCount > 0 ||
      tech.isBoomBotUpgrade ||
      tech.isIncendiary ||
      tech.isPulseLaser ||
      tech.isTokamak ||
      (tech.haveGunCheck("grenades") && !tech.isNeutronBomb)
    );
  },
  ...
  damageFromTech() {
    ...
    if (tech.isDamageCooldown)
      dmg *= m.lastKillCycle + tech.isDamageCooldownTime > m.cycle ? 0.5 : 4;
    if (tech.isDamageAfterKillNoRegen && m.lastKillCycle + 300 > m.cycle)
      dmg *= 2;
    if (
      tech.isDivisor &&
      b.activeGun !== undefined &&
      b.activeGun !== null &&
      b.guns[b.activeGun].ammo % 3 === 0
    )
      dmg *= 1.8;
    if (tech.isNoGroundDamage) dmg *= m.onGround ? 0.85 : 2;
    if (tech.isDilate) dmg *= 1.9 + 1.1 * Math.sin(m.cycle * 0.01);
    if (tech.isGunChoice && tech.buffedGun === b.inventoryGun)
      dmg *= 1 + 0.3 * b.inventory.length;
    if (powerUps.boost.endCycle > m.cycle) dmg *= 1 + powerUps.boost.damage;
    if (m.coupling && (m.fieldMode === 0 || m.fieldMode === 5))
      dmg *= 1 + 0.015 * m.coupling;
    if (
      m.isSneakAttack &&
      m.sneakAttackCycle + Math.min(100, 0.66 * (m.cycle - m.enterCloakCycle)) >
        m.cycle
    )
      dmg *= 4.5 * (1 + 0.033 * m.coupling);
    if (tech.deathSkipTime) dmg *= 1 + 0.6 * tech.deathSkipTime;
    if (tech.isTechDebt)
      dmg *=
        tech.totalCount > 20
          ? Math.pow(0.85, tech.totalCount - 20)
          : 4 - 0.15 * tech.totalCount;
    if (tech.isAnthropicDamage && tech.isDeathAvoidedThisLevel) dmg *= 2.71828;
    if (tech.isDupDamage) dmg *= 1 + Math.min(1, tech.duplicationChance());
    if (tech.isDamageForGuns)
      dmg *= 1 + 0.22 * Math.max(0, b.inventory.length - 1);
    if (tech.isOneGun && b.inventory.length < 2) dmg *= 1.3;
    if (tech.isAcidDmg && m.health > 1) dmg *= 1.35;
    if (tech.isRerollDamage)
      dmg *= 1 + Math.max(0, 0.05 * powerUps.research.count);
    ...
    if (tech.isSpeedDamage)
      dmg *= 1 + Math.min(1, (tech.speedAdded + player.speed) * 0.0193);
    if (tech.isAxion && tech.isHarmMACHO) dmg *= tech.isMoveMACHO ? 3 : 2;
    if (tech.isHarmDamage && m.lastHarmCycle + 480 > m.cycle) dmg *= 3;
    if (tech.lastHitDamage && m.lastHit)
      dmg *= 1 + tech.lastHitDamage * m.lastHit;
    if (tech.isLowHealthDmg)
      dmg *=
        1 + 0.7 * Math.max(0, 1 - (tech.isEnergyHealth ? m.energy : m.health));
  ...
  duplicationChance() {
    return Math.min(
      ...
      Math.max(
        0,
        (tech.isPowerUpsVanish ? 0.13 : 0) +
          (tech.isStimulatedEmission ? 0.2 : 0) +
          tech.duplication +
          tech.duplicateChance +
          0.05 * tech.isExtraGunField +
          m.duplicateChance +
          tech.fieldDuplicate +
          0.08 * tech.isDuplicateMobs +
          0.03 * tech.isMassProduction +
          0.04 * tech.isHealAttract +
          tech.cloakDuplication +
          (tech.isAnthropicTech && tech.isDeathAvoidedThisLevel ? 0.6 : 0) +
          0.06 * tech.isDupEnergy,
      ),
    );
  },
  ...
  tech: [
    {
      ...
      description:
        "<strong>+300</strong> maximum <strong class='color-h'>health</strong><br><strong>lose</strong> <strong class='color-h'>health</strong> after hard <strong>landings</strong>",
      maxCount: 1,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      name: "nitinol",
      description:
        "<strong>1.3x</strong> <strong>movement</strong> and <strong>jumping</strong><br><strong>0.8x</strong> <strong class='color-defense'>damage taken</strong>",
      maxCount: 1,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      ...
      description:
        "<strong>4x</strong> <em>fire rate</em><br>while <strong>firing</strong> your <strong>position</strong> is fixed",
      maxCount: 1,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      name: "Hilbert space",
      description:
        "<strong>4x</strong> <strong class='color-d'>damage</strong><br>after a <strong>collision</strong> enter an <strong class='alt'>alternate reality</strong>",
      maxCount: 1,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      name: "aperture",
      description:
        "every <strong>4</strong> seconds your <strong class='color-d'>damage</strong> cycles<br>between <strong>0.8x</strong> and <strong>3x</strong> <strong class='color-d'>damage</strong>",
      maxCount: 1,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      name: "diaphragm",
      description:
        "every <strong>4</strong> seconds your <strong class='color-defense'>damage taken</strong> cycles<br>between <strong>0.9x</strong> and <strong>0.2x</strong> <strong class='color-defense'>damage taken</strong>",
      maxCount: 1,
      count: 0,
      frequency: 2,
      frequencyDefault: 2,
    ...
    },
    {
      ...
      allowed() {
        return (
          (powerUps.research.count > 1 || build.isExperimentSelection) &&
          !m.isAltSkin &&
          !tech.isPiezo &&
          !tech.isRewindAvoidDeath &&
          !tech.isAnnihilation
        ); //&& !tech.isAmmoFromHealth && !tech.isRewindGun
      },
    ...
    },
    {
      ...
      effect() {
        ...
        for (let i = 0; i < powerUp.length; i++) {
          //find active heal power ups and adjust color live
          if (powerUp[i].name === "heal")
            powerUp[i].color = powerUps.heal.color;
        }
      },
      remove() {
    ...
    },
    {
      ...
      allowed() {
        return (
          !m.isAltSkin &&
          m.fieldUpgrades[m.fieldMode].name !== "standing wave" &&
          !tech.isRewindField &&
          !tech.isEnergyHealth
        );
      },
      requires:
        "not skinned, standing wave, max energy reduction, retrocausality, mass-energy",
      effect() {
    ...
    },
    {
      ...
      description:
        "when you <strong class='color-rewind'>rewind</strong> build scrap <strong class='color-bot'>bots</strong><br>that protect you for about <strong>9</strong> seconds",
      maxCount: 3,
      count: 0,
      frequency: 2,
      frequencyDefault: 2,
    ...
    },
    {
      ...
      description:
        "when you <strong class='color-rewind'>rewind</strong> drop several <strong>grenades</strong>", //<br>become <strong>invulnerable</strong> until they <strong class='color-e'>explode</strong>
      maxCount: 1,
      count: 0,
      frequency: 2,
      frequencyDefault: 2,
    ...
    },
    {
      name: "ordnance",
      description:
        "spawn a <strong class='color-g'>gun</strong>, gain <strong>2x</strong> <strong class='color-g'>gun</strong><strong class='color-m'>tech</strong> <em class='flicker'>frequency</em><br><strong>+6%</strong> <strong class='color-junk'>JUNK</strong><strong class='color-m'>tech</strong> chance",
      maxCount: 1,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      name: "arsenal",
      descriptionFunction() {
        return `<strong>1.25x</strong> <strong class='color-d'>damage</strong> per unequipped <strong class='color-g'>gun</strong><br><em>(${(1 + 0.25 * Math.max(0, b.inventory.length - 1)).toFixed(2)}x)</em>`;
      },
      maxCount: 1,
      count: 0,
      frequency: 1,
    ...
    },
    {
      name: "active cooling",
      descriptionFunction() {
        return `<strong>1.25x</strong> <em>fire rate</em> per unequipped <strong class='color-g'>gun</strong><br><em>(${(1 / Math.pow(0.8, Math.max(0, b.inventory.length - 1))).toFixed(2)}x)</em>`;
    ...
    },
    {
      ...
      descriptionFunction() {
        ...
        if (
          this.count > 0 &&
          Number.isInteger(tech.buffedGun) &&
          b.inventory.length
        ) {
    ...
    },
    {
      name: "generalist",
      description:
        "spawn <strong>7</strong> <strong class='color-g'>guns</strong>, but you can't <strong>switch</strong> <strong class='color-g'>guns</strong><br>your equipped <strong class='color-g'>gun</strong> cycles after each level",
      maxCount: 1,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
      ...
      effect() {
        ...
        for (let i = 0; i < 7; i++)
          powerUps.spawn(
            m.pos.x + 10 * Math.random(),
            m.pos.y + 10 * Math.random(),
            "gun",
          );
      },
      remove() {
    ...
    },
    {
      ...
      effect() {
        for (let i = b.inventory.length - 1; i > -1; i--) {
          //backwards because some tech can remove or add guns
          ...
          for (let j = 0, len = tech.tech.length; j < len; j++) {
            ...
            if (
              tech.tech[j].isGunTech &&
              tech.tech[j].allowed() &&
              !tech.tech[j].isJunk &&
              !tech.tech[j].isBadRandomOption &&
              tech.tech[j].count < tech.tech[j].maxCount
            ) {
              const regex = tech.tech[j].requires.search(
                b.guns[b.inventory[i]].name,
              ); //get string index of gun name
              const not = tech.tech[j].requires.search(" not "); //get string index of ' not '
              if (regex !== -1 && (not === -1 || not > regex))
                gunTechPool.push(j); //look for the gun name in the requirements, but the gun name needs to show up before the word ' not '
            }
          ...
          if (gunTechPool.length) {
            ...
            simulation.makeTextLog(
              `<span class='color-var'>tech</span>.giveTech("<span class='color-text'>${tech.tech[gunTechPool[index]].name}</span>")`,
              360,
            );
    ...
    },
    {
      ...
      effect() {
        ...
        for (let i = 0; i < b.guns.length; i++) {
          if (b.guns[i].have && b.guns[i].ammo !== Infinity)
            ammoCount += b.guns[i].ammo / b.guns[i].ammoPack;
        }
    ...
    },
    {
      ...
      descriptionFunction() {
        if (this.count === 0)
          this.gun = Math.floor(Math.random() * (b.guns.length - 1)); //don't pick laser
      ...
      effect() {
        ...
        simulation.makeTextLog(
          `${b.guns[this.gun].ammoPack} → ${2 * b.guns[this.gun].ammoPack} average <strong class='color-ammo'>ammo</strong> per ${powerUps.orb.ammo(1)} for <strong class='color-g'>${b.guns[this.gun].name}</strong>`,
        );
    ...
    },
    {
      ...
      effect() {
        let options = [];
        for (let i = 0; i < b.inventory.length; i++)
          options.push(b.inventory[i]);
        options = shuffle(options);
        for (let i = 0; i < options.length; i++) {
          const index = options[i];
          const scale = i < options.length / 2 ? 4 : 0.25;
          simulation.makeTextLog(
            `${b.guns[index].ammoPack.toFixed(1)} <span ${scale < 1 ? 'style="color: #f00;"' : ""}>→</span> ${(b.guns[index].ammoPack * scale).toFixed(1)} average <strong class='color-ammo'>ammo</strong> per ${powerUps.orb.ammo(1)} for <strong class='color-g'>${b.guns[index].name}</strong>`,
            Infinity,
          );
    ...
    },
    {
      name: "gun turret",
      description:
        "if <strong>crouching</strong><br><strong>0.3x</strong> <strong class='color-defense'>damage taken</strong>",
      maxCount: 1,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      ...
      descriptionFunction() {
        return `<strong class='color-d'>damage</strong> is proportional to your <strong class="color-speed">speed</strong><br>up to <strong>2x</strong> <strong class='color-d'>damage</strong> at <strong>55</strong> <strong class="color-speed">speed</strong> <em>(${(1 + Math.min(1, (tech.speedAdded + player.speed) * 0.0193)).toFixed(2)}x)</em>`;
      },
      maxCount: 1,
      count: 0,
      frequency: 1,
    ...
    },
    {
      ...
      description:
        "far away mobs take more <strong class='color-d'>damage</strong><br>up to <strong>1.3x</strong> <strong class='color-d'>damage</strong> at <strong>3000</strong> displacement",
      maxCount: 1,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      name: "regression",
      description:
        "bullet <strong>collisions</strong> increase <strong>vulnerability</strong> to<br><strong class='color-d'>damage</strong> by <strong>1.05x</strong> for mobs <em>(+1.025x for bosses)</em>",
      maxCount: 1,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      ...
      description:
        "<strong>1.2x</strong> <strong class='color-d'>damage</strong><br><strong>0.8x</strong> <em>fire rate</em>",
      maxCount: 1,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      ...
      descriptionFunction() {
        let damageTotal = 1;
        for (let i = 0; i < this.damageSoFar.length; i++)
          damageTotal *= this.damageSoFar[i];
        let currentDamage = "";
        if (this.count)
          currentDamage = `<br><em>(${damageTotal.toFixed(2)}x)</em>`;
        return (
          `randomly gain between <strong>1x</strong> and <strong>1.3x</strong> <strong class='color-d'>damage</strong>` +
          currentDamage
        );
      },
      maxCount: 9,
      ...
      remove() {
        if (this.count && m.alive)
          for (let i = 0; i < this.damageSoFar.length; i++)
            tech.damage /= this.damageSoFar[i];
    ...
    },
    {
      ...
      description:
        "your bullets last <strong>1.3x</strong> <strong>longer</strong>", //<br><em style = 'font-size: 83%'>drone spore worm flea missile foam wave neutron ice</em>",
      maxCount: 3,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      ...
      description:
        "if a mob is <strong>stunned</strong> it takes<br><strong>5x</strong> <strong class='color-d'>damage</strong> from bullet impacts",
      maxCount: 1,
      count: 0,
      frequency: 2,
      frequencyDefault: 2,
    ...
    },
    {
      name: "shear stress",
      description:
        "after mobs <strong>die</strong><br>they fire a <strong>nail</strong> at nearby mobs",
      maxCount: 9,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      ...
      description:
        "after mobs <strong>die</strong> they <strong class='color-e'>explode</strong>",
      maxCount: 1,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      name: "propagator",
      description:
        "after mobs <strong>die</strong> advance time <strong>0.5</strong> seconds<br><strong>1.6x</strong> <strong class='color-d'>damage</strong>",
      maxCount: 3,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      ...
      description:
        "<strong>3x</strong> <strong class='color-d'>damage</strong> to <strong>mobs</strong> below <strong>25%</strong> <strong>durability</strong>",
      maxCount: 1,
      count: 0,
      frequency: 3,
      frequencyDefault: 3,
    ...
    },
    {
      ...
      description:
        "<strong>mobs</strong> spawn with <strong>0.88x</strong> initial <strong>durability</strong>",
      maxCount: 3,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
      ...
      effect() {
        ...
        for (let i = 0; i < mob.length; i++) {
          if (mob.health > mobs.mobSpawnWithHealth)
            mob.health = mobs.mobSpawnWithHealth;
        }
      },
      remove() {
    ...
    },
    {
      ...
      description:
        "after mobs <strong>die</strong> you have a <strong>33%</strong> chance<br>to build scrap <strong class='color-bot'>bots</strong> that operate for <strong>15</strong> seconds",
      maxCount: 3,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      ...
      description:
        "after mobs <strong>die</strong><br>reset scrap <strong class='color-bot'>bots</strong> to <strong>15</strong> seconds of operation",
      maxCount: 1,
      count: 0,
      frequency: 3,
      frequencyDefault: 3,
    ...
    },
    {
      ...
      description:
        "a <strong class='color-bot'>bot</strong> fires <strong>nails</strong> at mobs in line of sight",
      maxCount: 9,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      ...
      description:
        "<strong>convert</strong> your bots to <strong>nail-bots</strong><br><strong>5x</strong> <em>fire rate</em> and <strong>1.4x</strong> nail <strong>velocity</strong>",
      maxCount: 1,
      count: 0,
      frequency: 3,
      frequencyDefault: 3,
    ...
    },
    {
      ...
      description:
        "a <strong class='color-bot'>bot</strong> sprays sticky <strong>foam</strong> at nearby mobs",
      maxCount: 9,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      ...
      description:
        "<strong>convert</strong> your bots to <strong>foam-bots</strong><br><strong>3x</strong> foam <strong>size</strong> and <em>fire rate</em>",
      maxCount: 1,
      count: 0,
      frequency: 3,
      frequencyDefault: 3,
    ...
    },
    {
      ...
      description:
        "a <strong class='color-bot'>bot</strong> emits <strong>expanding arcs</strong><br>aimed towards nearby mobs",
      maxCount: 9,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      ...
      description:
        "<strong>convert</strong> your bots to <strong>sound-bots</strong><br><strong>2.5x</strong> wave <em>fire rate</em> and <strong>2.5x</strong> <strong class='color-d'>damage</strong>",
      maxCount: 1,
      count: 0,
      frequency: 3,
      frequencyDefault: 3,
    ...
    },
    {
      ...
      description:
        "a <strong class='color-bot'>bot</strong> <strong>defends</strong> the space around you<br>ignites an <strong class='color-e'>explosion</strong> after hitting a mob",
      maxCount: 9,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      ...
      description:
        "<strong>convert</strong> your bots to <strong>boom-bots</strong><br><strong>4x</strong> <strong class='color-e'>explosion</strong> <strong class='color-d'>damage</strong> and size",
      maxCount: 1,
      count: 0,
      frequency: 3,
      frequencyDefault: 3,
    ...
    },
    {
      ...
      description:
        "a <strong class='color-bot'>bot</strong> uses <strong class='color-f'>energy</strong> to emit a <strong class='color-laser'>laser</strong> beam<br>that targets nearby mobs",
      maxCount: 9,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      ...
      description:
        "<strong>convert</strong> your bots to <strong>laser-bots</strong><br><strong>2.00x</strong> <strong class='color-d'>damage</strong>, efficiency, and range",
      maxCount: 1,
      count: 0,
      frequency: 3,
      frequencyDefault: 3,
    ...
    },
    {
      ...
      description:
        "<strong>convert</strong> your bots to <strong>orbital-bots</strong><br><strong>4x</strong> orbital <strong class='color-d'>damage</strong> and <strong>1.5x</strong> <strong>radius</strong>",
      maxCount: 1,
      count: 0,
      frequency: 3,
      frequencyDefault: 3,
    ...
    },
    {
      ...
      description:
        "a <strong class='color-bot'>bot</strong> <strong class='color-d'>damages</strong> mobs while it <strong>traces</strong> your path<br><strong>+8</strong> <strong class='color-f'>energy</strong> per second when nearby",
      maxCount: 9,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      ...
      description:
        "<strong>convert</strong> your bots to <strong>dynamo-bots</strong><br><strong>+24</strong> <strong class='color-f'>energy</strong> per second when nearby",
      maxCount: 1,
      count: 0,
      frequency: 3,
      frequencyDefault: 3,
    ...
    },
    {
      ...
      description:
        "for each permanent <strong class='color-bot'>bot</strong><br><strong>0.95x</strong> <strong class='color-defense'>damage taken</strong>",
      maxCount: 1,
      count: 0,
      frequency: 2,
      frequencyDefault: 2,
    ...
    },
    {
      name: "network effect",
      description:
        "for each permanent <strong class='color-bot'>bot</strong><br><strong>1.05x</strong> <strong class='color-d'>damage</strong>",
      maxCount: 1,
      count: 0,
      frequency: 2,
      frequencyDefault: 2,
    ...
    },
    {
      ...
      effect() {
        ...
        simulation.makeTextLog(
          `<span class='color-var'>m</span>.<span class='color-r'>research</span> <span class='color-symbol'>=</span> 0`,
        );
      },
      remove() {
    ...
    },
    {
      ...
      remove() {
        if (this.count > 0) {
          for (let i = 0, len = tech.tech.length; i < len; i++) {
            if (tech.tech[i].isBotTech)
              tech.tech[i].frequency = Math.ceil(tech.tech[i].frequency / 3);
          }
        }
    ...
    },
    {
      ...
      description:
        "<strong>double</strong> your current permanent <strong class='color-bot'>bots</strong><br>remove <strong>all</strong> <strong class='color-g'>guns</strong> in your inventory",
      maxCount: 1,
      count: 0,
      frequency: 2,
      frequencyDefault: 2,
    ...
    },
    {
      name: "mass driver",
      description:
        "<strong>4x</strong> <strong class='color-block'>block</strong> collision <strong class='color-d'>damage</strong>",
      maxCount: 1,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      ...
      description:
        "if <strong>holding</strong> a <strong class='color-block'>block</strong> <strong>0.1x</strong> <strong class='color-defense'>damage taken</strong><br>after <strong>throwing</strong> a <strong class='color-block'>block</strong> it expands <strong>3x</strong>",
      maxCount: 1,
      count: 0,
      frequency: 3,
      frequencyDefault: 3,
      allowed() {
        return (
          (tech.blockDamage > 0.075 || tech.isPrinter) &&
          m.fieldMode !== 8 &&
          m.fieldMode !== 9 &&
          !tech.isTokamak
        );
    ...
    },
    {
      name: "restitution",
      description:
        "<strong>2.5x</strong> <strong class='color-block'>block</strong> collision <strong class='color-d'>damage</strong><br>after <strong>throwing</strong> a <strong class='color-block'>block</strong> it becomes very <strong>bouncy</strong>",
      maxCount: 1,
      count: 0,
      frequency: 3,
      frequencyDefault: 3,
      allowed() {
        return (
          (tech.blockDamage > 0.075 || tech.isPrinter) &&
          m.fieldUpgrades[m.fieldMode].name !== "pilot wave" &&
          m.fieldUpgrades[m.fieldMode].name !== "wormhole" &&
          !tech.isTokamak
        );
      },
      requires: "mass driver, not pilot wave, tokamak, wormhole",
    ...
    },
    {
      name: "flywheel",
      description:
        "<strong>2.5x</strong> <strong class='color-block'>block</strong> collision <strong class='color-d'>damage</strong><br>after a mob <strong>dies</strong> its <strong class='color-block'>block</strong> is <strong>flung</strong> at mobs",
      maxCount: 1,
      count: 0,
      frequency: 3,
      frequencyDefault: 3,
...

    {
      ...
      descriptionFunction() {
        return `while your <strong>first</strong> <strong class='color-g'>gun</strong> is equipped<br><strong>0.85x</strong> <strong class='color-defense'>damage taken</strong> per <strong class='color-g'>gun</strong> <em>(${(0.85 ** b.inventory.length).toFixed(2)}x)</em>`;
      },
      maxCount: 1,
      count: 0,
      frequency: 1,
    ...
    },
    {
      name: "MACHO",
      description:
        "a massive compact halo object follows you<br><strong>0.4x</strong> <strong class='color-defense'>damage taken</strong> inside the <strong class='color-MACHO'>MACHO</strong>",
      maxCount: 1,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      ...
      description:
        "<strong>crouching</strong> pulls the <strong class='color-MACHO'>MACHO</strong> towards you<br> <strong>1.5x</strong> for all <strong class='color-MACHO'>MACHO</strong> effects",
      maxCount: 1,
      count: 0,
      frequency: 2,
      frequencyDefault: 2,
    ...
    },
    {
      name: "axion",
      description:
        "while inside the <strong class='color-MACHO'>MACHO</strong><br> <strong>2x</strong> <strong class='color-d'>damage</strong>",
      maxCount: 1,
      count: 0,
      frequency: 2,
      frequencyDefault: 2,
    ...
    },
    {
      ...
      description:
        "after losing <strong class='color-h'>health</strong> there is a chance<br>to rebuild your broken parts as <strong>drones</strong>",
      maxCount: 1,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      ...
      description:
        "after mob collisions<br><strong>0.3x</strong> <strong class='color-defense'>damage taken</strong> for <strong>10</strong> seconds",
      maxCount: 1,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      ...
      description:
        "if you <strong>collide</strong> with a mob<br>generate <strong>+2048</strong> <strong class='color-f'>energy</strong>",
      maxCount: 1,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      name: "ground state",
      description:
        "<strong>+300</strong> maximum <strong class='color-f'>energy</strong><br><strong>0.66x</strong> passive <strong class='color-f'>energy</strong> generation",
      maxCount: 1,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      ...
      description:
        "<strong>1.6x</strong> <strong class='color-d'>damage</strong><br>after mobs <strong>die</strong> <strong>–5</strong> <strong class='color-f'>energy</strong>",
      maxCount: 1,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      ...
      descriptionFunction() {
        return `use ${powerUps.orb.research(2)}<br><span style = 'font-size:94%;'><strong>1.01x</strong> <strong class='color-d'>damage</strong> per <strong class='color-f'>energy</strong> below <strong>100</strong> <em>(${(1 + Math.max(0, 1 - m.energy)).toFixed(2)}x)</em></span>`;
      },
      maxCount: 1,
      count: 0,
      frequency: 1,
    ...
    },
    {
      name: "overcharge",
      description:
        "<strong>+88</strong> maximum <strong class='color-f'>energy</strong><br><strong>+5%</strong> <strong class='color-junk'>JUNK</strong><strong class='color-m'>tech</strong> chance",
      maxCount: 9,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      name: "Maxwells demon",
      description:
        "<strong class='color-f'>energy</strong> above max decays <strong>30x</strong> slower<br><strong>+5%</strong> <strong class='color-junk'>JUNK</strong><strong class='color-m'>tech</strong> chance",
      maxCount: 1,
      count: 0,
      frequency: 2,
      frequencyDefault: 2,
    ...
    },
    {
      ...
      description:
        "if <strong>crouched</strong> <strong>7x</strong> passive <strong class='color-f'>energy</strong> generation<br>otherwise <strong>0x</strong> passive <strong class='color-f'>energy</strong> generation",
      maxCount: 1,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      ...
      description:
        "<strong>1.04x</strong> of <strong class='color-d'>damage</strong> done recovered as <strong class='color-f'>energy</strong>",
      maxCount: 9,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      name: "parasitism",
      description:
        "if a mob has <strong>died</strong> in the last <strong>5</strong> seconds<br><strong>2x</strong> <strong class='color-d'>damage</strong>, no passive <strong class='color-f'>energy</strong> generation",
      maxCount: 1,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      ...
      description:
        "if a mob has <strong>died</strong> in the last <strong>5</strong> seconds<br>generate <strong>0.05x</strong> max <strong class='color-f'>energy</strong> per second",
      maxCount: 1,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      name: "recycling",
      descriptionFunction() {
        return `if a mob has <strong>died</strong> in the last <strong>5</strong> seconds<br>recover <strong>0.005x</strong> max ${tech.isEnergyHealth ? "<strong class='color-f'>energy</strong>" : "<strong class='color-h'>health</strong>"} per second`;
      },
      description: "",
      maxCount: 1,
      count: 0,
    ...
    },
    {
      name: "torpor",
      description:
        "if a mob has <strong>not died</strong> in the last <strong>5</strong> seconds<br><strong>0.3x</strong> <strong class='color-defense'>damage taken</strong>",
      maxCount: 1,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      name: "homeostasis",
      descriptionFunction() {
        return `<strong>0.8x</strong> <strong class='color-defense'>damage taken</strong> per <strong class='color-h'>health</strong> below <strong>100</strong><br><em>(${(1 - Math.max(0, 1 - m.health) * 0.8).toFixed(2)}x)</em>`;
      },
      maxCount: 1,
      count: 0,
      frequency: 1,
    ...
    },
    {
      ...
      descriptionFunction() {
        return `<strong>1.007x</strong> <strong class='color-d'>damage</strong> per ${tech.isEnergyHealth ? "<strong class='color-f'>energy</strong>" : "<strong class='color-h'>health</strong>"} below <strong>100</strong><br><em>(${(1 + 0.7 * Math.max(0, 1 - (tech.isEnergyHealth ? m.energy : m.health))).toFixed(2)}x)</em>`;
      },
      maxCount: 1,
      count: 0,
      frequency: 1,
    ...
    },
    {
      ...
      description:
        "if your <strong class='color-h'>health</strong> is above <strong>100</strong><br><strong>1.35x</strong> <strong class='color-d'>damage</strong>",
      maxCount: 1,
      count: 0,
      frequency: 2,
      frequencyDefault: 2,
    ...
    },
    {
      ...
      effect() {
        ...
        if (!build.isExperimentSelection && !simulation.isTextLogOpen)
          simulation.circleFlare(0.04);
      },
      remove() {
    ...
    },
    {
      name: "accretion disk",
      descriptionFunction() {
        return `<strong>1.05x</strong> <strong class='color-d'>damage</strong> <em>(${(1 + 0.05 * powerUp.length).toFixed(2)}x)</em><br>for each <strong>power up</strong> that exists on this <strong>level</strong>`;
      },
      maxCount: 1,
      count: 0,
      frequency: 3,
    ...
    },
    {
      ...
      effect() {
        for (let i = 0; i < 13; i++)
          powerUps.spawn(
            m.pos.x + 60 * (Math.random() - 0.5),
            m.pos.y + 60 * (Math.random() - 0.5),
            "heal",
          );
        for (let i = 0, len = tech.tech.length; i < len; i++) {
    ...
    },
    {
      ...
      description:
        "after <strong>anthropic principle</strong> prevents your <strong>death</strong><br><strong>+60%</strong> <strong class='color-dup'>duplication</strong> chance for that level",
      maxCount: 1,
      count: 0,
      frequency: 3,
      frequencyDefault: 3,
    ...
    },
    {
      ...
      description:
        "after <strong>anthropic principle</strong> prevents your <strong>death</strong><br><strong>2.71828x</strong> <strong class='color-d'>damage</strong> for that level",
      maxCount: 1,
      count: 0,
      frequency: 3,
      frequencyDefault: 3,
    ...
    },
    {
      ...
      description:
        "<strong>0.7x</strong> <strong class='color-defense'>damage taken</strong><br>after <strong>dying</strong>, continue in an <strong class='alt'>alternate reality</strong>",
      maxCount: 1,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      ...
      description: `after a <strong>boss</strong> <strong>dies</strong> spawn ${powerUps.orb.research(5)}<br>after you <strong class='color-r'>research</strong> enter an <strong class='alt'>alternate reality</strong>`,
      maxCount: 1,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
      ...
      allowed() {
        return (
          !tech.isSwitchReality &&
          !tech.isCollisionRealitySwitch &&
          !tech.isJunkResearch
        );
      },
    ...
    },
    {
      name: "pseudoscience",
      description:
        "<strong class='color-r'>research</strong> <strong>2</strong> times</span> for <strong>free</strong>, but<br>add <strong>1%</strong> <strong class='color-junk'>JUNK</strong><strong class='color-m'>tech</strong> chance each time",
      maxCount: 1,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      ...
      description: `<strong>+5%</strong> <strong class='color-junk'>JUNK</strong><strong class='color-m'>tech</strong> chance<br><strong>47%</strong> chance to spawn ${powerUps.orb.research(1)} after consuming ${powerUps.orb.research(1)}`,
      maxCount: 1,
      count: 0,
      frequency: 2,
      frequencyDefault: 2,
      allowed() {
        return (
          (powerUps.research.count > 3 || build.isExperimentSelection) &&
          !tech.isSuperDeterminism
        );
      },
    ...
    },
    {
      ...
      descriptionFunction() {
        return `<strong>1.05x</strong> <strong class='color-d'>damage</strong> per ${powerUps.orb.research(1)} in your inventory<br><em>(${(1 + Math.max(0, 0.05 * powerUps.research.count)).toFixed(2)}x)</em>`;
      },
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      allowed() {
        return (
          powerUps.research.count < 1 &&
          !tech.isSuperDeterminism &&
          !tech.isRerollHaste
        );
      },
      requires:
        "no research, not superdeterminism, Ψ(t) collapse, perturbation theory",
      effect() {
        tech.isAnsatz = true;
      },
      remove() {
    ...
    },
    {
      name: "eternalism",
      description:
        "<strong>1.25x</strong> <strong class='color-d'>damage</strong><br><strong>time</strong> can't be <strong>paused</strong> <em>(time can be dilated)</em>",
      maxCount: 1,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
      allowed() {
        return (
          !tech.isPauseSwitchField &&
          !tech.isPauseEjectTech &&
    ...
    },
    {
      name: "brainstorming",
      description:
        "<strong class='color-m'>tech</strong> <strong class='color-choice'><span>ch</span><span>oi</span><span>ces</span></strong> <strong>randomize</strong><br>every <strong>1.5</strong> seconds for <strong>10</strong> seconds",
      maxCount: 1,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      ...
      description:
        "<strong class='color-m'>tech</strong> have an extra <strong class='color-f'>field</strong> or <strong class='color-g'>gun</strong> <strong class='color-choice'><span>ch</span><span>oi</span><span>ce</span></strong><br><strong>+5%</strong> chance to <strong class='color-dup'>duplicate</strong> spawned <strong>power ups</strong>",
      maxCount: 1,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      name: "emergence",
      description:
        "<strong class='color-m'>tech</strong>, <strong class='color-f'>fields</strong>, and <strong class='color-g'>guns</strong> have <strong>+1</strong> <strong class='color-choice'><span>ch</span><span>oi</span><span>ce</span></strong><br><strong>1.1x</strong> <strong class='color-d'>damage</strong>",
      maxCount: 9,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      ...
      description:
        "your next <strong class='color-m'>tech</strong> <strong class='color-choice'><span>ch</span><span>oi</span><span>ce</span></strong> has all possible <strong>options</strong><br><strong>+5%</strong> <strong class='color-junk'>JUNK</strong><strong class='color-m'>tech</strong> chance",
      maxCount: 1,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      name: "determinism",
      description:
        "spawn <strong>5</strong> <strong class='color-m'>tech</strong><br>only <strong>1</strong> <strong class='color-choice'><span>ch</span><span>oi</span><span>ce</span></strong> for <strong class='color-m'>tech</strong>, <strong class='color-f'>fields</strong>, and <strong class='color-g'>guns</strong>",
      maxCount: 1,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
      ...
      allowed() {
        return (
          !tech.extraChoices && !tech.isExtraGunField && !tech.isExtraBotOption
        );
      },
    ...
    },
    {
      name: "technical debt",
      descriptionFunction() {
        return `<strong>4x</strong> <strong class='color-d'>damage</strong> but lose <strong>0.15x</strong> <strong class='color-d'>damage</strong><br>for each <strong class='color-m'>tech</strong> you have learned <em>(${(tech.totalCount > 20 ? Math.pow(0.85, tech.totalCount - 20) : 4 - 0.15 * tech.totalCount).toFixed(2)}x)</em>`;
      },
      maxCount: 1,
      count: 0,
      frequency: 1,
    ...
    },
    {
      name: "dark patterns",
      description:
        "<strong>1.3x</strong> <strong class='color-d'>damage</strong><br><strong>+17%</strong> <strong class='color-junk'>JUNK</strong><strong class='color-m'>tech</strong> chance",
      maxCount: 9,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      name: "junk DNA",
      descriptionFunction() {
        return `increase <strong class='color-d'>damage</strong> by twice the<br><strong class='color-junk'>JUNK</strong><strong class='color-m'>tech</strong> chance <em>(${(1 + 2 * tech.junkChance).toFixed(2)}x)</em>`;
      },
      maxCount: 1,
      count: 0,
      frequency: 1,
      ...
      effect() {
        ...
        let cycle = () => {
          if (
            ...
            powerUps.research.count !== Infinity
          ) {
    ...
    },
    {
      ...
      description:
        "clicking <strong class='color-cancel'>cancel</strong> for a <strong class='color-f'>field</strong>, <strong class='color-m'>tech</strong>, or <strong class='color-g'>gun</strong><br>gives <strong>+5%</strong> power up <strong class='color-dup'>duplication</strong> chance",
      maxCount: 1,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      name: "replication",
      description:
        "<strong>+10%</strong> chance to <strong class='color-dup'>duplicate</strong> spawned <strong>power ups</strong><br><strong>+22%</strong> <strong class='color-junk'>JUNK</strong><strong class='color-m'>tech</strong> chance",
      maxCount: 9,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      name: "metastability",
      description:
        "<strong>+13%</strong> chance to <strong class='color-dup'>duplicate</strong> spawned <strong>power ups</strong><br><strong class='color-dup'>duplicates</strong> <strong class='color-e'>explode</strong> with a <strong>4</strong> second <strong>half-life</strong>",
      maxCount: 1,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      ...
      description:
        "<strong>+8%</strong> chance to <strong class='color-dup'>duplicate</strong> spawned <strong>power ups</strong><br><strong class='color-dup'>duplication</strong> also <strong class='color-dup'>duplicates</strong> <strong>mobs</strong>",
      maxCount: 1,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      ...
      description:
        "<strong>+20%</strong> chance to <strong class='color-dup'>duplicate</strong> spawned <strong>power ups</strong>,<br><strong>collisions</strong> <span class='color-remove'>eject</span> a random <strong class='color-m'>tech</strong>",
      maxCount: 1,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
      ...
      remove() {
        if (this.count > 0 && m.alive) {
          ...
          requestAnimationFrame(() => {
            const options = [];
            for (let i = 0, len = tech.tech.length; i < len; i++)
              if (tech.tech[i].isRemoveBenefit && tech.tech[i].count === 0)
                options.push(i);
    ...
    },
    {
      name: "Born rule",
      description:
        "<span class='color-remove'>eject</span> all your <strong class='color-m'>tech</strong>",
      maxCount: 1,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
...

    {
      name: "Occams razor",
      descriptionFunction() {
        return `randomly <span class='color-remove'>remove</span> <strong>half</strong> your <strong class='color-m'>tech</strong><br>for each removed <strong>${(1 + this.damagePerRemoved).toFixed(2)}x</strong> <strong class='color-d'>damage</strong> <em>(~${(this.count === 0 ? 1 + this.damagePerRemoved * 0.5 * tech.totalCount : this.damage).toFixed(2)}x)</em>`;
      },
      maxCount: 1,
      count: 0,
      frequency: 1,
      ...
      effect() {
        ...
        let removeCount = 0;
        for (let i = 0, len = pool.length * 0.5; i < len; i++)
          removeCount += tech.removeTech(pool[i]);
    ...
    },
    {
      ...
      description:
        "<span class='color-remove'>remove</span> <strong>1</strong> random <strong class='color-m'>tech</strong><br>spawn <strong>2</strong> new <strong class='color-g'>guns</strong>",
      maxCount: 1,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      ...
      description:
        "<span class='color-remove'>remove</span> <strong>1</strong> random <strong class='color-m'>tech</strong><br>spawn <strong>2</strong> <strong class='color-m'>tech</strong>",
      maxCount: 1,
      count: 0,
      frequency: 1,
      frequencyDefault: 1,
    ...
    },
    {
      ...
      allowed() {
        return (
          ((tech.haveGunCheck("nail gun") && tech.isNeedles) ||
            (tech.isNeedles && tech.haveGunCheck("shotgun"))) &&
          !tech.isNeedleIce
        );
      },
    ...
    },
    {
      ...
      allowed() {
        return (
          (!tech.isLargeHarpoon && tech.haveGunCheck("harpoon")) ||
          tech.isNeedles ||
          tech.isHookDefense
        );
      },
    ...
    },
    {
      name: "needle gun",
      description:
        "<strong>nail gun</strong> and <strong>shotgun</strong> fire mob piercing <strong>needles</strong>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
      ...
      allowed() {
        return (
          ((tech.haveGunCheck("nail gun") &&
            !tech.nailInstantFireRate &&
            !tech.nailRecoil &&
            !tech.isRicochet) ||
            (tech.haveGunCheck("shotgun") &&
              !tech.isNailShot &&
              !tech.isFoamShot &&
              !tech.isSporeWorm &&
              !tech.isSporeFlea)) &&
      ...
      requires:
        "nail gun, shotgun, not ice crystal, rivets, rotary cannon, pneumatic, incendiary, nail-shot, foam-shot, worm-shot, ice-shot",
      effect() {
        tech.isNeedles = true;
    ...
    },
    {
      ...
      description:
        "mobs below <strong>50%</strong> durability <strong>die</strong> after you shoot<br>them near their <strong>center</strong> with <strong>needles</strong> or <strong>rivets</strong>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      name: "rivet gun",
      description:
        "<strong>nail gun</strong> and <strong>shotgun</strong> slowly lob a heavy <strong>rivet</strong>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
      ...
      requires:
        "nail gun, shotgun, not ice crystal, needles, or pneumatic actuator",
      effect() {
        tech.isRivets = true;
        for (i = 0, len = b.guns.length; i < len; i++) {
          //find which gun
    ...
    },
    {
      ...
      description:
        "<strong>nail gun</strong> takes <strong>no</strong> time to ramp up<br>to its fastest <em>fire rate</em>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      allowed() {
        return (
          tech.haveGunCheck("nail gun") && !tech.isRivets && !tech.isNeedles
        ); // && !tech.isNailRadiation && !tech.isNailCrit
      },
      ...
      allowed() {
        return (
          tech.hookNails +
            ...
            tech.fragments +
            tech.nailsDeathMob +
            (tech.haveGunCheck("super balls") +
              (tech.haveGunCheck("mine") && !tech.isFoamMine) +
              tech.haveGunCheck("nail gun") +
              tech.isNeedles +
              tech.isNailShot +
              tech.isRivets) *
    ...
    },
    {
      ...
      description:
        "if <strong>nails</strong>, <strong>needles</strong>, or <strong>rivets</strong> strike mobs<br>near their <strong>center</strong> they can <strong class='color-e'>explode</strong>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
      ...
      allowed() {
        return (
          (tech.isNailShot ||
            ...
            tech.haveGunCheck("nail gun") ||
            tech.isRivets ||
            ((tech.isMineDrop || tech.haveGunCheck("mine")) &&
              !(tech.isFoamMine || tech.isSuperMine))) &&
          !tech.isIncendiary &&
          !tech.isCritKill
        );
      },
      requires:
        "nail gun, mine, needles, nails, rivets, not incendiary, stress concentration",
      effect() {
    ...
    },
    {
      ...
      description:
        "<strong>nails</strong>, <strong>needles</strong>, and <strong>rivets</strong> are <strong class='color-p'>radioactive</strong><br><strong>1.9x</strong> <strong class='color-p'>radioactive</strong> <strong class='color-d'>damage</strong> over <strong>3</strong> seconds",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
      ...
      allowed() {
        return (
          ...
          ((tech.isMineDrop || tech.haveGunCheck("mine")) &&
            !(tech.isFoamMine || tech.isSuperMine)) ||
          (tech.haveGunCheck("nail gun") && !tech.isShieldPierce) ||
          (tech.haveGunCheck("shotgun") && (tech.isNeedles || tech.isNailShot))
        );
      },
    ...
    },
    {
      ...
      description:
        "<span style = 'font-size:90%;'><strong>nails</strong>, <strong>needles</strong>, <strong>rivets</strong> are made of <strong class='color-p'>plutonium-238</strong></span><br><strong class='color-p'>radioactive</strong> <strong class='color-d'>damage</strong> lasts <strong>+3</strong>  seconds",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      description:
        "<span style = 'font-size:95%;'><strong>nails</strong>, <strong>needles</strong>, <strong>rivets</strong> are made of <strong class='color-p'>lithium-8</strong><br><strong>4x</strong> <strong class='color-p'>radioactive</strong> <strong class='color-d'>damage</strong> for <strong>1</strong> second</span>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      description:
        "<strong>1.7x</strong> <strong>shotgun</strong> <em>fire rate</em> and <strong>recoil</strong>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      description:
        "<strong>1.5x</strong> <strong>shotgun</strong> <strong class='color-d'>damage</strong><br><strong>shotgun</strong> <strong>recoil</strong> is <strong>reversed</strong>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 1,
    ...
    },
    {
      name: "repeater",
      description:
        "<strong>shotgun</strong> immediately fires again for no <strong class='color-ammo'>ammo</strong><br>reduced <strong>0.5x</strong> <strong>shotgun</strong> <em>fire rate</em>",
      isGunTech: true,
      maxCount: 9,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      description:
        "<strong>shotgun</strong> drives a long clip of <strong>nails</strong>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
      ...
      requires:
        "shotgun, not incendiary, rivets, foam-shot, worm-shot, ice-shot, needles",
      effect() {
    ...
    },
    {
      ...
      description:
        "<strong>shotgun</strong> sprays sticky <strong>foam</strong> bubbles",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
      ...
      requires:
        "shotgun, not incendiary, nail-shot, rivet, worm-shot, ice-shot, needle",
      effect() {
    ...
    },
    {
      ...
      description:
        "<strong>shotgun</strong> grows freezing <strong class='color-s'>ice IX</strong> crystals",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
      ...
      requires:
        "shotgun, not incendiary, nail-shot, rivet, foam-shot, worm-shot",
      effect() {
        tech.isIceShot = true;
      },
      remove() {
    ...
    },
    {
      name: "freezer burn",
      description:
        "mobs <strong class='color-s'>frozen</strong> while below <strong>33%</strong> durability <strong>die</strong>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
      ...
      allowed() {
        return (
          ...
          tech.isIceShot ||
          tech.isNeedleIce ||
          (m.coupling && (m.fieldMode === 3 || m.fieldMode === 0))
        );
      },
    ...
    },
    {
      name: "flash freeze",
      description:
        "mobs <strong class='color-s'>frozen</strong> while above <strong>66%</strong> durability<br>have their durability reduced to <strong>66%</strong>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      name: "crystallizer",
      description:
        "after <strong class='color-s'>frozen</strong> mobs <strong>die</strong> they<br>shatter into <strong class='color-s'>ice IX</strong> crystals",
      isGunTech: true,
      maxCount: 9,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      description:
        "after <strong>killing</strong> mobs with <strong class='color-s'>ice IX</strong><br><strong>+100</strong> <strong class='color-f'>energy</strong>",
      isGunTech: true,
      maxCount: 9,
      count: 0,
      frequency: 2,
    ...
    },
    {
      name: "superfluidity",
      description:
        "<strong class='color-s'>freeze</strong> effects are applied<br>to a small area around the target",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      description:
        "<strong>shotgun</strong>, <strong>rivets</strong>, <strong>super balls</strong>, and <strong>drones</strong><br>are loaded with <strong class='color-e'>explosives</strong>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 1,
      ...
      allowed() {
        return (
          (tech.haveGunCheck("shotgun") &&
            ...
            !tech.isSporeFlea &&
            !tech.isNeedles) ||
          ((tech.haveGunCheck("super balls") || tech.isSuperMine) &&
            !tech.isSuperBounce &&
            !tech.isFoamBall &&
            !tech.isSuperHarm) ||
          (tech.isRivets && !tech.isNailCrit) ||
          (m.fieldMode === 4 && simulation.molecularMode === 3) ||
          (tech.haveGunCheck("drones") &&
            !tech.isForeverDrones &&
            !tech.isDroneRadioactive &&
            !tech.isDroneTeleport)
        );
      },
      requires:
        "shotgun, super balls, rivets, drones, not irradiated drones, burst drones, polyurethane, Zectron",
      effect() {
    ...
    },
    {
      ...
      description:
        "<strong>super balls</strong> and <strong>harpoons</strong> colliding with <strong>mobs</strong><br>catalyzes a reaction that yields <strong>foam</strong> bubbles",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
      ...
      allowed() {
        return (
          ((tech.haveGunCheck("super balls") || tech.isSuperMine) &&
            !tech.isSuperBounce) ||
          (tech.haveGunCheck("harpoon") && !tech.fragments) ||
          tech.isHookDefense
        );
      },
    ...
    },
    {
      name: "autocannon",
      description:
        "fire <strong>+2</strong> <strong>super balls</strong> in a line<br><strong>1.4x</strong> <strong>super ball</strong> velocity and gravity",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      name: "super ball",
      description:
        "fire just <strong>1 large</strong> super <strong>ball</strong><br>that <strong>stuns</strong> mobs for <strong>2</strong> second",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      name: "phase velocity",
      description:
        "wave particles <strong>propagate</strong> faster as <strong>solids</strong><br><strong>1.4x</strong> wave <strong class='color-d'>damage</strong>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      name: "amplitude",
      description:
        "<strong>1.4x</strong> wave <strong class='color-d'>damage</strong><br><strong>1.4x</strong> wave bullet <strong>amplitude</strong>",
      isGunTech: true,
      maxCount: 3,
      count: 0,
      frequency: 2,
    ...
    },
    {
      name: "bound state",
      description:
        "wave packets <strong>reflect</strong> backwards <strong>2</strong> times<br><strong>0.66x</strong> wave <strong>range</strong>",
      isGunTech: true,
      maxCount: 9,
      count: 0,
      frequency: 2,
    ...
    },
    {
      name: "phonon",
      description:
        "waves are low <strong>frequency</strong>, high <strong class='color-d'>damage</strong><br><strong>expanding arcs</strong> that propagate through <strong>solids</strong>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 3,
      ...
      allowed() {
        return (
          tech.haveGunCheck("wave") &&
          !tech.isPhaseVelocity &&
          tech.waveReflections === 1
        );
      },
      ...
      remove() {
        ...
        if (this.count > 0) {
          ...
          b.guns[3].ammoPack *= this.ammoScale;
          if (tech.isInfiniteWaveAmmo) {
            b.guns[3].savedAmmo = Math.ceil(
              b.guns[3].savedAmmo * this.ammoScale,
    ...
    },
    {
      name: "isotropic",
      description:
        "<strong>waves</strong> expand in <strong>all</strong> directions<br><strong>0.6x</strong> <strong>range</strong> and <strong>1.5x</strong> <strong class='color-d'>damage</strong>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 3,
    ...
    },
    {
      ...
      description:
        "after a <strong class='color-block'>block</strong> gets vibrated by a <strong>phonon</strong><br>there is a chance it's <strong>flung</strong> at nearby mobs",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
      ...
      allowed() {
        return (
          (tech.isLongitudinal && tech.haveGunCheck("wave")) ||
          tech.isSoundBotUpgrade
        );
      },
    ...
    },
    {
      ...
      description:
        "after a <strong>mob</strong> gets vibrated by a <strong>phonon</strong><br>a new <strong>resonance wave</strong> expands",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      allowed() {
        return (
          tech.haveGunCheck("missiles", false) &&
    ...
    },
    {
      name: "cruise missile",
      description:
        "<strong>2x</strong> <strong>missile</strong> <strong class='color-e'>explosive</strong> <strong class='color-d'>damage</strong>, radius<br><strong>0.5x</strong> <strong>missile</strong> speed",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
      ...
      allowed() {
        return (
          (tech.haveGunCheck("missiles") && tech.missileFireCD === 45) ||
          (m.fieldMode === 4 && simulation.molecularMode === 1) ||
          tech.missileBotCount
        );
      },
    ...
    },
    {
      name: "ICBM",
      description:
        "<strong>1.75x</strong> <strong>missile</strong> <strong class='color-e'>explosive</strong> <strong class='color-d'>damage</strong>, radius<br><strong>0.5x</strong> <strong>missile</strong> speed",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      name: "iridium-192",
      description:
        "<strong class='color-e'>explosions</strong> release <strong class='color-p'>gamma radiation</strong><br><strong>2x</strong> <strong class='color-e'>explosion</strong> <strong class='color-d'>damage</strong> over <strong>4</strong> seconds",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 1,
      ...
      allowed() {
        return (
          !tech.isImmuneExplosion &&
          tech.explosiveRadius === 1 &&
          !tech.isSmallExplosion &&
          !tech.isBlockExplode &&
          !tech.fragments &&
          (tech.haveGunCheck("missiles") ||
            tech.missileBotCount ||
      ...
      requires:
        "an explosive damage source, not ammonium nitrate, nitroglycerin, chain reaction, fragmentation, electric armor",
      effect() {
    ...
    },
    {
      name: "fragmentation",
      description:
        "some <strong class='color-e'>detonations</strong> and collisions eject <strong>nails</strong><br><em style = 'font-size: 90%'>blocks, grenades, missiles, rivets, harpoon</em>",
      isGunTech: true,
      maxCount: 9,
      count: 0,
      frequency: 1,
      ...
      allowed() {
        return (
          !tech.isExplodeRadio &&
          ((tech.haveGunCheck("harpoon") && !tech.isFoamBall) ||
            (tech.haveGunCheck("grenades") && !tech.isNeutronBomb) ||
            tech.haveGunCheck("missiles") ||
            (m.fieldMode === 4 && simulation.molecularMode === 1) ||
            tech.missileBotCount ||
      ...
      requires:
        "grenades, missiles, rivets, harpoon, or mass driver, not iridium-192, not polyurethane foam",
      effect() {
        tech.fragments++;
      },
      remove() {
    ...
    },
    {
      ...
      description:
        "<strong>1.25x</strong> <strong class='color-e'>explosive</strong> <strong class='color-d'>damage</strong>, radius",
      isGunTech: true,
      maxCount: 9,
      count: 0,
      frequency: 2,
    ...
    },
    {
      name: "nitroglycerin",
      description:
        "<strong>1.66x</strong> <strong class='color-e'>explosive</strong> <strong class='color-d'>damage</strong><br><strong>0.66x</strong> smaller <strong class='color-e'>explosive</strong> <strong>radius</strong>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
      ...
      allowed() {
        return (
          !tech.isExplodeRadio &&
          tech.hasExplosiveDamageCheck() &&
          !tech.isExplosionHarm
        );
      },
    ...
    },
    {
      ...
      description:
        "<strong>1.7x</strong> <strong class='color-e'>explosive</strong> <strong>radius</strong><br><strong>1.4x</strong> <strong class='color-e'>explosive</strong> <strong class='color-defense'>damage taken</strong>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      allowed() {
        return (
          tech.haveGunCheck("spores") ||
          tech.haveGunCheck("mine") ||
          (!tech.isExplodeRadio && tech.hasExplosiveDamageCheck())
        );
      },
    ...
    },
    {
      name: "MIRV",
      description:
        "fire <strong>+1</strong> <strong>missile</strong> or <strong>grenade</strong> per shot<br><strong>0.88x</strong> <strong class='color-e'>explosion</strong> <strong class='color-d'>damage</strong> and <strong>radius</strong>",
      isGunTech: true,
      maxCount: 9,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      description:
        "<strong>grenades</strong> <strong class='color-e'>explode</strong> on map <strong>collisions</strong><br><strong class='color-e'>explosions</strong> drain <strong class='color-f'>energy</strong>, not <strong class='color-h'>health</strong>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      name: "vacuum bomb",
      description:
        "<strong>grenades</strong> fire slower, <strong class='color-e'>explode</strong> bigger,<br>and <strong>suck</strong> everything towards them",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      name: "chain reaction",
      description:
        "<strong>1.33x</strong> <strong>grenade</strong> radius and <strong class='color-d'>damage</strong><br><strong class='color-block'>blocks</strong> caught in <strong class='color-e'>explosions</strong> also <strong class='color-e'>explode</strong>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      name: "flame test",
      description:
        "after <strong>grenades</strong> detonate they release<br>a colorful <strong>cluster</strong> of small <strong class='color-e'>explosions</strong>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
      ...
      allowed() {
        return (
          tech.haveGunCheck("grenades") &&
          !tech.isNeutronBomb &&
          !tech.isCircleExplode &&
          !tech.isPetalsExplode
        );
      },
    ...
    },
    {
      name: "pyrotechnics",
      description:
        "after <strong>grenades</strong> detonate they release<br>a colorful <strong>circle</strong> of <strong class='color-e'>explosions</strong>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      name: "fireworks",
      description:
        "after <strong>grenades</strong> detonate they release<br>colorful <strong>petals</strong> of <strong class='color-e'>explosions</strong>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      name: "neutron bomb",
      description:
        "<strong>grenades</strong> are <strong class='color-p'>irradiated</strong> with <strong class='color-p'>Cf-252</strong><br>does <strong class='color-p'>radioactive</strong> <strong class='color-d'>damage</strong> over time",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
      ...
      requires:
        "grenades, not fragmentation, vacuum bomb, iridium-192, pyrotechnics, fireworks, flame test, chain reaction",
      effect() {
    ...
    },
    {
      ...
      description:
        "<strong>1.2x</strong> <strong class='color-p'>radioactive</strong> range<br>objects in range of the bomb are <strong>slowed</strong>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      description:
        "after a mob or shield <strong>dies</strong>,<br>leftover <strong class='color-p'>radiation</strong> <strong>spreads</strong> to a nearby mob",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      description:
        "<strong>1.5x</strong> <strong class='color-p'>radiation</strong> <strong class='color-d'>damage</strong><br><em style = 'font-size:93%;'>nail, drone, neutron bomb, iridium, cosmic string, deflect</em>",
      isGunTech: true,
      maxCount: 9,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      description:
        "reduce <strong class='color-p'>radioactive</strong> effects on you by <strong>0.8x</strong><br><em>neutron bomb, drones, explosions, slime</em>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
      ...
      allowed() {
        return (
          tech.isNeutronBomb || tech.isDroneRadioactive || tech.isExplodeRadio
        );
      },
    ...
    },
    {
      name: "ricochet",
      description:
        "after <strong>nails</strong> hit a mob they <strong>rebound</strong> towards<br>a new mob with <strong>2.8x</strong> <strong class='color-d'>damage</strong> per bounce",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
      ...
      allowed() {
        ...
        return (
          ...
          tech.nailsDeathMob ||
          (tech.haveGunCheck("mine") &&
            !(tech.isLaserMine || tech.isFoamMine || tech.isSuperMine)) ||
          (tech.haveGunCheck("nail gun") &&
            !tech.isRivets &&
            !tech.isNeedles) ||
          (tech.haveGunCheck("shotgun") &&
            (tech.isNeedles || tech.isNailShot) &&
            !tech.isRivets &&
            !tech.isNeedles)
        );
      },
    ...
    },
    {
      name: "booby trap",
      description:
        "<strong>50%</strong> chance to drop a <strong>mine</strong> from <strong>power ups</strong><br><strong>+30%</strong> <strong class='color-junk'>JUNK</strong><strong class='color-m'>tech</strong> chance",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      description:
        "instead of nails <strong>mines</strong> catalyze a reaction<br>that yields <strong>foam</strong> bubbles",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
      ...
      requires:
        "mines, not blast ball, ricochet, irradiated nails, supercritical fission",
      effect() {
    ...
    },
    {
      ...
      requires:
        "mines, not elephants toothpaste, ricochet, irradiated nails, supercritical fission",
      effect() {
    ...
    },
    {
      ...
      description:
        "<strong>mines</strong> laid while you are <strong>crouched</strong><br>use <strong class='color-f'>energy</strong> to emit <strong>3</strong> unaimed <strong class='color-laser'>lasers</strong>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      description:
        "during their <strong>growth</strong> phase<br><strong>1.7x</strong> <strong class='color-p' style='letter-spacing: 2px;'>sporangium</strong> discharge",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      // },
      description:
        "<strong class='color-p' style='letter-spacing: 2px;'>sporangium</strong> <strong>infect</strong> mobs they attach to<br><strong>infected</strong> mobs <strong>resurrect</strong> and attack other mobs",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      name: "colony",
      description:
        "<strong>1.6x</strong> <strong class='color-p' style='letter-spacing: 2px;'>sporangium</strong> discharge<br><strong>33%</strong> chance to discharge something different",
    ...
    },
    {
      ...
      allowed() {
        return (
          tech.haveGunCheck("spores") ||
          tech.sporesOnDeath > 0 ||
          (m.fieldMode === 4 && simulation.molecularMode === 0) ||
          tech.isSporeWorm ||
          tech.isSporeFlea
        );
      },
    ...
    },
    {
      name: "necrophage",
      description:
        "if <strong>foam</strong>, <strong class='color-p' style='letter-spacing: -0.8px;'>fleas</strong>, or <strong class='color-p' style='letter-spacing: -0.8px;'>worms</strong> <strong>kill</strong> their target<br>they grow 3 <strong>copies</strong>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
      ...
      allowed() {
        return (
          tech.haveGunCheck("foam") ||
          tech.isFoamBall ||
          tech.isFoamBotUpgrade ||
          tech.isFoamShot ||
          tech.isSporeWorm ||
          tech.isSporeFlea ||
          tech.isFoamMine
    ...
    },
    {
      name: "siphonaptera",
      description:
        "<strong class='color-p' style='letter-spacing: 2px;'>spores</strong> metamorphose into <strong class='color-p' style='letter-spacing: -0.8px;'>fleas</strong><br><strong>shotgun</strong> fires <strong class='color-p' style='letter-spacing: -0.8px;'>fleas</strong>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 3,
    ...
    },
    {
      name: "nematodes",
      description:
        "<strong class='color-p' style='letter-spacing: 2px;'>spores</strong> metamorphose into <strong class='color-p' style='letter-spacing: -0.8px;'>worms</strong><br><strong>shotgun</strong> fires <strong class='color-p' style='letter-spacing: -0.8px;'>worms</strong>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 3,
    ...
    },
    {
      name: "K-selection",
      description:
        "<strong>1.37x</strong> <strong class='color-p' style='letter-spacing: -0.8px;'>worm</strong> and <strong class='color-p' style='letter-spacing: -0.8px;'>flea</strong> <strong class='color-d'>damage</strong>",
      isGunTech: true,
      maxCount: 3,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      allowed() {
        return (
          (tech.isSporeFollow &&
            (tech.haveGunCheck("spores") ||
              (tech.haveGunCheck("shotgun") && tech.isSporeWorm))) ||
          tech.haveGunCheck("drones") ||
          (m.fieldMode === 4 &&
            (simulation.molecularMode === 0 || simulation.molecularMode === 3))
        );
      },
    ...
    },
    {
      ...
      allowed() {
        return (
          tech.haveGunCheck("drones", false) &&
          !tech.isDroneRespawn &&
          tech.bulletsLastLonger === 1 &&
          !tech.isDronesTravel &&
          (build.isExperimentSelection || powerUps.research.count > 1)
    ...
    },
    {
      name: "delivery drone",
      description:
        "if a <strong>drone</strong> picks up a <strong>power up</strong>,<br>it becomes <strong>larger</strong>, <strong>faster</strong>, and more <strong>durable</strong>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      description:
        "after a <strong>drone</strong> expires it will use <strong>-5</strong> <strong class='color-f'>energy</strong><br>and a nearby <strong class='color-block'>block</strong> to <strong>replicate</strong> itself",
    ...
    },
    {
      ...
      description:
        "<strong>drones</strong> rapidly <strong>rush</strong> towards their target<br><strong>1.33x</strong> <strong>drone</strong> collision <strong class='color-d'>damage</strong>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 3,
      ...
      requires:
        "drones, molecular assembler, not irradiated drones, incendiary",
      effect() {
    ...
    },
    {
      ...
      description:
        "<strong>1.66x</strong> <strong>drones</strong> <strong>rush</strong> frequency<br><strong>1.44x</strong> <strong>drone</strong> collision <strong class='color-d'>damage</strong>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      description:
        "<strong>0.5x</strong> <strong>drone</strong> duration<br><strong>2x</strong> <strong>drone</strong> <strong class='color-p'>radiation</strong> <strong class='color-d'>damage</strong>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      description:
        "<strong>1.66x</strong> <strong>drone</strong> acceleration<br><strong>1.33x</strong> <strong class='color-p'>radiation</strong> <strong class='color-d'>damage</strong>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      description:
        "<strong>foam</strong> bubbles are electrically charged<br>causing <strong>attraction</strong> to nearby <strong>mobs</strong>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 1,
    ...
    },
    {
      ...
      description:
        "<strong>foam</strong>, <strong>wave</strong>, and <strong>super ball</strong> positions are erratic<br><strong>1.5x</strong> <strong>foam</strong>, <strong>wave</strong>, and <strong>super ball</strong> <strong class='color-d'>damage</strong>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 1,
      ...
      allowed() {
        return (
          ...
          tech.isFoamBall ||
          tech.isFoamMine ||
          (tech.haveGunCheck("wave") && !tech.is360Longitudinal) ||
          (tech.haveGunCheck("super balls") && !tech.isSuperHarm) ||
          tech.isSoundBotUpgrade
        );
      },
    ...
    },
    {
      name: "aerogel",
      description:
        "<strong>foam</strong> bubbles <strong>float</strong> with <strong>0.5x</strong> <strong>foam</strong> duration<br><strong>2.8x</strong> <strong>foam</strong> <strong class='color-d'>damage</strong>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      description:
        "<strong>1.4x</strong> <strong>foam</strong> <strong class='color-d'>damage</strong>",
      isGunTech: true,
      maxCount: 9,
      count: 0,
      frequency: 2,
    ...
    },
    {
      name: "cavitation",
      description:
        "<strong>25%</strong> chance to discharge a huge <strong>foam</strong> bubble<br><strong>2x</strong> <strong>foam</strong> gun <strong>recoil</strong>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      description:
        "if you have below <strong>300</strong> <strong class='color-ammo'>ammo</strong><br><strong>2x</strong> <strong>foam</strong> <strong class='color-g'>gun</strong> bubble <strong>size</strong>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      description:
        "build up <strong>charge</strong> while firing <strong>foam</strong> <strong class='color-g'>gun</strong><br>after firing <strong>discharge</strong> <strong>foam</strong> bubbles",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      allowed() {
        return (
          tech.blockDamage > 0.075 ||
          tech.isRailGun ||
          (tech.haveGunCheck("foam") && tech.isFoamPressure) ||
          tech.isTokamak ||
          tech.isPulseLaser ||
          tech.isPlasmaBall
        );
    ...
    },
    {
      ...
      allowed() {
        return (
          tech.haveGunCheck("harpoon") &&
          !tech.isFilament &&
          !tech.isHarpoonPowerUp &&
          !tech.isBoostReplaceAmmo
        );
      },
    ...
    },
    {
      name: "alternator",
      description:
        "<strong>0.05x</strong> <strong>harpoon</strong> <strong class='color-f'>energy</strong> cost",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      description:
        "if you <strong>collide</strong> with a <strong>mob</strong><br>fire <strong>harpoons</strong> at nearby <strong>mobs</strong>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      allowed() {
        return (
          tech.haveGunCheck("harpoon") && b.guns[9].ammo >= this.removeAmmo()
        );
      },
      requires: "harpoon",
      effect() {
    ...
    },
    {
      ...
      description:
        "after using <strong>harpoon</strong>/<strong>grapple</strong> to collect <strong>power ups</strong><br><strong>1.8x</strong> <strong>harpoon</strong> or <strong>grapple</strong> <strong class='color-d'>damage</strong> for <strong>8</strong> seconds",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
      ...
      allowed() {
        return (
          ((tech.haveGunCheck("harpoon") && !tech.isRailGun) ||
            m.fieldMode === 10) &&
          !tech.isHarpoonFullHealth
        );
      },
    ...
    },
    {
      name: "brittle",
      description:
        "<strong>2.2x</strong> <strong>harpoon</strong>/<strong>grapple</strong> <strong class='color-d'>damage</strong><br>to <strong>mobs</strong> at maximum <strong>durability</strong>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
      ...
      allowed() {
        return (
          (tech.haveGunCheck("harpoon") || m.fieldMode === 10) &&
          !tech.isHarpoonPowerUp
        );
      },
    ...
    },
    {
      ...
      allowed() {
        return (
          ((tech.haveGunCheck("wave") && tech.isInfiniteWaveAmmo) ||
            tech.haveGunCheck("laser") ||
            (tech.haveGunCheck("harpoon") && !tech.isRailGun)) &&
          !tech.isEnergyNoAmmo
        );
      },
      ...
      effect() {
        ...
        for (let i = powerUp.length - 1; i > -1; i--) {
          if (powerUp[i].name === "ammo") {
            powerUps.spawn(
              powerUp[i].position.x + 50 * (Math.random() - 0.5),
              powerUp[i].position.y + 50 * (Math.random() - 0.5),
              "boost",
            );
    ...
    },
    {
      ...
      description:
        "gain <strong>3</strong> random <strong class='color-laser'>laser</strong> <strong class='color-g'>gun</strong><strong class='color-m'>tech</strong><br><strong class='color-laser'>laser</strong> only turns <strong>off</strong> if you have no <strong class='color-f'>energy</strong>",
      // isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
        ...
        requestAnimationFrame(() => {
          ...
          for (let j = 0; j < 3; j++) {
            ...
            for (let i = options.length - 1; i > -1; i--) {
              const index = options[i];
              if (
                !(tech.tech[index].count < tech.tech[index].maxCount) ||
            ...
            if (options.length) {
              const index = options[Math.floor(Math.random() * options.length)];
              simulation.makeTextLog(
                `<span class='color-var'>tech</span>.giveTech("<span class='color-text'>${tech.tech[index].name}</span>") <em>//optical amplifier</em>`,
                360,
              );
              tech.giveTech(index);
              techGiven++;
          ...
          if (techGiven > 0) {
            ...
            //eject if none found
            simulation.makeTextLog(
              `0 <span class='color-var'>tech</span> found <em>//optical amplifier</em>`,
            );
            const loop = () => {
    ...
    },
    {
      ...
      description:
        "<strong class='color-laser'>lasers</strong> push <strong>mobs</strong> and <strong class='color-block'>blocks</strong>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
      ...
      allowed() {
        return (
          (tech.haveGunCheck("laser") && !tech.isPulseLaser) ||
          tech.isLaserBotUpgrade ||
          tech.isLaserField
        );
      },
    ...
    },
    {
      name: "iridescence",
      description:
        "if <strong class='color-laser'>laser</strong> beams hit mobs near their <strong>center</strong><br><strong>2x</strong> <strong class='color-laser'>laser</strong> <strong class='color-d'>damage</strong>",
      isGunTech: true,
      maxCount: 9,
      count: 0,
      frequency: 2,
    ...
    },
    {
      name: "lens",
      description:
        "<strong>2.5x</strong> <strong class='color-laser'>laser</strong> <strong class='color-g'>gun</strong> <strong class='color-d'>damage</strong> if it passes<br>through a revolving <strong>90°</strong> arc circular lens", //<span style='font-size: 125%;'>π</span> / 2</strong>
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      name: "compound lens",
      description:
        "<strong>1.4x</strong> <strong class='color-laser'>laser</strong> lens <strong class='color-d'>damage</strong><br><strong>+25°</strong> lens arc",
      isGunTech: true,
      maxCount: 9,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      description:
        "<strong>+2</strong> <strong class='color-laser'>laser</strong> beam reflections",
      isGunTech: true,
      maxCount: 3,
      count: 0,
      frequency: 1,
    ...
    },
    {
      ...
      allowed() {
        return (
          tech.haveGunCheck("laser") && !tech.isWideLaser && !tech.historyLaser
        );
      },
    ...
    },
    {
      ...
      description:
        "<strong class='color-laser'>laser</strong> <strong class='color-g'>gun</strong> beam is <strong>wider</strong> and doesn't <strong>reflect</strong><br><strong>3.2x</strong> <strong class='color-laser'>laser</strong> <strong class='color-d'>damage</strong>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
      ...
      requires:
        "laser gun, not specular reflection, diffraction grating, slow light, pulse",
      effect() {
    ...
    },
    {
      name: "output coupler",
      description:
        "<strong>1.3x</strong> <strong class='color-laser'>laser</strong> <strong class='color-g'>gun</strong> beam <strong>width</strong><br><strong>1.3x</strong> <strong class='color-laser'>laser</strong> <strong class='color-d'>damage</strong>",
      isGunTech: true,
      maxCount: 9,
      count: 0,
      frequency: 2,
    ...
    },
    {
      name: "slow light",
      description:
        "<strong class='color-laser'>laser</strong> <strong class='color-g'>gun</strong> beam is <strong>spread</strong> into your recent <strong>past</strong><br><strong>4x</strong> full beam <strong class='color-d'>damage</strong>",
      isGunTech: true,
      maxCount: 9,
      count: 0,
      frequency: 1,
      ...
      requires:
        "laser gun, not specular reflection, diffraction grating, diffuse beam",
      effect() {
    ...
    },
    {
      name: "infrared diode",
      description:
        "<strong>0.4x</strong> <strong class='color-laser'>laser</strong> <strong class='color-f'>energy</strong> cost<br><em>infrared light is outside visual perception</em>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 1,
    ...
    },
    {
      name: "dye laser",
      description:
        "<strong>0.75x</strong> <strong class='color-laser'>laser</strong> <strong class='color-f'>energy</strong> cost<br><strong>1.25x</strong> <strong class='color-laser'>laser</strong> <strong class='color-d'>damage</strong>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 1,
    ...
    },
    {
      ...
      description:
        "<strong>3x</strong> <strong class='color-laser'>laser</strong> <strong class='color-f'>energy</strong> cost<br><strong>3x</strong> <strong class='color-laser'>laser</strong> <strong class='color-d'>damage</strong>",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 1,
    ...
    },
    {
      name: "pulse",
      description:
        "charge your <strong class='color-f'>energy</strong> and release it as a<br><strong class='color-laser'>laser</strong> pulse that initiates an <strong class='color-e'>explosion</strong> cluster",
      isGunTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    //**************************************************
    {
      ...
      description:
        "<strong>1.5x</strong> <strong>standing wave</strong> deflection <strong class='color-f'>energy</strong> efficiency<br>shield deflection radius holds it's max range", //<strong>standing wave</strong> oscillates in a 3rd dimension<br>
      isFieldTech: true,
      maxCount: 9,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      description:
        "if <strong>deflecting</strong> drains all your <strong class='color-f'>energy</strong><br>emit <strong class='color-laser'>laser</strong> beams that scale with max <strong class='color-f'>energy</strong>",
      isFieldTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      allowed() {
        return (
          (m.fieldMode === 1 || m.fieldMode === 8 || m.fieldMode === 6) &&
          (build.isExperimentSelection || powerUps.research.count > 1)
        );
      },
    ...
    },
    {
      name: "expansion",
      description:
        "using <strong>standing wave</strong> field <strong>expands</strong> its <strong>radius</strong><br><strong>+77</strong> maximum <strong class='color-f'>energy</strong>",
      isFieldTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      descriptionFunction() {
        return `<strong>1.0023x</strong> <strong class='color-d'>damage</strong> per <strong class='color-f'>energy</strong><br><em>(${(1 + 0.23 * m.maxEnergy).toFixed(2)}x damage at max energy)</em>`;
      },
    ...
    },
    {
      name: "bremsstrahlung",
      description:
        "<strong>deflecting</strong> and thrown <strong class='color-block'>blocks</strong><br>do braking <strong class='color-d'>damage</strong> to mobs",
      isFieldTech: true,
      maxCount: 9,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      description:
        "bremsstrahlung's effects are <strong class='color-p'>radioactive</strong><br><strong>3.5x</strong> <strong class='color-d'>damage</strong> over <strong>3</strong> seconds",
      isFieldTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
      ...
      allowed() {
        return (
          (m.fieldMode === 1 || m.fieldMode === 2 || m.fieldMode === 8) &&
          tech.blockDmg
        );
      },
    ...
    },
    {
      name: "flux pinning",
      description:
        "after <strong>deflecting</strong> a mob<br>it is <strong>stunned</strong> for up to <strong>4</strong> seconds",
      isFieldTech: true,
      maxCount: 9,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      description:
        "<strong>1.55x</strong> perfect diamagnetism <strong>radius</strong><br><strong>+22°</strong> perfect diamagnetism circular <strong>arc</strong>",
      isFieldTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      descriptionFunction() {
        return `increase <strong class='color-d'>damage</strong> by your last ${tech.isEnergyHealth ? "<strong class='color-f'>energy</strong>" : "<strong class='color-h'>health</strong>"} loss<br><em>(${(1 + tech.lastHitDamage * m.lastHit).toFixed(2)}x damage)</em>`;
      },
      isFieldTech: true,
      maxCount: 9,
      count: 0,
    ...
    },
    {
      name: "annihilation",
      description:
        "after <strong>colliding</strong> with non-boss mobs<br>they are <strong>annihilated</strong> and <strong>–10</strong> <strong class='color-f'>energy</strong>",
      isFieldTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      name: "inertial mass",
      description:
        "<strong>negative mass</strong> is larger and <strong>faster</strong>", //<br><strong class='color-block'>blocks</strong> also move <strong>horizontally</strong> with the field
      isFieldTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    // },
    {
      ...
      description:
        "hold <strong>crouch</strong> and use your <strong class='color-f'>field</strong> to <strong>print</strong> a <strong class='color-block'>block</strong><br> with <strong>1.8x</strong> density, <strong class='color-d'>damage</strong>, and launch speed",
    ...
    },
    {
      ...
      description:
        "after picking up a <strong>power up</strong><br><strong>+200</strong> <strong class='color-f'>energy</strong>",
      isFieldTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      description:
        "after <strong>deflecting</strong> mobs<br><strong>molecular assembler</strong> generates <strong>+50</strong> <strong class='color-f'>energy</strong>",
      isFieldTech: true,
      maxCount: 9,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      description:
        "<strong>1.35x</strong> <strong class='color-d'>damage</strong><br><strong>0.7x</strong> <em>fire rate</em>",
      isFieldTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      name: "tokamak",
      description:
        "throwing a <strong class='color-block'>block</strong> converts it into <strong class='color-f'>energy</strong><br>and a pulsed fusion <strong class='color-e'>explosion</strong>",
      isFieldTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
      ...
      allowed() {
        return (
          (m.fieldMode === 5 || m.fieldMode === 4 || m.fieldMode === 10) &&
          !tech.isPrinter &&
          !tech.isReel &&
          !tech.hookNails
        );
      ...
      requires:
        "plasma torch, molecular assembler, grappling hook, not printer, reel, swarf",
      effect() {
        tech.isTokamak = true;
      },
      remove() {
    ...
    },
    {
      ...
      description:
        "if your <strong class='color-f'>field</strong> is active<br><strong>0.1x</strong> <strong class='color-defense'>damage taken</strong>",
      isFieldTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      effect() {
        ...
        if (build.isExperimentSelection) {
          document
            .getElementById("field-" + m.fieldMode)
            .classList.remove("build-field-selected");
          document
            .getElementById("field-0")
            .classList.add("build-field-selected");
        }
    ...
    },
    {
      ...
      allowed() {
        return (
          (tech.plasmaBotCount || m.fieldMode === 5) &&
          (build.isExperimentSelection || powerUps.research.count > 1) &&
          !tech.isPlasmaBall
        );
      },
    ...
    },
    {
      name: "extruder",
      description:
        "<strong>extrude</strong> a thin hot wire of <strong class='color-plasma'>plasma</strong><br>increases <strong class='color-d'>damage</strong> and <strong class='color-f'>energy</strong> cost",
      isFieldTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      description:
        "<strong class='color-plasma'>extrude</strong> metals at a higher <strong class='color-plasma'>temperature</strong><br>increases effective <strong>radius</strong> and <strong class='color-d'>damage</strong>",
      isFieldTech: true,
      maxCount: 9,
      count: 0,
      frequency: 2,
    ...
    },
    {
      name: "plasma ball",
      description:
        "<strong>grow</strong> an expanding <strong>ball</strong> of <strong class='color-plasma'>plasma</strong><br>increases <strong class='color-d'>damage</strong> and <strong class='color-f'>energy</strong> cost",
      isFieldTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
      ...
      allowed() {
        return (
          m.fieldMode === 5 && !tech.isExtruder && tech.isPlasmaRange === 1
        );
      },
    ...
    },
    {
      ...
      description:
        "increase the <strong>range</strong> and <strong>frequency</strong><br>of <strong class='color-plasma'>plasma</strong> ball's <strong>electric arc</strong> ",
      isFieldTech: true,
      maxCount: 9,
      count: 0,
      frequency: 2,
    ...
    },
    {
      name: "retrocausality",
      description:
        "<strong>time dilation</strong> uses <strong class='color-f'>energy</strong> to <strong>rewind</strong> your<br><strong class='color-h'>health</strong>, <strong>velocity</strong>, and <strong>position</strong> up to <strong>10</strong> seconds",
      isFieldTech: true,
      maxCount: 1,
      count: 0,
      frequency: 1,
    ...
    },
    {
      ...
      description:
        "when not <strong>moving</strong> time dilation <strong style='letter-spacing: 2px;'>stops time</strong><br><strong>0.6x</strong> <strong class='color-defense'>damage taken</strong>",
      isFieldTech: true,
      maxCount: 1,
      count: 0,
      frequency: 1,
    ...
    },
    {
      name: "time crystals",
      descriptionFunction() {
        return `<strong>2.5x</strong> passive <strong class='color-f'>energy</strong> generation<br><em>(+${(150 * m.fieldRegen * 60).toFixed(1)} energy per second)</em>`;
      },
      isFieldTech: true,
      maxCount: 1,
      count: 0,
      ...
      effect() {
        ...
        this.descriptionFunction = function () {
          return `<strong>2.5x</strong> passive <strong class='color-f'>energy</strong> generation<br><em>(+${(60 * m.fieldRegen * 60).toFixed(1)} energy per second)</em>`;
        };
      },
      remove() {
    ...
    },
    {
      ...
      description:
        "while <strong class='color-cloaked'>cloaked</strong> you are <strong>intangible</strong><br>to <strong class='color-block'>blocks</strong> and mobs, but <strong>mobs</strong> drain <strong class='color-f'>energy</strong>",
      isFieldTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
      ...
      remove() {
        if (tech.isIntangible) {
          ...
          player.collisionFilter.mask =
            cat.body | cat.map | cat.mob | cat.mobBullet | cat.mobShield; //normal collisions
        }
      },
    },
    {
      ...
      description:
        "after <strong class='color-cloaked'>cloaking</strong> recover <strong>0.75x</strong><br>of your last <strong class='color-h'>health</strong> lost",
      isFieldTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      description:
        "after <strong class='color-cloaked'>decloaking</strong><br><strong>stun</strong> nearby mobs for 2 second",
      isFieldTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      description:
        "<strong>2.1x</strong> <strong class='color-d'>damage</strong><br>to <strong>mobs</strong> at maximum <strong>durability</strong>",
      isFieldTech: true,
      maxCount: 1,
      count: 0,
      frequency: 1,
    ...
    },
    {
      ...
      effect() {
        ...
        for (let j = 0, len = 4; j < len; j++)
          powerUps.spawn(
            level.exit.x + 100 * (Math.random() - 0.5),
            level.exit.y - 100 + 100 * (Math.random() - 0.5),
            "research",
            false,
          );
      },
    ...
    },
    {
      ...
      allowed() {
        return (
          (m.fieldMode === 8 || m.fieldMode === 6 || m.fieldMode === 9) &&
          (build.isExperimentSelection || powerUps.research.count > 2)
        );
      },
    ...
    },
    {
      ...
      description:
        "after a <strong class='color-block'>block</strong> falls into a <strong class='color-worm'>wormhole</strong><br>spawn a <strong class='color-p' style='letter-spacing: 2px;'>worm</strong>",
      isFieldTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      name: "cosmic string",
      description:
        "after <strong>tunneling</strong> through mobs with a <strong class='color-worm'>wormhole</strong><br><strong>stun</strong> them and do <strong class='color-p'>radioactive</strong> <strong class='color-d'>damage</strong>",
      isFieldTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      name: "invariant",
      description:
        "while placing your <strong class='color-worm'>wormhole</strong><br>use <strong class='color-f'>energy</strong> to <strong>pause</strong> time",
      isFieldTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      description:
        "<strong class='color-worm'>wormholes</strong> can tunnel through <strong>anything</strong><br>for <strong>2x</strong> <strong class='color-f'>energy</strong> cost",
      isFieldTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      name: "CIWS",
      description:
        "<strong>grappling hook</strong> uses <strong>10</strong> <strong class='color-f'>energy</strong><br> to fire <strong>harpoons</strong> at nearby mobs",
      isFieldTech: true,
      maxCount: 1,
      count: 0,
      frequency: 2,
    ...
    },
    {
      ...
      description:
        "after <strong>grappling hook</strong> impacts something<br>eject <strong>nails</strong> splinters towards nearby mobs",
      isFieldTech: true,
      maxCount: 3,
      count: 0,
      frequency: 2,
    ...
    },
    {
      name: "reel",
      description:
        "<strong>5x</strong> <strong class='color-block'>block</strong> collision <strong class='color-d'>damage</strong><br>up to <strong>+100</strong> <strong class='color-f'>energy</strong> after reeling in <strong class='color-block'>blocks</strong>",
      isFieldTech: true,
      maxCount: 1,
      count: 0,
      frequency: 1,
    ...
    // },
    {
      name: "swap meet",
      description:
        "normal <strong class='color-m'>tech</strong> become <strong class='color-junk'>JUNK</strong><br>and <strong class='color-junk'>JUNK</strong> become normal <strong class='color-m'>tech</strong>",
      maxCount: 1,
      count: 0,
      frequency: 0,
      isJunk: true,
      ...
      descriptionFunction() {
        ...
        const loop = () => {
          if (
            ...
            this.damage = 1 + dmg;
            if (document.getElementById(`damage-JUNK-id${this.id}`))
              document.getElementById(`damage-JUNK-id${this.id}`).innerHTML =
                this.text;
            setTimeout(() => {
              loop();
            }, delay);
      ...
      descriptionFunction() {
        ...
        const loop = () => {
          if (
            ...
            //&& (!simulation.isChoosing || this.count === 0) //simulation.paused ||
            count += 4.5;
            const waves =
              2 * Math.sin(count * 0.0133) +
              Math.sin(count * 0.013) +
              0.5 * Math.sin(count * 0.031) +
              0.33 * Math.sin(count * 0.03);
            ...
            if (document.getElementById(`boost-JUNK-id${this.id}`))
              document.getElementById(`boost-JUNK-id${this.id}`).innerHTML =
                this.text;
            setTimeout(() => {
              loop();
            }, this.delay);
    ...
    },
    {
      name: "placebo",
      description:
        "<strong>7.77x</strong> <strong class='color-d'>damage</strong>",
      maxCount: 1,
      count: 0,
      frequency: 0,
      isJunk: true,
    ...
    },
    {
      ...
      description:
        "build <strong>100</strong> scrap <strong class='color-bot'>bots</strong><br>bots might last for <strong>30</strong> seconds",
      maxCount: 1,
      count: 0,
      frequency: 0,
      isJunk: true,
      ...
      effect() {
        for (let i = 0; i < 100; i++) {
          ...
          bullet[bullet.length - 1].endCycle =
            simulation.cycle + 800 + 1000 * Math.random(); //15 seconds
        }
      },
      remove() {},
    },
    ...
    },
    {
      name: "panpsychism",
      description:
        "awaken all <strong class='color-block'>blocks</strong><br><strong class='color-block'>blocks</strong> have a chance to spawn power ups",
      maxCount: 1,
      count: 0,
      frequency: 0,
      isJunk: true,
        ...
        setInterval(() => {
          for (let i = body.length - 1; i > -1; i--) {
            if (!body[i].isNotHoldable) {
              ...
              if (!body[i].isAboutToBeRemoved)
                mob[mob.length - 1].isDropPowerUp = true;
              body.splice(i, 1);
            }
          }
        }, 6000);
    ...
    },
    {
      ...
      effect() {
        setInterval(() => {
          ...
          fireBlock(
            player.position.x + 600 * (Math.random() - 0.5),
            player.position.y - 500 - 500 * Math.random(),
          );
    ...
    },
    {
      ...
      description:
        "<strong>10x</strong> current <strong class='color-m'>tech</strong> <em class='flicker'>frequency</em>",
      maxCount: 1,
      count: 0,
      frequency: 1,
      isJunk: true,
      ...
      remove() {
        if (this.count) {
          for (let i = 0, len = tech.tech.length; i < len; i++) {
            if (tech.tech[i].count > 0 && tech.tech[i].frequency > 1)
              tech.tech[i].frequency /= 10;
          }
        }
      },
    },
    {
      ...
      effect() {
        ...
        setInterval(() => {
          if (
            powerUps.boost.endCycle < m.cycle &&
            !simulation.paused &&
            m.alive
          ) {
            for (let i = 0; i < mob.length; i++) {
              if (mob[i].distanceToPlayer2() < 400000) {
                //650
    ...
    },
    {
      ...
      description:
        "spawn 5 <strong class='color-f'>field</strong> power ups, but every 12 seconds<br>teleport a second into your future or past",
      maxCount: 1,
      count: 0,
      frequency: 0,
      isJunk: true,
        ...
        function loop() {
          if (!simulation.paused && m.alive) {
            if (!(simulation.cycle % 720)) {
              requestAnimationFrame(() => {
                if (simulation.cycle % 1440 > 720) {
                  //kinda alternate between each option
                  m.rewind(60);
    ...
    // },
    {
      name: "discount",
      description:
        "get 3 random <strong class='color-junk'>JUNK</strong><strong class='color-m'>tech</strong> for the price of 1!",
      maxCount: 1,
      count: 0,
      frequency: 0,
      isJunk: true,
    ...
    // },
    {
      ...
      description:
        "instantly spawn 5 <strong class='color-m'>tech</strong>, but add a chance to<br>remove everything with a 5 minute <strong>half-life</strong>",
      maxCount: 1,
      count: 0,
      frequency: 0,
      frequencyDefault: 0,
    ...
    },
    {
      name: "harvest",
      description:
        "convert all the mobs on this level into <strong class='color-ammo'>ammo</strong>",
      maxCount: 1,
      count: 0,
      frequency: 0,
      frequencyDefault: 0,
    ...
    },
    {
      name: "brainstorm",
      description:
        "the <strong class='color-m'>tech</strong> <strong class='color-choice'><span>ch</span><span>oi</span><span>ce</span></strong> menu <strong>randomizes</strong><br>every <strong>0.5</strong> seconds for <strong>10</strong> seconds",
      maxCount: 1,
      count: 0,
      frequency: 0,
      frequencyDefault: 0,
    ...
    },
    {
      ...
      allowed() {
        return (
          !tech.isFallingDamage && !tech.isOverHeal && !tech.isEnergyHealth
        );
      },
      ...
      effect() {
        ...
        m.setMaxHealth();
        for (let i = 0; i < b.guns.length; i++)
          b.guns[i].ammo = b.guns[i].ammo * Math.pow(2, 10);
        simulation.updateGunHUD();
      },
      remove() {},
    },
      ...
      effect() {
        m.look = () => {
          ...
          m.transSmoothX =
            canvas.width2 -
            m.pos.x -
            (simulation.mouse.x - canvas.width2) * scale;
          m.transSmoothY =
            canvas.height2 -
            m.pos.y -
            (simulation.mouse.y - canvas.height2) * scale;
    ...
    },
    {
      ...
      description:
        "when you touch walls you speed up instead of slowing down. It's kinda fun.",
      maxCount: 1,
      count: 0,
      frequency: 0,
      isJunk: true,
    ...
    // },
    {
      ...
      effect() {
        ...
        const colors = shuffle([
          powerUps.research.color,
          powerUps.heal.color,
          powerUps.ammo.color,
          powerUps.ammo.color,
          powerUps.field.color,
          powerUps.gun.color,
        ]);
    ...
    },
    {
      ...
      description:
        "emit 2 sine waveforms at 853 Hz and 960 Hz<br><em>lower your volume</em>",
      maxCount: 1,
      count: 0,
      frequency: 0,
      isJunk: true,
      ...
      effect: () => {
        ...
        setTimeout(() => {
          sound.suspend();
          powerUps.spawn(
            m.pos.x + 160 * (Math.random() - 0.5),
            m.pos.y + 160 * (Math.random() - 0.5),
            "heal",
          );
          setTimeout(() => {
            sound.resume();
    ...
    },
    {
      name: "automatic",
      description:
        "you can't fire when moving<br>always <strong>fire</strong> when at <strong>rest</strong>",
      maxCount: 1,
      count: 0,
      frequency: 0,
      isJunk: true,
    ...
    },
    {
      name: "what the block?",
      description:
        "throwing a <strong class='color-block'>block</strong> throws <strong>you</strong> instead",
      maxCount: 1,
      count: 0,
      frequency: 0,
      isInstant: true,
    ...
    },
    {
      ...
      effect() {
        m.look = function () {
          //always on mouse look
          m.angle =
            (((m.pos.x + m.pos.y) / 100 + Math.PI) % Math.PI) * 2 - Math.PI;
          //smoothed mouse look translations
          const scale = 0.8;
          m.transSmoothX =
            canvas.width2 -
    ...
    },
    {
      name: "p-zombie",
      description:
        "set your <strong class='color-h'>health</strong> to <strong>1</strong><br>all mobs, not bosses, die and <strong>resurrect</strong> as zombies",
      maxCount: 1,
      count: 0,
      frequency: 0,
      isInstant: true,
      ...
      effect() {
        ...
        m.displayHealth();
        for (let i = mob.length - 1; i > -1; i--) {
          //replace mobs with zombies
    ...
    },
    {
      name: "decomposers",
      description:
        "after they die <strong>mobs</strong> leave behind <strong>spawns</strong>",
      maxCount: 1,
      count: 0,
      frequency: 0,
      isInstant: true,
    ...
    // },
    {
      name: "Fourier analysis",
      description:
        "your aiming is now controlled by this equation:<br><span style = 'font-size:80%;'>2sin(0.0133t) + sin(0.013t) + 0.5sin(0.031t)+ 0.33sin(0.03t)</span>",
      maxCount: 1,
      count: 0,
      frequency: 0,
      isJunk: true,
      ...
      effect() {
        m.look = () => {
          m.angle =
            2 * Math.sin(m.cycle * 0.0133) +
            Math.sin(m.cycle * 0.013) +
            0.5 * Math.sin(m.cycle * 0.031) +
            0.33 * Math.sin(m.cycle * 0.03);
          const scale = 0.8;
          simulation.mouse.y;
          m.transSmoothX =
            canvas.width2 -
    ...
    },
    {
      ...
      description:
        "spawn a <strong class='color-g'>gun</strong><br><strong>remove</strong> your active <strong class='color-g'>gun</strong>",
      maxCount: 1,
      count: 0,
      frequency: 0,
      isInstant: true,
    ...
    },
    {
      name: "probability",
      description:
        "<strong>100x</strong> <em class='flicker'>frequency</em><br>of one random <strong class='color-m'>tech</strong>",
      maxCount: 1,
      count: 0,
      frequency: 0,
      isInstant: true,
    ...
    },
    {
      ...
      effect() {
        ...
        for (let i = 0, len = tech.tech.length; i < len; i++)
          tech.tech[i].name = tech.tech[i].name.shuffle();
      },
      remove() {},
    },
    {
      name: "quantum leap",
      description:
        "become an <strong class='alt'>alternate</strong> version of yourself<br>every <strong>20</strong> seconds",
      maxCount: 1,
      count: 0,
      frequency: 0,
      isInstant: true,
    ...
    },
    {
      ...
      effect() {
        setInterval(() => {
          ...
          simulation.makeTextLog(
            `simulation.score <span class='color-symbol'>=</span> ${score.toFixed(0)}`,
          );
    ...
    },
    {
      name: "pop-ups",
      description:
        "sign up to learn endless easy ways to win n-gon<br>that Landgreen doesn't want you to know!!!1!!",
      maxCount: 1,
      count: 0,
      frequency: 0,
      isInstant: true,
      ...
      effect() {
        setInterval(() => {
          alert(
            `The best combo is ${tech.tech[Math.floor(Math.random() * tech.tech.length)].name} with ${tech.tech[Math.floor(Math.random() * tech.tech.length)].name}!`,
          );
    ...
    },
    {
      name: "repartitioning",
      description:
        "set the <em class='flicker'>frequency</em> of finding normal <strong class='color-m'>tech</strong> to <strong>0</strong><br>spawn 5 <strong class='color-m'>tech</strong>",
      maxCount: 1,
      count: 0,
      frequency: 0,
      isInstant: true,
    ...
    },
    {
      name: "defragment",
      description:
        "set the <em class='flicker'>frequency</em> of finding <strong class='color-junk'>JUNK</strong><strong class='color-m'>tech</strong> to zero",
      maxCount: 1,
      count: 0,
      frequency: 0,
      isInstant: true,
      ...
      effect() {
        setInterval(() => {
          if (!simulation.paused)
            ctx.rotate(0.001 * Math.sin(simulation.cycle * 0.01));
        }, 16);
      },
      remove() {},
    },
    ...
    // },
    {
      name: "umbra",
      description:
        "produce a blue glow around everything<br>and probably some simulation lag",
      maxCount: 1,
      count: 0,
      frequency: 0,
      isInstant: true,
    ...
    },
    {
      name: "rewind",
      description:
        "every 10 seconds <strong class='color-rewind'>rewind</strong> <strong>2</strong> seconds",
      maxCount: 9,
      count: 0,
      frequency: 0,
      isInstant: true,
    ...
    },
    {
      name: "undo",
      description:
        "every 4 seconds <strong class='color-rewind'>rewind</strong> <strong>1/2</strong> a second",
      maxCount: 9,
      count: 0,
      frequency: 0,
      isInstant: true,
    ...
    },
    {
      ...
      description:
        "convert your <strong class='color-f'>energy</strong> into <strong class='color-block'>blocks</strong>",
      maxCount: 9,
      count: 0,
      frequency: 0,
      isInstant: true,
    ...
    },
    {
      name: "reincarnation",
      description:
        "kill all mobs and spawn new ones<br>(also spawn a few extra mobs for fun)",
      maxCount: 3,
      count: 0,
      frequency: 0,
      isInstant: true,
      ...
      effect() {
        ...
        for (let i = 0, len = mob.length; i < len; i++) {
          if (mob[i].alive && !mob[i].shield && !mob[i].isBadTarget) {
            const pick =
              spawn.pickList[Math.floor(Math.random() * spawn.pickList.length)];
    ...
    },
    {
      name: "expert system",
      description:
        "spawn a <strong class='color-m'>tech</strong> power up<br><strong>+64%</strong> <strong class='color-junk'>JUNK</strong><strong class='color-m'>tech</strong> chance",
      maxCount: 9,
      count: 0,
      frequency: 0,
      isInstant: true,
    ...
    },
    {
      ...
      description:
        "every 10 seconds drain your <strong class='color-f'>energy</strong><br>return it doubled 5 seconds later",
      maxCount: 9,
      count: 0,
      frequency: 0,
      isInstant: true,
        ...
        for (let i = 0; i < 120; i++) {
          setTimeout(() => {
            ...
            b.missile(
              where,
              -Math.PI / 2 +
                0.2 * (Math.random() - 0.5) * Math.sqrt(tech.missileCount),
              -2,
            );
          }, i * 1000);
        }
        ...
        setInterval(() => {
          if (!simulation.paused && document.visibilityState !== "hidden") {
            b.grenade(
              ...
              -Math.PI / 2,
            ); //fire different angles for each grenade
    ...
    },
    {
      name: "wall jump",
      description:
        "no knees or toes are drawn on the player<br>you can wall climb though",
      maxCount: 1,
      count: 0,
      frequency: 0,
      isJunk: true,
    ...
    },
    {
      name: "diegesis",
      description:
        "indicate fire cooldown</strong><br>through a rotation of your head",
      maxCount: 1,
      count: 0,
      frequency: 0,
      isJunk: true,
    ...
    },
    {
      name: "n-gone",
      description:
        "become invisible to yourself<br><em>mobs can still see you</em>",
      maxCount: 1,
      count: 0,
      frequency: 0,
      isJunk: true,
    ...
    // },
    {
      ...
      allowed() {
        return (
          !m.isShipMode &&
          !m.isAltSkin &&
          m.fieldUpgrades[m.fieldMode].name !== "negative mass"
        );
      },
      requires: "",
      effect() {
        ...
        for (let i = 0; i < tech.tech.length; i++) {
          if (tech.tech[i].name === "relativistic rotation")
            tech.tech[i].frequency = 10;
        }
      },
      remove() {},
    },
    {
      ...
      description:
        "turning the ship rotates the universe instead<br><strong>2x</strong> <strong class='color-d'>damage</strong>",
      maxCount: 1,
      count: 0,
      frequency: 0,
      isInstant: true,
    ...
    },
    {
      name: "assimilation",
      description:
        "all your <strong class='color-bot'>bots</strong> are converted to the <strong>same</strong> random model",
      maxCount: 1,
      count: 0,
      frequency: 0,
      isBotTech: true,
    ...
    },
    {
      name: "stun",
      description:
        "<strong>stun</strong> all mobs for up to <strong>8</strong> seconds",
      maxCount: 9,
      count: 0,
      frequency: 0,
      isInstant: true,
    ...
    },
    {
      name: "translucent",
      description:
        "spawn <strong>3</strong> <strong class='color-g'>gun</strong> power ups<br>your <strong class='color-g'>bullets</strong> and bots are transparent",
      maxCount: 1,
      count: 0,
      frequency: 0,
      isInstant: true,
    ...
    },
    {
      name: "difficulty",
      description:
        "spawn a power up that lets you<br>adjust the simulation <strong>difficulty</strong> parameters",
      maxCount: 1,
      count: 0,
      frequency: 0,
      isInstant: true,
    ...
    },
    {
      ...
      effect() {
        ...
        simulation.makeTextLog(
          `<span class='color-var'>m</span>.<span class='color-r'>research</span> <span class='color-symbol'>--</span><br>${powerUps.research.count}`,
        );
      },
      remove() {},
    },
    {
      ...
      effect() {
        ...
        for (let i = 0, len = 9; i < len; i++) {
          ...
          spawn.randomLevelBoss(
            m.pos.x + range * Math.cos(angle),
            m.pos.y + range * Math.sin(angle),
            spawn.nonCollideBossList,
          );
        }
    ...
    },
    {
      ...
      effect() {
        ...
        simulation.makeTextLog(
          `spawn<span class='color-symbol'>.</span>${this.mobType}<span class='color-symbol'>(</span>x<span class='color-symbol'>,</span>y<span class='color-symbol'>)</span>`,
        );
      },
      remove() {},
    },
    {
      ...
      effect() {
        ...
        for (let i = 0; i < 30; i++) {
          const where = Vector.add(
            m.pos,
            Vector.mult(
              Vector.rotate(unit, Math.random() * 2 * Math.PI),
              2000 + 1200 * Math.random(),
            ),
          );
    ...
    },
    {
      ...
      descriptionFunction() {
        const loop = () => {
          if (
            (simulation.paused || simulation.isChoosing) &&
            m.alive &&
            !build.isExperimentSelection
          ) {
            //&& (!simulation.isChoosing || this.count === 0)
            let b = []; //produce next row
            b.push(
              this.rule(
                this.state[this.state.length - 1],
                this.state[this.state.length - 1].length - 1,
                0,
                1,
              ),
            ); //left edge wrap around
            for (
              let i = 1;
              i < this.state[this.state.length - 1].length - 1;
              i++
            ) {
              //apply rule to the rest of the array
              b.push(
                this.rule(this.state[this.state.length - 1], i - 1, i, i + 1),
              );
            }
            b.push(
              this.rule(
                this.state[this.state.length - 1],
                this.state[this.state.length - 1].length - 2,
                this.state[this.state.length - 1].length - 1,
                0,
              ),
            ); //right edge wrap around
            this.state.push(b);
            if (document.getElementById(`cellular-rule-id${this.id}`))
              document.getElementById(`cellular-rule-id${this.id}`).innerHTML =
                this.outputText(); //convert to squares and send HTML
            if (
              this.count &&
              this.researchSpawned < 12 &&
              !(this.state.length % 10)
            ) {
              this.researchSpawned++;
              powerUps.spawn(
                m.pos.x - 50 + 100 * (Math.random() - 0.5),
                m.pos.y + 100 * (Math.random() - 0.5),
                "research",
              );
            }
            setTimeout(
    ...
    },
    {
      ...
      effect() {
        const urls = [
          "https://scratch.mit.edu/projects/14005697/fullscreen/",
          "https://scratch.mit.edu/projects/22573757/fullscreen/",
          "https://scratch.mit.edu/projects/41429974/fullscreen/",
          "https://scratch.mit.edu/projects/43690666/fullscreen/",
          "https://codepen.io/lilgreenland/full/ozXNWZ",
          "https://codepen.io/lilgreenland/full/wzARJY",
          "classic/7-1-2017/",
          "classic/4-15-2018/",
          "classic/7-11-2019/",
          "classic/9-8-2019/",
    ...
    },
    {
      name: "beforeunload",
      description:
        "<strong>75%</strong> of the time when you attempt to <strong>exit</strong> n-gon<br>you are prompted to <strong>cancel</strong> or continue.<br>Each time you <strong>cancel</strong> gain <strong>1.25x</strong> <strong class='color-d'>damage</strong>.",
      maxCount: 1,
      count: 0,
      frequency: 1,
      isJunk: true,
    ...
    },
    {
      name: "tinker",
      description:
        "<strong>permanently</strong> unlock <strong class='color-junk'>JUNK</strong><strong class='color-m'>tech</strong> in experiment mode<br><em>this effect is stored for future visits</em>",
      maxCount: 1,
      count: 0,
      frequency: 0,
      frequencyDefault: 0,
    ...
    //**************************************************
    {
      ...
      requires: "NOT EXPERIMENT MODE",
      effect() {
        if (localSettings.loreCount > lore.conversation.length - 1) {
          //reward for people done with lore chapters (or on the final chapter)
          for (let i = mob.length - 1; i > -1; i--) {
            //replace mobs with starters
        ...
        setTimeout(() => {
          //a short delay, I can't remember why
          lore.techCount++;
