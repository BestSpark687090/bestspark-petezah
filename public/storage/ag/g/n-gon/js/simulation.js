...
const simulation = {
      ...
      for (let i = 0, len = mob.length; i < len; i++) {
        if (
          Vector.magnitudeSquared(
            Vector.sub(mob[i].position, simulation.mouseInGame),
          ) <
          mob[i].radius * mob[i].radius
        ) {
          console.log(mob[i]);
        }
      }
  ...
  drawCircle() {
    ...
    while (i--) {
      ...
      ctx.arc(
        simulation.drawList[i].x,
        simulation.drawList[i].y,
        simulation.drawList[i].radius,
        0,
        2 * Math.PI,
      );
  ...
  boldActiveGunHUD() {
    if (b.inventory.length > 0) {
      for (let i = 0, len = b.inventory.length; i < len; ++i) {
        if (
          b.inventory[i] === b.activeGun &&
          document.getElementById(b.activeGun)
        ) {
  ...
  makeTextLog(text, time = 240) {
    if (
      !localSettings.isHideHUD &&
      simulation.isTextLogOpen &&
      !build.isExperimentSelection
    ) {
      if (simulation.lastLogTime > m.cycle) {
        //if there is an older message
        document.getElementById("text-log").innerHTML =
          document.getElementById("text-log").innerHTML + "<br>" + text;
  ...
  zoomScale: 1000,
  isAutoZoom: true,
  setZoom(zoomScale = simulation.zoomScale) {
    ...
    if (simulation.isAutoZoom) {
      simulation.ephemera.push({
        ...
        do() {
          ...
          if (
            this.count < 1 &&
            this.currentLevel === level.onLevel &&
            simulation.isAutoZoom
          ) {
  ...
  translatePlayerAndCamera(where) {
    ...
    const change = {
      x: before.x - player.position.x,
      y: before.y - player.position.y,
    };
    ...
    for (let i = 0; i < bullet.length; i++) {
      if (bullet[i].botType) {
        if (
          Vector.magnitudeSquared(
            Vector.sub(bullet[i].position, player.position),
          ) > 1000000
        ) {
          //far away bots teleport to player
          ...
          //close bots maintain relative distance to player on teleport
          Matter.Body.setPosition(
            bullet[i],
            Vector.sub(bullet[i].position, change),
          );
        }
      }
    }
  ...
  firstRun: true,
  splashReturn() {
    document.getElementById("previous-seed").innerHTML =
      `previous seed: <span style="font-size:80%;">${Math.initialSeed}</span><br>`;
  ...
  startGame(isBuildRun = false, isTrainingRun = false) {
    ...
    simulation.clearMap();
    if (!isBuildRun) {
      //if a build run logic flow returns to "experiment-button").addEventListener
    ...
    simulation.makeTextLog(
      `Math.seed <span class='color-symbol'>=</span> ${Math.initialSeed}`,
    );
    simulation.makeTextLog(
      `<span class='color-var'>const</span> engine <span class='color-symbol'>=</span> Engine.create(); <em>//simulation begin</em>`,
    );
    simulation.makeTextLog(
      `engine.timing.timeScale <span class='color-symbol'>=</span> 1`,
    );
      ...
      do() {
        if (!(m.cycle % 60)) {
          ...
          if (m.energy > m.maxEnergy) {
            m.energy =
              m.maxEnergy + (m.energy - m.maxEnergy) * tech.overfillDrain; //every second energy above max energy loses 25%
            if (m.energy > 1000000) m.energy = 1000000;
          }
          if (m.pos.y > simulation.fallHeight) {
            // if 4000px deep
            if (level.fallMode === "start") {
            ...
            } else if (level.fallMode === "position") {
              //fall and stay in the same horizontal position
              simulation.ephemera.push({
                name: "slow player",
              ...
              const before = { x: player.position.x, y: player.position.y };
              let posXClamped = Math.min(
                Math.max(level.fallModeBounds.left, player.position.x),
                level.fallModeBounds.right,
              );
              Matter.Body.setPosition(player, {
                x: posXClamped,
                y: level.enter.y - 4000,
              });

              // translate camera smoothly to preserve illusion to endless fall
              const change = {
                x: before.x - posXClamped,
                y: before.y - player.position.y,
              };
              m.transX += change.x;
              m.transY += change.y;
          ...
          if (m.lastKillCycle + 300 > m.cycle) {
          ...
          if (!(m.cycle % 420)) {
            ...
            if (
              Matter.Query.point(map, m.pos).length > 0 ||
              Matter.Query.point(map, player.position).length > 0
            ) {
            ...
            if (
              tech.cyclicImmunity &&
              m.immuneCycle < m.cycle + tech.cyclicImmunity
            )
              m.immuneCycle = m.cycle + tech.cyclicImmunity; //player is immune to damage for 60 cycles
  ...
  clearMap() {
    ...
    if (m.alive) {
      ...
      if (b.guns[10].have) {
        //do you have mines as a gun
        let count = 0;
        for (i = 0, len = bullet.length; i < len; i++) {
          //count mines left on map
          if (
            (bullet[i].bulletType === "mine" &&
              (!tech.isMineSentry || bullet[i].shots === undefined)) ||
        ...
        if (tech.ammoCap)
          b.guns[10].ammo = Math.min(tech.ammoCap, b.guns[10].ammo);
        simulation.updateGunHUD();
      }
      ...
      if (tech.isMutualism && !tech.isEnergyHealth) {
        for (let i = 0; i < bullet.length; i++) {
          if (bullet[i].isMutualismActive) {
            m.health +=
              0.01 + 0.01 * (bullet[i].isSpore || bullet[i].isFlea ? 0 : 1);
      ...
      if (tech.isEndLevelPowerUp) {
        for (let i = 0; i < powerUp.length; i++) {
          if (powerUp[i].name === "tech") {
            ...
            if (m.fieldMode === 0)
              m.setField(
                Math.ceil(Math.random() * (m.fieldUpgrades.length - 1)),
              ); //pick a random field, but not field 0
          } else {
            powerUp[i].effect();
          }
        }
    ...
    if (tech.isHealAttract && m.alive) {
      //send health power ups to the next level
      let healCount = 0;
      for (let i = 0, len = powerUp.length; i < len; i++) {
        if (
          powerUp[i].name === "heal" &&
          Vector.magnitudeSquared(Vector.sub(powerUp[i].position, m.pos)) <
            1000000
        )
          healCount++;
      }
        ...
        if (healCount > 0) {
          ...
          if (!simulation.paused && !simulation.isChoosing) {
            ...
            powerUps.directSpawn(
              level.enter.x + 50 + 100 * (Math.random() - 0.5),
              level.enter.y - 60 + 100 * (Math.random() - 0.5),
              "heal",
            );
          }
        }
    ...
    if (tech.isDronesTravel && m.alive) {
      ...
      requestAnimationFrame(() => {
        b.delayDrones(
          { x: level.enter.x + 50, y: level.enter.y - 60 },
          droneCount,
        );
      });
    ...
    if (tech.isQuantumEraser && m.alive) {
      ...
      let cycle = () => {
        //run after waiting a cycle for the map to be cleared
        const types = [
          "heal",
          "ammo",
          "heal",
  ...
  testingOutput() {
    ...
    ctx.fillText(
      `(${simulation.mouseInGame.x.toFixed(1)}, ${simulation.mouseInGame.y.toFixed(1)})`,
      simulation.mouse.x,
      simulation.mouse.y - 20,
    );
  },
  sight: {
    //credit to Cornbread2100 for adding this algorithm to n-gon
    // square: 0,
    intersectMap: [], //this is precalculated in simulation.draw.lineOfSightPrecalculation()
    getIntersection(v1, v1End, domain) {
      const intersections = simulation.sight.getIntersections(
        v1,
        v1End,
        domain,
      );
      var best = {
        x: v1End.x,
        y: v1End.y,
        dist: (v1End.x - v1.x) ** 2 + (v1End.y - v1.y) ** 2,
      };
      for (const intersection of intersections) {
        const dist =
          (intersection.x - v1.x) ** 2 + (intersection.y - v1.y) ** 2;
        if (dist < best.dist)
          best = { x: intersection.x, y: intersection.y, dist: dist };
      }
      ...
      for (const obj of domain) {
        for (var i = 0; i < obj.vertices.length - 1; i++) {
          ...
          if (results.onLine1 && results.onLine2)
            intersections.push({ x: results.x, y: results.y });
        }
        results = simulation.checkLineIntersection(
          v1,
          v1End,
          obj.vertices[obj.vertices.length - 1],
          obj.vertices[0],
        );
        if (results.onLine1 && results.onLine2)
          intersections.push({ x: results.x, y: results.y });
    ...
    circleLoS(pos, radius) {
      ...
      function circleLineCollisions(a, b, c, radius) {
        ...
        if (distance == radius) {
          ...
          const distance1 = {
            a: Math.sqrt(
              (intersection1.x - a.x) ** 2 + (intersection1.y - a.y) ** 2,
            ),
            b: Math.sqrt(
              (intersection1.x - b.x) ** 2 + (intersection1.y - b.y) ** 2,
            ),
          };
          const distance2 = {
            a: Math.sqrt(
              (intersection2.x - a.x) ** 2 + (intersection2.y - a.y) ** 2,
            ),
            b: Math.sqrt(
              (intersection2.x - b.x) ** 2 + (intersection2.y - b.y) ** 2,
            ),
          };
          const result = [];
        ...
        for (var i = 0; i < obj.vertices.length; i++) {
          ...
          if (Matter.Query.ray(map, pos, queryPoint).length == 0) {
            var distance = Math.sqrt(
              (vertex.x - pos.x) ** 2 + (vertex.y - pos.y) ** 2,
            );
            ...
            if (best.dist >= distance)
              best = { x: endPoint.x, y: endPoint.y, dist: distance };
            vertices.push(best);
            ...
            if (best.dist >= radius)
              best = { x: endPoint.x, y: endPoint.y, dist: radius };
            vertices.push(best);
      ...
      for (const line of outerCollisions) {
        for (const vertex of line) {
          ...
          const angle = Math.atan2(vertex.y - pos.y, vertex.x - pos.x);
          const queryPoint = {
            x: Math.cos(angle + Math.PI) + vertex.x,
            y: Math.sin(angle + Math.PI) + vertex.y,
          };
          if (
            Math.abs(distance - radius) < 1 &&
            Matter.Query.ray(map, pos, queryPoint).length == 0
          )
            circleCollisions.push(vertex);
        }
      }
      for (var i = 0; i < circleCollisions.length; i++) {
        ...
        if (
          ...
          angle1 / Math.abs(angle1) != angle2 / Math.abs(angle2)
        ) {
          // if the arc between the to points crosses over the left side (+/- pi radians)
          const newAngle1 =
            (Math.PI - Math.abs(angle1)) * (angle1 / Math.abs(angle1));
          const newAngle2 =
            (Math.PI - Math.abs(angle2)) * (angle2 / Math.abs(angle2));
      ...
      vertices.sort(
        (a, b) =>
          Math.atan2(a.y - pos.y, a.x - pos.x) -
          Math.atan2(b.y - pos.y, b.x - pos.x),
      );
      return vertices;
    },
  },
  draw: {
    ...
    lineOfSightPrecalculation() {
      ...
      for (var i = 0; i < map.length; i++) {
        ...
        for (var j = 0; j < obj.vertices.length - 1; j++) {
          var intersections = simulation.sight.getIntersections(
            obj.vertices[j],
            obj.vertices[j + 1],
            restOfMap,
          );
        ...
        intersections = simulation.sight.getIntersections(
          obj.vertices[obj.vertices.length - 1],
          obj.vertices[0],
          restOfMap,
        );
        ...
        if (vertices.length) {
          ...
          for (var i = 1; i < vertices.length; i++) {
            var currentDistance = Math.sqrt(
              (vertices[i - 1].x - pos.x) ** 2 +
                (vertices[i - 1].y - pos.y) ** 2,
            );
            var newDistance = Math.sqrt(
              (vertices[i].x - pos.x) ** 2 + (vertices[i].y - pos.y) ** 2,
            );
            if (
              Math.abs(currentDistance - radius) < 1 &&
              Math.abs(newDistance - radius) < 1
            ) {
              const currentAngle = Math.atan2(
                vertices[i - 1].y - pos.y,
                vertices[i - 1].x - pos.x,
              );
          ...
          newDistance = Math.sqrt(
            (vertices[0].x - pos.x) ** 2 + (vertices[0].y - pos.y) ** 2,
          );
          currentDistance = Math.sqrt(
            (vertices[vertices.length - 1].x - pos.x) ** 2 +
              (vertices[vertices.length - 1].y - pos.y) ** 2,
          );
          if (
            Math.abs(currentDistance - radius) < 1 &&
            Math.abs(newDistance - radius) < 1
            ...
            const currentAngle = Math.atan2(
              vertices[vertices.length - 1].y - pos.y,
              vertices[vertices.length - 1].x - pos.x,
            );
            const newAngle = Math.atan2(
              vertices[0].y - pos.y,
              vertices[0].x - pos.x,
    ...
    cons() {
      ...
      for (let i = 0, len = cons.length; i < len; ++i) {
        ...
        ctx.lineTo(
          cons[i].bodyB.position.x + cons[i].pointB.x,
          cons[i].bodyB.position.y + cons[i].pointB.y,
        );
      }
  ...
  constructCycle() {
    if (
      simulation.isConstructionMode &&
      simulation.constructMouseDownPosition
    ) {
  ...
  enableConstructMode() {
      ...
      if (simulation.testing && simulation.constructMouseDownPosition) {
        ...
        if (e.button === 1) {
          ...
          simulation.outputMapString(
            `${Math.floor(simulation.constructMouseDownPosition.x)}, ${Math.floor(simulation.constructMouseDownPosition.y)}`,
          );
        } else if (
          simulation.mouseInGame.x > simulation.constructMouseDownPosition.x &&
          simulation.mouseInGame.y > simulation.constructMouseDownPosition.y
        ) {
          //make sure that the width and height are positive
          if (e.button === 0) {
            //add map
            if (level.isProcedural) {
              simulation.outputMapString(
                `spawn.mapRect(x+${x}, ${y}, ${dx}, ${dy});\n`,
              );
            } else {
              simulation.outputMapString(
    ...
    document.body.addEventListener("keydown", (event) => {
      // e.keyCode   z=90  m=77 b=66  shift = 16  c = 67
      if (
        simulation.testing &&
        event.code === "KeyZ" &&
        simulation.constructMapString.length
      ) {
        if (
          simulation.constructMapString[
            simulation.constructMapString.length - 1
          ][6] === "m"
        ) {
          //remove map from current level
          ...
          //remove body from current level
