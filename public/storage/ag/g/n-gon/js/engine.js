      ...
      if (mob[k].alive) {
        ...
        function collideMob(obj) {
          ...
          if (
            ...
            if (tech.isHarpoonDefense) {
              //fire harpoons at mobs after getting hit
              const maxCount = 10 + 3 * tech.extraHarpoons; //scale the number of hooks fired
              let count = maxCount - 1;
              const angle = Math.atan2(
                mob[k].position.y - player.position.y,
                mob[k].position.x - player.position.x,
              );
              ...
              for (; count > 0; count--) {
                b.harpoon(
                  m.pos,
                  mob[k],
                  angle + (count * 2 * Math.PI) / maxCount,
                  0.75,
                  true,
                  7,
                );
            ...
            if (obj.classType === "bullet" && obj.speed > obj.minDmgSpeed) {
              ...
              if (tech.isLessDamageReduction && !mob[k].shield)
                mob[k].damageReduction *= mob[k].isBoss
                  ? mob[k].isFinalBoss
                    ? 1.0005
                    : 1.0025
                  : 1.05;
            ...
            if (obj.classType === "body" && obj.speed > 6) {
              ...
              if (v > 9) {
                if (tech.blockDmg) {
                  //electricity
                  Matter.Body.setVelocity(mob[k], {
                    x: 0.5 * mob[k].velocity.x,
                    y: 0.5 * mob[k].velocity.y,
                  });
                  if (
                ...
                if (stunTime > 0.5 && mob[k].memory !== Infinity)
                  mobs.statusStun(mob[k], 60 + 60 * Math.sqrt(stunTime));
                if (
                  mob[k].alive &&
