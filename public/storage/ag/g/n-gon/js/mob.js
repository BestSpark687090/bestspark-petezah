  ...
  statusSlow(who, cycles = 60) {
    ...
    if (tech.isAoESlow) {
      ...
      for (let i = 0, len = mob.length; i < len; i++) {
        if (
          who !== mob[i] &&
          Vector.magnitudeSquared(Vector.sub(who.position, mob[i].position)) <
            range2 + mob[i].radius
        )
          applySlow(mob[i]);
      }
    ...
    function applySlow(whom) {
      if (!whom.shield && !whom.isShielded && whom.alive) {
        if (
          tech.isIceMaxHealthLoss &&
          whom.health > 0.66 &&
          whom.damageReduction > 0
        )
          whom.health = 0.66;
        if (
          tech.isIceKill &&
    ...
    mob[i] = Matter.Bodies.polygon(xPos, yPos, sides, radius, {
      ...
      locatePlayer() {
        ...
        this.seePlayer.recall =
          this.memory + Math.round(this.memory * Math.random()); //cycles before mob falls a sleep
          ...
          if (
            ...
            if (!m.isCloak) {
              for (let i = 0; i < depth; i++) {
                //if lost player lock onto a player location in history
                let history = m.history[(m.cycle - 10 * i) % 600];
                if (
                  Matter.Query.ray(map, this.position, history.position)
                    .length === 0
                ) {
                  this.seePlayer.recall =
                    this.memory + Math.round(this.memory * Math.random()); //cycles before mob falls a sleep
      ...
      seePlayerByDistOrLOS() {
        if (!(simulation.cycle % this.seePlayerFreq)) {
          if (
            (this.distanceToPlayer2() < this.seeAtDistance2 ||
              Matter.Query.ray(map, this.position, this.playerPosRandomY())
                .length === 0) && //&& Matter.Query.ray(body, this.position, this.playerPosRandomY()).length === 0
            !m.isCloak
          ) {
            this.foundPlayer();
          } else if (this.seePlayer.recall) {
      ...
      wing(a, radius = 250, ellipticity = 0.4, dmg = 0.0006) {
        ...
        const where = Vector.add(
          this.position,
          Vector.mult(perp, radius + 0.8 * this.radius),
        );
        ...
        const hitPlayer = Matter.Query.ray(
          [player],
          this.position,
          Vector.add(this.position, Vector.mult(perp, radius * 2.05)),
          minorRadius,
        );
      ...
      springAttack() {
        ...
        if (
          this.seePlayer.recall &&
          Matter.Query.ray(map, this.position, this.seePlayer.position)
            .length === 0
        ) {
          if (!(simulation.cycle % (this.seePlayerFreq * 2))) {
            ...
            const goal = Vector.add(
              this.position,
              Vector.mult(unit, stepRange),
            );
          ...
          for (let i = 0; i < array.length; ++i) {
            if (!array[i].isNotHoldable) {
              ...
              if (radius2 < range * range && radius2 > 10000) {
                const curlVector = Vector.mult(
                  Vector.perp(Vector.normalise(sub)),
                  mag,
                );
                //apply curl force
                ...
                if (isAntiGravity)
                  array[i].force.y -= 0.8 * simulation.g * array[i].mass;
      ...
      pullPlayer() {
        if (
          this.seePlayer.yes &&
          Vector.magnitudeSquared(Vector.sub(this.position, player.position)) <
            1000000
        ) {
          ...
          player.force.x -=
            simulation.accelScale *
            0.00113 *
            player.mass *
            Math.cos(angle) *
            (m.onGround ? 2 : 1);
          player.force.y -=
            simulation.accelScale * 0.00084 * player.mass * Math.sin(angle);
      ...
      hoverOverPlayer() {
        if (this.seePlayer.recall) {
          ...
          if (
            this.position.y >
            this.seePlayer.position.y - this.hoverElevation + rangeY
          ) {
          ...
          if (
            this.position.x >
            this.seePlayer.position.x + this.hoverXOff + rangeX
          ) {
      ...
      grow() {
        if (this.seePlayer.recall) {
          if (this.radius < 80) {
            ...
            if (this.isShielded) {
              //remove shield if shielded when growing
        ...
        if (this.seePlayer.recall && !(simulation.cycle % this.blinkRate)) {
          ...
          if (distMag < this.blinkLength) {
            Matter.Body.translate(
              this,
              Vector.mult(unitVector, distMag + rando),
            );
          } else {
            Matter.Body.translate(
              this,
              Vector.mult(unitVector, this.blinkLength + rando),
            );
          }
      ...
      bomb() {
        ...
        if (
          ...
          spawn.bomb(
            this.position.x,
            this.position.y + this.radius * 0.7,
            9 + Math.ceil(this.radius / 15),
            5,
          );
          //add spin and speed
      ...
      fire() {
        ...
        if (this.seePlayer.recall) {
          ...
          if (!(simulation.cycle % this.seePlayerFreq)) {
            ...
            this.fireDir.y -=
              Math.abs(this.seePlayer.position.x - this.position.x) / 2500; //gives the bullet an arc  //was    / 1600
          }
      ...
      explode(mass = this.mass) {
        if (m.immuneCycle < m.cycle) {
          m.damage(
            Math.min(Math.max(0.02 * Math.sqrt(mass), 0.01), 0.35) *
              simulation.dmgScale,
          );
      ...
      healthBar() {
        //draw health by mob //most health bars are drawn in mobs.healthbar();
        if (this.seePlayer.recall) {
          ...
          if (dmg !== Infinity) {
            ...
            if (tech.isFarAwayDmg)
              dmg *=
                1 +
                Math.sqrt(
                  Math.max(500, Math.min(3000, this.distanceToPlayer())) - 500,
                ) *
                  0.0067; //up to 33% dmg at max range of 3000
            ...
            if (
              ...
              m.energy += Math.min(this.health, dmg) * tech.energySiphon;
      ...
      isDropPowerUp: true,
      death() {
        if (tech.collidePowerUps && this.isDropPowerUp)
          powerUps.randomize(this.position); //needs to run before onDeath spawns power ups
        ...
        if (this.isDropPowerUp) {
          ...
          if (tech.iceIXOnDeath && this.isSlowed) {
            for (
              let i = 0,
                len =
                  2 * Math.sqrt(Math.min(this.mass, 25)) * tech.iceIXOnDeath;
              i < len;
              i++
            )
              b.iceIX(3, Math.random() * 2 * Math.PI, this.position);
          }
          if (tech.deathSpawnsFromBoss || tech.deathSpawns) {
            ...
            const len = Math.min(
              12,
              spawns *
                Math.ceil(Math.random() * simulation.difficulty * spawns),
            );
            for (let i = 0; i < len; i++) {
              spawn.spawns(
          ...
          if (tech.healSpawn && Math.random() < tech.healSpawn) {
            powerUps.spawn(
              this.position.x + 20 * (Math.random() - 0.5),
              this.position.y + 20 * (Math.random() - 0.5),
              "heal",
            );
            simulation.drawList.push({
              x: this.position.x,
          ...
          if (tech.deathSkipTime && !m.isBodiesAsleep) {
            requestAnimationFrame(() => {
              simulation.timePlayerSkip(
                (this.isBoss ? 45 : 25) * tech.deathSkipTime,
              );
          ...
          if (Math.random() < tech.sporesOnDeath) {
            const amount = Math.min(
              25,
              Math.floor(2 + this.mass * (0.5 + 0.5 * Math.random())),
            );
            if (tech.isSporeFlea) {
              const len = amount / 2;
              for (let i = 0; i < len; i++) {
            ...
            b.explosion(
              ...
              Math.min(
                700,
                Math.sqrt(this.mass + 6) * (30 + 60 * Math.random()),
              ),
            );
          ...
          if (tech.isBotSpawnerReset) {
            for (let i = 0, len = bullet.length; i < len; i++) {
              if (bullet[i].botType && bullet[i].endCycle !== Infinity)
                bullet[i].endCycle = simulation.cycle + 900; //15 seconds
            }
          }
          ...
          if (tech.isMobDeathImmunity) {
            ...
            if (m.immuneCycle < m.cycle + immuneTime)
        ...
        if (tech.isRadioactive) {
          ...
          for (let i = 0, len = this.status.length; i < len; i++) {
            if (this.status[i].type === "dot")
              dmgTotal +=
                this.status[i].dmg *
                (this.status[i].endCycle - simulation.cycle);
          }
          if (dmgTotal > 0) {
            //look for closest mob
            ...
            if (closestIndex) {
              ...
              ctx.moveTo(this.position.x, this.position.y);
              ctx.lineTo(
                mob[closestIndex].position.x,
                mob[closestIndex].position.y,
              );
      ...
      replace(i) {
        ...
        if (
          ...
          if (v.length > 5 && body.length < 35 && Math.random() < 0.25) {
            ...
            body[len2] = Matter.Bodies.fromVertices(
              this.position.x,
              this.position.y,
              v,
            );
          ...
          if (tech.isMobBlockFling) {
            ...
            if (!who.isNotHoldable) {
              ...
              Matter.Body.setAngularVelocity(
                who,
                (0.5 + 0.2 * Math.random()) * (Math.random() < 0.5 ? -1 : 1),
              );
