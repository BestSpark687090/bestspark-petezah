...
class HorizontalSlider extends Phaser.GameObjects.Container {
  constructor({
    ...
    if (notches.frame !== "") {
      this.notches = scene.add
        .image(notches.x, notches.y, texture, notches.frame)
        .setOrigin(0);

      this.add(this.notches);
    }

    this.trackEmpty = scene.add
      .image(track.x, track.y, texture, track.emptyFrame)
      .setOrigin(0);
    this.trackFull = scene.add
      .image(track.x, track.y, texture, track.fullFrame)
      .setOrigin(0);

    if (slider.upFrame !== "") {
      this.slider = scene.add
        .image(slider.x, slider.y, texture, slider.upFrame)
        .setOrigin(0);
    } else {
      ...
      this.slider = scene.add
        .rectangle(slider.x, slider.y, slider.width, slider.height, 0xff0000)
        .setOrigin(0)
        .setVisible(false);
    }

    this.sliderUpFrame = slider.upFrame;
    this.sliderDownFrame =
      slider.downFrame !== "" ? slider.downFrame : slider.upFrame;
    ...
    this.sliderRight =
      slider.right > 0
        ? slider.right
        : this.trackEmpty.width - this.slider.width;
    ...
    if (this.enableTrackJump) {
      const hitRect = new Phaser.Geom.Rectangle(
        0,
        0,
        this.trackEmpty.width,
        this.trackEmpty.height,
      );
  ...
  setOrigin(x = 0.5, y = x) {
    ...
    this.sliderRight =
      slider.right > 0
        ? offsetX + slider.right
        : offsetX + (width - this.slider.displayWidth);
    this.sliderRange = this.sliderRight - this.sliderLeft;
  ...
  setSlider(percent, duration = 0, ease = "linear") {
    this.setValue(
      Phaser.Math.FromPercent(percent, this.values.min, this.values.max),
    );
  ...
  updateSlider(isSkipUpdate) {
    ...
    values.percent = Phaser.Math.Percent(
      values.current,
      values.min,
      values.max,
    );
    ...
    if (this.slider.visible) {
      this.cropRect.width =
        this.sliderRange * values.percent + this.slider.width / 2;
    } else {
