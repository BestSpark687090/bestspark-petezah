    ...
    _PostToRuntimeMaybeSync(name, data, dispatchOpts) {
      if (this._iRuntime.UsesWorker())
        ...
        this._iRuntime
          ...
          [
            "_OnMessageFromDOM"
          ]({ type: "event", component: this._componentId, handler: name, dispatchOpts: dispatchOpts || null, data: data, responseId: null });
    }
  ...
  window.RuntimeInterface = class RuntimeInterface {
    ...
    async CreateWorker(url, baseUrl, workerOpts) {
      ...
      if (this._exportType === "cordova" && this._isFileProtocol) {
        ...
        const arrayBuffer =
          await this.CordovaFetchLocalFileAsArrayBuffer(filePath);
    ...
    async _MaybeGetCordovaScriptURL(url) {
      if (
        ...
        const arrayBuffer =
          await this.CordovaFetchLocalFileAsArrayBuffer(filename);
        const blob = new Blob([arrayBuffer], {
          type: "application/javascript",
        });
...
("use strict");
{
  if (window["C3_IsSupported"]) {
    ...
    window["c3_runtimeInterface"] = new self.RuntimeInterface({
      ...
      projectScripts: [["scripts/project/FrameRateLimit.js"]],
...
("use strict");
{
  ...
  const MOBILE_ADS_INITIALIZATION_TIMEOUT_MS = 5e3;
  let idfaAvailable = false;
  ...
  async function StartAdmobPlus(resolver, debug) {
    debug = typeof debug !== "undefined" ? debug : configureOptions.debug;
      ...
      document.addEventListener("deviceready", async () => {
        try {
          const timeoutId = setTimeout(() => {
            resolve(false);
            Failure(resolver, "failure to initialize");
          }, MOBILE_ADS_INITIALIZATION_TIMEOUT_MS);
          await self["admob"]["start"]();
          clearTimeout(timeoutId);
          if (!!debug) {
            const testId = await TestId();
            UpdateAdmobPlusRequest("testDeviceIds", [testId]);
          }
          resolve(true);
        } catch (error) {
          resolve(false);
          Failure(resolver, "Initialization failure");
        }
      });
    });
    return startPromise;
  ...
  async function TestId() {
    if (self["cordova"]["platformId"] === "android")
      return self.C3AdUtilsMD5(self["device"]["uuid"]).toUpperCase();
    if (self["cordova"]["platformId"] === "ios") {
      const idfaPlugin = self["cordova"]["plugins"]["idfa"];
      const idfaInfo = await idfaPlugin["getInfo"]();
      if (!idfaInfo["trackingLimited"]) {
        idfaAvailable = true;
        return self.C3AdUtilsMD5(idfaInfo["idfa"]).toUpperCase();
      }
    }
  }
  function GetAdUnit(adUnit, type) {
    if (self["cordova"]["platformId"] === "android") return adUnit;
    if (self["cordova"]["platformId"] === "ios") {
      const hasTestAdUnit = !!testAdUnitsiOS[type];
      if (configureOptions.debug && !idfaAvailable && hasTestAdUnit) {
        console.log("[C3 advert]", "USING TEST AD UNITS");
        return testAdUnitsiOS[type];
      }
      return adUnit;
    }
  }
...
("use strict");
{
  ...
  const Success = C3AdUtils.Success;
  const Failure = C3AdUtils.Failure;
  const BANNER = 0;
  const LARGE_BANNER = 1;
  const MEDIUM_RECTANGLE = 2;
  const FULL_BANNER = 3;
  const LEADERBOARD = 4;
  const SMART_BANNER = 5;
  const Events = {
    get LOAD() {
  ...
  class C3BannerAd {
    ...
    Load(adPersonalization, resolver) {
      ...
      const options = {
        ...
        offset: 0,
        npa: adPersonalization === "NON_PERSONALIZED" ? "1" : "0",
      };
      this._ad = new self["admob"]["BannerAd"](options);
      this._ad["load"]();
    }
    async Show(resolver) {
      if (!this._ad) return Failure(resolver, "banner ad not created");
      if (this._adLoading) return Failure(resolver, "banner ad still loading");
      if (this._adShowing) return Success(resolver, "banner ad already shown");
      await this._ad["show"]();
      this._adShowing = true;
      Success(resolver, "banner shown");
    }
    async Hide(resolver) {
      if (!this._ad) return Failure(resolver, "banner ad not created");
      if (this._adLoading) return Failure(resolver, "banner ad still loading");
      if (!this._adShowing)
        return Success(resolver, "banner ad not being shown");
      await this._ad["hide"]();
      this._adShowing = false;
      Success(resolver, "banner ad hidden");
    }
    IsLoading() {
...
("use strict");
{
  ...
  class C3InterstitialAd {
    ...
    ResolveLoadPromise(result) {
      ...
      if (this._loadPromiseResolver) this._loadPromiseResolver(result);
      this._loadPromise = null;
      this._loadPromiseResolver = null;
      this._loadPromise = this.GetLoadPromise();
    }
    ...
    async Show(resolver) {
      ...
      const show_ad = async () => {
        if (this._loadSuccess) {
          _AddShowListeners.call(this, resolver);
          await this._ad["show"]();
        } else {
          _AddLoadListeners.call(this);
          this._loadSuccess = false;
          ...
          if (this._loadSuccess) {
            _AddShowListeners.call(this, resolver);
            await this._ad["show"]();
          } else Failure(resolver, "interstitial ad failed to load");
        }
      };
      if (this._adLoading) {
        await this.GetLoadPromise();
  ...
  function _AddLoadListeners(resolver) {
    ...
    const onFailLoad = (event) => {
      ...
      if (resolver) Failure(resolver, "interstitial ad failed to load");
      document.removeEventListener(Events.LOAD, onLoad);
      document.removeEventListener(Events.LOAD_FAIL, onFailLoad);
    };
    document.addEventListener(Events.LOAD, onLoad);
  ...
  function _AddShowListeners(resolver) {
    const onShow = (event) => {
      ...
      if (resolver) Success(resolver, "interstitial ad show");
    ...
    const onShowFail = (event) => {
      this._adShowing = false;
      if (resolver) Failure(resolver, "interstitial ad failed to show");
      document.removeEventListener(Events.SHOW, onShow);
      document.removeEventListener(Events.SHOW_FAIL, onShowFail);
    };
...
("use strict");
{
      ...
      const show_ad = async () => {
        if (this._loadSuccess) {
          ...
          if (this._loadSuccess) {
            _AddShowListeners.call(this, resolver);
            await this._ad["show"]();
          } else Failure(resolver, "rewarded ad failed to load");
        }
      };
      if (this._adLoading) {
        await this.GetLoadPromise();
  ...
  function _AddLoadListeners(resolver) {
    const onLoad = (event) => {
      this._adLoading = false;
      this.ResolveLoadPromise(true);
      if (resolver) Success(resolver, "rewarded ad load");
      document.removeEventListener(Events.LOAD, onLoad);
      document.removeEventListener(Events.LOAD_FAIL, onFailLoad);
    };
    const onFailLoad = (event) => {
      this._adLoading = false;
      this.ResolveLoadPromise(false);
      if (resolver) Failure(resolver, "rewarded ad failed to load");
      document.removeEventListener(Events.LOAD, onLoad);
      document.removeEventListener(Events.LOAD_FAIL, onFailLoad);
    };
    document.addEventListener(Events.LOAD, onLoad);
  ...
  function _AddShowListeners(resolver) {
    ...
    const onShowFail = (event) => {
      this._adShowing = false;
      if (resolver) Failure(resolver, "rewarded ad failed to show");
      document.removeEventListener(Events.SHOW, onShow);
      document.removeEventListener(Events.SHOW_FAIL, onShowFail);
    ...
    const onDismiss = (event) => {
      if (this._reward !== null) {
        ...
        const amount = String(this._reward["amount"]);
        if (resolver) Success(resolver, JSON.stringify([type, amount]));
      } else if (resolver) Failure(resolver, "closed with no reward");
      this._reward = null;
      this._adShowing = false;
      this._adLoading = true;
      this._loadSuccess = false;
...
("use strict");
{
      ...
      const show_ad = async () => {
        if (this._loadSuccess) {
          ...
          if (this._loadSuccess) {
            _AddShowListeners.call(this, resolver);
            await this._ad["show"]();
          } else Failure(resolver, "rewarded interstitial ad failed to load");
        }
      };
      if (this._adLoading) {
        await this.GetLoadPromise();
  ...
  function _AddLoadListeners(resolver) {
    const onLoad = (event) => {
      this._adLoading = false;
      this.ResolveLoadPromise(true);
      if (resolver) Success(resolver, "rewarded interstitial ad load");
      document.removeEventListener(Events.LOAD, onLoad);
      document.removeEventListener(Events.LOAD_FAIL, onFailLoad);
    };
    const onFailLoad = (event) => {
...
("use strict");
{
  ...
  const Success = C3AdUtils.Success;
  const Failure = C3AdUtils.Failure;
  const IOS = self["cordova"] ? self["cordova"]["platformId"] === "ios" : false;
  const ANDROID = self["cordova"]
    ? self["cordova"]["platformId"] === "android"
    : false;
  const UNKNOWN = 0;
  ...
  class C3Consent {
    constructor() {
      ...
      this._hasConsentForm = false;
    }
    async Reset() {
      if (!!C3AdUtils.ConfigurationOptions.debug)
    ...
    async RequestTrackingAuthorization() {
      if (IOS) await self["admob"]["requestTrackingAuthorization"]();
    }
    ...
    async RequestIDFA(resolver) {
      if (ANDROID) return Success(resolver, "not-determined");
      if (IOS) {
        await _UpdateRequestLocationInEEAorUnknown.call(this);
        if (this._inEEAorUnknown === "true")
          await this.RequestTrackingAuthorization();
        else {
          await this.GetTrackingAuthorizationStatus();
          if (this._trackingStatusString === "not-determined") {
            await _HasConsentForm.call(this);
            if (this._hasConsentForm) await this.ShowIDFAMessage();
            else await this.RequestTrackingAuthorization();
          }
        }
        await this.GetTrackingAuthorizationStatus();
        return Success(resolver, this._trackingStatusString);
      }
    }
    async RequestInfoUpdateForTesting(debug, debugLocation, resolver) {
      const ret = await _RequestInfoUpdateForTesting.call(
        this,
        debug,
        debugLocation,
        (error) => {
          if (error)
            if (typeof error === "object") Failure(resolver, error.message);
            else Failure(resolver, error);
          else Failure(resolver, "requestInfoUpdate error");
        },
      );
      return ret;
    }
    ...
    async ShowConsentForm(showConsent, showOutOfEEA = false) {
      await _UpdateConsentStatus.call(this);
      switch (showConsent) {
        case "eu": {
          const hasConsentForm = await _HasConsentForm.call(this);
          const needsConsent = _NeedsConsent.call(this);
          if (needsConsent && hasConsentForm) {
        ...
        case "always": {
          const hasConsentForm = await _HasConsentForm.call(this);
          const isInEEA = await _UpdateRequestLocationInEEAorUnknown.call(this);
          if (hasConsentForm && isInEEA === "true") {
            const form = await self["consent"]["loadForm"]();
            await form.show();
    ...
    async ShowIDFAMessage(showConsent) {
      await this.ShowConsentForm("always", true);
    }
  ...
  async function _UpdateRequestLocationInEEAorUnknown() {
    ...
    if (IOS) {
      ...
      if (gdprApplies) return (this._inEEAorUnknown = "true");
      else return (this._inEEAorUnknown = "false");
    }
  }
  ...
  async function _RequestInfoUpdateForTesting(debug, debugLocation, onError) {
    ...
    if (!!debug) {
      ...
      const testId = await C3AdUtils.TestId;
      testDeviceIds = [testId];
    }
    try {
      if (!!debug)
      ...
      else await self["consent"]["requestInfoUpdate"]();
      return true;
    } catch (error) {
...
{
  const C3AdUtils = self.C3AdUtils;
  const C3Consent = self.C3Consent;
  const C3BannerAd = self.C3BannerAd;
  const C3InterstitialAd = self.C3InterstitialAd;
  const C3RewardedAd = self.C3RewardedAd;
  const C3RewardedInterstitialAd = self.C3RewardedInterstitialAd;
  let bannerAd = null;
  ...
  addHandler("CreateBannerAdvert", async (id, size, debug, position, fn) => {
    if (!(await C3AdUtils.StartAdmobPlus(fn, debug))) return;
    const adPersonalization = await C3Consent.GetAdPersonalization();
    if (!bannerAd) bannerAd = new C3BannerAd(id, size, position, fn);
  ...
  addHandler("CreateInterstitialAdvert", async (id, debug, fn) => {
    if (interstitialAd)
      return C3AdUtils.Success(fn, "interstitial ad already created");
    if (!(await C3AdUtils.StartAdmobPlus(fn, debug))) return;
    const adPersonalization = await C3Consent.GetAdPersonalization();
  ...
  addHandler("CreateVideoAdvert", async (id, debug, fn) => {
    if (videoAd || rewardedAd)
      return C3AdUtils.Success(fn, "video ad already created");
    if (!(await C3AdUtils.StartAdmobPlus(fn, debug))) return;
    const adPersonalization = await C3Consent.GetAdPersonalization();
    videoAd = new C3RewardedAd(id, fn);
  ...
  addHandler("CreateRewardedAdvert", async (id, debug, fn) => {
    if (rewardedAd || videoAd)
      return C3AdUtils.Success(fn, "rewarded ad already created");
    if (!(await C3AdUtils.StartAdmobPlus(fn, debug))) return;
    const adPersonalization = await C3Consent.GetAdPersonalization();
  ...
  addHandler("CreateRewardedInterstitialAdvert", async (id, debug, fn) => {
    if (rewardedInterstitialAd)
      return C3AdUtils.Success(fn, "rewarded interstitial ad already created");
    if (!(await C3AdUtils.StartAdmobPlus(fn, debug))) return;
    const adPersonalization = await C3Consent.GetAdPersonalization();
  ...
  addHandler(
    ...
    async (
      ...
      if (firstCall) await C3Consent.Reset();
      if (
        !(await C3Consent.RequestInfoUpdateForTesting(debug, debugLocation, fn))
      )
        return C3Consent.FailureResponse(
          fn,
          "failure to update consent information",
        );
      if (firstCall)
        if (showOnStartUp) await C3Consent.ShowConsentForm("eu");
        else await C3Consent.UpdateConsentStatus();
      else await C3Consent.ShowConsentForm("always");
      await C3Consent.SuccessResponse(fn);
    },
  );
  ...
  addHandler("SetMaxAdContentRating", async (label, fn) => {
    if (!(await C3AdUtils.StartAdmobPlus(fn))) return;
  ...
  if (!self.C3MobileAdvertsAPI) self.C3MobileAdvertsAPI = {};
...
("use strict");
{
  ...
  const api = {};
  const addHandler = (name, fn) => (api[name] = fn);
  const sleep = (t) => new Promise((r) => setTimeout(r, t));
  let bannerState = null;
  let intState = null;
  let videoState = null;
  let rewardedState = null;
  let rewardedIntState = null;
  ...
  addHandler("CreateBannerAdvert", async (...a) => {
    ...
    await sleep(50);
    if (bannerState) fn("Banner already exists");
    else {
      bannerState = "ready";
  ...
  addHandler("ShowBannerAdvert", async (...a) => {
    ...
    await sleep(50);
    if (bannerState != "ready") fn("Banner cannot be shown");
    else {
      bannerState = "shown";
  ...
  addHandler("CreateInterstitialAdvert", async (...a) => {
    ...
    await sleep(50);
    if (intState) fn("Intersitial already exists");
    else {
      intState = "ready";
  ...
  addHandler("ShowInterstitialAdvert", async (...a) => {
    ...
    await sleep(50);
    if (intState != "ready") fn("Cannot show interstitial");
    else {
      intState = null;
  ...
  addHandler("CreateVideoAdvert", async (...a) => {
    ...
    await sleep(50);
    if (videoState) fn("Video already exists");
    else {
      videoState = "ready";
  ...
  addHandler("ShowVideoAdvert", async (...a) => {
    ...
    await sleep(50);
    if (videoState != "ready") fn("Cannot show video");
    else {
      videoState = null;
  ...
  addHandler("CreateRewardedAdvert", async (...a) => {
    ...
    await sleep(50);
    if (rewardedState) fn("Rewarded already exists");
    else {
      rewardedState = "ready";
      fn(null, "Created rewarded");
    }
  });
  addHandler("ShowRewardedAdvert", async (...a) => {
    const [data, fn] = getArgument("ShowRewardedAdvert", a);
    await sleep(50);
    if (rewardedState != "ready") fn("Cannot show rewarded");
    else {
      rewardedState = null;
      fn(null, '["example type", 20]');
    }
  ...
  addHandler("CreateRewardedInterstitialAdvert", async (...a) => {
    ...
    await sleep(50);
    if (rewardedIntState) fn("Rewarded intersitial already exists");
    else {
  ...
  addHandler("ShowRewardedInterstitialAdvert", async (...a) => {
    ...
    await sleep(50);
    if (rewardedIntState != "ready") fn("Cannot show rewarded interstitial");
    else {
  ...
  addHandler("Configure", async (...a) => {
    ...
    await sleep(50);
    fn(null, STATUS_PERSONALISED + "&&true");
  });
...
("use strict");
{
  ...
  const isReady = async (resolver) => {
    ...
    readyPromise = new Promise((resolve, reject) => {
      self["adConfig"]({ preloadAdBreaks: "on", onReady: () => resolve(true) });
    });
    return readyPromise;
  };
  const suspendRuntime = () => {
    if (api["suspendRuntime"]) api["suspendRuntime"]();
  };
  const resumeRuntime = () => {
    if (api["resumeRuntime"]) api["resumeRuntime"]();
  };
  addHandler("CreateBannerAdvert", async (id, size, debug, position, fn) => {
  ...
  addHandler("CreateInterstitialAdvert", async (id, debug, fn) => {
    if (!(await isReady(fn))) return;
    C3AdUtils.Success(fn, "interstitial ad created");
  });
  addHandler("ShowInterstitialAdvert", async (fn) => {
    if (!(await isReady(fn))) return;
    self["adBreak"]({
  ...
  addHandler("CreateRewardedAdvert", async (id, debug, fn) => {
    if (!(await isReady(fn))) return;
    C3AdUtils.Success(fn, "rewarded ad created");
  });
  addHandler("ShowRewardedAdvert", async (fn) => {
    if (!(await isReady(fn))) return;
    self["adBreak"]({
    ...
    self["adBreak"]({
      ...
      adBreakDone: (placementInfo) => {
        switch (placementInfo["breakStatus"]) {
          ...
          default: {
            console.table(placementInfo);
            C3AdUtils.Failure(fn, "rewarded interstitial ad failed to show");
          }
        }
      },
      beforeReward: (showAdFunc) => {
  ...
  addHandler("TagForChildDirectedTreatment", async (label, fn) => {
    C3AdUtils.Failure(fn, "tagging for children not supported in web platform");
  });
  addHandler("TagForUnderAgeOfConsent", async (label, fn) => {
    C3AdUtils.Failure(
      fn,
...
("use strict");
{
  ...
  const HANDLER_CLASS = class MobileAdvertHandler extends self.DOMHandler {
    constructor(iRuntime) {
      ...
      const addScriptHandler = (name) => [
        name,
        (data) => this._AddScript(data[0], data[1]),
      ];
      this.AddRuntimeMessageHandlers([
        ...
        addScriptHandler("AddScript"),
      ]);
    }
    _GetApi() {
      if (self["cordova"]) return self.C3MobileAdvertsAPI.real;
      else if (!self["cordova"] && !USE_EMULATOR)
        return self.C3MobileAdvertsAPI.web;
      else if (USE_EMULATOR) return self.C3MobileAdvertsAPI.fake;
    }
    _SuspendRuntime() {
      const runtimeInterface = this.GetRuntimeInterface();
      runtimeInterface.PostToRuntimeComponent("runtime", "visibilitychange", {
        hidden: true,
      });
    }
    _ResumeRuntime() {
    ...
    async _CallMethod(name, data) {
      ...
      if (!api["suspendRuntime"])
        api["suspendRuntime"] = () => this._SuspendRuntime();
      if (!api["resumeRuntime"])
        api["resumeRuntime"] = () => this._ResumeRuntime();
      if (!api) {
        if (!hasShownWarning) {
