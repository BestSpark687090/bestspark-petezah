...
("use strict");
{
  ...
  window.DOMElementHandler = class DOMElementHandler extends self.DOMHandler {
    ...
    AddDOMElementMessageHandlers(arr) {
      for (const [handler, func] of arr)
        this.AddDOMElementMessageHandler(handler, func);
    }
    _OnCreate(e) {
      const elementId = e["elementId"];
...
("use strict");
{
  ...
  window.RuntimeInterface = class RuntimeInterface {
    constructor(opts) {
      ...
      if (
        this._exportType === "playable-ad-single-file" ||
        this._exportType === "playable-ad-zip" ||
    ...
    async _InitWorker(opts, port2) {
      const workerMainUrl = opts.workerMainUrl;
      if (this._exportType === "preview") {
        ...
        this._worker = await this.CreateWorker(workerMainUrl, {
          type: "module",
          name: "Runtime",
        });
      ...
      this._worker.postMessage(
        Object.assign(this._GetCommonRuntimeOptions(opts), {
          ...
          scriptsInEventsPath: opts.scriptsInEventsPath,
        }),
    ...
    async _InitDOM(opts, port2) {
      ...
      this._runtimeDomHandler._AddDefaultCanvasEventHandlers(canvas);
      this._runtimeDomHandler._AddDefaultHTMLWrapEventHandlers(htmlWrap);
      const runtimeScriptList = await Promise.all(
        opts.runtimeScriptList.map((url) =>
          this._MaybeGetPlatformSpecificScriptURL(url),
        ),
      );
      await Promise.all(runtimeScriptList.map((url) => AddScript(url)));
      const projectMainScriptPath = opts.projectMainScriptPath;
      const scriptsInEventsPath = opts.scriptsInEventsPath;
      if (projectMainScriptPath)
        try {
          await AddScript(projectMainScriptPath);
          if (
            this._exportType === "preview" &&
            !globalThis.C3_ProjectMainScriptOK
          )
          ...
          alert(
            `Failed to load the project main script (${projectMainScriptPath}). Check all your JavaScript code has valid syntax, all imports are written correctly, and that an exception was not thrown running the script. Press F12 and check the console for error details.`,
          );
        }
      if (scriptsInEventsPath)
        try {
          await AddScript(scriptsInEventsPath);
          if (this._exportType === "preview" && !globalThis.C3.ScriptsInEvents)
            throw new Error("scripts in events did not run to completion");
        } catch (err) {
          this._RemoveLoadingMessage();
          console.error("Error loading scripts in events: ", err);
          alert(
            `Failed to load scripts in events. Check all your JavaScript code has valid syntax, all imports are written correctly, and that an exception was not thrown running the 'Imports for events' script. Press F12 and check the console for error details.`,
          );
        }
    ...
    async CreateWorker(url, workerOpts) {
    ...
    ["_PlayableAdFetchBlob"](url) {
      ...
      else throw new Error("missing file: " + url);
    }
    _GetPermissionAPI() {
      const api =
        window["cordova"] &&
...
("use strict");
{
  ...
  self.JobSchedulerDOM = class JobSchedulerDOM {
    ...
    async Init() {
      ...
      const dispatchWorkerScriptUrl =
        this._runtimeInterface.GetScriptFolder() + DISPATCH_WORKER_SCRIPT_NAME;
    ...
    async _CreateJobWorker() {
      ...
      const jobWorkerScriptUrl =
        this._runtimeInterface.GetScriptFolder() + JOB_WORKER_SCRIPT_NAME;
  ...
  self.C3WebAudioBuffer = class C3WebAudioBuffer extends self.C3AudioBuffer {
    ...
    async _Fetch() {
      ...
      if (
        ...
        iRuntime.GetExportType() === "playable-ad-single-file" &&
        iRuntime.IsRelativeURL(this._url)
      ) {
        const blob = iRuntime["_PlayableAdFetchBlob"](this._url);
  ...
  const HANDLER_CLASS = class MouseDOMHandler extends self.DOMHandler {
    constructor(iRuntime) {
      ...
      this.AddRuntimeMessageHandlers([
        ...
        ["request-pointer-lock", (e) => this._OnRequestPointerLock(e)],
...
// scripts/plugins/Avix_PokiSDK_ForC3/c3runtime/domSide.js
("use strict");
...
("use strict");
{
  ...
  function GetAndroidInsetTop(AndroidNotch) {
    return new Promise((resolve, reject) => {
      AndroidNotch["getInsetTop"](resolve, reject);
    });
  }
  function GetAndroidInsetRight(AndroidNotch) {
    return new Promise((resolve, reject) => {
      AndroidNotch["getInsetRight"](resolve, reject);
    });
  }
  function GetAndroidInsetBottom(AndroidNotch) {
    return new Promise((resolve, reject) => {
  ...
  function GetAndroidInsetLeft(AndroidNotch) {
    return new Promise((resolve, reject) => {
      AndroidNotch["getInsetLeft"](resolve, reject);
    });
  }
      ...
      if (AndroidNotch)
        try {
          return await Promise.all([
            GetAndroidInsetTop(AndroidNotch),
            GetAndroidInsetRight(AndroidNotch),
            GetAndroidInsetBottom(AndroidNotch),
            GetAndroidInsetLeft(AndroidNotch),
          ]);
        } catch (err) {
          console.error(
...
("use strict");
{
  ...
  const HANDLER_CLASS = class TextInputDOMHandler extends self.DOMElementHandler {
    constructor(iRuntime) {
      super(iRuntime, DOM_COMPONENT_ID);
      this.AddDOMElementMessageHandler("scroll-to-bottom", (elem) =>
        this._OnScrollToBottom(elem),
      );
    }
    CreateElement(elementId, e) {
      let elem;
