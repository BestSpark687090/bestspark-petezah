...
window.DOMElementHandler = class extends self.DOMHandler {
  ...
  _OnSetVisible(a) {
    this._autoAttach &&
      (this._elementMap.get(a.elementId).style.display = a.isVisible
        ? ""
        : "none");
  }
  _OnUpdatePosition(a) {
...
window.RuntimeInterface = class a {
  ...
  async _InitDOM(b, c) {
    ...
    Array.isArray(b.workerDependencyScripts) &&
      d.unshift(...b.workerDependencyScripts);
...
self.RuntimeInterface.AddDOMHandlerClass(
  class extends self.DOMElementHandler {
    constructor(a) {
      super(a, "text-input");
      this.AddDOMElementMessageHandler("scroll-to-bottom", (b) =>
        this._OnScrollToBottom(b),
      );
    }
    CreateElement(a, b) {
      let c;
      ...
      c.addEventListener("input", () =>
        this.PostToRuntimeElement("change", a, { text: c.value }),
      );
      b.id && (c.id = b.id);
...
self.AudioDOMHandler = class extends self.DOMHandler {
  ...
  *audioInstancesByTag(a) {
  ...
  async _AddEffect(a) {
    ...
    else if ("stereopan" === b) d = new self.C3AudioStereoPanFX(this, ...d);
  ...
  _SetEffectParam(a) {
    ...
    !a ||
      0 > b ||
      b >= a.length ||
      (a[b].SetParam(c, d, e, f), this._PostUpdatedFxState());
  }
  _RemoveEffects(a) {
    a = a.tag.toLowerCase();
...
self.C3AudioInstance = class {
  ...
  SetPan(a, b, c, d, e, f) {
    this._isPannerEnabled &&
      (this.SetPanXYA(a, b, c),
      (a = self.AudioDOMHandler.ToDegrees),
      this._pannerConeParams[0] !== a(d) &&
        ((this._pannerConeParams[0] = a(d)),
        (this._pannerNode.coneInnerAngle = a(d))),
      this._pannerConeParams[1] !== a(e) &&
        ((this._pannerConeParams[1] = a(e)),
        (this._pannerNode.coneOuterAngle = a(e))),
      this._pannerConeParams[2] !== f &&
        ((this._pannerConeParams[2] = f),
  ...
  class extends self.DOMHandler {
    ...
    _OnReadyForSWMessages() {
      window.C3_RegisterSW &&
        ...
        window.OfflineClientInfo.SetMessageCallback((a) =>
          this.PostToRuntime("sw-message", a.data),
        );
    }
    _OnOnlineStateChanged(a) {
      this.PostToRuntime("online-state", { isOnline: a });
    }
    ...
    _OnRequestFullscreen(a) {
      if (
        ...
        let c;
        a.requestFullscreen
          ? (c = a.requestFullscreen(b))
          : a.mozRequestFullScreen
            ? (c = a.mozRequestFullScreen(b))
            : a.msRequestFullscreen
              ? (c = a.msRequestFullscreen(b))
    ...
    _OnGetDocumentCSSStyle(a) {
      ...
      try {
        ...
        return c
          ? { isOk: !0, result: window.getComputedStyle(c).getPropertyValue(b) }
          : { isOk: !1 };
      } catch (c) {
        return (
          console.warn("[Browser] Failed to get style: ", c),
          { isOk: !1 }
        );
      }
    }
...
self.RuntimeInterface.AddDOMHandlerClass(
  class extends self.DOMHandler {
    ...
    _GetSafeAreaInset() {
      ...
      a = [
        a.getPropertyValue("--temp-sai-top"),
        a.getPropertyValue("--temp-sai-right"),
        a.getPropertyValue("--temp-sai-bottom"),
        a.getPropertyValue("--temp-sai-left"),
      ].map((c) => {
        c = parseInt(c, 10);
  ...
  class extends self.DOMHandler {
    ...
    async _RequestMotionPermission() {
      ...
      try {
        ...
        return (
          console.warn("[Touch] Failed to request motion permission: ", a),
          !1
        );
      }
    }
