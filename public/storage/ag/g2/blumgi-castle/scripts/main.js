...
window.RuntimeInterface = class a {
  ...
  _GetWorkerURL(b) {
    b = this._workerScriptURLs.hasOwnProperty(b)
      ...
      : b.endsWith("/workermain.js") &&
        ...
        : "playable-ad" === this._exportType &&
            this._localFileBlobs.hasOwnProperty(b.toLowerCase())
  ...
  async _InitWorker(b, c) {
    ...
    if ("cordova" === this._exportType)
      for (let g = 0, h = b.projectScripts.length; g < h; ++g) {
        const k = b.projectScripts[g],
          l = k[0];
        if (
          l === b.mainProjectScript ||
          "scriptsInEvents.js" === l ||
          l.endsWith("/scriptsInEvents.js")
        )
          k[1] = await this._MaybeGetCordovaScriptURL(l);
      }
    ...
    self.c3_callFunction = (g, h) =>
      this._runtimeDomHandler._InvokeFunctionFromJS(g, h);
  ...
  CordovaFetchLocalFile(b) {
    const c =
      window.cordova.file.applicationDirectory + "www/" + b.toLowerCase();
    return new Promise((d, e) => {
...
self.AudioDOMHandler = class extends self.DOMHandler {
  ...
  *audioInstancesByTag(a) {
    ...
    0 < m &&
      ...
      (this._audioContext.getOutputTimestamp
        ? ((a = this._audioContext.getOutputTimestamp()),
          (m = m - a.performanceTime / 1e3 + a.contextTime))
        : (m = m - performance.now() / 1e3 + this._audioContext.currentTime));
  ...
  async _OnLoadState(a) {
    ...
    if (3 !== b)
      for (var c of this._audioInstances)
        (c.IsMusic() && 1 === b) || ((c.IsMusic() || 2 !== b) && c.Stop());
