...
async function CreateCacheFromFileList(cacheName, fileList, bypassCache) {
  const responses = await Promise.all(
    fileList.map((url) => fetchWithBypass(url, bypassCache)),
  );
  let allOk = true;
  ...
  try {
    return await Promise.all(
      responses.map((response, i) => cache.put(fileList[i], response)),
    );
  } catch (err) {
...
async function UpdateCheck(isFirst) {
  try {
    ...
    console.log(
      CONSOLE_PREFIX + "Caching " + fileList.length + " files for offline use",
    );
    ...
    if (isUpdatePending) {
      ...
      console.log(
        CONSOLE_PREFIX + "All resources saved, offline support ready",
      );
...
async function GetCacheNameToUse(availableCacheNames, doUpdateCheck) {
  ...
  await Promise.all(
    availableCacheNames.slice(0, -1).map((c) => caches.delete(c)),
  );
...
async function HandleFetch(event, doUpdateCheck) {
  ...
  const useCacheName = await GetCacheNameToUse(
    availableCacheNames,
    doUpdateCheck,
  );
  ...
  if (IsUrlInLazyLoadList(event.request.url, lazyLoadList))
    try {
      ...
      console.warn(
        CONSOLE_PREFIX + "Error caching '" + event.request.url + "': ",
        err,
      );
    }
