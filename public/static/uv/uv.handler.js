...
async function __uvHook(window, config = {}, bare = '/bare/') {
    ...
    switch (event.that.tagName) {
      case 'HEAD':
        event.data.value = __uv
          .sourceHtml(event.data.value.replace(/<head(.*)>(.*)<\/head>/s, '<op-head$1>$2</op-head>'))
          .replace(/<op-head(.*)>(.*)<\/op-head>/s, '<head$1>$2</head>');
        break;
      case 'BODY':
        event.data.value = __uv
          .sourceHtml(event.data.value.replace(/<body(.*)>(.*)<\/body>/s, '<op-body$1>$2</op-body>'))
  ...
  client.element.on('getAttribute', (event) => {
    if (client.element.hasAttribute.call(event.that, __uv.attributePrefix + '-attr-' + event.data.name)) {
      event.respondWith(event.target.call(event.that, __uv.attributePrefix + '-attr-' + event.data.name));
    }
  });
  ...
  client.url.on('createObjectURL', (event) => {
    ...
    if (url.startsWith('blob:' + location.origin)) {
      let newUrl =
        'blob:' +
        (__uv.meta.url.href !== 'about:blank' ? __uv.meta.url.origin : window.parent.__uv.meta.url.origin) +
  ...
  client.storage.on('key', (event) => {
    if (event.that.__uv$storageObj) {
      event.respondWith(client.nativeMethods.keys.call(null, event.that.__uv$storageObj)[event.data.index] || null);
    }
  });
    ...
    event.data.protocols = [
      ...
      __uv.encodeProtocol(
        JSON.stringify({
          remote,
          headers,
          forward_headers: ['accept', 'accept-encoding', 'accept-language', 'sec-websocket-extensions', 'sec-websocket-key', 'sec-websocket-version']
        })
      )
    ];
