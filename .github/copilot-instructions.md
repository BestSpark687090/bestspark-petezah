...
- Node engine: package.json declares `"type": "module"` and requires Node >= 22.
  ...
  - Build step: For this current branch there is a build step; the app is
    static-first and serves files directly from `public/`, the build step _does_
    compile submodules and do other thingss (Like transform images and generate
    the sitemap)
